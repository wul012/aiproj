...
from minigpt.registry_ack_pub import build_registry_ack_pub
from minigpt.registry_ack_pub_artifacts import write_ack_pub_artifacts_outputs
...
class RandomizedHoldoutPublicationRegistryDownstreamConsumerAckBundlePublicationReceiptPacketIndexPublicationCheckTests(unittest.TestCase):
    ...
    def test_cli_require_pass_returns_one_on_tampered_publication(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((root / "cli-check" / REGISTRY_ACK_PUB_CHECK_JSON_FILENAME).is_file())
    ...
    def test_outputs_and_cli_are_wired(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            outputs = write_ack_pub_check_artifacts_outputs(report, root / "check")
            cli_main([str(publication_path.parent), "--out-dir", str(root / "cli-check"), "--require-pass", "--force"])

        self.assertEqual(set(outputs), {"json", "csv", "text", "markdown", "html"})
        self.assertTrue(outputs["json"].endswith(REGISTRY_ACK_PUB_CHECK_JSON_FILENAME))
        self.assertIn("contract_check_ready=True", render_ack_pub_check_artifacts_text(report))
        self.assertIn("Checks", render_ack_pub_check_artifacts_markdown(report))
        self.assertIn("publication contract check", render_ack_pub_check_artifacts_html(report))


def ready_check_inputs(root: Path) -> tuple[dict[str, object], Path]:
    review, review_path = ready_publication_inputs(root)
    publication = build_registry_ack_pub(
        review,
        receipt_packet_index_review_path=review_path,
    )
    outputs = write_ack_pub_artifacts_outputs(publication, root / "publication")
