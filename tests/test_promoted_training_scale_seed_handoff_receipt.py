...
class PromotedTrainingScaleSeedHandoffReceiptTests(unittest.TestCase):
    ...
    def test_receipt_checker_requires_v2_ci_regression_contract_fields(self) -> None:
        missing_fields = {
            "schema_version": 2,
            "receipt_type": RECEIPT_TYPE,
            "automation_decision": "continue",
        ...
        self.assertTrue(
            any("schema_version >= 2 receipt must include handoff_batch_maturity_ci_regression_count" in issue for issue in failed["issues"])
        )
        self.assertEqual(passed["status"], "pass")
        self.assertEqual(passed["handoff_batch_maturity_ci_regression_count"], 2)
    ...
    def test_execute_script_can_write_receipt_check_outputs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(check_payload["selected_handoff_batch_maturity_ci_regression_count"], 0)
            self.assertEqual(check_payload["handoff_batch_maturity_ci_regression_count"], 0)
            self.assertEqual(check_payload["comparison_exclusion_reasons"], [])
            ...
            self.assertIn("receipt_check_status=pass", completed.stdout)
            self.assertIn("receipt_decision=continue", completed.stdout)
            self.assertIn("receipt_handoff_batch_maturity_ci_regression_count=0", completed.stdout)
    ...
    def test_execute_script_can_write_handoff_assurance_outputs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(assurance_payload["embedded_receipt_check_receipt_schema_version"], 2)
            self.assertEqual(
                assurance_payload["embedded_receipt_check_receipt_selected_handoff_batch_maturity_ci_regression_count"],
                0,
            )
            self.assertEqual(
                assurance_payload["embedded_receipt_check_receipt_handoff_batch_maturity_ci_regression_count"],
                0,
            )
            self.assertEqual(assurance_payload["embedded_receipt_check_receipt_comparison_exclusion_reasons"], [])
            ...
            self.assertIn("handoff_assurance_embedded_receipt_check_receipt_schema_version", csv_text)
            self.assertIn("Handoff assurance receipt schema version", markdown)
            ...
            self.assertIn("handoff_assurance_embedded_receipt_check_receipt_schema_version=2", completed.stdout)
            ...
            self.assertIn(
                ...
                assurance_text.read_text(encoding="utf-8"),
            )
            self.assertIn("handoff_assurance_status=pass", assurance_text.read_text(encoding="utf-8"))

    def test_handoff_assurance_exposes_v2_receipt_ci_regression_contract(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            seed = write_seed_tree(
                root,
            ...
            self.assertEqual(check["status"], "pass")
            self.assertEqual(check["embedded_receipt_check_receipt_schema_version"], 2)
            ...
            self.assertIn("handoff_assurance_embedded_receipt_check_receipt_schema_version=2", rendered)
            self.assertIn(
    ...
    def test_embedded_receipt_check_rejects_tampered_receipt_ci_regression_sidecar(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            seed = write_seed_tree(
                root,
            ...
            report = load_promoted_training_scale_seed_handoff_report(script_out)
            receipt_path = Path(report["receipt_check"]["receipt_path"])
            receipt = json.loads(receipt_path.read_text(encoding="utf-8"))
            receipt["handoff_batch_maturity_ci_regression_count"] = 0
            ...
            self.assertTrue(
                any(
                    "receipt_check.receipt_path.handoff_batch_maturity_ci_regression_count" in issue
    ...
    def test_handoff_assurance_smoke_script_passes(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(summary["checks"]["handoff_assurance_embedded_receipt_check_receipt_schema_version"], 2)
            self.assertEqual(
                summary["checks"][
            ...
            self.assertEqual(
                summary["checks"]["handoff_assurance_embedded_receipt_check_receipt_comparison_exclusion_reasons"],
                [],
            )
