...
from minigpt.bounded_objective_loss_signal_bridge_target_only_memory_stagnation_aware_suffix_replay_comparison_artifacts import (
    render_stagnation_aware_suffix_replay_comparison_html,
    render_stagnation_aware_suffix_replay_comparison_markdown,
    render_stagnation_aware_suffix_replay_comparison_text,
    write_stagnation_aware_suffix_replay_comparison_outputs,
)
...
from scripts.run_bounded_objective_loss_signal_bridge_target_only_memory_stagnation_aware_suffix_replay_comparison import (
    main as cli_main,
)
...
class StagnationAwareSuffixReplayComparisonTests(unittest.TestCase):
    def test_partial_replay_is_ready_but_not_recovered(self) -> None:
        ...
        self.assertIn("stagnation_aware_suffix", report["decision"])
        self.assertTrue(report["summary"]["bounded_objective_loss_signal_bridge_target_only_memory_stagnation_aware_suffix_replay_comparison_ready"])
        self.assertEqual(report["summary"]["route"], "bounded_objective_loss_signal_bridge_target_only_memory_stagnation_aware_suffix")
    ...
    def test_contract_recovered_still_requires_holdout(self) -> None:
        ...
        self.assertEqual(report["decision"], "bounded_objective_loss_signal_bridge_target_only_memory_stagnation_aware_suffix_replay_contract_recovered_holdout_required")
    ...
    def test_training_run_must_be_ready(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            training["summary"]["bounded_objective_loss_signal_bridge_target_only_memory_stagnation_aware_suffix_training_ready"] = False
            report = build_stagnation_aware_suffix_replay_comparison(objective_contract(), training, generator_runner=fake_runner({}))
    ...
    def test_outputs_and_cli_are_wired(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            report = build_stagnation_aware_suffix_replay_comparison(objective_contract(), training_run(run_dir), generator_runner=fake_runner({}))
            outputs = write_stagnation_aware_suffix_replay_comparison_outputs(report, root / "out")
            cli_main([
                "--objective-contract",
        ...
        self.assertTrue(outputs["json"].endswith(TARGET_ONLY_MEMORY_STAGNATION_AWARE_SUFFIX_REPLAY_COMPARISON_JSON_FILENAME))
        self.assertIn("stagnation_aware_suffix_replay_comparison_ready=True", render_stagnation_aware_suffix_replay_comparison_text(report))
        self.assertIn("Replay Rows", render_stagnation_aware_suffix_replay_comparison_markdown(report))
        self.assertIn("Replay Rows", render_stagnation_aware_suffix_replay_comparison_html(report))
