...
class TinyStandardBenchmarkSmokeTests(unittest.TestCase):
    def test_capped_standard_suite_keeps_all_cases_with_lower_token_budget(self) -> None:
        ...
        self.assertIn("Evidence chain: train, eval suite, generation quality, benchmark scorecard.", corpus)
        ...
        text = render_summary(
            {
                ...
                "benchmark_scorecard": {"overall_status": "warn", "overall_score": 66.5},
                "commands": [{"name": "train", "status": "pass"}],
            }
        )
