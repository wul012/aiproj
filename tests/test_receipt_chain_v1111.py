...
from minigpt.receipt_chain_check_v1110 import build_receipt_check_v1110
from minigpt.receipt_chain_check_v1110_artifacts import write_check_v1110_artifacts_outputs
...
from minigpt.receipt_chain_v1111_artifacts import (
    render_receipt_v1111_artifacts_html,
    render_receipt_v1111_artifacts_markdown,
    render_receipt_v1111_artifacts_text,
    write_receipt_v1111_artifacts_outputs,
)
...
from minigpt.receipt_chain_v1109_artifacts import write_receipt_v1109_artifacts_outputs
...
class RandomizedHoldoutPublicationReceiptIndexReceiptIndexReceiptIndexReceiptIndexReceiptIndexReceiptIndexV1111Tests(unittest.TestCase):
    def test_receipt_index_accepts_ready_receipt_and_check(self) -> None:
        with short_temp_dir() as tmp:
            ...
            report = build_receipt_chain_v1111(
                receipt,
                check,
    ...
    def test_outputs_and_cli_are_wired(self) -> None:
        with short_temp_dir() as tmp:
            ...
            outputs = write_receipt_v1111_artifacts_outputs(report, root / "index")
        ...
        self.assertTrue(outputs["json"].endswith(RECEIPT_CHAIN_V1111_JSON_FILENAME))
        self.assertIn("index_ready=True", render_receipt_v1111_artifacts_text(report))
        self.assertIn("Receipt Index Rows", render_receipt_v1111_artifacts_markdown(report))
        self.assertIn("receipt index", render_receipt_v1111_artifacts_html(report))
...
def ready_index_inputs(root: Path) -> tuple[dict[str, object], Path, dict[str, object], Path]:
    ...
    receipt_outputs = write_receipt_v1109_artifacts_outputs(receipt, root / "receipt")
    receipt_path = Path(receipt_outputs["json"])
    check = build_receipt_check_v1110(receipt, receipt_path=receipt_path)
    check_outputs = write_check_v1110_artifacts_outputs(check, root / "receipt-check")
