...
from scripts.compare_model_capability_route_promotion_bounded_real_replay_prompt_aligned_checkpoint import main as cli_main
...
def training_report(*, ready: bool = True) -> dict:
    return {
        ...
        "summary": {"prompt_aligned_training_ready": ready},
        "artifacts": [{"key": "checkpoint", "exists": ready, "path": "checkpoint.pt", "size": 10}],
    }


class ModelCapabilityRoutePromotionBoundedRealReplayPromptAlignedCheckpointComparisonTests(unittest.TestCase):
    def test_regression_is_valid_comparison_but_blocks_improvement(self) -> None:
        report = build_model_capability_route_promotion_bounded_real_replay_prompt_aligned_checkpoint_comparison(
        ...
        self.assertEqual(report["decision"], "model_capability_route_promotion_bounded_real_replay_prompt_aligned_checkpoint_regressed")
        self.assertTrue(report["summary"]["bounded_prompt_aligned_checkpoint_comparison_ready"])
        self.assertTrue(report["summary"]["prompt_aligned_checkpoint_regressed"])
    ...
    def test_training_evidence_must_be_ready_when_provided(self) -> None:
        ...
        self.assertIn("prompt_aligned_training_ready_when_provided", [issue["id"] for issue in report["issues"]])
    ...
    def test_outputs_and_cli_are_wired(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            training_outputs = write_model_capability_route_promotion_bounded_real_replay_prompt_aligned_training_run_outputs(training, root / "training")
            ...
            report = build_model_capability_route_promotion_bounded_real_replay_prompt_aligned_checkpoint_comparison(baseline, prompt, training)
            outputs = write_model_capability_route_promotion_bounded_real_replay_prompt_aligned_checkpoint_comparison_outputs(report, root / "comparison")
            cli_main(
                [
                    "--baseline-replay",
        ...
        self.assertIn("prompt_aligned_checkpoint_regressed=True", render_model_capability_route_promotion_bounded_real_replay_prompt_aligned_checkpoint_comparison_text(report))
        self.assertIn("Case Comparison", render_model_capability_route_promotion_bounded_real_replay_prompt_aligned_checkpoint_comparison_markdown(report))
        self.assertIn("Prompt-aligned passed", render_model_capability_route_promotion_bounded_real_replay_prompt_aligned_checkpoint_comparison_html(report))
