...
from scripts.run_model_capability_route_promotion_bounded_objective_replay_comparison import main as cli_main


class BoundedObjectiveReplayComparisonTests(unittest.TestCase):
    def test_zero_hit_replay_is_ready_but_not_recovered(self) -> None:
        ...
        self.assertEqual(report["decision"], "model_capability_route_promotion_bounded_objective_replay_zero_hit")
        self.assertTrue(report["summary"]["bounded_objective_replay_comparison_ready"])
    ...
    def test_all_cases_pass_marks_contract_recovered_but_not_promotion_ready(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            run_dir = write_fake_run(root)
            report = build_model_capability_route_promotion_bounded_objective_replay_comparison(
        ...
        self.assertEqual(report["decision"], "model_capability_route_promotion_bounded_objective_replay_contract_recovered_holdout_required")
    ...
    def test_training_evidence_must_be_ready(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            run_dir = write_fake_run(root)
            training = training_run(run_dir)
            training["summary"]["bounded_objective_training_ready"] = False
            report = build_model_capability_route_promotion_bounded_objective_replay_comparison(
                objective_contract(),
                training,
                generator_runner=fake_runner(" fixed loss"),
    ...
    def test_outputs_and_cli_are_wired(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            outputs = write_bounded_objective_replay_comparison_outputs(report, root / "out")
            cli_main(
                [
                    "--objective-contract",
        ...
        self.assertTrue(outputs["json"].endswith(BOUNDED_OBJECTIVE_REPLAY_COMPARISON_JSON_FILENAME))
        self.assertIn("bounded_objective_replay_comparison_ready=True", render_bounded_objective_replay_comparison_text(report))
        self.assertIn("Replay Rows", render_bounded_objective_replay_comparison_markdown(report))
        self.assertIn("Replay Rows", render_bounded_objective_replay_comparison_html(report))
...
def training_run(run_dir: Path) -> dict:
    return {
        "status": "pass",
        "run_dir": str(run_dir),
        "summary": {
