...
from minigpt.receipt_chain_review_v1038_artifacts import (
    render_review_v1038_artifacts_html,
    render_review_v1038_artifacts_markdown,
    render_review_v1038_artifacts_text,
    write_review_v1038_artifacts_outputs,
)
from minigpt.receipt_chain_v1037 import build_receipt_chain_v1037
from minigpt.receipt_chain_v1037_artifacts import write_receipt_v1037_artifacts_outputs
...
class RandomizedHoldoutPublicationReceiptIndexReceiptIndexReceiptIndexReceiptIndexReceiptIndexReceiptIndexReviewV1038Tests(unittest.TestCase):
    def test_review_accepts_ready_receipt_index(self) -> None:
        with short_temp_dir() as tmp:
            ...
            report = build_receipt_review_v1038(
                index,
    ...
    def test_outputs_and_cli_are_wired(self) -> None:
        with short_temp_dir() as tmp:
            ...
            outputs = write_review_v1038_artifacts_outputs(report, root / "review")
            cli_main([str(index_path.parent), "--out-dir", str(root / "cli-review"), "--require-review-ready", "--require-lookup-ready", "--force"])

        self.assertEqual(set(outputs), {"json", "csv", "text", "markdown", "html"})
        self.assertTrue(outputs["json"].endswith(RECEIPT_REVIEW_V1038_JSON_FILENAME))
        self.assertIn("review_ready=True", render_review_v1038_artifacts_text(report))
        self.assertIn("Review Summary", render_review_v1038_artifacts_markdown(report))
        self.assertIn("Review Summary", render_review_v1038_artifacts_html(report))
...
def ready_review_inputs(root: Path) -> tuple[dict[str, object], Path]:
    ...
    index = build_receipt_chain_v1037(
        receipt,
        check,
        receipt_path=receipt_path,
        receipt_check_path=check_path,
    )
    outputs = write_receipt_v1037_artifacts_outputs(index, root / "index")
