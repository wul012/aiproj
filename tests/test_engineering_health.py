...
class EngineeringHealthTests(unittest.TestCase):
    def test_build_steps_runs_stable_health_gates_in_order(self) -> None:
        ...
        self.assertEqual(steps[4].command[:3], ("python", "-B", "scripts/check_name_budget.py"))
        self.assertIn(str(out_dir / "name-budget"), steps[4].command)
        self.assertEqual(steps[5].command[:3], ("python", "-B", "scripts/check_type_analysis.py"))
        self.assertIn(str(out_dir / "type-analysis"), steps[5].command)
        self.assertEqual(steps[6].command[:3], ("python", "-B", "scripts/check_model_capability_honest_measurement.py"))
        self.assertIn(str(out_dir / "model-capability-honest-measurement"), steps[6].command)
        self.assertEqual(steps[7].command[:3], ("python", "-B", "scripts/check_artifact_schema_guard.py"))
        self.assertIn(str(out_dir / "artifact-schema-guard"), steps[7].command)
        self.assertEqual(steps[8].command[:3], ("python", "-B", "scripts/check_file_size_ratchet.py"))
        self.assertIn(str(out_dir / "file-size-ratchet"), steps[8].command)
        self.assertEqual(steps[9].command[:3], ("python", "-B", "scripts/check_aiproj_track_closeout.py"))
        self.assertIn(str(out_dir / "aiproj-track-closeout"), steps[9].command)
        self.assertEqual(steps[10].command, ("python", "-B", "scripts/check_normalization_guard.py"))
