...
class PlaygroundTests(unittest.TestCase):
    def test_build_payload_reads_sampling_report(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(any(profile["id"] == "suppress_newline_tokens" for profile in payload["generation_profiles"]))
            self.assertEqual(payload["defaults"]["generation_profile"], "default")
    ...
    def test_render_playground_escapes_text_and_has_controls(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("generationProfileSelect", html)
            self.assertIn("suppress_newline_tokens", html)
            self.assertIn("generation_profile", html)
            self.assertIn("--generation-profile", html)
            self.assertIn("/api/generation-profiles", html)
            self.assertIn("loadGenerationProfiles", html)
            self.assertIn("populateGenerationProfileSelect", html)
            ...
            self.assertIn("requestHistoryTable", html)
            self.assertIn("requestHistoryBody", html)
            self.assertIn("refreshRequestHistoryButton", html)
            self.assertIn("requestHistoryStatusFilter", html)
            self.assertIn("requestHistoryEndpointFilter", html)
            self.assertIn("requestHistoryCheckpointFilter", html)
            self.assertIn("requestHistoryLimitInput", html)
            self.assertIn("requestHistoryExportLink", html)
            self.assertIn("/api/request-history", html)
            self.assertIn("/api/request-history-detail", html)
            self.assertIn("format=csv", html)
            self.assertIn("loadRequestHistory", html)
            self.assertIn("renderRequestHistory", html)
            self.assertIn("showRequestHistoryDetail", html)
            self.assertIn("requestHistoryDetailUrl", html)
            self.assertIn("requestHistoryDetailPanel", html)
            self.assertIn("requestHistoryDetailOutput", html)
            self.assertIn("requestHistoryFilters", html)
            self.assertIn("requestHistoryQuery", html)
            self.assertIn("updateRequestHistoryExportLink", html)
            self.assertIn("matching_records", html)
            self.assertIn("log_index", html)
            self.assertIn("request_history_${logIndex}.json", html)
