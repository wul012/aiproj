...
from minigpt.model_capability_route_promotion_bounded_objective_unassisted_repair_seed_revision_replay_comparison_artifacts import (
    ...
    write_bounded_objective_unassisted_repair_seed_revision_replay_comparison_outputs,
)
...
from scripts.run_model_capability_route_promotion_bounded_objective_unassisted_repair_seed_revision_replay_comparison import main as cli_main


class BoundedObjectiveUnassistedRepairSeedRevisionReplayComparisonTests(unittest.TestCase):
    def test_zero_hit_replay_is_ready_but_not_recovered(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            report = build_model_capability_route_promotion_bounded_objective_unassisted_repair_seed_revision_replay_comparison(
                objective_contract(),
                seed_revision_training_run(run_dir),
                generator_runner=fake_runner("wixed w"),
            )

        self.assertEqual(report["status"], "pass")
        self.assertEqual(report["decision"], "model_capability_route_promotion_bounded_objective_unassisted_repair_seed_revision_replay_zero_hit")
        self.assertTrue(report["summary"]["bounded_objective_unassisted_repair_seed_revision_replay_comparison_ready"])
        self.assertFalse(report["summary"]["objective_contract_recovered"])
        self.assertFalse(report["summary"]["decoder_anchor_used"])
        self.assertEqual(report["summary"]["any_hit_case_count"], 0)
        self.assertEqual(report["comparison"]["training_source"], "model_capability_route_promotion_bounded_objective_unassisted_repair_seed_revision_training_run")
        self.assertEqual(report["interpretation"]["model_quality_claim"], "not_improved")
    ...
    def test_all_cases_pass_marks_contract_recovered_but_holdout_required(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            run_dir = write_fake_run(root)
            report = build_model_capability_route_promotion_bounded_objective_unassisted_repair_seed_revision_replay_comparison(
        ...
        self.assertEqual(
            report["decision"],
            "model_capability_route_promotion_bounded_objective_unassisted_repair_seed_revision_replay_contract_recovered_holdout_required",
        )
        self.assertTrue(report["summary"]["objective_contract_recovered"])
        self.assertFalse(report["summary"]["promotion_ready"])
        self.assertEqual(report["summary"]["next_step"], "run_unchanged_bounded_suite_holdout_replay")
        self.assertEqual(resolve_exit_code(report, require_comparison_ready=True, require_objective_pass=True), 0)

    def test_training_evidence_must_be_seed_revision_ready(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            run_dir = write_fake_run(root)
            training = seed_revision_training_run(run_dir)
            training["summary"]["bounded_objective_unassisted_repair_seed_revision_training_ready"] = False
            report = build_model_capability_route_promotion_bounded_objective_unassisted_repair_seed_revision_replay_comparison(
                objective_contract(),
                training,
                generator_runner=fake_runner(" fixed loss"),
        ...
        self.assertEqual(report["decision"], "fix_model_capability_route_promotion_bounded_objective_unassisted_repair_seed_revision_replay_comparison")
    ...
    def test_outputs_and_cli_are_wired(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write_json_payload(seed_revision_training_run(run_dir), training_path)
            ...
            outputs = write_bounded_objective_unassisted_repair_seed_revision_replay_comparison_outputs(report, root / "out")
            cli_main(
                [
                    "--objective-contract",
        ...
        self.assertTrue(outputs["json"].endswith(BOUNDED_OBJECTIVE_UNASSISTED_REPAIR_SEED_REVISION_REPLAY_COMPARISON_JSON_FILENAME))
        self.assertIn("bounded_objective_unassisted_repair_seed_revision_replay_comparison_ready=True", render_bounded_objective_unassisted_repair_seed_revision_replay_comparison_text(report))
        self.assertIn("Replay Rows", render_bounded_objective_unassisted_repair_seed_revision_replay_comparison_markdown(report))
        self.assertIn("Replay Rows", render_bounded_objective_unassisted_repair_seed_revision_replay_comparison_html(report))
...
def seed_revision_training_run(run_dir: Path) -> dict:
    return {
        "status": "pass",
        "run_dir": str(run_dir),
        "summary": {
