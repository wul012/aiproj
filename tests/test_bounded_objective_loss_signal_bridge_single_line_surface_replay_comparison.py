...
from minigpt.bounded_objective_loss_signal_bridge_single_line_surface_replay_comparison_artifacts import (
...
from scripts.run_bounded_objective_loss_signal_bridge_single_line_surface_replay_comparison import main as cli_main
...
class BoundedObjectiveLossSignalBridgeSingleLineSurfaceReplayComparisonTests(unittest.TestCase):
    def test_partial_replay_is_ready_but_not_recovered(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            run_dir = write_fake_run(Path(tmp))
            report = build_bounded_objective_loss_signal_bridge_single_line_surface_replay_comparison(
                objective_contract(),
        ...
        self.assertEqual(report["decision"], "bounded_objective_loss_signal_bridge_single_line_surface_replay_partial_required_term_hit")
        self.assertTrue(report["summary"]["bounded_objective_loss_signal_bridge_single_line_surface_replay_comparison_ready"])
        self.assertFalse(report["summary"]["objective_contract_recovered"])
        self.assertEqual(report["summary"]["any_hit_case_count"], 1)
        self.assertEqual(report["summary"]["route"], "bounded_objective_loss_signal_bridge_single_line_surface")
    ...
    def test_contract_recovered_still_requires_holdout(self) -> None:
        ...
        self.assertEqual(report["decision"], "bounded_objective_loss_signal_bridge_single_line_surface_replay_contract_recovered_holdout_required")
    ...
    def test_training_run_must_be_ready(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            training["summary"]["bounded_objective_loss_signal_bridge_single_line_surface_training_ready"] = False
            report = build_bounded_objective_loss_signal_bridge_single_line_surface_replay_comparison(
                objective_contract(),
                training,
                generator_runner=fake_runner({}),
    ...
    def test_outputs_and_cli_are_wired(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            training_path = root / "training" / LOSS_SIGNAL_BRIDGE_SINGLE_LINE_SURFACE_TRAINING_RUN_JSON_FILENAME
            ...
            self.assertEqual(locate_single_line_surface_training_run(training_path.parent), training_path)
            report = build_bounded_objective_loss_signal_bridge_single_line_surface_replay_comparison(
                objective_contract(),
                training_run(run_dir),
                generator_runner=fake_runner({}),
            )
            outputs = write_single_line_surface_replay_comparison_outputs(report, root / "out")
            cli_main(
                [
                    "--objective-contract",
        ...
        self.assertTrue(outputs["json"].endswith(LOSS_SIGNAL_BRIDGE_SINGLE_LINE_SURFACE_REPLAY_COMPARISON_JSON_FILENAME))
        self.assertIn("bounded_objective_loss_signal_bridge_single_line_surface_replay_comparison_ready=True", render_single_line_surface_replay_comparison_text(report))
        self.assertIn("Replay Rows", render_single_line_surface_replay_comparison_markdown(report))
        self.assertIn("Replay Rows", render_single_line_surface_replay_comparison_html(report))
