...
from minigpt.model_capability_required_term_pair_readiness_pair_prompt_transfer_contract_patch_artifacts import (
...
class PairPromptTransferContractPatchTests(unittest.TestCase):
    def test_patch_adds_non_heldout_pair_transfer_rows(self) -> None:
        report = build_pair_prompt_transfer_contract_patch(
            repair_plan=repair_plan_fixture(),
            base_contract_report=base_contract_fixture(),
        )

        self.assertEqual(report["status"], "pass")
        self.assertEqual(report["decision"], "pair_readiness_pair_prompt_transfer_contract_patch_ready")
        self.assertEqual(report["summary"]["pair_transfer_row_count"], len(PAIR_PROMPT_TRANSFER_ROWS))
    ...
    def test_patch_fails_when_plan_is_wrong_artifact(self) -> None:
        plan = repair_plan_fixture()
        plan["plan"]["proposed_next_artifact"] = "pair_readiness_direct_completion_surface_contract"
        report = build_pair_prompt_transfer_contract_patch(
            repair_plan=plan,
            base_contract_report=base_contract_fixture(),
        )
    ...
    def test_outputs_render_all_formats(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            outputs = write_pair_prompt_transfer_contract_patch_outputs(report, Path(tmp))

        self.assertEqual(set(outputs), {"json", "csv", "text", "markdown", "html"})
        self.assertIn("patch_ready=True", render_pair_prompt_transfer_contract_patch_text(report))
        self.assertIn("Pair Prompt Transfer Contract Patch", render_pair_prompt_transfer_contract_patch_markdown(report))
        self.assertIn("MiniGPT pair prompt transfer contract patch", render_pair_prompt_transfer_contract_patch_html(report))
...
def repair_plan_fixture() -> dict[str, object]:
    return {
        ...
        "plan": {"proposed_next_artifact": "pair_readiness_pair_prompt_transfer_contract_patch"},
