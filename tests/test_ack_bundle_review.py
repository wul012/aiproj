...
from minigpt.ack_bundle_index import build_ack_bundle_index
from minigpt.ack_bundle_index_artifacts import write_ack_index_artifacts_outputs
...
class RandomizedHoldoutPublicationRegistryDownstreamConsumerAckBundlePublicationReceiptPacketIndexPublicationIndexReviewTests(unittest.TestCase):
    def test_publication_index_review_accepts_ready_index(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            report = build_ack_bundle_review(
                index,
    ...
    def test_outputs_and_cli_are_wired(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            index, index_path = ready_review_inputs(root)
            self.assertEqual(locate_ack_bundle_review(index_path.parent), index_path)
            report = build_ack_bundle_review(
                index,
                publication_index_path=index_path,
            )
            outputs = write_review_artifacts_outputs(report, root / "review")
            cli_main(
                [
        ...
        self.assertTrue(outputs["json"].endswith(ACK_BUNDLE_REVIEW_JSON_FILENAME))
        self.assertIn("randomized_holdout_publication_registry_downstream_consumer_ack_bundle_publication_receipt_packet_index_publication_index_review_ready=True", render_review_artifacts_text(report))
        self.assertIn("Publication index rows", render_review_artifacts_markdown(report))
        self.assertIn("publication index review", render_review_artifacts_html(report))
...
def ready_review_inputs(root: Path) -> tuple[dict[str, object], Path]:
    ...
    index = build_ack_bundle_index(
        publication,
        check,
        publication_path=publication_path,
        publication_check_path=check_path,
    )
    index_outputs = write_ack_index_artifacts_outputs(index, root / "publication-index")
