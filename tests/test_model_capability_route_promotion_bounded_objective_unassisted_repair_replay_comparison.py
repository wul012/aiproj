...
from minigpt.model_capability_route_promotion_bounded_objective_unassisted_repair_replay_comparison_artifacts import (
    ...
    write_bounded_objective_unassisted_repair_replay_comparison_outputs,
)
...
from scripts.run_model_capability_route_promotion_bounded_objective_unassisted_repair_replay_comparison import main as cli_main


class BoundedObjectiveUnassistedRepairReplayComparisonTests(unittest.TestCase):
    def test_zero_hit_replay_is_ready_but_not_recovered(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            report = build_model_capability_route_promotion_bounded_objective_unassisted_repair_replay_comparison(
                objective_contract(),
                unassisted_training_run(run_dir),
                generator_runner=fake_runner("wixed w"),
            )

        self.assertEqual(report["status"], "pass")
        self.assertEqual(report["decision"], "model_capability_route_promotion_bounded_objective_unassisted_repair_replay_zero_hit")
        self.assertTrue(report["summary"]["bounded_objective_unassisted_repair_replay_comparison_ready"])
    ...
    def test_all_cases_pass_marks_contract_recovered_but_holdout_required(self) -> None:
        ...
        self.assertEqual(report["decision"], "model_capability_route_promotion_bounded_objective_unassisted_repair_replay_contract_recovered_holdout_required")
    ...
    def test_training_evidence_must_be_unassisted_ready(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            run_dir = write_fake_run(root)
            training = unassisted_training_run(run_dir)
    ...
    def test_outputs_and_cli_are_wired(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write_json_payload(unassisted_training_run(run_dir), training_path)
            ...
            outputs = write_bounded_objective_unassisted_repair_replay_comparison_outputs(report, root / "out")
            cli_main(
                [
                    "--objective-contract",
        ...
        self.assertTrue(outputs["json"].endswith(BOUNDED_OBJECTIVE_UNASSISTED_REPAIR_REPLAY_COMPARISON_JSON_FILENAME))
        self.assertIn("bounded_objective_unassisted_repair_replay_comparison_ready=True", render_bounded_objective_unassisted_repair_replay_comparison_text(report))
        self.assertIn("Replay Rows", render_bounded_objective_unassisted_repair_replay_comparison_markdown(report))
        self.assertIn("Replay Rows", render_bounded_objective_unassisted_repair_replay_comparison_html(report))
...
def unassisted_training_run(run_dir: Path) -> dict:
    return {
        "status": "pass",
        "run_dir": str(run_dir),
        "summary": {
