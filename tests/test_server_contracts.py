...
class ServerContractsSplitTests(unittest.TestCase):
    ...
    def test_contract_module_builds_request_and_pair_payloads(self) -> None:
        ...
        self.assertEqual(request.generation_profile, "default")
        ...
        profiled = server_contracts.parse_generation_request(
            {"prompt": "hello", "generation_profile": "suppress_newline_tokens", "blocked_token_texts": ["x"]}
        )
        self.assertEqual(profiled.generation_profile, "suppress_newline_tokens")
        self.assertEqual(profiled.blocked_token_texts, ("\n", "\r", "x"))
        self.assertTrue(any(item["id"] == "suppress_newline_tokens" for item in server_contracts.generation_profile_options()))
        with self.assertRaisesRegex(ValueError, "unknown generation_profile"):
            server_contracts.parse_generation_request({"prompt": "hello", "generation_profile": "unknown"})
