...
from minigpt.model_capability_route_promotion_bounded_real_replay_prompt_aligned_checkpoint_comparison_artifacts import (
...
from scripts.diagnose_model_capability_route_promotion_bounded_real_replay_prompt_aligned_failure import main as cli_main
...
def comparison_report() -> dict:
    return {
        "status": "pass",
        "summary": {"promotion_ready": False, "prompt_aligned_checkpoint_regressed": True, "pass_rate_delta": -0.4},
    }
...
def seed_report() -> dict:
    ...
    return {
        "status": "pass",
        "summary": {"prompt_aligned_seed_revision_ready": True, "exact_prompt_answer_count": 1},
...
def training_report() -> dict:
    return {
        "status": "pass",
        "summary": {"prompt_aligned_training_ready": True, "final_val_loss": 4.2},
    }


class ModelCapabilityRoutePromotionBoundedRealReplayPromptAlignedFailureDiagnosticTests(unittest.TestCase):
    def test_diagnoses_zero_hit_fragmentation_after_prompt_alignment(self) -> None:
        ...
        self.assertEqual(report["decision"], "model_capability_route_promotion_bounded_real_replay_prompt_aligned_failure_diagnostic_ready")
        self.assertTrue(report["summary"]["prompt_aligned_failure_diagnostic_ready"])
        self.assertEqual(report["summary"]["zero_hit_case_count"], 1)
        self.assertEqual(report["summary"]["prompt_in_corpus_count"], 1)
        self.assertIn("exact_prompts_present_but_generation_unanchored", [row["cause_id"] for row in report["root_causes"]])
    ...
    def test_diagnostic_fails_when_comparison_is_not_ready(self) -> None:
        ...
        self.assertIn("prompt_aligned_comparison_passed", [issue["id"] for issue in report["issues"]])
    ...
    def test_outputs_and_cli_are_wired(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            comparison_outputs = write_model_capability_route_promotion_bounded_real_replay_prompt_aligned_checkpoint_comparison_outputs(comparison_report(), root / "comparison")
            seed_outputs = write_model_capability_route_promotion_bounded_real_replay_prompt_aligned_seed_revision_outputs(seed_report(), root / "seed")
            training_outputs = write_model_capability_route_promotion_bounded_real_replay_prompt_aligned_training_run_outputs(training_report(), root / "training")
            self.assertEqual(locate_prompt_aligned_replay(Path(replay_outputs["json"]).parent), Path(replay_outputs["json"]))
            self.assertEqual(locate_prompt_aligned_comparison(Path(comparison_outputs["json"]).parent), Path(comparison_outputs["json"]))
            self.assertEqual(locate_prompt_aligned_seed_revision(Path(seed_outputs["json"]).parent), Path(seed_outputs["json"]))
            self.assertEqual(locate_prompt_aligned_training_run(Path(training_outputs["json"]).parent), Path(training_outputs["json"]))
            report = build_model_capability_route_promotion_bounded_real_replay_prompt_aligned_failure_diagnostic(
                replay_report(),
                comparison_report(),
                seed_report(),
            ...
            outputs = write_model_capability_route_promotion_bounded_real_replay_prompt_aligned_failure_diagnostic_outputs(report, root / "diagnostic")
            cli_main(
                [
        ...
        self.assertIn("zero_hit_case_count=1", render_model_capability_route_promotion_bounded_real_replay_prompt_aligned_failure_diagnostic_text(report))
        self.assertIn("Root Causes", render_model_capability_route_promotion_bounded_real_replay_prompt_aligned_failure_diagnostic_markdown(report))
        self.assertIn("Zero hit", render_model_capability_route_promotion_bounded_real_replay_prompt_aligned_failure_diagnostic_html(report))
