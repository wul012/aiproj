...
from minigpt.receipt_chain_v1117_artifacts import (
...
from minigpt.receipt_chain_review_v1116 import build_receipt_review_v1116
from minigpt.receipt_chain_review_v1116_artifacts import write_review_v1116_artifacts_outputs
from minigpt.report_utils import write_json_payload
from scripts.record_receipt_chain_v1117 import main as cli_main
from tests.test_receipt_chain_review_v1116 import ready_review_inputs as ready_v1116_index_inputs
...
class RandomizedHoldoutPublicationReceiptIndexReceiptIndexReceiptIndexReceiptIndexReceiptIndexReceiptIndexReceiptv1117Tests(unittest.TestCase):
    ...
    def test_outputs_and_cli_are_wired(self) -> None:
        with short_temp_dir() as tmp:
            ...
            outputs = write_receipt_v1117_artifacts_outputs(report, root / "receipt")
            cli_main([str(review_path.parent), "--out-dir", str(root / "cli-receipt"), "--require-receipt-ready", "--force"])
        ...
        self.assertIn("receipt_ready=True", render_receipt_v1117_artifacts_text(report))
        self.assertIn("Consumer Receipts", render_receipt_v1117_artifacts_markdown(report))
        self.assertIn("receipt", render_receipt_v1117_artifacts_html(report))


def ready_receipt_inputs(root: Path) -> tuple[dict[str, object], Path]:
    index, index_path = ready_v1116_index_inputs(root)
    review = build_receipt_review_v1116(
        index,
        receipt_index_path=index_path,
    )
    outputs = write_review_v1116_artifacts_outputs(review, root / "review")
