...
from minigpt.unassisted_loss_suffix_repair_replay_comparison_v1155 import (
...
from scripts.run_unassisted_loss_suffix_repair_replay_comparison_v1155 import main as cli_main


class UnassistedLossSuffixRepairReplayComparisonV1155Tests(unittest.TestCase):
    def test_partial_signal_is_ready_but_not_recovered(self) -> None:
        ...
        self.assertEqual(report["decision"], "unassisted_loss_suffix_repair_replay_partial_signal")
        self.assertTrue(report["summary"]["unassisted_loss_suffix_repair_replay_comparison_ready"])
    ...
    def test_all_full_pair_hits_mark_recovery_candidate(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            report = build_unassisted_loss_suffix_repair_replay_comparison_v1155(
                handoff,
                holdout_prompts=holdout_prompts(),
                generator_runner=fake_runner(["fixed loss", "fixed loss", "fixed loss", "fixed loss", "fixed loss"]),
            )

        self.assertEqual(report["decision"], "unassisted_loss_suffix_repair_replay_full_pair_recovered_candidate")
        self.assertEqual(report["summary"]["full_pair_case_count"], 5)
    ...
    def test_target_contaminated_holdout_prompt_fails_preflight(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            report = build_unassisted_loss_suffix_repair_replay_comparison_v1155(
                ...
                generator_runner=fake_runner(["fixed loss"] * 5),
            )
    ...
    def test_archived_handoff_falls_back_to_sibling_run_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            report = build_unassisted_loss_suffix_repair_replay_comparison_v1155(
                ...
                precomputed_generations=generation_rows(["fixed loss", "fixed loss", "fixed loss", "fixed loss", "fixed loss"]),
            )
    ...
    def test_outputs_and_cli_accept_precomputed_generations(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write_json_payload(generation_rows(["fixed loss", "fixed loss", "fixed loss", "fixed loss", "fixed loss"]), generations_path)
            report = build_unassisted_loss_suffix_repair_replay_comparison_v1155(
                handoff,
                holdout_prompts=holdout_prompts(),
                precomputed_generations=generation_rows(["fixed loss", "fixed loss", "fixed loss", "fixed loss", "fixed loss"]),
            )
            outputs = write_unassisted_loss_suffix_repair_replay_comparison_v1155_outputs(report, root / "replay")
            cli_main(
                [
                    "--handoff",
            ...
            self.assertEqual(locate_v1154_training_handoff(handoff_path.parent), handoff_path)
            self.assertEqual(set(outputs), {"json", "csv", "text", "markdown", "html", "generation_rows"})
            self.assertTrue(outputs["json"].endswith(f"{UNASSISTED_LOSS_SUFFIX_REPAIR_REPLAY_COMPARISON_V1155_STEM}.json"))
...
def holdout_prompts() -> list[dict[str, object]]:
    return [
        ...
        {"case_id": "holdout-05", "prompt": "review:", "expected_terms": ["fixed", "loss"], "max_new_tokens": 8, "temperature": 0.2, "top_k": 5},
    ]
