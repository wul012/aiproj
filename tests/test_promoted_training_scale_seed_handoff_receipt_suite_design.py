...
class PromotedTrainingScaleSeedHandoffReceiptSuiteDesignTests(unittest.TestCase):
    ...
    def test_script_embeds_suite_design_receipt_contract_through_assurance(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(receipt["handoff_batch_maturity_ci_regression_reason_counts"], {})
            self.assertEqual(receipt["selected_handoff_batch_maturity_ci_regression_reason_counts"], {})
            ...
            self.assertEqual(receipt_check["schema_version"], 5)
            self.assertEqual(receipt_check["handoff_batch_maturity_ci_regression_reason_counts"], {})
            self.assertEqual(receipt_check["handoff_batch_maturity_suite_design_regression_count"], 2)
            self.assertEqual(embedded_check["receipt_schema_version"], 5)
            self.assertEqual(embedded_check["receipt_handoff_batch_maturity_ci_regression_reason_counts"], {})
            self.assertEqual(embedded_check["receipt_handoff_batch_maturity_suite_design_regression_count"], 2)
            self.assertEqual(assurance["embedded_receipt_check_receipt_schema_version"], 5)
            self.assertEqual(assurance["embedded_receipt_check_receipt_handoff_batch_maturity_ci_regression_reason_counts"], {})
            self.assertEqual(
