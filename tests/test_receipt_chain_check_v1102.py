...
from minigpt.receipt_chain_check_v1102_artifacts import (
    render_check_v1102_artifacts_html,
    render_check_v1102_artifacts_markdown,
    render_check_v1102_artifacts_text,
    write_check_v1102_artifacts_outputs,
)
...
class RandomizedHoldoutPublicationReceiptIndexReceiptIndexReceiptIndexReceiptIndexReceiptIndexReceiptIndexReceiptCheckV1102Tests(unittest.TestCase):
    def test_receipt_check_accepts_rebuildable_receipt(self) -> None:
        with short_temp_dir() as tmp:
            ...
            report = build_receipt_check_v1102(
                receipt,
    ...
    def test_cli_require_pass_returns_one_on_tampered_receipt(self) -> None:
        with short_temp_dir() as tmp:
            ...
            self.assertTrue((root / "cli-check" / RECEIPT_CHECK_V1102_JSON_FILENAME).is_file())
    ...
    def test_outputs_and_cli_are_wired(self) -> None:
        with short_temp_dir() as tmp:
            ...
            outputs = write_check_v1102_artifacts_outputs(report, root / "check")
            cli_main([str(receipt_path.parent), "--out-dir", str(root / "cli-check"), "--require-pass", "--force"])

        self.assertEqual(set(outputs), {"json", "csv", "text", "markdown", "html"})
        self.assertTrue(outputs["json"].endswith(RECEIPT_CHECK_V1102_JSON_FILENAME))
        self.assertIn("contract_check_ready=True", render_check_v1102_artifacts_text(report))
        self.assertIn("Checks", render_check_v1102_artifacts_markdown(report))
        self.assertIn("receipt", render_check_v1102_artifacts_html(report))
...
def ready_check_inputs(root: Path) -> tuple[dict[str, object], Path]:
    ...
    outputs = write_receipt_v1101_artifacts_outputs(receipt, root / "receipt")
