...
Version 197 continues the focused benchmark scorecard helper cleanup in the artifact layer. `benchmark_scorecard_artifacts.py` now imports `as_dict` and `list_of_dicts` from `report_utils`, removing local `_dict` / `_list_of_dicts` helpers and making artifact exports keep valid rows even when mixed lists contain non-dict noise.
...
- Removed local `_dict()` and `_list_of_dicts()` helper definitions from `src/minigpt/benchmark_scorecard_artifacts.py`.
- Imported `as_dict as _dict` and `list_of_dicts as _list_of_dicts` from `src/minigpt/report_utils.py`, keeping artifact rendering call sites stable.
- Improved artifact-row tolerance: mixed component/drilldown/rubric lists now skip non-dict rows while preserving valid rows in CSV/Markdown/HTML outputs.
- Kept the scope deliberately small: this completes the benchmark scorecard artifact half of the helper cleanup, not a broad repo migration.
- Verified focused artifact and scorecard tests, syntax checks, full unittest discovery, source encoding, and docs/evidence alignment.
