...
Version 158 splits release gate profile comparison JSON/CSV/Markdown/HTML artifact writers into `release_gate_comparison_artifacts.py`. The comparison schema, CLI, and legacy `release_gate_comparison.py` exports stay unchanged, while the main comparison module drops from 581 lines to 284 lines.
...
| Local inference and UI | Playground server, checkpoint selector, streaming generation, cancellation/timeout controls, request history, pair artifacts, extracted server contracts, split playground assets, extracted generator class, extracted server log-event builders, extracted checkpoint/model-info payload builders | `server.py`, `server_checkpoints.py`, `server_logging.py`, `server_generator.py`, `server_contracts.py`, `request_history.py`, `pair_artifacts.py`, `playground.py`, `playground_assets.py`, `playground_style.py`, `playground_script.py`, server-checkpoint, server-logging, server-generator, server-contract, request-history, pair-artifact, and playground-asset tests plus Playwright screenshots | Keep HTTP routing and model generation stable while extracting pure contracts, generator, logging, and payload helpers |
...
- Added `src/minigpt/release_gate_comparison_artifacts.py` for JSON, CSV, delta CSV, Markdown, and HTML writers/renderers.
- Kept `build_release_gate_profile_comparison()` and all legacy `release_gate_comparison.py` exports compatible.
- Reduced `release_gate_comparison.py` from 581 lines to 284 lines by moving artifact output code to a dedicated module.
