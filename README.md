...
- Maintenance maturity: improving. v83-v108 reduced repeated report helpers through `report_utils`; v109 turns over-fragmented utility migrations into a runnable batching policy; v110 turns large-module concern into a runnable pressure report; v111-v136 split several high-pressure rendering, service, registry, dashboard, comparison, and artifact boundaries; v137-v141 enriched generation-quality and scorecard promotion evidence; v142-v145 hardened source and CI workflow hygiene after real CI findings; v146-v151 carry the new CI hygiene evidence through project audit, release bundle, release readiness, readiness comparison, registry tracking, and maturity summary review; v152 performs a small CI hygiene quality hardening without adding a new report layer; v153-v154 move release bundle and release readiness artifact rendering out of their builders; v155 moves server request-log event construction out of the HTTP handler; v156 moves checkpoint/model-info payload construction out of `server_contracts.py`; v157 moves registry leaderboard HTML fragments out of `registry_render.py`; v158 moves release gate profile comparison artifact writers out of the comparison builder; v159 moves server HTTP response/file helpers out of the route handler; v160 moves training scale promotion artifact writers out of the promotion decision builder; v161 moves dataset card artifact writers out of the dataset evidence builder; v162 moves model card artifact writers out of the project-level model summary builder; v163 moves benchmark scorecard comparison delta and summary logic out of the comparison entrypoint; v164 moves request-history HTTP endpoint handling out of the server route handler; v165 moves training scale plan artifact writers out of the scale planner; v166 moves project audit request-history and CI workflow hygiene context checks out of the audit builder; v167 moves playground request-history JavaScript out of the main playground script; v168 moves promoted seed artifact rendering out of the seed builder; v169 moves promoted seed handoff artifact rendering out of the execution handoff builder; v170 moves release readiness comparison artifact rendering out of the delta comparison builder; v171 moves GET route dispatch out of the HTTP handler; v172 moves eval suite artifact rendering out of the fixed-prompt benchmark builder; v173 moves training scale run decision artifact rendering out of the execute-candidate selector; v174 moves training scale gate artifact rendering out of the policy gate builder; v175 moves promoted training scale comparison artifact rendering out of the promoted-run comparison builder; v176 moves benchmark scorecard decision artifact writers out of the promotion-decision builder; v177 moves dashboard section and helper rendering out of the dashboard HTML entrypoint; v178 moves POST route handling out of the HTTP handler; v179 moves maturity narrative summary and section/evidence helpers out of the narrative entrypoint; v180 moves training-scale workflow artifact rendering out of the workflow orchestrator; v181 moves CI workflow hygiene artifact rendering out of the policy checker; v182 moves benchmark scorecard comparison section rendering out of its artifact writer; v183 moves module-pressure scanning out of maintenance batching/proposal policy; v184 moves benchmark scorecard comparison helper logic out of the deltas builder; v185 moves training scale promotion index helper logic out of the index builder; v186 switches back to functionality by turning real train output directories into promotion-review evidence; v187 connects that evidence to fixed eval suite coverage; v188 connects it to generation quality flags; v189 connects it to benchmark scorecard scoring; v190 removes generation-quality path friction in the scorecard builder so the real chain no longer needs manual copy glue; v191 adds portfolio-level same-checkpoint pair evidence while keeping scorecard language honest about reproducibility versus model improvement; v192 lets the same portfolio chain compare an external baseline checkpoint against the newly trained candidate; v193 applies that external baseline control to batch variant matrices; v194 splits training run evidence component logic out of the orchestration module; v195 moves repeated timestamp helpers in the training evaluation chain to `report_utils.utc_now`; v196 moves benchmark scorecard dict/list normalization helpers to `report_utils` and protects mixed-list pair-result handling with a focused test; v197 moves benchmark scorecard artifact dict/list helpers to `report_utils` and makes artifact row export tolerate mixed lists without dropping valid rows; v198 returns to model-eval by adding a standard Chinese benchmark prompt suite for broader repeated evaluation; v199 connects that suite into training portfolio and batch planning; v200 carries the same suite selection into training scale planning, gated runs, and consolidated workflow evidence; v201 carries the selected suite through promoted seed and seed-handoff evidence; v202 adds suite-consistency evidence to training-scale and promoted comparisons so mixed-suite deltas are not mistaken for clean model-quality gains; v203 turns that evidence into an opt-in strict decision guard for clean model-quality comparisons; v204 carries that evidence into the one-command training-scale workflow; v205 carries the same strict guard evidence into the controlled handoff report; v206-v261 propagate handoff suite/batch review context through promotion, indexes, promoted comparisons, promoted decisions, promoted seeds, and seed handoff; v262-v267 split several high-pressure promoted/scale artifact and helper boundaries; v268 adds an opt-in clean batch-review gate to standalone run decisions; v269 carries that gate into the consolidated workflow; v270 carries it into execution handoff validation; v271 carries it into promotion acceptance; v272 carries it into promotion indexing; v273 carries the clean batch-review guard into promoted comparison filtering, artifact output, CLI output, and tests; v274 carries clean batch-review evidence into promoted baseline decisions and rejects dirty clean-required promoted rows as baseline candidates; v275 carries selected clean batch-review evidence into promoted next-cycle seeds; v276 carries that same evidence into promoted seed handoff reports before the next plan command is executed; v277 adds an opt-in seed handoff clean batch-review requirement for automation; v278 summarizes requested seed handoff requirements in one automation gate; v279 turns that gate into a direct automation decision contract with exit code and requirement counts; v280 adds a top-level automation summary that combines gate and handoff execution blockers; v281 writes compact automation receipt artifacts for external scripts; v282 adds a receipt checker CLI for CI-style validation and exit behavior; v283 lets the checker discover receipts from output directories and write check artifacts; v284 lets the handoff execution script run that checker inline and archive receipt-check outputs before the final exit decision; v285 persists the same receipt-check result inside the handoff JSON/CSV/Markdown/HTML artifacts; v286 adds an independent embedded receipt-check verifier for the main handoff report; v287 extends that verifier to validate receipt/check sidecar files; v288 lets the handoff execution script run the embedded verifier inline and archive its outputs; v289 persists the inline embedded-check summary and output paths back into the main handoff JSON/CSV/Markdown/HTML artifacts; v290 adds a directory-level handoff assurance checker for the main report and embedded-check sidecars; v291 lets the handoff execution script run that assurance checker inline and embed its result back into the main report; v292 promotes the inline assurance chain into a GitHub Actions smoke step before coverage; v293 gives that smoke step its own summary JSON/text artifacts for easier CI review; v294 adds workflow hygiene checks that require the smoke step and its before-coverage order; v295 adds line-number evidence to that order check for easier CI drift diagnosis; v296 carries those order counts into CLI stdout and project audit context; v297 carries the same counts into release bundle and release readiness evidence; v298 compares those order-violation deltas across release readiness reports; v299 carries those order-violation deltas through registry and maturity summaries; v300 carries the same signal into maturity narrative review; v301 carries it into training portfolio comparison blocker actions; v302 carries that summary into training portfolio batch reports and CLI output. The latest module pressure smoke is `pass` with zero warn modules.
...
The assurance checker accepts the handoff output directory or main report JSON. It recomputes the embedded receipt-check, compares it with `embedded_receipt_check` in the main report, opens the embedded-check JSON/text sidecars referenced by `embedded_receipt_check_outputs`, and writes `promoted_training_scale_seed_handoff_assurance.json` plus `.txt` when requested. This is the compact CI/交接入口 for the receipt evidence chain.
