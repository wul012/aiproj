...
Version 172 splits eval suite artifact writers into `eval_suite_artifacts.py`. Prompt cases, prompt suite loading, prompt result construction, eval report schema, script usage, and old `minigpt.eval_suite` exports stay unchanged, while JSON, CSV, SVG, and HTML output rendering move to a focused artifact module.
...
- Added `src/minigpt/eval_suite_artifacts.py` for eval suite JSON, CSV, SVG, and HTML output writers.
- Kept prompt case validation, prompt suite loading, prompt result construction, eval report schema, `scripts/eval_suite.py` usage, and old `minigpt.eval_suite` exports intact.
- Reduced `eval_suite.py` from 517 lines to 322 lines by moving output rendering to a dedicated artifact module.
- Added direct facade identity coverage in `tests/test_eval_suite.py`.
- Confirmed full unittest discovery passes with 394 tests and module pressure remains `pass` with zero warn modules.
