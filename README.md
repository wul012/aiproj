...
Version 208 carries promotion-index handoff suite guard evidence into the promoted training-scale comparison. The promoted comparison now preserves `handoff_require_suite_consistency`, `handoff_suite_consistency`, `handoff_suite_mismatch_count`, and `handoff_selected_suite_path` per promoted input, summarizes handoff suite counts, and exposes the fields in JSON/CSV/Markdown/HTML/CLI output without changing comparison or baseline-selection semantics.
...
- Carried promotion-index handoff suite guard fields into promoted training-scale comparison rows.
- Added comparison summary counts for handoff strict-suite inputs, consistent handoff suites, total handoff mismatches, selected-suite references, and comparison-ready handoff mismatches.
- Exposed those fields in promoted comparison JSON, CSV, Markdown, HTML, and CLI stdout while preserving promoted-only comparison semantics.
- Added promoted comparison tests and CLI smoke proving downstream comparison reports retain the promotion-index suite boundary.
- Verified focused comparison tests, promoted comparison smoke, artifact-field smoke, syntax, full unittest discovery, source encoding, and docs/evidence alignment.
...
The promoted decision writes `promoted_training_scale_decision.json`, `promoted_training_scale_decision.csv`, `promoted_training_scale_decision.md`, and `promoted_training_scale_decision.html`. It selects a stable promoted baseline only after the promoted comparison has completed, records the selected suite path for the next seed, and marks the decision as `accepted`, `review`, or `blocked` based on readiness, gate status, batch status, suite evidence, and upstream comparison state. Use `--require-suite-consistency` when the promoted baseline must come from a same-suite comparison.
