...
Version 177 splits dashboard section/helper rendering into `dashboard_sections.py`. Dashboard payload building, artifact collection, dashboard writing, legacy `minigpt.dashboard.render_dashboard_html` export, and the public `minigpt.dashboard_render.render_dashboard_html` entrypoint stay unchanged, while style, stats grid, artifact cards, content sections, links, images, formatting, and escaping move to a focused section-rendering module.
...
- Added `src/minigpt/dashboard_sections.py` for dashboard style, stats grid, artifact cards, manifest/dataset/model/training/eval/pair/prediction/attention/chat sections, links, images, formatting, and escaping helpers.
- Kept dashboard payload building, artifact collection, `write_dashboard()`, `scripts/build_dashboard.py`, legacy `minigpt.dashboard.render_dashboard_html`, and public `minigpt.dashboard_render.render_dashboard_html` intact.
- Reduced `dashboard_render.py` from 492 lines to 82 lines by moving section/helper rendering to a dedicated module.
- Added direct section helper coverage in `tests/test_dashboard_render.py`.
- Confirmed full unittest discovery passes with 399 tests and module pressure remains `pass` with zero warn modules.
