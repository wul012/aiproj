...
Version 239 adds a non-blocking test coverage report path. CI now runs unittest through `scripts/run_test_coverage.py`, records coverage JSON/CSV/Markdown/HTML evidence, and leaves fail-under disabled until a baseline is reviewed.
...
- Added `scripts/run_test_coverage.py`.
- Added `minigpt.test_coverage_report` JSON/CSV/Markdown/HTML coverage evidence.
- CI now runs unittest through the coverage report script.
- Coverage remains non-blocking: `threshold_enabled=false` and no `fail_under`.
- Archived command-output evidence in `c/239`.
