...
Version 269 carries the clean batch-review gate into the consolidated training-scale workflow. The one-command workflow can now block execute selection when batch review actions, blockers, or maturity coverage regressions are present, while default workflow behavior remains compatible.
...
- `run_training_scale_workflow()` now accepts `decision_require_clean_batch_review`.
- `scripts/run_training_scale_workflow.py` exposes `--decision-require-clean-batch-review`.
- Workflow summary/artifacts now carry `decision_require_clean_batch_review` and `clean_batch_review_status`.
- Nested decision artifacts receive the same clean batch-review requirement, so one-command workflows can block review-mode execute selection.
- Archived command-output and workflow gate evidence in `c/269`.
