...
Version 246 carries release readiness coverage regressions into the run registry. Registry summaries, CSV, HTML stats, delta leaders, and CLI output now track `coverage-regressed` runs, coverage regression counts, coverage percent deltas, and coverage gap deltas.
...
- Registry run summaries now classify readiness comparisons with coverage regressions as `coverage-regressed`.
- Registry delta summaries count test coverage regressions and max coverage percent/gap deltas.
- Registry CSV, HTML stat cards, readiness cells, delta leaders, and CLI output expose coverage regression evidence.
- Focused registry tests and a registry smoke prove the new fields are machine-readable and human-visible.
- Archived command-output evidence in `c/246`.
