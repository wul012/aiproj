...
Version 206 carries the handoff suite guard into the training-scale promotion acceptance layer. `build_training_scale_promotion()` now reads `suite_guard` from `training_scale_handoff.json` and surfaces `handoff_require_suite_consistency`, `handoff_suite_consistency`, `handoff_suite_mismatch_count`, and `handoff_selected_suite_path` in promotion JSON/CSV/Markdown/HTML without changing the promoted/review/blocked decision rules.
...
- Carried handoff suite guard state into `build_training_scale_promotion()`.
- Added promotion summary fields for `handoff_require_suite_consistency`, `handoff_suite_consistency`, `handoff_suite_mismatch_count`, and `handoff_selected_suite_path`.
- Exposed those fields in promotion JSON, CSV, Markdown, and HTML while preserving the promoted/review/blocked status rules.
- Added promotion tests proving suite guard evidence reaches promotion artifacts without downgrading an otherwise promoted handoff.
- Verified focused promotion tests, promotion smoke, artifact-field smoke, syntax, full unittest discovery, source encoding, and docs/evidence alignment.
