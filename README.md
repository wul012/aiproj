...
Version 211 carries promoted training-scale seed handoff suite guard evidence into the promoted seed handoff layer. The seed handoff now preserves selected handoff suite guard fields alongside seed/plan suite paths, exposes them in JSON/CSV/Markdown/HTML/CLI output, and keeps command validation and execution semantics unchanged.
...
- Carried promoted training-scale seed handoff suite guard fields into promoted seed handoff evidence.
- Added seed handoff summary fields for selected handoff suite status, selected handoff mismatch count, selected handoff suite path, and upstream handoff mismatch totals.
- Exposed those fields in seed handoff JSON, CSV, Markdown, HTML, and CLI stdout while preserving command validation, planned-mode review, and execute-mode behavior.
- Consolidated same-meaning `first_present` fallbacks and repeated UTC timestamp wrappers through `report_utils` for a small contract-preserving helper cleanup.
- Added promoted seed handoff tests and CLI smoke proving the handoff entrypoint retains the seed's upstream suite boundary.
- Verified focused seed handoff tests, seed handoff smoke, artifact-field smoke, syntax, full unittest discovery, source encoding, and docs/evidence alignment.
...
Its summary compares `seed_suite_path` with `plan_suite_path` and keeps `selected_handoff_suite_consistency` / `handoff_suite_mismatch_total`, so a review can confirm that the standard suite and upstream clean-comparison boundary were preserved after executing the next-plan command.
