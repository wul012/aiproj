...
Version 300 carries CI workflow order-violation regressions into the maturity narrative so the final portfolio-facing review can flag smoke-before-coverage drift as a release-quality concern.
...
It also consumes maturity narrative release-readiness coverage regression fields: if the best-scoring portfolio is `coverage-regressed` or carries coverage regression counts, the comparison emits a blocker review action before that run can be promoted as the next clean baseline.
