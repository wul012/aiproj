...
Version 267 splits training-scale run comparison artifact writers into a focused module. Run loading, baseline selection, delta scoring, suite consistency, batch-review aggregation, and recommendations stay in the comparison builder while JSON/CSV/Markdown/HTML rendering moves to the artifact layer.
...
- `training_scale_run_comparison_artifacts.py` now owns run-comparison JSON/CSV/Markdown/HTML writers and HTML helper rendering.
- `training_scale_run_comparison.py` keeps run loading, baseline selection, readiness deltas, suite consistency, batch-review aggregation, and recommendations while re-exporting artifact functions for compatibility.
- Main comparison file is down from 542 to 311 lines; artifact writer sits at 249 lines.
- Archived command-output and artifact-boundary evidence in `c/267`.
