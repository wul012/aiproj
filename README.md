...
Version 250 carries the coverage governance signal into training portfolio comparison. A best-scoring portfolio is now blocked from promotion when its maturity narrative reports release-readiness test coverage regressions, and JSON/CSV/Markdown/HTML outputs expose the coverage review context.
...
- Training portfolio comparison now reads maturity narrative release-readiness coverage fields.
- Coverage-regressed portfolios are counted in the summary and named in Markdown/HTML outputs.
- A best-scoring portfolio with coverage regressions now gets a blocker review action instead of being silently promotable.
- CSV/Markdown/HTML comparison outputs include coverage trend, regression counts, and delta fields.
- Archived command-output evidence in `c/250`.
