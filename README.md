...
Version 160 splits training scale promotion JSON/CSV/Markdown/HTML artifact writers into `training_scale_promotion_artifacts.py`. The promotion decision schema, CLI, and legacy `training_scale_promotion.py` exports stay unchanged, while the main promotion module drops from 575 lines to 350 lines.
...
- Added `src/minigpt/training_scale_promotion_artifacts.py` for JSON, CSV, Markdown, and HTML promotion artifact writers/renderers.
- Kept `build_training_scale_promotion()`, `scripts/build_training_scale_promotion.py`, and legacy `training_scale_promotion.py` exports compatible.
- Reduced `training_scale_promotion.py` from 575 lines to 350 lines by moving artifact output code to a dedicated module.
