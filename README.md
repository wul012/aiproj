...
Version 240 turns the v239 coverage baseline into a conservative CI coverage gate. `scripts/run_test_coverage.py` now accepts `--fail-under`, writes the same JSON/CSV/Markdown/HTML evidence before exiting, and CI uses `--fail-under 80` while the observed local baseline remains around 90%.
...
- Added `--fail-under` support to `scripts/run_test_coverage.py`.
- Coverage reports now expose `status`, `decision`, `fail_under`, and `coverage_gap`.
- CI uses `--fail-under 80`, guarded by CI workflow hygiene.
- The gate writes evidence before returning failure, so blocked runs remain inspectable.
- Archived command-output evidence in `c/240`.
