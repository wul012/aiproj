...
Version 214 adds promoted seed handoff clean-evidence readiness. The seed handoff now turns the v212/v213 alignment verdict into machine-readable `seed_handoff_clean_evidence_ready`, `seed_handoff_clean_evidence_status`, and `seed_handoff_clean_evidence_detail` fields across summary, CSV, Markdown, HTML, and CLI output while keeping command validation and execution semantics unchanged.
...
- Added `seed_handoff_clean_evidence_ready`, `seed_handoff_clean_evidence_status`, and `seed_handoff_clean_evidence_detail` to promoted seed handoff summary output.
- Exported the clean-evidence readiness status through CSV, Markdown, HTML, and the CLI smoke output.
- Kept readiness as evidence metadata only: it does not change planned-mode validation or execute-mode command behavior.
- Added tests proving pending-plan, consistent, mismatch, and missing alignment map to `pending-plan`, `ready`, `review`, and `incomplete` readiness states.
- Verified focused seed handoff tests, clean-evidence smoke, artifact-field smoke, syntax, full unittest discovery, source encoding, and docs/evidence alignment.
