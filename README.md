...
Version 277 adds an opt-in clean batch-review requirement to promoted seed handoff automation. The seed handoff layer now persists `clean_batch_review_requirement`, prints strict CLI diagnostics, and exits non-zero only when `--require-clean-batch-review` is requested and selected clean-required handoff evidence is not clean.
...
Its summary compares `selected_handoff_selected_suite_path`, `seed_suite_path`, and `plan_suite_path`, emits `seed_handoff_suite_alignment_status`, turns `pending-plan`, `consistent`, `mismatch`, and `missing` into review recommendations, and exposes clean-evidence readiness/status-domain fields. The CLI also accepts `--require-clean-evidence` and `--require-clean-batch-review` for opt-in automation gates, prints structured gate diagnostics, and persists the gate decisions as `clean_evidence_requirement` and `clean_batch_review_requirement`; default command validation and execution behavior remain unchanged.
Library callers can pass `require_clean_evidence=True` or `require_clean_batch_review=True` to `build_promoted_training_scale_seed_handoff()` and receive the same requirement objects that the CLI writes to artifacts.
