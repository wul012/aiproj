...
Version 253 carries batch comparison review actions into gated scale runs and scale-run comparisons. The scale-run JSON/CSV/Markdown/HTML evidence now keeps batch review counts, blocker counts, coverage regressions, and blocker reasons visible when a run becomes promotion or readiness input.
...
- Gated scale runs now preserve batch comparison review counts, blocker counts, coverage regression counts, and blocker reasons in JSON/CSV/Markdown/HTML evidence.
- Scale-run comparisons aggregate those batch review fields and include them in readiness scoring context, Markdown/HTML summaries, CSV exports, and recommendations.
- Recommendations now warn when a scale run or run comparison still contains batch comparison review or blocker actions.
- Archived command-output evidence in `c/253`.
