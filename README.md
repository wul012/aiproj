...
Version 125 applies the v110 module pressure audit to the local inference server with a contract-preserving generator split. It keeps the same `minigpt.server.MiniGPTGenerator` import path, `create_handler()` default generator factory, HTTP routes, SSE events, request logging, and pair generation flow while moving the PyTorch checkpoint/tokenizer loading plus token generation class into `server_generator.py`.
...
- Added `server_generator.py` for `MiniGPTGenerator`, including checkpoint loading, tokenizer loading, device selection, full generation, and token streaming.
- Updated `server.py` to import and re-export `MiniGPTGenerator`, keeping `create_handler()` defaults and existing `from minigpt.server import MiniGPTGenerator` users stable.
- Added `tests/test_server_generator.py` to lock legacy export parity, default tokenizer-path resolution, explicit tokenizer-path handling, and lazy loading state.
- Kept HTTP routes, SSE event names, request-history logging, pair generation artifact writing, and server contract helpers unchanged.
- Used the v110 pressure check and v124 next-pressure note as evidence: `server.py` drops from 678 nonblank lines to 588 nonblank lines, warn modules drop from 2 to 1, and the next largest module becomes `comparison.py`.
