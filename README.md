...
Version 156 splits MiniGPT checkpoint discovery, health, model-info, checkpoint list, and checkpoint comparison payloads into `server_checkpoints.py`. The existing `server_contracts.py` and `server.py` facades still export the same checkpoint helpers, while module pressure stays `pass` with zero warn modules.
...
- Added `src/minigpt/server_checkpoints.py` for checkpoint discovery, health, model-info, checkpoint list, and checkpoint comparison payloads.
- Kept `src/minigpt/server_contracts.py` and `src/minigpt/server.py` as legacy facades for checkpoint helpers.
- Reduced `server_contracts.py` from 604 lines to 275 lines by moving checkpoint/model-info helper chains to a pure module.
- Added server checkpoint pure tests plus facade identity tests, while retaining existing HTTP server tests.
- Confirmed module pressure remains `pass` with zero warn modules; the largest module moves to `src/minigpt/registry_render.py`.
