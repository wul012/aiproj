...
Version 159 splits server HTTP response, SSE, static file, and JSON body helpers into `server_http.py`. The `create_handler()` routes, local inference APIs, and legacy `server.py` facade exports stay unchanged, while the main server module drops from 580 lines to 541 lines.
...
- Added `src/minigpt/server_http.py` for JSON/text responses, SSE headers/events, static file responses, request-body parsing, and guarded run-file serving.
- Kept `create_handler()`, `run_server()`, local inference routes, and legacy `server.py` facade exports compatible.
- Reduced `server.py` from 580 lines to 541 lines by moving pure HTTP helper behavior to a dedicated module.
- Added direct HTTP helper tests for response headers, JSON parsing, SSE formatting, file serving, and directory escape protection.
