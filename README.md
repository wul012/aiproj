...
Version 192 turns the v191 self-pair baseline into an optional real baseline-vs-candidate workflow. `run_training_portfolio.py` now accepts `--pair-baseline-checkpoint`, `--pair-baseline-tokenizer`, `--pair-baseline-id`, and `--pair-candidate-id`; when provided, portfolio pair batch compares that external baseline checkpoint against the newly trained candidate checkpoint, while the default remains the v191 same-checkpoint reproducibility baseline.
...
- Added optional pair baseline CLI flags to `scripts/run_training_portfolio.py`: `--pair-baseline-checkpoint`, `--pair-baseline-tokenizer`, `--pair-baseline-id`, and `--pair-candidate-id`.
- Added `pair_config` to training portfolio reports so JSON, Markdown, and HTML show whether the pair evidence is `same_checkpoint_baseline` or `external_baseline`.
- Updated `src/minigpt/pair_batch.py` so `comparison.same_checkpoint` prefers real checkpoint paths over display ids.
- Kept default behavior unchanged: without baseline flags, portfolio still uses the v191 same-checkpoint reproducibility baseline.
- Verified a real CPU PyTorch baseline checkpoint plus candidate portfolio: 11/11 steps completed, 18/18 artifacts present, `pair_config.mode=external_baseline`, scorecard pair mode `cross_checkpoint_or_unknown`, and pair same flags `[False]`.
