...
Version 226 adds eval-suite coverage readiness evidence. Fixed-prompt reports now expose representative prompt coverage and comparison readiness, so small ad hoc suites are separated from broader standard suites before making checkpoint-quality claims.
...
- Added eval-suite `coverage` evidence with representative prompt status and comparison readiness status.
- Checks recommended task types, recommended difficulties, minimum case count, tag breadth, hard-task comparison coverage, and comparison blockers.
- Exposes the same coverage block at the top level and under `benchmark.coverage` for downstream consumers.
- Renders coverage readiness in eval-suite HTML reports.
- Verifies that `builtin:standard-zh` is comparison-ready while tiny ad hoc suites remain warning-level evidence.
- Archived Playwright-rendered report evidence in `c/226`.
