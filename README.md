...
Version 164 splits request-history HTTP endpoint handling into `server_request_history.py`. The request-history JSON/CSV/detail contracts and `minigpt.server` facade exports stay unchanged, while `server.py` drops from 541 lines to 519 lines.
...
- Added `src/minigpt/server_request_history.py` for `/api/request-history` and `/api/request-history-detail` handling.
- Kept request-history JSON, CSV, detail, BAD_REQUEST, and NOT_FOUND behavior compatible through the existing server tests.
- Reduced `server.py` from 541 lines to 519 lines by moving request-history endpoint branching to a dedicated helper module.
- Added direct endpoint-helper coverage in `tests/test_server_request_history.py` alongside existing HTTP server integration tests.
