...
Version 215 hardens the promoted seed handoff clean-evidence readiness contract. The seed handoff now exposes a typed status domain for `ready`, `pending-plan`, `review`, and `incomplete`, carries that domain through summary, CSV, Markdown, HTML, and CLI output, and keeps command validation and execution semantics unchanged.
...
- Added `SeedHandoffCleanEvidenceStatus`, `SeedHandoffCleanEvidenceReadiness`, and `SEED_HANDOFF_CLEAN_EVIDENCE_STATUSES` to make clean-evidence readiness a stable typed contract.
- Added `seed_handoff_clean_evidence_status_domain` to summary, CSV, Markdown, HTML, and CLI output.
- Kept the v214 readiness semantics unchanged: the domain is schema evidence, not a blocker or execution policy.
- Added a focused contract test proving the public status domain remains `ready`, `pending-plan`, `review`, and `incomplete`.
- Verified focused seed handoff tests, status-domain smoke, artifact-field smoke, syntax, full unittest discovery, source encoding, and docs/evidence alignment.
...
Its summary compares `selected_handoff_selected_suite_path`, `seed_suite_path`, and `plan_suite_path`, emits `seed_handoff_suite_alignment_status`, and turns `pending-plan`, `consistent`, `mismatch`, and `missing` into review recommendations without changing command execution behavior.
