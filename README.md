...
Version 219 turns the promoted seed handoff clean-evidence gate result into a library-level contract and finishes the pending `utc_now` cleanup. The CLI now asks `build_promoted_training_scale_seed_handoff()` to attach `clean_evidence_requirement`, exported status domains include `not-required/pass/fail`, artifact renderers expose the requirement domain, and duplicate local `_utc_now()` wrappers have been replaced with `report_utils.utc_now`.
...
- Added `build_seed_handoff_clean_evidence_requirement()` as the reusable source of truth for `not-required` / `pass` / `fail`.
- Added `require_clean_evidence` to `build_promoted_training_scale_seed_handoff()` so CLI and library callers share one requirement contract.
- Exported `SEED_HANDOFF_CLEAN_EVIDENCE_REQUIREMENT_STATUSES` and added requirement status-domain output to CSV/Markdown/HTML.
- Removed remaining local `_utc_now()` wrappers that only forwarded to `report_utils.utc_now`.
- Verified focused seed handoff tests, requirement-domain smoke, duplicate `_utc_now` scan, full unittest discovery, source encoding, and docs/evidence alignment.
