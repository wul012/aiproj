...
Version 190 removes a real benchmark-chain path friction. `build_benchmark_scorecard()` now accepts generation quality evidence from both `generation_quality/` and `generation-quality/`, including the same variants under `eval_suite/`, so real train -> eval -> quality -> scorecard smoke runs no longer need a manual directory copy.
...
- Updated `src/minigpt/benchmark_scorecard.py` so scorecard loading accepts `generation_quality/generation_quality.json`, `generation-quality/generation_quality.json`, `eval_suite/generation_quality/generation_quality.json`, and `eval_suite/generation-quality/generation_quality.json`.
- Updated evidence completeness scoring to treat all four generation-quality locations as valid quality evidence.
- Added a focused scorecard regression test proving underscore-style `generation_quality/` works without a manual copy.
- Verified a real CPU PyTorch train -> eval suite -> generation quality -> benchmark scorecard -> training evidence chain with no generation-quality directory copy step.
- Kept the scorecard scoring rules unchanged; this is path compatibility and workflow friction removal, not a benchmark policy change.
