...
Version 155 splits MiniGPT server request-log event construction into `server_logging.py`. The existing `server.py` facade still exports the log-event helpers, while HTTP handlers keep routing and append duties and module pressure stays `pass` with zero warn modules.
...
- Added `src/minigpt/server_logging.py` for generation and pair-generation request-history event construction.
- Kept `src/minigpt/server.py` as the legacy facade for `build_generation_log_event` and `build_pair_generation_log_event`.
- Reduced handler responsibilities: HTTP code now routes, parses, sends responses, and appends logs; the event schema lives in a pure helper module.
- Added pure logging tests plus facade identity tests, while retaining existing HTTP server tests.
