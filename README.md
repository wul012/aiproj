...
Version 237 adds an opt-in CLI gate for training portfolio comparison blocker actions. Reports are still written first, but `--fail-on-blocker-action` now exits non-zero when structured review actions include blockers.
...
- `scripts/compare_training_portfolios.py` now prints review action and blocker counts.
- `--fail-on-blocker-action` exits non-zero after writing outputs when blocker actions exist.
- Default CLI behavior remains compatible and exits zero for review/blocker reports unless the flag is set.
- Tests cover both default and gated CLI behavior.
- Archived command-output evidence in `c/237`.
