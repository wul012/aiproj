...
Version 195 is a small helper consolidation across the training evaluation chain. `benchmark_scorecard.py`, `training_portfolio.py`, and `training_portfolio_batch.py` now import `utc_now` from `report_utils` instead of carrying identical local timestamp helpers, keeping generated-at and step timing behavior unchanged while reducing repeated utility code.
...
- Removed duplicate local `utc_now()` definitions from `src/minigpt/benchmark_scorecard.py`, `src/minigpt/training_portfolio.py`, and `src/minigpt/training_portfolio_batch.py`.
- Imported `utc_now` from `src/minigpt/report_utils.py` in those modules, keeping generated-at and step timing call sites unchanged.
- Kept the scope deliberately small: other modules with local `_utc_now()` helpers are left for later targeted migrations.
- Verified focused benchmark scorecard, training portfolio, and training portfolio batch tests still pass.
- Verified syntax checks, full unittest discovery, source encoding, and docs/evidence alignment.
