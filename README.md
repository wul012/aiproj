...
Version 259 carries promoted-comparison handoff batch review context into promoted training scale baseline decisions. The selected baseline, decision summary, CSV/Markdown/HTML outputs, CLI prints, and recommendations now keep selected batch review/blocker risk visible while preserving the existing accepted/review/blocked decision semantics.
...
- Promoted training scale baseline decision now preserves promoted-comparison handoff selected batch review status, selected review/blocker counts, and handoff-level batch comparison totals.
- CSV/Markdown/HTML decision outputs and CLI prints expose selected and comparison-ready batch review/blocker context beside suite guard and baseline choice evidence.
- Decision recommendations now warn when the selected promoted baseline still carries handoff batch blocker or review actions, without changing baseline acceptance semantics.
- Archived command-output and Playwright/Chrome HTML evidence in `c/259`.
...
The promoted seed writes `promoted_training_scale_seed.json`, `promoted_training_scale_seed.csv`, `promoted_training_scale_seed.md`, and `promoted_training_scale_seed.html`. It records the selected promoted baseline, selected handoff suite guard, validates the next corpus sources, and prints the follow-up `plan_training_scale.py` command only when the decision and corpus handoff are complete.
