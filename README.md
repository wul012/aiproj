...
Version 201 carries the standard Chinese benchmark suite selection one step further, into the promoted seed handoff. `plan_training_scale.py`, `run_training_scale_workflow.py`, and `build_promoted_training_scale_seed.py` now keep `standard-zh` visible through plan, gated run, workflow, seed, and seed-handoff evidence, while `--suite-name default` cleanly falls back to the default prompt file suite instead of inventing a fake builtin suite.
...
- Added suite inheritance to `build_promoted_training_scale_seed()`: selected promoted runs with `suite_path=builtin:standard-zh` generate next-cycle `plan_training_scale.py ... --suite-name standard-zh` commands.
- Added `--suite` / `--suite-name` overrides to `scripts/build_promoted_training_scale_seed.py`, with `--suite-name default` returning to the default file suite instead of producing `builtin:default`.
- Extended promoted seed and seed-handoff JSON/CSV/Markdown/HTML summaries with baseline, seed, and generated-plan suite paths.
- Hardened `training_scale_plan.py` and `training_portfolio.py` default suite-name handling so direct API and CLI paths share the same default-suite semantics.
- Verified focused promoted seed/handoff, portfolio, batch, and training-scale tests, real seed/handoff smoke, syntax, full unittest discovery, source encoding, and docs/evidence alignment.
