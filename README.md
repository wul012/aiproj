...
Version 220 carries the promoted seed handoff clean-evidence requirement into recommendations. When `require_clean_evidence=True`, the report now records a clear pass/fail action alongside the existing alignment recommendation, so JSON/Markdown/HTML readers can see whether the handoff is usable as clean comparison evidence or must be executed/fixed first.
...
- Added clean-evidence requirement recommendations when `require_clean_evidence=True`.
- Records `Clean-evidence requirement passed` when the handoff is clean evidence ready.
- Records `Clean-evidence requirement failed` plus readiness detail when the gate is required but still pending/review/incomplete.
- Kept default `require_clean_evidence=False` recommendations unchanged to avoid report noise.
- Verified focused seed handoff tests, recommendation smoke, full unittest discovery, source encoding, and docs/evidence alignment.
