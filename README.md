...
Version 228 carries scorecard eval-suite readiness into benchmark scorecard comparisons. Cross-run scorecard comparison reports now expose baseline eval comparison status, non-comparison-ready run counts, and recommendations that keep score deltas provisional when eval evidence is not comparison-ready.
...
- Scorecard comparison run summaries now carry `eval_suite_coverage_status` and `eval_suite_comparison_status`.
- Comparison summary now records baseline eval comparison status, known readiness count, non-comparison-ready run count, and affected run names.
- CSV, Markdown, and HTML comparison artifacts expose eval comparison readiness next to score/rubric deltas.
- Recommendations mark scorecard deltas as provisional when any compared run is not eval-suite comparison-ready.
- Baseline-specific recommendation warns when the baseline itself is not comparison-ready.
- Archived command-output evidence in `c/228`.
