...
Version 184 splits benchmark scorecard comparison helper logic out of `benchmark_scorecard_comparison_deltas.py` into `benchmark_scorecard_comparison_helpers.py`. The scorecard comparison report schema, builder functions, script behavior, artifact writers, and legacy imports stay unchanged, while the deltas module returns to comparison builders and the helpers module owns relation/normalization/explanation plumbing.
...
- Added `src/minigpt/benchmark_scorecard_comparison_helpers.py` for scorecard comparison relation, normalization, explanation, and helper plumbing.
- Kept `benchmark_scorecard_comparison_deltas.py` as the public builder facade for run/case/group deltas, summary, recommendations, and best-run selection.
- Reduced `benchmark_scorecard_comparison_deltas.py` to builder orchestration by moving helper logic into the new module.
- Added facade identity tests so old comparison imports still resolve to the same public delta builders.
- Confirmed focused comparison tests, source encoding hygiene, and full unittest discovery pass.
