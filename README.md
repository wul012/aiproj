...
Version 178 splits MiniGPT server POST route handling into `server_post_routes.py`. `/api/generate`, `/api/generate-stream`, `/api/generate-pair`, and `/api/generate-pair-artifact` keep the same request parsing, checkpoint resolution, generation, SSE timeout/cancel behavior, pair artifact writing, request logging, and legacy `minigpt.server` facade exports, while `server.py` returns to handler assembly and server startup.
...
- Added `src/minigpt/server_post_routes.py` for `/api/generate`, `/api/generate-stream`, `/api/generate-pair`, and `/api/generate-pair-artifact` POST route handling.
- Kept `create_handler()`, `run_server()`, request logging, HTTP helpers, checkpoint discovery, request parsing, pair artifact writing, SSE timeout/cancel behavior, and legacy `minigpt.server` exports intact.
- Reduced `server.py` from 496 lines to 240 lines by moving POST route dispatch and generation flow into a dedicated module.
- Added `tests/test_server_post_routes.py` for facade identity, normal generation helper flow, and pair artifact helper flow.
- Confirmed full unittest discovery passes with 402 tests and module pressure remains `pass` with zero warn modules.
