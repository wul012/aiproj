...
Version 241 carries the coverage gate result into project audit evidence. `scripts/audit_project.py` can now read `test_coverage_report.json`, project audit summaries expose coverage status/percent/fail-under/gap, and the audit keeps missing or failing coverage evidence as review-level governance context.
...
- Project audit can read `test_coverage_report.json`.
- Audit summaries now expose coverage status, decision, percent, fail-under, and gap.
- Audit Markdown/HTML surfaces coverage context beside CI workflow and request history context.
- `scripts/audit_project.py` accepts `--test-coverage-report`.
- Archived command-output evidence in `c/241`.
