...
Version 191 closes the remaining pair-evidence gap in the real training portfolio chain. `build_training_portfolio_plan()` now runs `scripts/pair_batch.py` after generation-quality analysis and before scorecard construction, using the trained checkpoint as a same-checkpoint reproducibility baseline. `build_benchmark_scorecard()` marks that mode as `same_checkpoint_baseline` and caps pair component scores at 90 so this self-check is not overstated as cross-checkpoint model improvement.
...
- Updated `src/minigpt/training_portfolio.py` so execute mode runs `scripts/pair_batch.py` between generation quality and benchmark scorecard.
- Added `pair_batch` and `pair_batch_html` artifacts to the portfolio plan, giving scorecard a direct `pair_batch/pair_generation_batch.*` evidence group.
- Updated `src/minigpt/benchmark_scorecard.py` and artifact renderers so same-checkpoint pair evidence is labeled as `same_checkpoint_baseline`.
- Capped same-checkpoint pair consistency and delta-stability component scores at 90, preserving reproducibility value without presenting it as cross-checkpoint model improvement.
- Verified a real CPU PyTorch portfolio run: 11/11 steps completed, 18/18 artifacts present, scorecard status `pass`, pair mode `same_checkpoint_baseline`.
