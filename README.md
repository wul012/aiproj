...
Version 217 adds structured diagnostics to the promoted seed handoff clean-evidence CLI gate. Default validation and execution behavior stay unchanged, and `--require-clean-evidence` now prints the required status, ready flag, and detail before reporting pass/fail.
...
- Added `clean_evidence_required_status`, `clean_evidence_required_ready`, and `clean_evidence_required_detail` lines when `--require-clean-evidence` is used.
- Kept `clean_evidence_required=pass/fail` and the v216 exit-code behavior unchanged.
- Preserved default seed handoff behavior when the flag is not provided.
- Added focused assertions for both passing and failing gate diagnostics.
- Verified focused seed handoff tests, gate-diagnostics smoke, artifact smoke, syntax, full unittest discovery, source encoding, and docs/evidence alignment.
