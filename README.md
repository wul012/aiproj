...
Version 264 splits promoted decision handoff review helper logic into a focused module. The promoted baseline decision contract stays unchanged while selected handoff suite and batch-review summary/recommendation logic leaves the main decision builder.
...
- `promoted_training_scale_decision_review.py` now owns promoted decision selected handoff suite/batch-review summary projection and batch-review recommendations.
- `promoted_training_scale_decision.py` keeps the baseline selection and artifact surface; it is down from 716 to 600 lines, with rendering still a later split target.
- Added focused helper tests for selected context, derived comparison-ready totals, and recommendation precedence.
- Archived command-output and helper-boundary evidence in `c/264`.
