...
Version 142 hardens the source encoding hygiene gate against local/CI Python parser drift. `source_encoding_hygiene.py` now records a target Python version, flags Python 3.12+ same-quote f-string expression patterns before they reach Python 3.11 CI, and exposes compatibility status in JSON/CSV/Markdown/HTML reports plus the CLI output.
...
- Added `target_python` to `build_source_encoding_report` and the source encoding policy payload, defaulting to Python 3.11 to match CI.
- Added a compatibility scanner for Python 3.12+ same-quote f-string expression syntax that can pass on newer local Python but fail on Python 3.11 CI.
- Added `compatibility_ok`, `compatibility_error`, `compatibility_error_count`, and `compatibility_error_paths` to JSON/CSV/Markdown/HTML reports.
- Updated `scripts/check_source_encoding.py` with `--target-python`, plus CLI output for `compatibility_error_count` and `target_python`.
- Strengthened `tests/test_source_encoding_hygiene.py` across Python 3.13 and Python 3.11 paths, including HTML visibility for compatibility-only offenders.
- Reran source encoding tests, Python 3.11 tests, source encoding smoke, maintenance smoke, compile checks, and full unittest discovery for `c/142`.
