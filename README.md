...
Version 243 carries coverage gate evidence into release gate policy. Standard/review/strict release gate profiles now require the `test_coverage_report` audit check, legacy remains compatible with older bundles, and gate reports expose coverage status/percent/fail-under/gap beside release and audit status.
...
- Standard/review/strict release gate profiles now require the `test_coverage_report` audit check.
- Legacy profile still allows older bundles without coverage audit evidence.
- Gate summaries and Markdown/HTML outputs expose coverage status, percent, fail-under, and gap.
- `scripts/check_release_gate.py` and profile comparison support `--allow-missing-test-coverage`.
- Archived command-output evidence in `c/243`.
