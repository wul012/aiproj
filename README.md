...
Version 174 splits training scale gate artifact writers into `training_scale_gate_artifacts.py`. Policy profiles, plan loading, gate checks, status summaries, recommendations, script/workflow usage, and old `minigpt.training_scale_gate` exports stay unchanged, while JSON, CSV, Markdown, and HTML output rendering move to a focused artifact module.
...
- Added `src/minigpt/training_scale_gate_artifacts.py` for training scale gate JSON, CSV, Markdown, and HTML output writers.
- Kept policy profiles, plan loading, gate check construction, status summaries, recommendations, `scripts/check_training_scale_gate.py`, workflow usage, and old `minigpt.training_scale_gate` exports intact.
- Reduced `training_scale_gate.py` from 508 lines to 314 lines by moving output rendering to a dedicated artifact module.
- Added direct facade identity coverage in `tests/test_training_scale_gate.py`.
- Confirmed full unittest discovery passes with 396 tests and module pressure remains `pass` with zero warn modules.
