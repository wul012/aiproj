...
Version 216 adds an opt-in clean-evidence requirement to the promoted seed handoff CLI. Default validation and execution behavior stay unchanged, but automation can now pass `--require-clean-evidence` to fail unless `seed_handoff_clean_evidence_ready` is true.
...
- Added `--require-clean-evidence` to `scripts/execute_promoted_training_scale_seed.py`.
- Kept default seed handoff behavior unchanged: planned-mode validation and execute-mode command behavior still work without requiring clean evidence.
- Made the new flag exit non-zero only when explicitly requested and `seed_handoff_clean_evidence_ready` is false.
- Added focused tests proving a completed, consistent handoff passes the requirement while a pending-plan handoff fails only when the flag is enabled.
- Verified focused seed handoff tests, opt-in gate smoke, artifact smoke, syntax, full unittest discovery, source encoding, and docs/evidence alignment.
