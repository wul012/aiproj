...
Version 182 splits benchmark scorecard comparison Markdown/HTML section rendering into `benchmark_scorecard_comparison_sections.py`. The comparison report schema, JSON/CSV artifact writers, case-delta CSV, script behavior, and legacy imports from `benchmark_scorecard_comparison.py` and `benchmark_scorecard_comparison_artifacts.py` stay unchanged, while the artifact module returns to file writing and output orchestration.
...
- Added `src/minigpt/benchmark_scorecard_comparison_sections.py` for benchmark scorecard comparison Markdown/HTML section rendering.
- Kept comparison report schema, JSON/CSV writers, case-delta CSV, script behavior, and old comparison/artifact imports intact.
- Reduced `benchmark_scorecard_comparison_artifacts.py` from 460 lines to 150 lines by leaving file writing and output orchestration in the artifact module.
- Added facade identity tests so old render imports from `benchmark_scorecard_comparison.py` and `benchmark_scorecard_comparison_artifacts.py` stay tied to the new section renderer.
- Confirmed focused comparison tests, source encoding hygiene, and maintenance module pressure remain clean, with full unittest discovery passing.
