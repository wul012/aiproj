...
Version 233 exposes the best-scoring portfolio's maturity status in training portfolio comparison Markdown and HTML outputs, so the review boundary added in v232 is visible in human-facing reports as well as JSON.
...
- Markdown summary now shows `Best score maturity`.
- HTML stats now show `Best score maturity`.
- Existing comparison and artifact tests assert the human-facing outputs expose the v232 review guard.
- Keeps JSON behavior unchanged while improving report readability.
- Archived command-output evidence in `c/233`.
