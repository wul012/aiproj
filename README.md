...
Version 268 adds an opt-in clean batch-review gate to training-scale run decisions. Default decisions still allow review-mode evidence, while automation can now require zero batch review actions, blockers, and maturity coverage regressions before an execute command is selected.
...
- Maintenance maturity: improving. v83-v108 reduced repeated report helpers through `report_utils`; v109 turns over-fragmented utility migrations into a runnable batching policy; v110 turns large-module concern into a runnable pressure report; v111-v136 split several high-pressure rendering, service, registry, dashboard, comparison, and artifact boundaries; v137-v141 enriched generation-quality and scorecard promotion evidence; v142-v145 hardened source and CI workflow hygiene after real CI findings; v146-v151 carry the new CI hygiene evidence through project audit, release bundle, release readiness, readiness comparison, registry tracking, and maturity summary review; v152 performs a small CI hygiene quality hardening without adding a new report layer; v153-v154 move release bundle and release readiness artifact rendering out of their builders; v155 moves server request-log event construction out of the HTTP handler; v156 moves checkpoint/model-info payload construction out of `server_contracts.py`; v157 moves registry leaderboard HTML fragments out of `registry_render.py`; v158 moves release gate profile comparison artifact writers out of the comparison builder; v159 moves server HTTP response/file helpers out of the route handler; v160 moves training scale promotion artifact writers out of the promotion decision builder; v161 moves dataset card artifact writers out of the dataset evidence builder; v162 moves model card artifact writers out of the project-level model summary builder; v163 moves benchmark scorecard comparison delta and summary logic out of the comparison entrypoint; v164 moves request-history HTTP endpoint handling out of the server route handler; v165 moves training scale plan artifact writers out of the scale planner; v166 moves project audit request-history and CI workflow hygiene context checks out of the audit builder; v167 moves playground request-history JavaScript out of the main playground script; v168 moves promoted seed artifact rendering out of the seed builder; v169 moves promoted seed handoff artifact rendering out of the execution handoff builder; v170 moves release readiness comparison artifact rendering out of the delta comparison builder; v171 moves GET route dispatch out of the HTTP handler; v172 moves eval suite artifact rendering out of the fixed-prompt benchmark builder; v173 moves training scale run decision artifact rendering out of the execute-candidate selector; v174 moves training scale gate artifact rendering out of the policy gate builder; v175 moves promoted training scale comparison artifact rendering out of the promoted-run comparison builder; v176 moves benchmark scorecard decision artifact writers out of the promotion-decision builder; v177 moves dashboard section and helper rendering out of the dashboard HTML entrypoint; v178 moves POST route handling out of the HTTP handler; v179 moves maturity narrative summary and section/evidence helpers out of the narrative entrypoint; v180 moves training-scale workflow artifact rendering out of the workflow orchestrator; v181 moves CI workflow hygiene artifact rendering out of the policy checker; v182 moves benchmark scorecard comparison section rendering out of its artifact writer; v183 moves module-pressure scanning out of maintenance batching/proposal policy; v184 moves benchmark scorecard comparison helper logic out of the deltas builder; v185 moves training scale promotion index helper logic out of the index builder; v186 switches back to functionality by turning real train output directories into promotion-review evidence; v187 connects that evidence to fixed eval suite coverage; v188 connects it to generation quality flags; v189 connects it to benchmark scorecard scoring; v190 removes generation-quality path friction in the scorecard builder so the real chain no longer needs manual copy glue; v191 adds portfolio-level same-checkpoint pair evidence while keeping scorecard language honest about reproducibility versus model improvement; v192 lets the same portfolio chain compare an external baseline checkpoint against the newly trained candidate; v193 applies that external baseline control to batch variant matrices; v194 splits training run evidence component logic out of the orchestration module; v195 moves repeated timestamp helpers in the training evaluation chain to `report_utils.utc_now`; v196 moves benchmark scorecard dict/list normalization helpers to `report_utils` and protects mixed-list pair-result handling with a focused test; v197 moves benchmark scorecard artifact dict/list helpers to `report_utils` and makes artifact row export tolerate mixed lists without dropping valid rows; v198 returns to model-eval by adding a standard Chinese benchmark prompt suite for broader repeated evaluation; v199 connects that suite into training portfolio and batch planning; v200 carries the same suite selection into training scale planning, gated runs, and consolidated workflow evidence; v201 carries the selected suite through promoted seed and seed-handoff evidence; v202 adds suite-consistency evidence to training-scale and promoted comparisons so mixed-suite deltas are not mistaken for clean model-quality gains; v203 turns that evidence into an opt-in strict decision guard for clean model-quality comparisons; v204 carries that guard into the one-command training-scale workflow so consolidated runs cannot bypass it; v205 carries the same strict guard evidence into the controlled handoff report so reviewed execute commands keep the suite boundary visible; v206 carries that handoff suite guard into promotion acceptance evidence without changing promotion status semantics; v207 carries that promotion acceptance evidence one step upstream into the promotion index so compare-ready selection keeps the handoff suite boundary visible; v208 carries the same index evidence into promoted-only comparison so downstream comparison reports keep the handoff boundary visible while still relying on real run comparison for scoring; v209 carries the selected promoted comparison handoff guard into baseline decision evidence so the chosen baseline keeps both real suite path and upstream handoff context; v210 carries that decision handoff guard into the next-cycle promoted seed while preserving command-generation semantics; v211 carries the seed's selected handoff guard into the seed handoff report without changing command validation or execution; v212 turns those carried suite paths into an explicit seed-handoff alignment verdict without making it a blocker; v213 turns that verdict into review recommendations while preserving the same execution rules; v214 turns the same verdict into machine-readable clean-evidence readiness fields without changing blocker semantics; v215 hardens the readiness status domain as a typed, exported contract; v216 adds an opt-in CLI clean-evidence requirement for automation; v217 adds structured diagnostics to that gate; v218 persists the gate decision into handoff artifacts; v219 promotes that requirement decision to a reusable library contract and removes remaining duplicate `_utc_now()` wrappers; v220 carries requirement pass/fail into recommendations for clearer automation and human review; v221 starts targeted numeric helper consolidation with explicit none/default semantics; v222 extends the same numeric helper consolidation into benchmark scorecard scoring, comparison, and promotion decision helpers; v223 carries the numeric helper consolidation into generation-quality analysis and artifact rendering; v224 carries the same helper consolidation into release-gate policy checks and artifact rendering; v225 carries the helper consolidation into training-scale run and promotion chains; v226 returns to model-eval evidence by adding eval-suite coverage and comparison readiness; v227 lets benchmark scorecards consume that readiness before treating eval coverage as pass evidence; v228 carries the same readiness into cross-scorecard comparison reports and recommendations; v229 carries readiness into scorecard promotion decisions as review evidence; v230 carries that decision readiness into the maturity narrative and portfolio review status; v231 adds an integration test that protects the full eval-readiness chain; v232 carries maturity-review status into training portfolio comparison best-score decisions; v233 exposes the same best-score maturity status in human-facing comparison outputs; v234 refines the recommendation language for best-score versus non-leading maturity reviews; v235 adds named maturity-review portfolio tracking to the comparison summary and outputs; v236 turns portfolio comparison concerns into structured review actions; v237 adds an opt-in CLI gate for blocker review actions; v238 adds Dependabot automation for Actions and Python dependency updates; v239 records a non-blocking unittest coverage baseline; v240 turns that baseline into a conservative `--fail-under 80` CI coverage gate; v241 carries that coverage gate status into project audit evidence; v242 carries the same gate status into release bundle handoff evidence; v243 makes the release gate policy consume that coverage evidence; v244 surfaces the same evidence in release readiness dashboards; v245 compares that coverage evidence across readiness dashboards; v246-v249 carry coverage regressions through registry, maturity summary, maturity narrative, and an integration test; v250 makes training portfolio comparison consume that signal before promoting the best-scoring run; v251 splits the portfolio review and recommendation logic out of the comparison builder while preserving the same output contract; v252 carries those review actions back into training portfolio batch summaries and recommendations; v253 carries batch comparison review/blocker counts into gated scale runs and scale-run comparisons; v254 carries the selected-run batch review status into run decisions and execute-command evidence; v255 carries the same decision batch review context into controlled handoff validation and execution evidence; v256 carries handoff batch review context into promotion acceptance evidence; v257 carries that same handoff batch review context into promotion indexes without changing promoted-only compare semantics; v258 carries it into promoted comparisons so compare-ready inputs keep selected batch review/blocker risk visible; v259 carries it into promoted baseline decisions so the selected baseline keeps that risk visible; v260 carries it into promoted next-cycle seeds so the generated plan handoff keeps that risk visible; v261 carries it into promoted seed handoff reports so validation/execution evidence keeps that risk visible; v262 splits the seed handoff review and clean-evidence helper logic out of the orchestration module so the boundary stays under the module-pressure threshold; v263 splits promoted seed suite-guard and batch-review helper logic out of the next-cycle seed orchestration module; v264 splits promoted decision selected handoff review helper logic out of the baseline decision builder; v265 splits promoted decision artifact writers out of the baseline decision builder; v266 splits training-scale handoff artifact writers out of the validation/execution builder; v267 splits training-scale run comparison artifact writers out of the comparison builder; v268 adds an opt-in clean batch-review gate to run decisions. The latest module pressure smoke is `pass` with zero warn modules.
...
- `build_training_scale_run_decision()` now accepts `require_clean_batch_review`.
- `scripts/decide_training_scale_run.py` exposes `--require-clean-batch-review` for automation that wants clean execute evidence.
- Decision summaries and artifacts now include `require_clean_batch_review` and `clean_batch_review_status`.
- Default behavior stays unchanged: review-mode batch evidence can still produce a review decision.
- Archived command-output and gate evidence in `c/268`.
