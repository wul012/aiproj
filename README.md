...
Version 170 splits release readiness comparison artifact writers into `release_readiness_comparison_artifacts.py`. The comparison schema, baseline override, status/panel/CI workflow delta logic, registry consumption, and old `minigpt.release_readiness_comparison` exports stay unchanged, while `release_readiness_comparison.py` drops from 521 lines to 242 lines.
...
- Added `src/minigpt/release_readiness_comparison_artifacts.py` for release readiness comparison JSON, CSV, delta CSV, Markdown, and HTML output writers.
- Kept the existing comparison schema, baseline override, status/panel/CI workflow delta logic, registry consumption, and `minigpt.release_readiness_comparison` facade exports intact.
- Reduced `release_readiness_comparison.py` from 521 lines to 242 lines by moving output rendering to a dedicated artifact module.
- Added direct facade identity coverage in `tests/test_release_readiness_comparison.py`.
- Confirmed module pressure remains `pass` with zero warn modules.
