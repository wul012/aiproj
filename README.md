...
Version 254 carries batch comparison review actions into the training scale run decision layer. Decision JSON/CSV/Markdown/HTML evidence now records selected-run batch review status, selected blocker/review counts, comparison-level review totals, and recommendation text before generating execute commands.
...
- Run decisions now preserve selected-run batch review status, selected review/blocker counts, and comparison-level batch review totals.
- CSV/Markdown/HTML decision artifacts expose those fields so execute commands keep the upstream batch comparison risks visible.
- A selected run with batch comparison blocker actions is downgraded to `review` instead of being presented as fully `ready`.
- Archived command-output evidence in `c/254`.
