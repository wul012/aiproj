...
Version 352 adds exact-source dedupe and reproducible snapshot evidence to prepared dataset versions, so larger corpus preparation can skip duplicate source files while preserving included/skipped source counts and skip reasons.
...
| Data and experiment governance | Dataset preparation, dataset quality, dataset cards with extracted artifact writers, run manifests, experiment cards, model cards with extracted artifact writers, project audits and release bundles with extracted request-history, benchmark-history, CI-workflow hygiene, coverage, and tiny scorecard plan digest gate context helpers | `data_prep`, `data_quality`, `dataset_card`, `dataset_card_artifacts`, `manifest`, `experiment_card`, `model_card`, `model_card_artifacts`, `project_audit`, `project_audit_contexts`, `release_bundle` modules and tests | Dataset version scale, dedupe policy, reproducible corpus snapshots |
