...
Version 200 carries the standard Chinese benchmark suite selection into the training scale workflow. `plan_training_scale.py` and `run_training_scale_workflow.py` now accept `--suite-name standard-zh`, scale plans record `suite={"mode":"builtin","path":"builtin:standard-zh"}`, gated runs hand that suite to the portfolio batch layer, and plan/run/workflow reports display the chosen suite path.
...
- Added `suite_name` and `suite_path` support to `build_training_scale_plan()`, recording `suite.mode/name/path` and the compatibility field `suite_path`.
- Updated generated training-scale batch handoff commands to include `--suite-name standard-zh` when a built-in suite is selected.
- Carried suite metadata through `run_training_scale_plan()` and `run_training_scale_workflow()` summaries so plan, gated run, and workflow reports show `builtin:standard-zh`.
- Added `--suite` and `--suite-name standard-zh` to `scripts/plan_training_scale.py` and `scripts/run_training_scale_workflow.py`.
- Verified focused training-scale plan/run/workflow tests, plan/workflow smoke commands, syntax, full unittest discovery, source encoding, and docs/evidence alignment.
