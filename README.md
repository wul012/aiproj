...
Version 209 carries promoted-comparison handoff suite guard evidence into the promoted baseline decision. The baseline selector now preserves the selected run's handoff suite guard fields, summarizes upstream handoff suite counts, and exposes them in JSON/CSV/Markdown/HTML/CLI output without changing strict suite-consistency enforcement or baseline scoring semantics.
...
- Carried promoted-comparison handoff suite guard fields into promoted baseline decision rows and selected baseline evidence.
- Added decision summary fields for selected handoff suite status, selected handoff mismatch count, selected handoff suite path, and upstream handoff suite totals.
- Exposed those fields in decision JSON, CSV, Markdown, HTML, and CLI stdout while preserving strict suite-consistency and baseline scoring semantics.
- Added promoted decision tests and CLI smoke proving selected baseline evidence retains the promoted-comparison handoff boundary.
- Verified focused decision tests, promoted decision smoke, artifact-field smoke, syntax, full unittest discovery, source encoding, and docs/evidence alignment.
