...
- Shared report utility helpers for JSON/CSV output, artifact-row checks, command display, Markdown cells, HTML escaping, and defensive list/dict normalization, now used by gated training scale run, training scale run comparison, training scale run decision, promoted baseline decision, promoted seed, promoted seed handoff, and controlled training scale handoff modules
...
- Use the v75-v89 workflow/handoff/promotion/index/promoted-comparison/decision/seed/seed-handoff/report-utility chain on a real larger Chinese corpus.
