...
Version 271 carries the handoff clean batch-review requirement into training-scale promotion acceptance. Promotion reports now preserve the handoff clean batch-review guard, render it in artifacts, print it from the CLI, and block promotion when a clean batch-review requirement is not satisfied.
...
- `build_training_scale_promotion()` now builds a `clean_batch_review_guard` from handoff evidence.
- Promotion acceptance blocks when handoff requires clean batch-review evidence but the status is not `clean`.
- Promotion CSV/Markdown/HTML now render `handoff_require_clean_batch_review` and `handoff_clean_batch_review_status`.
- `scripts/build_training_scale_promotion.py` prints `handoff_clean_batch_review_status`.
- Archived command-output and promotion gate evidence in `c/271`.
