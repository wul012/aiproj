...
Version 154 splits release readiness artifact rendering into `release_readiness_artifacts.py`. The existing `release_readiness.py` facade still exports the same render/write functions, while module pressure stays `pass` with zero warn modules after moving dashboard JSON/Markdown/HTML output helpers out of the readiness builder.
...
- Added `src/minigpt/release_readiness_artifacts.py` for release readiness JSON/Markdown/HTML rendering and writing.
- Kept `src/minigpt/release_readiness.py` as the legacy facade for `render_release_readiness_*` and `write_release_readiness_*`.
- Reduced `release_readiness.py` from 628 lines to roughly 415 lines by moving dashboard artifact helpers and HTML style into the artifact module.
- Added facade identity tests so scripts and existing imports keep working.
