...
Version 171 splits MiniGPT server GET route handling into `server_routes.py`. Health, checkpoints, checkpoint comparison, model-info, request-history, playground, and run-file GET semantics stay unchanged, while POST generation, streaming, pair generation, logging, and the old `minigpt.server` facade exports remain in `server.py`.
...
- Added `src/minigpt/server_routes.py` for GET route dispatch covering health, checkpoints, checkpoint comparison, model-info, request-history, playground, and run-file serving.
- Kept POST generation, streaming generation, pair generation, request logging, generator caching, HTTP helpers, and old `minigpt.server` facade exports intact.
- Reduced `server.py` from 519 lines to 491 lines by moving GET dispatch to a focused route module.
- Added direct facade identity coverage in `tests/test_server_routes.py`.
- Confirmed full unittest discovery passes with 393 tests and module pressure remains `pass` with zero warn modules.
