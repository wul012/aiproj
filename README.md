...
Version 198 adds a standard Chinese benchmark prompt suite. The default five-case suite remains compatible, while `standard_zh_prompt_suite()`, `builtin:standard-zh`, `data/standard_zh_eval_prompts.json`, and `--suite-name standard-zh` give eval and pair-batch workflows a broader ten-case benchmark covering continuation, QA, summary, structured output, reasoning, classification, rewrite, safety boundary, self-check, and comparison prompts.
...
- Added `src/minigpt/eval_suites.py` with `standard_zh_prompt_suite()` and built-in suite name resolution.
- Added `data/standard_zh_eval_prompts.json`, matching the built-in standard suite for file-based workflows.
- Added `load_builtin_prompt_suite()` and `builtin:standard-zh` support while keeping the existing default suite and `data/eval_prompts.json` compatible.
- Added `--suite-name standard-zh` to `scripts/eval_suite.py` and `scripts/pair_batch.py`.
- Verified standard suite coverage, data-file parity, script help, syntax, full unittest discovery, source encoding, and docs/evidence alignment.
