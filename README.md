...
Version 210 carries promoted baseline decision handoff suite guard evidence into the promoted training-scale seed. The next-cycle seed now preserves selected handoff suite guard fields alongside the inherited suite path, summarizes them in seed outputs, and exposes them in JSON/CSV/Markdown/HTML/CLI output without changing next-plan command generation.
...
- Carried promoted baseline decision handoff suite guard fields into promoted training-scale seed evidence.
- Added seed summary fields for selected handoff suite status, selected handoff mismatch count, selected handoff suite path, and upstream handoff mismatch totals.
- Exposed those fields in seed JSON, CSV, Markdown, HTML, and CLI stdout while preserving next-plan command generation and inherited suite path behavior.
- Added promoted seed tests and CLI smoke proving next-cycle seed evidence retains the baseline-decision handoff boundary.
- Verified focused seed tests, promoted seed smoke, artifact-field smoke, syntax, full unittest discovery, source encoding, and docs/evidence alignment.
...
Its summary also compares `seed_suite_path` with `plan_suite_path`, so a review can confirm that the standard suite was preserved after executing the next-plan command.
