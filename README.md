...
- Shared report utility helpers for JSON/CSV output, artifact-row checks, command display, Markdown cells, HTML escaping, and defensive list/dict normalization, now used by generation quality, promoted training scale comparison, training scale promotion index, training scale promotion, training scale workflow, training scale planner, training scale gate, gated training scale run, training scale run comparison, training scale run decision, promoted baseline decision, promoted seed, promoted seed handoff, and controlled training scale handoff modules
