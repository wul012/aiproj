...
Version 221 starts a targeted numeric-helper consolidation. `report_utils` now exposes `number_or_none()` and `number_or_default()`, and the first migration covers training run evidence and training scale gate conversion helpers while preserving their different `None` versus default-zero semantics.
...
- Added `number_or_none()` and `number_or_default()` to `report_utils`.
- Migrated `training_run_evidence_components.py` to use `number_or_none()` while preserving missing-value semantics.
- Migrated `training_scale_gate.py` to use `number_or_default()` while preserving default-zero gate behavior.
- Added report-utils tests for empty strings, `None`, booleans, bad values, ints, and floats.
- Verified focused report-utils/training evidence/gate tests, helper scans, full unittest discovery, source encoding, and docs/evidence alignment.
