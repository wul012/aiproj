...
Version 144 upgrades the GitHub Actions workflow to Node 24 native action versions. `actions/checkout@v6` and `actions/setup-python@v6` both advertise `using: node24`, so the CI no longer relies on the v143 force-runtime environment variable that still left a Node.js 20 annotation.
...
- Upgraded `.github/workflows/ci.yml` from `actions/checkout@v4` to `actions/checkout@v6`.
- Upgraded `.github/workflows/ci.yml` from `actions/setup-python@v5` to `actions/setup-python@v6`.
- Removed `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24`, because v143 showed that forcing Node 24 still leaves a Node 20 target annotation.
- Updated `tests/test_ci_workflow.py` to assert the workflow uses Node 24 native action majors and no longer uses the force-runtime env.
- Reran the CI workflow test, source encoding smoke, compile checks, maintenance smoke, full unittest discovery, and action metadata checks for `c/144`.
