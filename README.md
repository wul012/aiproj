...
Version 203 turns v202 suite-consistency evidence into an optional strict decision guard. Training-scale run decision and promoted baseline decision now accept `--require-suite-consistency`, blocking mixed or missing benchmark-suite comparisons only when a clean model-quality comparison is required, while the default mode still preserves governance triage compatibility.
...
- Added `require_suite_consistency` to training-scale run decision and promoted baseline decision builders.
- Added `--require-suite-consistency` to `scripts/decide_training_scale_run.py` and `scripts/decide_promoted_training_scale_baseline.py`.
- Kept default behavior compatible: mixed-suite comparisons remain usable for governance triage, with warnings.
- When strict mode is enabled, mixed or missing suite comparisons reject all candidates and record `benchmark suite consistency is ...` rejection reasons.
- Verified focused strict-guard tests, strict CLI smokes, syntax, full unittest discovery, source encoding, and docs/evidence alignment.
