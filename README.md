...
Version 213 adds alignment-aware promoted seed handoff recommendations. The seed handoff now turns `pending-plan`, `consistent`, `mismatch`, and `missing` suite alignment verdicts into concrete review actions while keeping command validation and execution semantics unchanged.
...
- Added alignment-aware seed handoff recommendations for `pending-plan`, `consistent`, `mismatch`, and `missing` suite verdicts.
- Kept recommendations as review guidance only: they do not change planned-mode validation or execute-mode command behavior.
- Added tests proving mismatch and missing alignment produce review recommendations without blocking handoff execution.
- Preserved existing seed handoff artifact rendering because recommendations already flow through Markdown and HTML outputs.
- Verified focused seed handoff tests, recommendation smoke, artifact-field smoke, syntax, full unittest discovery, source encoding, and docs/evidence alignment.
