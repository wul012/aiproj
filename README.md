...
Version 186 adds training run evidence reports for real MiniGPT checkpoint directories. `scripts/build_training_run_evidence.py` reads a run produced by `scripts/train.py`, checks checkpoint/tokenizer/config/metrics/manifest/sample/eval evidence, writes JSON/CSV/Markdown/HTML outputs, and is now inserted into the training portfolio pipeline immediately after the real train step.
...
- Added `src/minigpt/training_run_evidence.py` and `src/minigpt/training_run_evidence_artifacts.py` to summarize real run directories into JSON/CSV/Markdown/HTML evidence.
- Added `scripts/build_training_run_evidence.py` for CLI review of checkpoint, tokenizer, train config, metrics, run manifest, sample, and optional eval-suite evidence.
- Inserted `training_run_evidence` into `build_training_portfolio_plan()` immediately after the train step, so portfolio execution now records whether a real checkpoint run is promotion-review ready.
- Verified a real CPU PyTorch smoke run that produced checkpoint, tokenizer, metrics, manifest, loss curve, and sample output, then converted it into a `review` evidence report because eval-suite evidence was intentionally not present yet.
- Confirmed focused training-run evidence and training-portfolio tests, source encoding hygiene, and Playwright/Chrome HTML rendering.
