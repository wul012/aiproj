...
Version 258 carries promotion-index handoff batch review context into promoted training scale comparison. Promoted comparison rows, summaries, CSV/Markdown/HTML outputs, CLI prints, and recommendations now keep comparison-ready selected batch review/blocker risk visible while preserving promoted-only compare semantics.
...
- Promoted training scale comparison now preserves promotion-index handoff selected batch review status, selected review/blocker counts, and handoff-level batch comparison totals.
- CSV/Markdown/HTML comparison outputs and CLI prints expose comparison-ready batch review/blocker counts beside suite guard and promoted-only comparison evidence.
- Promoted comparison recommendations now warn when compared promoted inputs still carry selected handoff batch review/blocker actions, without changing promoted-only compare selection.
- Archived command-output evidence in `c/258`.
...
The promoted decision writes `promoted_training_scale_decision.json`, `promoted_training_scale_decision.csv`, `promoted_training_scale_decision.md`, and `promoted_training_scale_decision.html`. It selects a stable promoted baseline only after the promoted comparison has completed, records the selected suite path and selected handoff suite guard for the next seed, and marks the decision as `accepted`, `review`, or `blocked` based on readiness, gate status, batch status, suite evidence, and upstream comparison state. Use `--require-suite-consistency` when the promoted baseline must come from a same-suite comparison.
