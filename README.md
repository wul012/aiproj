...
Version 148 carries the CI workflow hygiene evidence from release bundle into the release readiness dashboard. The dashboard now resolves `ci_workflow_hygiene.json`, shows a dedicated CI Workflow Hygiene panel, prints CI workflow status from the CLI, and treats failed hygiene as a review signal rather than a release gate hard block.
...
| Release and maturity governance | Registry, project audit, release bundle with request-history and CI-workflow hygiene evidence, release gate profiles, release readiness dashboards with CI workflow hygiene context, maturity summaries and narratives | release, readiness, maturity, audit tests plus versioned screenshots | Keep governance useful while avoiding more report-only fragmentation |
...
- Added optional `ci_workflow_hygiene_path` input to `build_release_readiness_dashboard`.
- Let release readiness resolve `ci_workflow_hygiene.json` from release bundle inputs or the default `runs/ci-workflow-hygiene` location.
- Added a dedicated `CI Workflow Hygiene` readiness panel with status, failed checks, and Node 24 native action count.
- Kept CI workflow hygiene failures as `review` warnings, not release gate hard blocks.
- Added `--ci-workflow-hygiene` to `scripts/build_release_readiness.py` and CLI status output.
