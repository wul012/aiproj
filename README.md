...
Version 244 carries coverage gate evidence into the release readiness dashboard. The dashboard can read `test_coverage_report.json`, fall back to bundle coverage context, and surface coverage status/percent/fail-under/gap as a dedicated readiness panel plus Markdown/HTML summary fields.
...
- Release readiness dashboards can read `test_coverage_report.json`.
- Readiness summaries now expose coverage status, percent, fail-under, and gap.
- A dedicated `Test Coverage Gate` panel uses either the report or bundle summary/context fallback.
- `scripts/build_release_readiness.py` accepts `--test-coverage-report`.
- Archived command-output evidence in `c/244`.
