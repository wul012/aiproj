...
Version 342 lifts the CI tiny scorecard plan digest gate into CI workflow hygiene summaries and project audit context, so downstream governance can consume the gate without scanning raw check rows.
...
- Model capability maturity: medium. The architecture and evaluation loop are real, and v342 makes CPU tiny scorecard plan digest gate readiness visible to downstream governance, but the tiny run remains plumbing evidence rather than a model-quality claim.
