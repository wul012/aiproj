...
Version 162 splits model card JSON/Markdown/HTML artifact writers into `model_card_artifacts.py`. The model card schema, CLI, and legacy `model_card.py` exports stay unchanged, while the main model card module drops from 556 lines to 257 lines.
...
- Added `src/minigpt/model_card_artifacts.py` for JSON, Markdown, and HTML model card writers/renderers.
- Kept `build_model_card()`, `scripts/build_model_card.py`, and legacy `model_card.py` exports compatible.
- Reduced `model_card.py` from 556 lines to 257 lines by moving artifact output code to a dedicated module.
- Added facade identity coverage so old imports continue to point at the artifact implementation.
