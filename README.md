...
Version 180 splits training-scale workflow JSON/CSV/Markdown/HTML rendering and writing into `training_scale_workflow_artifacts.py`. The workflow JSON schema, nested plan/run/comparison/decision artifacts, CLI behavior, and legacy `training_scale_workflow.py` imports stay unchanged, while the main module returns to orchestration, profile execution, summary calculation, and recommendation assembly.
...
- Added `src/minigpt/training_scale_workflow_artifacts.py` for workflow JSON/CSV/Markdown/HTML rendering and writing.
- Kept `run_training_scale_workflow()`, workflow JSON schema, nested artifact directories, script behavior, and legacy workflow imports intact.
- Reduced `training_scale_workflow.py` from 485 lines to 248 lines by moving artifact output helpers to the dedicated module.
- Added facade identity tests so old imports from `minigpt.training_scale_workflow` stay tied to the new artifact implementation.
- Confirmed full unittest discovery passes with 404 tests and module pressure remains `pass` with zero warn modules.
