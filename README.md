...
Version 204 carries the strict suite-consistency guard into the consolidated training-scale workflow. `run_training_scale_workflow()` and `scripts/run_training_scale_workflow.py` now accept `decision_require_suite_consistency` / `--decision-require-suite-consistency`, and the workflow JSON/CSV/Markdown/HTML summaries show whether the workflow decision required same-suite comparison evidence.
...
- Added `decision_require_suite_consistency` to `run_training_scale_workflow()`.
- Added `--decision-require-suite-consistency` to `scripts/run_training_scale_workflow.py`.
- Carried strict suite guard state into workflow JSON, summary, CSV, Markdown, and HTML stats.
- Added workflow tests proving the strict guard reaches the nested training-scale decision artifact.
- Verified focused workflow/decision tests, workflow CLI smoke, syntax, full unittest discovery, source encoding, and docs/evidence alignment.
