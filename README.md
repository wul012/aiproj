...
Version 193 carries the v192 external baseline pair workflow into `run_training_portfolio_batch.py`. Batch plans now accept the same baseline checkpoint/tokenizer/id parameters, let variants override them, and show pair mode counts in JSON, CSV, Markdown, and HTML so a matrix of candidate runs does not silently fall back to same-checkpoint self-checks.
...
- Added batch-level pair baseline CLI flags to `scripts/run_training_portfolio_batch.py`: `--pair-baseline-checkpoint`, `--pair-baseline-tokenizer`, `--pair-baseline-id`, and `--pair-candidate-id`.
- Added these keys to training portfolio batch variant overrides, so a matrix can share one baseline or override a specific variant baseline/id.
- Added `summary.pair_mode_counts` and per-variant `pair_mode` output so JSON, CSV, Markdown, and HTML expose whether variants are using `same_checkpoint_baseline` or `external_baseline`.
- Kept default behavior unchanged: without baseline flags, batch variants still use the v191/v192 same-checkpoint reproducibility baseline.
- Verified a batch dry-run using default variants and a shared external baseline: 2/2 variants planned with `pair_mode_counts={"external_baseline": 2}`, and batch CSV/Markdown/HTML all show external baseline mode.
