...
Version 205 carries the strict suite-consistency guard from the consolidated workflow into the controlled handoff layer. `build_training_scale_handoff()` and `scripts/execute_training_scale_handoff.py` now surface `decision_require_suite_consistency`, `suite_consistency`, `suite_mismatch_count`, and `selected_suite_path` in the handoff JSON/CSV/Markdown/HTML evidence, so the reviewed execute command no longer loses the workflow's same-suite boundary.
...
- Carried strict suite guard state from the workflow decision into `build_training_scale_handoff()`.
- Added `suite_guard` plus `decision_require_suite_consistency`, `suite_consistency`, `suite_mismatch_count`, and `selected_suite_path` to handoff JSON, CSV, Markdown, and HTML evidence.
- Added handoff tests proving the workflow/decision suite guard reaches the reviewed execute command evidence.
- Verified focused handoff tests, planned handoff smoke, executed handoff smoke, artifact-field smoke, syntax, full unittest discovery, source encoding, and docs/evidence alignment.
