...
Version 185 splits training scale promotion index helper logic out of `training_scale_promotion_index.py` into `training_scale_promotion_index_helpers.py`. The promotion index schema, JSON/CSV/Markdown/HTML outputs, compare-command construction, script behavior, and public API stay unchanged, while the index module returns to loading, building, rendering, and artifact writing.
...
- Added `src/minigpt/training_scale_promotion_index_helpers.py` for promotion row construction, name resolution, baseline selection, compare-command inputs, summary, and recommendations.
- Kept `training_scale_promotion_index.py` as the public builder and artifact facade for loading promotions, rendering Markdown/HTML, and writing JSON/CSV/Markdown/HTML outputs.
- Reduced `training_scale_promotion_index.py` to index orchestration and report presentation by moving internal helper logic into the new module.
- Added helper smoke coverage so compare-input and summary helpers stay aligned with the public index builder.
- Confirmed focused training scale promotion index tests, source encoding hygiene, and full unittest discovery pass.
