...
Version 194 is a contract-preserving training run evidence refactor. `training_run_evidence.py` has been reduced from 580 lines to 151 lines by moving readiness checks, summary construction, section builders, recommendations, and value helpers into `training_run_evidence_components.py`, while the public builder and artifact writer facade remain unchanged.
...
- Added `src/minigpt/training_run_evidence_components.py` for readiness checks, summary construction, section builders, recommendations, and value conversion helpers.
- Reduced `src/minigpt/training_run_evidence.py` from 580 lines to 151 lines so it focuses on file loading, artifact row collection, and report assembly.
- Kept the public API unchanged: `build_training_run_evidence()` and all training run evidence artifact writer re-exports still live behind `training_run_evidence.py`.
- Left behavior unchanged: existing training run evidence tests still assert ready/review/blocked states, eval/quality/scorecard summaries, rendered outputs, and HTML escaping.
- Verified focused training run evidence tests, syntax checks, full unittest discovery, source encoding, and docs/evidence alignment.
