...
Version 227 carries eval-suite coverage readiness into benchmark scorecards. Scorecards now keep legacy case-count behavior for old eval artifacts, while using v226 `coverage.comparison_status` to warn on suites that are runnable but not comparison-ready.
...
- Benchmark scorecards now read eval-suite `coverage` from either `report.coverage` or `benchmark.coverage`.
- Legacy eval artifacts without coverage still use the old case-count score, preserving existing runs.
- New eval artifacts with `comparison_status != pass` cap eval coverage at warning-level score.
- Scorecard summary now exposes eval-suite coverage and comparison statuses.
- Recommendations now call out comparison-readiness blockers before checkpoint quality claims.
- Archived command-output evidence in `c/227`.
