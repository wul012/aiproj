...
Version 282 adds a promoted seed handoff automation receipt checker CLI, so CI can validate receipt structure, print stable key/value diagnostics, and exit from the receipt decision.
...
```powershell
python scripts/check_promoted_seed_handoff_receipt.py runs/training-scale-workflow/promoted-seed-handoff/promoted_training_scale_seed_handoff_automation_receipt.json
```

The checker prints `receipt_check_status`, `receipt_decision`, `receipt_exit_code`, `receipt_blocking_source`, and failed requirements. By default it exits non-zero for invalid receipts and for valid `stop` decisions; pass `--allow-stop` when a CI job only wants to validate receipt structure.
