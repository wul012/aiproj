...
Version 179 splits maturity narrative summary and section/evidence helpers into `maturity_narrative_summary.py` and `maturity_narrative_sections.py`. The narrative JSON schema, recommendations, warnings, Markdown/HTML artifact writers, CLI behavior, and legacy `maturity_narrative.py` entrypoint stay unchanged, while the main module returns to input discovery, JSON loading, and report assembly.
...
- Added `src/minigpt/maturity_narrative_summary.py` for portfolio summary, recommendations, warnings, scorecard/dataset summaries, and portfolio status calculation.
- Added `src/minigpt/maturity_narrative_sections.py` for human-readable narrative sections, section claims, status-from-counts, and evidence matrix rows.
- Kept `build_maturity_narrative()`, JSON schema, artifact writers, CLI behavior, and legacy maturity narrative imports intact.
- Reduced `maturity_narrative.py` from 488 lines to 109 lines by moving summary and section/evidence helpers to dedicated modules.
- Confirmed full unittest discovery passes with 403 tests and module pressure remains `pass` with zero warn modules.
