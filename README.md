...
Version 242 carries the coverage gate result from project audit into release bundle evidence. `scripts/build_release_bundle.py` can now read `test_coverage_report.json`, release bundle summaries expose coverage status/percent/fail-under/gap, and bundle artifacts include coverage JSON/Markdown/HTML evidence for release-style handoff.
...
- Release bundles can read `test_coverage_report.json`.
- Bundle summaries now expose coverage status, decision, percent, fail-under, and gap.
- Bundle inputs, artifacts, and context carry coverage report paths plus JSON/Markdown/HTML evidence.
- `scripts/build_release_bundle.py` accepts `--test-coverage-report`.
- Archived command-output evidence in `c/242`.
