...
Version 189 enriches real training run evidence with benchmark scorecard summaries. `scripts/build_training_run_evidence.py` now reads `benchmark-scorecard/benchmark_scorecard.json`, records overall benchmark status/score, rubric status and average score, weakest rubric case, weakest task/difficulty groups, and carries those fields into JSON/CSV/Markdown/HTML reports and CLI output.
...
- Enriched `src/minigpt/training_run_evidence.py` with a `scorecard` section that reads `benchmark-scorecard/benchmark_scorecard.json` from a real run directory.
- Added benchmark overall status/score, component count, rubric status and average score, weakest rubric case, weakest task type, weakest difficulty, dominant generation flag, and generation flag count to training run evidence.
- Extended JSON/CSV/Markdown/HTML outputs and `scripts/build_training_run_evidence.py` CLI output so reviewers can see consolidated benchmark risk without opening a separate scorecard report.
- Preserved the boundary from v188: missing or non-pass benchmark scorecard is a non-blocking `review` warning, not a core training artifact failure.
- Verified a real CPU PyTorch train smoke followed by fixed eval suite, generation quality analysis, benchmark scorecard generation, and training-run evidence generation.
