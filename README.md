...
Version 167 splits the playground request-history JavaScript fragment into `playground_request_history_script.py`. The rendered playground HTML, request-history query/detail/export contracts, and `playground_script()` entrypoint stay unchanged, while `playground_script.py` drops from 532 lines to 389 lines.
...
- Added `src/minigpt/playground_request_history_script.py` for the request-history table, filters, CSV link, detail URL, detail panel, and render/load helpers.
- Kept the existing `playground_script(page_data)` entrypoint and rendered playground contracts intact.
- Reduced `playground_script.py` from 532 lines to 389 lines by moving a stable UI sub-boundary into a dedicated fragment module.
- Added direct request-history fragment coverage in `tests/test_playground_request_history_script.py` and a full-script inclusion assertion in `tests/test_playground_asset_modules.py`.
