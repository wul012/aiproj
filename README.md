...
Version 181 splits CI workflow hygiene JSON/CSV/Markdown/HTML rendering and writing into `ci_workflow_hygiene_artifacts.py`. The CI workflow policy checks, TypedDict report schema, action parsing, script behavior, and legacy `ci_workflow_hygiene.py` imports stay unchanged, while the main module returns to workflow parsing, checks, summary construction, and recommendations.
...
- Added `src/minigpt/ci_workflow_hygiene_artifacts.py` for CI workflow hygiene JSON/CSV/Markdown/HTML rendering and writing.
- Kept `build_ci_workflow_hygiene_report()`, TypedDict report schema, action parsing, policy checks, script behavior, and legacy CI hygiene imports intact.
- Reduced `ci_workflow_hygiene.py` from 470 lines to 263 lines by moving artifact output helpers to the dedicated module.
- Added facade identity tests so old imports from `minigpt.ci_workflow_hygiene` stay tied to the new artifact implementation.
- Confirmed full unittest discovery passes with 405 tests and module pressure remains `pass` with zero warn modules.
