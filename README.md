...
Version 252 carries training portfolio comparison review actions back into the batch layer. Batch reports now summarize comparison review counts, blocker counts, coverage-regressed portfolios, and blocker reasons so variant matrix runs cannot hide downstream baseline-selection risks.
...
- Batch execution now records `comparison_review_summary` after writing the portfolio comparison.
- Batch `execution` includes `comparison_review_action_count` and `comparison_blocker_action_count`.
- Markdown/HTML batch reports show review actions, blocker actions, maturity reviews, coverage-regressed portfolios, and blocker reasons.
- Recommendations now prioritize comparison blocker/review actions before telling the user to choose a baseline.
- Archived command-output evidence in `c/252`.
