...
Version 175 splits promoted training scale comparison artifact writers into `promoted_training_scale_comparison_artifacts.py`. Promotion index loading, promoted-run filtering, comparison input resolution, baseline handling, training-scale run comparison reuse, blockers, recommendations, script usage, and old `minigpt.promoted_training_scale_comparison` exports stay unchanged, while JSON, CSV, Markdown, and HTML output rendering move to a focused artifact module.
...
- Added `src/minigpt/promoted_training_scale_comparison_artifacts.py` for promoted training scale comparison JSON, CSV, Markdown, and HTML output writers.
- Kept promotion index loading, promoted-run filtering, comparison input resolution, baseline handling, training-scale run comparison reuse, blockers, recommendations, `scripts/compare_promoted_training_scale_runs.py`, and old `minigpt.promoted_training_scale_comparison` exports intact.
- Reduced `promoted_training_scale_comparison.py` from 499 lines to 236 lines by moving output rendering to a dedicated artifact module.
- Added direct facade identity coverage in `tests/test_promoted_training_scale_comparison.py`.
- Confirmed full unittest discovery passes with 397 tests and module pressure remains `pass` with zero warn modules.
