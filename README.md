...
Version 245 carries coverage gate evidence into release readiness comparison. Multi-readiness comparisons now track coverage status, percent, fail-under, gap, percent delta, gap delta, changed coverage panels, and test coverage regression counts.
...
| Release and maturity governance | Registry, project audit, release bundle with request-history and CI-workflow hygiene evidence, split release bundle artifacts, release gate profiles using shared numeric helpers, release readiness dashboards with split artifact writers, release readiness comparisons with split artifact writers, registry tracking with CI workflow hygiene context, maturity summaries and narratives with CI workflow regression context | `release_gate.py`, `release_gate_artifacts.py`, `release_readiness.py`, `release_readiness_artifacts.py`, `release_readiness_comparison.py`, `release_readiness_comparison_artifacts.py`, release, readiness, maturity, audit tests plus versioned screenshots | Keep governance useful while avoiding more report-only fragmentation |
...
- Release readiness comparison rows now include coverage status, percent, fail-under, and gap.
- Deltas now include coverage percent and gap changes.
- Comparison summary counts test coverage regressions.
- Markdown/HTML/CSV outputs surface coverage columns and regression recommendations.
- Archived command-output evidence in `c/245`.
