...
Version 202 adds suite-consistency evidence to training-scale and promoted-run comparisons. Run comparison, run decision, promoted comparison, and promoted baseline decision reports now show whether compared runs used the same benchmark suite, which suite path was selected, and when mixed suites should be treated as governance triage rather than clean model-quality deltas.
...
- Added `suite_consistency`, `suite_paths`, `suite_mismatch_count`, and `suite_relation` to training-scale run comparison evidence.
- Carried comparison suite status into training-scale run decision reports, including `selected_suite_path` and mixed-suite recommendations.
- Carried suite path and suite consistency into promoted comparison and promoted baseline decision reports.
- Added focused mixed-suite tests for ordinary training-scale comparison/decision and promoted comparison/decision.
- Verified focused suite-consistency tests, mixed-suite smokes, syntax, full unittest discovery, source encoding, and docs/evidence alignment.
...
The promoted decision writes `promoted_training_scale_decision.json`, `promoted_training_scale_decision.csv`, `promoted_training_scale_decision.md`, and `promoted_training_scale_decision.html`. It selects a stable promoted baseline only after the promoted comparison has completed, records the selected suite path for the next seed, and marks the decision as `accepted`, `review`, or `blocked` based on readiness, gate status, batch status, suite evidence, and upstream comparison state.
