...
Version 223 extends targeted numeric-helper consolidation into the generation-quality chain. Generation-quality analysis and artifact rendering now consume `report_utils.number_or_none()` while preserving existing float-or-missing ratio semantics.
...
| Release and maturity governance | Registry, project audit, release bundle with request-history and CI-workflow hygiene evidence, split release bundle artifacts, release gate profiles, release readiness dashboards with split artifact writers, release readiness comparisons with split artifact writers, registry tracking with CI workflow hygiene context, maturity summaries and narratives with CI workflow regression context | `release_readiness.py`, `release_readiness_artifacts.py`, `release_readiness_comparison.py`, `release_readiness_comparison_artifacts.py`, release, readiness, maturity, audit tests plus versioned screenshots | Keep governance useful while avoiding more report-only fragmentation |
...
- Migrated generation-quality analysis `_number()` to `report_utils.number_or_none()`.
- Migrated generation-quality artifact rendering `_number()` to `report_utils.number_or_none()`.
- Preserved ratio labels, average calculations, missing-value handling, and generation-quality artifact schema.
- Verified focused generation-quality/report-utils tests, helper scans, full unittest discovery, source encoding, and docs/evidence alignment.
