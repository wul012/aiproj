...
Version 222 extends targeted numeric-helper consolidation into the benchmark scorecard chain. Scorecard builders, scoring helpers, comparison helper deltas, and promotion decision helpers now consume `report_utils.number_or_none()` / `number_or_default()` while preserving existing missing-value and default-zero semantics.
...
- Migrated benchmark scorecard `_number()` helpers to `report_utils.number_or_none()`.
- Migrated benchmark scorecard decision `_int()` / `_int_or_none()` helpers to `number_or_default()` / `number_or_none()`.
- Preserved scorecard missing-value semantics and promotion decision default-zero semantics.
- Verified focused benchmark scorecard/decision/comparison/report-utils tests, helper scans, full unittest discovery, source encoding, and docs/evidence alignment.
