...
Version 176 splits benchmark scorecard decision artifact writers into `benchmark_scorecard_decision_artifacts.py`. Scorecard comparison loading, candidate evaluation, baseline exclusion, rubric threshold checks, generation-quality review items, selected-run ranking, decision status, recommendations, script usage, and old `minigpt.benchmark_scorecard_decision` exports stay unchanged, while JSON, CSV, Markdown, and HTML output rendering move to a focused artifact module.
...
- Added `src/minigpt/benchmark_scorecard_decision_artifacts.py` for benchmark scorecard decision JSON, CSV, Markdown, and HTML output writers.
- Kept scorecard comparison loading, candidate evaluation, baseline exclusion, rubric threshold checks, generation-quality review items, selected-run ranking, decision status, recommendations, `scripts/build_benchmark_scorecard_decision.py`, and old `minigpt.benchmark_scorecard_decision` exports intact.
- Reduced `benchmark_scorecard_decision.py` from 494 lines to 263 lines by moving output rendering to a dedicated artifact module.
- Added direct facade identity coverage in `tests/test_benchmark_scorecard_decision.py`.
- Confirmed full unittest discovery passes with 398 tests and module pressure remains `pass` with zero warn modules.
