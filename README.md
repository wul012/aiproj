...
Version 218 persists the promoted seed handoff clean-evidence gate result into artifacts. The CLI still keeps default behavior unchanged, but every run now writes a `clean_evidence_requirement` object before JSON/CSV/Markdown/HTML output, so pass/fail/not-required gate decisions remain auditable after stdout is gone.
...
- Added a `clean_evidence_requirement` object to CLI-written seed handoff reports.
- Persisted requirement status as `not-required`, `pass`, or `fail` before JSON/CSV/Markdown/HTML outputs are written.
- Added CSV, Markdown, and HTML rendering for the clean-evidence gate result.
- Kept v216/v217 stdout diagnostics and exit-code behavior unchanged.
- Verified focused seed handoff tests, gate-artifact smoke, syntax, full unittest discovery, source encoding, and docs/evidence alignment.
