...
Version 207 carries the handoff suite guard one step upstream into the training-scale promotion index. The index now preserves handoff suite evidence per promotion and exposes `handoff_require_suite_consistency`, `handoff_suite_consistency`, `handoff_suite_mismatch_count`, and `handoff_selected_suite_path` in promotion JSON/CSV/Markdown/HTML without changing compare-ready selection rules.
...
- Carried promotion handoff suite guard evidence one step upstream into the training-scale promotion index.
- Added promotion index summary fields for `handoff_require_suite_consistency`, `handoff_suite_consistency`, `handoff_suite_mismatch_count`, and `handoff_selected_suite_path`.
- Exposed those fields in promotion index JSON, CSV, Markdown, and HTML while preserving compare-ready selection rules.
- Added promotion index tests and CLI smoke proving compare-ready promoted inputs retain the handoff suite boundary.
- Verified focused index tests, promotion index smoke, artifact-field smoke, syntax, full unittest discovery, source encoding, and docs/evidence alignment.
