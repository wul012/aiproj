...
Version 165 splits training scale plan artifact rendering into `training_scale_plan_artifacts.py`. The plan schema, variants JSON, Markdown/HTML output, CLI behavior, and `minigpt.training_scale_plan` facade exports stay unchanged, while `training_scale_plan.py` drops from 541 lines to 313 lines.
...
- Added `src/minigpt/training_scale_plan_artifacts.py` for training-scale plan JSON, variants JSON, CSV, Markdown, and HTML output.
- Kept the existing `minigpt.training_scale_plan` facade exports intact for scripts and downstream training-scale modules.
- Reduced `training_scale_plan.py` from 541 lines to 313 lines by moving artifact rendering away from scale-plan construction.
- Added facade identity coverage in `tests/test_training_scale_plan.py` alongside existing plan, renderer, and batch-compatible variants tests.
