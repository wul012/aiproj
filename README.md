...
Version 173 splits training scale run decision artifact writers into `training_scale_run_decision_artifacts.py`. Candidate selection, rejection reasons, readiness thresholds, gate-pass policy, execute command construction, script/workflow usage, and old `minigpt.training_scale_run_decision` exports stay unchanged, while JSON, CSV, Markdown, and HTML output rendering move to a focused artifact module.
...
- Added `src/minigpt/training_scale_run_decision_artifacts.py` for training scale run decision JSON, CSV, Markdown, and HTML output writers.
- Kept candidate selection, rejection reasons, readiness thresholds, gate-pass policy, execute command construction, `scripts/decide_training_scale_run.py`, workflow usage, and old `minigpt.training_scale_run_decision` exports intact.
- Reduced `training_scale_run_decision.py` from 513 lines to 308 lines by moving output rendering to a dedicated artifact module.
- Added direct facade identity coverage in `tests/test_training_scale_run_decision.py`.
- Confirmed full unittest discovery passes with 395 tests and module pressure remains `pass` with zero warn modules.
