...
Version 143 opts GitHub Actions into the Node 24 JavaScript action runtime before the hosted runner default changes. The CI workflow now sets `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true`, and a lightweight workflow test guards that setting so the current Node.js 20 deprecation warning does not come back silently.
...
- Added top-level `env: FORCE_JAVASCRIPT_ACTIONS_TO_NODE24: "true"` to `.github/workflows/ci.yml`.
- Kept the existing Python 3.11 source encoding and unittest steps unchanged, so this is a runtime-policy maintenance release rather than a test matrix redesign.
- Added `tests/test_ci_workflow.py` to assert the Node 24 opt-in stays present beside `actions/checkout@v4` and `actions/setup-python@v5`.
- Reran the CI workflow test, source encoding smoke, compile checks, maintenance smoke, and full unittest discovery for `c/143`.
