...
Version 163 splits benchmark scorecard comparison delta, summary, recommendation, and best-run logic into `benchmark_scorecard_comparison_deltas.py`. The comparison schema, CLI, artifact exports, and decision consumers stay unchanged, while `benchmark_scorecard_comparison.py` drops from 549 lines to 134 lines.
...
- Added `src/minigpt/benchmark_scorecard_comparison_deltas.py` for run deltas, case deltas, task/difficulty group deltas, summary, recommendations, and best-run selection.
- Kept `build_benchmark_scorecard_comparison()`, scorecard comparison schema, artifact facade exports, and benchmark promotion decision consumers compatible.
- Reduced `benchmark_scorecard_comparison.py` from 549 lines to 134 lines by moving pure comparison calculations to the delta module.
- Added direct delta-module coverage in `tests/test_benchmark_scorecard_comparison_deltas.py` alongside existing comparison, artifact, and decision tests.
