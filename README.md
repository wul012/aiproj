...
Version 196 is a focused benchmark scorecard helper consolidation. `benchmark_scorecard.py` now imports `as_dict` and `list_of_dicts` from `report_utils` instead of keeping local `_dict` and duplicate `_list_of_dicts` helpers, preserving mixed-list filtering behavior while reducing repeated utility code in the evaluation chain.
...
- Removed local `_dict()` and duplicate `_list_of_dicts()` helper definitions from `src/minigpt/benchmark_scorecard.py`.
- Imported `as_dict as _dict` and `list_of_dicts as _list_of_dicts` from `src/minigpt/report_utils.py`, keeping scorecard call sites stable.
- Preserved pair-result normalization behavior: mixed `results` arrays still ignore non-dict items and keep dict comparisons available for max-delta and same-checkpoint checks.
- Kept the scope deliberately small: this is the benchmark scorecard portion of the helper cleanup, not a broad repo migration.
- Verified focused benchmark scorecard tests, syntax checks, full unittest discovery, source encoding, and docs/evidence alignment.
