...
Version 248 carries maturity-summary coverage regression context into the maturity narrative. The human-facing narrative now explains `coverage-regressed` release trends, coverage regression counts, coverage gap deltas, and why the portfolio remains review-level.
...
- Maturity narrative summary now carries release readiness coverage regression counts and max coverage gap/percent deltas.
- `coverage-regressed` release trends now make the portfolio narrative `review` even when benchmark/data/request evidence is otherwise clean.
- Release Quality Trend claims explain coverage regression counts and max gap deltas.
- Markdown/HTML stats and CLI output expose coverage regression context for human review.
- Archived command-output evidence in `c/248`.
