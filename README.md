...
Version 316 extends the CPU-friendly tiny standard benchmark smoke with a same-checkpoint pair baseline before the benchmark scorecard, so evidence completeness now covers eval suite, generation quality, pair batch, and scorecard while still separating reproducibility from model improvement.
