...
Version 188 enriches real training run evidence with generation quality summaries. `scripts/build_training_run_evidence.py` now reads `generation_quality/generation_quality.json`, records overall quality status, pass/warn/fail case counts, total flags, dominant flag, diversity/repetition metrics, and carries those fields into JSON/CSV/Markdown/HTML reports and CLI output.
...
- Enriched `src/minigpt/training_run_evidence.py` with a `quality` section that reads `generation_quality/generation_quality.json` from a real run directory.
- Added generation quality status, pass/warn/fail case counts, average unique ratio, repeated n-gram ratio, max repeat run, total flags, and dominant flag to training run evidence.
- Extended JSON/CSV/Markdown/HTML outputs and `scripts/build_training_run_evidence.py` CLI output so reviewers can see output-quality risk without opening a separate report.
- Preserved the boundary from v187: missing generation quality is a non-blocking `review` warning, and failed quality flags keep evidence under review rather than pretending the model is fully ready.
- Verified a real CPU PyTorch train smoke followed by fixed eval suite, generation quality analysis, and training-run evidence generation.
