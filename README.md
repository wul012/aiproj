...
Version 251 splits training portfolio comparison review logic into a dedicated module. The comparison builder keeps artifact loading, delta calculation, and summary assembly, while review actions, recommendations, and coverage-regression predicates live in `training_portfolio_comparison_review.py`.
...
- Added `training_portfolio_comparison_review.py` for review statuses, coverage-regression predicates, review actions, and recommendations.
- Reduced `training_portfolio_comparison.py` from 659 lines to 465 lines while keeping artifact loading, delta calculation, and summary assembly there.
- Added direct tests for tolerant coverage-regression parsing and best-score blocker action generation.
- Existing portfolio comparison and artifact tests still pass, proving JSON/CSV/Markdown/HTML behavior stayed compatible.
- Archived command-output evidence in `c/251`.
