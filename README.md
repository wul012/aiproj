...
Version 265 splits promoted decision artifact writers into a focused module. The promoted baseline decision contract stays unchanged while CSV/Markdown/HTML/JSON rendering leaves the main decision builder.
...
- `promoted_training_scale_decision_artifacts.py` now owns promoted decision JSON/CSV/Markdown/HTML writers and HTML helper rendering.
- `promoted_training_scale_decision.py` keeps baseline selection, rejection rules, summary assembly, and recommendations while re-exporting artifact functions for compatibility.
- Main decision file is down from 600 to 313 lines; artifact writer sits at 317 lines.
- Archived command-output and artifact-boundary evidence in `c/265`.
