...
Version 270 carries the clean batch-review workflow requirement into the training-scale execution handoff. Handoff validation now preserves the workflow clean batch-review guard, renders it in artifacts, prints it from the CLI, and blocks execution handoff when a clean batch-review requirement is not satisfied.
...
- `build_training_scale_handoff()` now builds a `clean_batch_review_guard` from workflow and nested decision evidence.
- Handoff validation blocks execution when workflow requires clean batch-review evidence but the status is not `clean`.
- Handoff CSV/Markdown/HTML now render `decision_require_clean_batch_review` and `clean_batch_review_status`.
- `scripts/execute_training_scale_handoff.py` prints `clean_batch_review_status`.
- Archived command-output and handoff gate evidence in `c/270`.
