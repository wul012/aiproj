...
- Playground server that serves the UI and exposes `/api/health`, `/api/checkpoints`, `/api/checkpoint-compare`, `/api/model-info`, `/api/generate`, `/api/generate-stream`, `/api/request-history`, `/api/generate-pair`, and `/api/generate-pair-artifact` for local live generation and review
...
- Request history endpoint `/api/request-history` that reads `inference_requests.jsonl`, returns recent normalized generation records, counts invalid log lines, summarizes returned statuses/endpoints, and powers a playground Request History table
...
The playground server turns that browser surface into a local API client for `/api/health`, `/api/checkpoints`, `/api/checkpoint-compare`, `/api/model-info`, `/api/generate`, `/api/generate-stream`, `/api/request-history`, `/api/generate-pair`, and `/api/generate-pair-artifact`, with local request limits, selected checkpoint routing, streaming token events, side-by-side generation, checkpoint comparison shortcuts, saved pair artifacts, request history, and JSONL inference logs.
...
- Add request detail export or filtering for the playground history table.
