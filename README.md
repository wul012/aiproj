...
Version 157 splits registry leaderboard HTML rendering into `registry_leaderboards.py`. The registry payload and `render_registry_html()` facade stay unchanged, while `registry_render.py` drops from 588 lines to 461 lines.
...
- Added `src/minigpt/registry_leaderboards.py` for loss, rubric, pair delta, and release readiness delta leaderboard HTML fragments.
- Kept `render_registry_html()` and registry payload keys unchanged.
- Reduced `registry_render.py` from 588 lines to 461 lines by moving isolated leaderboard blocks to a pure helper module.
- Added direct leaderboard tests for escaping, empty-state messages, and relative report links.
