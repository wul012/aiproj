...
Version 169 splits promoted training scale seed handoff artifact writers into `promoted_training_scale_seed_handoff_artifacts.py`. The handoff schema, command execution, timeout handling, plan artifact detection, and old `minigpt.promoted_training_scale_seed_handoff` exports stay unchanged, while `promoted_training_scale_seed_handoff.py` drops from 530 lines to 301 lines.
...
- Added `src/minigpt/promoted_training_scale_seed_handoff_artifacts.py` for promoted seed handoff JSON, CSV, Markdown, and HTML output writers.
- Kept the existing handoff schema, command execution, timeout handling, plan artifact detection, and `minigpt.promoted_training_scale_seed_handoff` facade exports intact.
- Reduced `promoted_training_scale_seed_handoff.py` from 530 lines to 301 lines by moving output rendering to a dedicated artifact module.
- Added direct facade identity coverage in `tests/test_promoted_training_scale_seed_handoff.py`.
