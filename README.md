...
Version 257 carries promotion-level handoff batch review context into the training scale promotion index. Promotion index rows, summaries, CSV/Markdown/HTML outputs, CLI prints, and recommendations now keep selected batch review/blocker risk visible while preserving promoted-only compare input semantics.
...
- Training scale promotion index rows now preserve handoff selected batch review status, selected review/blocker counts, and handoff-level batch comparison totals from promotion reports.
- CSV/Markdown/HTML index outputs and CLI prints expose those fields beside suite guard and compare-ready evidence.
- Promotion index recommendations now warn when promoted compare inputs still carry selected handoff batch review/blocker actions, without changing promoted-only compare selection.
- Archived command-output evidence in `c/257`.
