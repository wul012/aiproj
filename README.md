...
Version 153 splits release bundle artifact rendering into `release_bundle_artifacts.py`. The existing `release_bundle.py` facade still exports the same render/write functions, while module pressure returns to `pass` with zero warn modules after removing the report-output block from the release bundle builder.
...
- Added `src/minigpt/release_bundle_artifacts.py` for release bundle JSON/Markdown/HTML rendering and writing.
- Kept `src/minigpt/release_bundle.py` as the legacy facade for `render_release_bundle_*` and `write_release_bundle_*`.
- Reduced `release_bundle.py` from 709 lines to roughly 409 lines by moving artifact helpers and HTML style into the artifact module.
- Added facade identity tests so scripts and existing imports keep working.
- Confirmed module pressure returns to `pass` with zero warn modules.
