...
Version 199 connects the standard Chinese benchmark suite into the training portfolio workflow. `run_training_portfolio.py` and `run_training_portfolio_batch.py` now accept `--suite-name standard-zh`, portfolio plans record `suite={"mode":"builtin","path":"builtin:standard-zh"}`, and eval/pair-batch steps pass `--suite-name` instead of a file path when a built-in suite is selected.
...
- Added `suite_name` support to `build_training_portfolio_plan()`, with a recorded `suite` object and `suite_path="builtin:<name>"` compatibility field.
- Updated portfolio eval-suite and pair-batch steps to emit `--suite-name standard-zh` when a built-in suite is selected.
- Added `--suite-name standard-zh` to `scripts/run_training_portfolio.py` and `scripts/run_training_portfolio_batch.py`.
- Added batch-level and variant-level `suite_name` support, including variant override behavior for `suite_path`.
- Verified focused portfolio/batch/eval-suite tests, dry-run portfolio and batch smoke, syntax, full unittest discovery, source encoding, and docs/evidence alignment.
