...
Version 168 splits promoted training scale seed artifact writers into `promoted_training_scale_seed_artifacts.py`. The seed schema, next-plan command generation, script behavior, and old `minigpt.promoted_training_scale_seed` exports stay unchanged, while `promoted_training_scale_seed.py` drops from 530 lines to 319 lines.
...
- Added `src/minigpt/promoted_training_scale_seed_artifacts.py` for promoted seed JSON, CSV, Markdown, and HTML output writers.
- Kept the existing promoted seed schema, next-plan command generation, scripts, and `minigpt.promoted_training_scale_seed` facade exports intact.
- Reduced `promoted_training_scale_seed.py` from 530 lines to 319 lines by moving output rendering to a dedicated artifact module.
- Added direct facade identity coverage in `tests/test_promoted_training_scale_seed.py`.
