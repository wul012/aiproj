...
Version 286 adds an embedded receipt-check verifier for promoted seed handoff reports, so the main JSON can be independently rechecked against a freshly computed automation receipt decision.
...
The embedded verifier accepts either `promoted_training_scale_seed_handoff.json` or its output directory. It recomputes the automation receipt check from the handoff report, compares it with the embedded `receipt_check`, validates `receipt_check_outputs`, and writes `promoted_training_scale_seed_handoff_embedded_receipt_check.json` plus `.txt` when `--out-dir` is provided. Like the receipt checker, a structurally consistent `stop` decision exits non-zero unless `--allow-stop` is used.
