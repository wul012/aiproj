...
Version 266 splits training-scale handoff artifact writers into a focused module. Handoff validation, execution, suite guard, and batch-review context stay in the handoff builder while JSON/CSV/Markdown/HTML rendering moves to the artifact layer.
...
- `training_scale_handoff_artifacts.py` now owns handoff JSON/CSV/Markdown/HTML writers and HTML helper rendering.
- `training_scale_handoff.py` keeps workflow loading, decision command validation, optional command execution, suite guard propagation, artifact discovery, summary assembly, and recommendations while re-exporting artifact functions for compatibility.
- Main handoff file is down from 540 to 297 lines; artifact writer sits at 261 lines.
- Archived command-output and artifact-boundary evidence in `c/266`.
