...
Version 299 carries CI workflow order-violation deltas through registry and maturity summaries so cross-project governance can see smoke-before-coverage drift even when CI pass/fail status stays unchanged.
...
## Latest v299 checkpoint

- Registry run summaries now carry `release_readiness_ci_workflow_order_regression_count` and `release_readiness_max_ci_workflow_order_violation_delta`.
- Registry rankings now treat CI order violation delta as a regression signal and preserve it in delta rows and leaderboard ordering.
- Registry render and leaderboard views now show CI order regressions alongside CI workflow regressions.
- Maturity summaries and maturity artifact views now expose the CI order regression count and max violation delta.
- Tests cover registry, registry-ranking, and maturity consumers of the new order regression summary fields.
- Archived registry/maturity CI order evidence in `c/299`.
