...
Version 255 carries selected-run batch review status into the controlled training scale handoff. Handoff JSON/CSV/Markdown/HTML evidence now keeps decision-level batch review/blocker counts visible while validating or executing the reviewed command.
...
- Controlled training scale handoffs now preserve selected-run batch review status and decision-level batch review/blocker totals.
- CSV/Markdown/HTML handoff artifacts expose selected review/blocker counts and comparison-level totals beside suite guard and artifact checks.
- Handoff recommendations now distinguish review-level batch actions from blocker-level batch actions before execution evidence is treated as clean.
- Archived command-output evidence in `c/255`.
