...
Version 187 enriches real training run evidence with fixed eval suite summaries. `scripts/build_training_run_evidence.py` now reads `eval_suite/eval_suite.json`, records case count, task/difficulty coverage, average continuation length, and average unique-character count, then carries those fields into JSON/CSV/Markdown/HTML reports and CLI output.
...
- Enriched `src/minigpt/training_run_evidence.py` with an `evaluation` section that reads `eval_suite/eval_suite.json` from a real run directory.
- Added eval case count, result count, suite metadata, task type coverage, difficulty coverage, average continuation characters, and average unique characters to the evidence summary.
- Extended JSON/CSV/Markdown/HTML outputs and `scripts/build_training_run_evidence.py` CLI output so reviewers can see whether a real checkpoint was evaluated by fixed prompts.
- Preserved the boundary from v186: missing eval suite is a non-blocking `review` warning by default, while a real train+eval run can reach `ready`.
- Verified a real CPU PyTorch train smoke followed by fixed eval suite and training-run evidence generation.
