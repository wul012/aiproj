...
Version 263 splits promoted seed review helper logic into a focused module. The promoted next-cycle seed contract stays unchanged, while suite-guard and handoff batch-review mapping leave the orchestration file.
...
- `promoted_training_scale_seed_review.py` now owns promoted seed suite-guard mapping, handoff batch-review mapping, summary projection, and batch-review recommendations.
- `promoted_training_scale_seed.py` stays as the next-cycle seed orchestration surface while preserving the same JSON/CSV/Markdown/HTML and CLI contract.
- Added focused helper tests for summary/selected fallback behavior, suite guard propagation, and recommendation reuse.
- Archived command-output and helper-boundary evidence in `c/263`.
