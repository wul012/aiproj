...
Version 247 carries registry-level coverage regressions into the maturity summary. Maturity overview, Markdown/HTML trend context, CLI output, and recommendations now mark `coverage-regressed` readiness trends as review-worthy governance risk.
...
- Maturity summary now reads registry-level `test_coverage_regression_count`, status-change counts, and max coverage percent/gap deltas.
- `coverage-regressed` release readiness trends now downgrade otherwise-pass maturity summaries to `warn`.
- Markdown/HTML outputs show test coverage regression context beside CI workflow regression context.
- CLI output exposes coverage regression counts for quick maturity smoke checks.
- Archived command-output evidence in `c/247`.
