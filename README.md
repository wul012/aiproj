...
Version 166 splits project audit request-history and CI workflow hygiene context checks into `project_audit_contexts.py`. The audit schema, artifact writers, CLI behavior, and `minigpt.project_audit` facade exports stay unchanged, while `project_audit.py` drops from 538 lines to 404 lines.
...
| Local inference and UI | Playground server, checkpoint selector, streaming generation, cancellation/timeout controls, request history, pair artifacts, extracted server contracts, split playground assets, extracted generator class, extracted server log-event builders, extracted checkpoint/model-info payload builders, extracted HTTP response helpers, extracted request-history endpoint handlers | `server.py`, `server_http.py`, `server_request_history.py`, `server_checkpoints.py`, `server_logging.py`, `server_generator.py`, `server_contracts.py`, `request_history.py`, `pair_artifacts.py`, `playground.py`, `playground_assets.py`, `playground_style.py`, `playground_script.py`, server-http, server-request-history, server-checkpoint, server-logging, server-generator, server-contract, request-history, pair-artifact, and playground-asset tests plus Playwright screenshots | Keep HTTP routing and model generation stable while extracting pure contracts, generator, logging, payload, and response helpers |
...
- Added `src/minigpt/project_audit_contexts.py` for request-history summary checks, request-history context, CI workflow hygiene checks, and CI workflow context.
- Kept the existing `minigpt.project_audit` schema, artifact writers, facade exports, and `scripts/audit_project.py` CLI behavior intact.
- Reduced `project_audit.py` from 538 lines to 404 lines by moving external governance context mapping to a dedicated helper module.
- Added direct context-helper coverage in `tests/test_project_audit.py` alongside existing complete-project, warning, output, escaping, and facade tests.
