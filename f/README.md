...
v1289 把 v1287 的提交尖峰放到逐步显微镜下,问 v1288 因果判决留下的机制问题:val 崩塌那几步里到底什么坏了?方法上的关键是**延续原轨迹本身**——v1288 分支臂会重置 Adam 动量,这里不可用,于是逐行复刻 train_to_grok 循环(同 init/种子/先评估后更新/舍入),G0 要求每个 100 步粗行与提交 v1287 曲线**逐位相等**(9 格 ~740 行全部通过——"复刻循环=原循环"的数据证书),窗口内每步记录 train/val 准确率、48-bin 嵌入功率谱、总范数、该步真实反传的 loss 与梯度范数。判决 **`spike_preserves_circuit`**(G0/G1/G2 全绿、deep_bar 三点稳定;八连清洁链,预算逐数命中 9 runs / 73,700 步 / 15,620 密集步):173 个合格事件(提交普查只见 24)、134 个深事件(min val<0.5),**134/134 嵌入傅里叶电路完好**——r 中位 1.007、全体最差 0.979、零 destroyed 零 mid,share 在崩塌中还在继续爬升。显微镜附赠四个发现:post-grok 状态是**准周期弛豫振荡器**(高 lr 每 ~50 步一次:loss 慢充电、单个梯度脉冲放电——中位 240×、典范格极端 9.5M×——3–8 步愈合、范数锯齿);100 步网格普查性失明(150/173 事件完全隐形、隐藏深度中位 0.46);两种时间尺度(典范事件 fall 89–187 步最长 254,高 lr 是 1–3 步针尖,电路都完好);**旋转发生在尖峰处**(13/161 跨尖峰旋转 top-5 集,全部 lr≤4e-3,8e-3 集锁定 0/55——banked 旋转动力学部分回答)。三层弧收束:v1287 现象(亚稳)→ v1288 原因(wd)→ v1289 机制(读出断、电路存)——亚稳性是浅的,结构从未丢失,纯化是连续精炼而非反复重建。诚实边界:"电路"仅指嵌入(=绑定 unembedding)傅里叶结构,坏的是网络中段、未定位(banked);lead 中位 79 ≈ 尖峰间隔,是振荡器充电相而非独立预警(预注册非裁决)。预注册 `e3a1c63b` 后零改动。cache 与五格式报告见 `f/1289/解释/grok_spike_microscopy_v1289/`,单图见 `f/1289/图片/grok_spike_microscopy_v1289.png`。

## v1288

v1288 对 v1287 发现的 post-grok 亚稳性问最锋利的因果问题：谁在反复破坏已泛化的解？设计 = **成对分支臂**：对 9 个尖峰易发格确定性重跑到已验证健康的 v1287 梯子点（P1 把 8e-3/1338 从正落在尖峰里的 1.4× 点换到已验证的 1.8× 点），抓取权重态后分成两条延续臂——wd=1.0 与 wd=0.0，两臂共享 optimizer 重置混淆，臂间唯一差异就是 wd；G0 要求分支曲线与提交的 v1287 缓存**逐位相等**。判决 **`spikes_are_wd_driven`**（G0/G1/G2 全绿、三 bar 稳定；自 v1280 以来第二个非 review，本弧第一个正面因果判决）：wd=1 臂在 8/9 格复现尖峰（14 个，最深 0.31，全部自愈），wd=0 臂 **9/9 零尖峰**（min val ≥0.909）。反截尾跑证实 v1287 的两个"死亡"都是截尾：旧视界后 **100 步内**满血复活（0.431→0.996、0.373→0.999；4e-3/1338 的 +1000 新端点又恰好落进一个浅涡 0.881——端点抽签递归重演，本身即亚稳论断的再确认）。次级：范数方向全中（wd1 收缩 0.953 / wd0 增长 1.753）；纯度冻结预测受天花板限制（中位 wash，如实报告），唯一有余量的 canonical 格给出教科书解离（wd1 爬 0.372→0.574 / wd0 冻结）。机理收束：**纯化与亚稳性是 wd 雕刻的一体两面**；canonical 配方的早停之所以保护，是它停在 wd 还没来得及推下山脊的时刻。预注册 `b07f1b1f` 后零改动（七连清洁链），预算精确命中 29 runs / 117,000 步。banked：尖峰微观触发（稠密范数采样）；canonical 旋转动力学。cache 与五格式报告见 `f/1288/解释/grok_spike_anatomy_v1288/`，单图见 `f/1288/图片/grok_spike_anatomy_v1288.png`。

## v1287

v1287 把 v1286 只能从端点推断的 post-grok 纯化段变成直接观测：P1 缓存审计发现所有已提交 run 的 post-grok 时间几乎为零（五格恰在 t_gen 当步早停）——弧上全部纯度数字都是"grok 时刻的纯度"。设计：11 个 v1285/v1286 格延长到 3× t_gen 视界，`grok_stop_val=2.0` 经既有配置字段关闭早停（训练代码零改动），纯度在各格**已提交缓存的 top-5 频率集**上度量、缓存 final_share 为锚。正式判决 `review`（substrate_unsound：4e-3/1338、1339 终点 heldout 0.431/0.373——brief 里明文预告的 post-grok 稳定性防护开火）。防护抓到的东西比原问题大：**grok 后的解是亚稳的**——11 格里 9 格（含 canonical）出现 train+val 同崩的尖峰、全部 100–300 步自愈，两个"死亡格"只是恰在视界最后一格被截尾在尖峰中间（5300 步 val=1.0 → 5400 步 0.431 且 train 同步崩）。纯化本身真实且普遍（探索性：own 纯度 canonical 0.31→0.50–0.63 近翻倍、8e-3→0.88–0.91，延长后 4e-3 的 0.93 反超 8e-3——v1286 的"随 lr 单调"是截断快照）；canonical 的纯化会**旋转频率集**（set_match ✗ 3/3，预注册 climb 指标原则性失明、旋转诊断按设计接住——review 双重成立）；低 lr 的 post-grok 训练把 heldout 提到 1.0。预注册 `19a429c8` 后零改动（六连清洁链），前缀门 33/33、t_mem/t_gen 11/11 逐位复现、预算精确命中 44 次 / 407,800 步。banked：尖峰解剖与旋转动力学。cache 与五格式报告见 `f/1287/解释/grok_purification_v1287/`，单图见 `f/1287/图片/grok_purification_v1287.png`。

## v1286

v1286 检验 v1284/v1285 拼出的统一假说：建造速率由有效 lr 设定、泛化在建造完成时触发——若真，F 应在 v1281 的 8 倍压缩下守恒。**没有**：F 从 1e-3 的 0.56–0.72 掉到 ~0.25（各 lr 中位 0.20/0.27/0.28），不变量证伪。正式判决 `review`（bar_instability：2e-3 组中位 0.196 恰骑 0.2 分箱线，G2 兜底防护对真实骑线数据履职）。相对时间图揭示更细的真相：压缩格在 grok 时刻的**绝对**份额更高（0.45–0.55 vs 0.29）——F 低是分母的功劳，**高 lr 的回路在 grok 之后继续大幅纯化**（终点份额随 lr 单调升 0.31→0.55–0.59→0.62–0.72→0.71–0.81）：lr 解耦建造与泛化，并让同宽度终点更纯（v1285 的宽度-纯度梯度原来是 lr 条件性的）。三区域收束：慢深 grokking（F≈0.6，grok 后不纯化）/ 压缩 grokking（F≈0.25，强 post-grok 纯化）/ 相位边界（F≈0.25）。预注册 `2b31c62d` 后代码零改动（五连清洁链），前缀门 76/76、v1281 参照逐位复现，预算 84/112 次 / 63,400 步。banked：post-grok 纯化段的定量刻画。cache 与五格式报告见 `f/1286/解释/grok_lr_compression_v1286/`，单图见 `f/1286/图片/grok-lr-compression-v1286.png`。

## v1285

v1285 把 v1284 的问题带到慢雕刻叙事唯一可能幸存的地方：d=128 经典深平台（t_mem=100、t_gen 10,500–12,700）。判决 **`deep_plateau_sculpts`**——自 v1280 以来第一个非 review：F = {0.720, 0.556, 0.600} 中位 0.600，三 bar 一致、无离散，三格完整跑全部**逐位复现 v1279 参照**（前缀门 43/43）。深平台真的在建造（val 离开静息带前 56–72% 的回路功率增量已就位，C(t) 沿一万步平台单调爬升 0.15→0.30 无末端速成）——v1284 的"平台不拥有建造"正式定性为边界效应，慢雕刻在它被讲述的地方幸存；resolution note 已回写 v1284 简报。奖励：v1188 历史测量 0.307 三重复现（0.305/0.312/0.305），宽度-纯度梯度闭合 0.92@16 → 0.31@128；经典平台静息于 val 0.11–0.21 而非 chance（P1 发现，bar 由此适配为 {0.25,0.30,0.35}，预注册前披露）。预注册 `46fc9734` 后代码零改动（四连清洁链），预算 46/50 次 / 191,600 步。banked：lr × F 扫描（8 倍时间压缩下 F 是否守恒）。cache 与五格式报告见 `f/1285/解释/grok_deep_plateau_v1285/`，单图见 `f/1285/图片/grok-deep-plateau-v1285.png`。

## v1284

v1284 问 grokking 平台期在干什么：慢雕刻回路（经典叙事）还是等待后速成？方法 = 确定性截断重训当权重快照（经典训练代码零改动，前缀确定性门 43/43 自验证通过；该门在测试阶段就抓过假快照函数的相对步数错误）。P1 先从 v1277 缓存解决终点：耦合相终点回路**更浓**（w=16 top-5 份额 0.85–0.92 vs w=32 的 0.68–0.72）。6 条轨迹（含宽度匹配的 w=24 跨相对），预注册 `74970587` 后代码零改动（三连清洁链），预算 49/76 次 / 60,600 步。判决 **`review`（mixed_fractions）**——均匀性即发现：**两相在同一相对日程上建造回路**（F ≈ 0.25 双相同值；w=24 对 F 0.251 vs 0.240，延迟 seed 的 C(t) 曲线只是 ~3× 时间膨胀）——平台不拥有建造，慢雕刻叙事需修正。描述性：终点浓度随宽度平滑下降（0.91→0.85→0.76，衔接 v1277 端点）；边界平台是漏的。banked：d=128 经典深平台的同款测量。cache 与五格式报告见 `f/1284/解释/grok_circuit_timing_v1284/`，单图见 `f/1284/图片/grok-circuit-timing-v1284.png`。

## v1283

v1283 给 v1282 bank 的"从未记忆却泛化"现象定位了门。P1 缓存法证（33 格，免费）先重塑问题：d=128 的延迟相位在每个稳定 lr 下都存在（延迟被压缩 ~11,100→~700 步但从未消失）——"grokking 是 lr 假象"的强形式被证伪；而 w=16 在任何 lr 下都从未 grok（1e-3 下 t_mem≈t_gen）——**耦合是宽度门控的**；且 max_gap 分布双峰、[0.41, 0.79] 空洞，阈值预冻结在洞内。边界实验（{20,24,28}×3 seeds + 16/32 锚点重跑，预注册 `5146a306`，其后零代码改动，13/16 GPU，13/13 泛化成功）判决 **`review`（mixed_widths）**——mixed 本身即发现：**开启是陡峭双峰跳变，临界宽度 ≈24，seed 决定落在哪侧**；13 个新格没有一个落入 [0.5,0.7]，coupled 格延迟精确为 0、delayed 格 ≥300，`graded` 分支被干净排除。v1277 之问三层解构完成：w≥32 差异=lr 饥饿；w≤20 根本不在 grokking；边界 w≈24 陡峭且随机。banked：耦合相是否不经记忆解直接长出傅里叶回路（机制版）。cache 与五格式报告见 `f/1283/解释/grok_delay_gate_v1283/`，单图见 `f/1283/图片/grok-delay-gate-v1283.png`。

## v1282

v1282 跑 v1281 控制的宽度版，检验弧综合假设对起点之问（v1277"窄快 5 倍"）的预测：{16,32,64} × lr=4e-3 对照 v1281 的 d=128 地板（中位 1,400）。预注册 `bf1cbf4b`，9/12 GPU，参照零重跑。判决 **`review`（broken_cells）**：w=16 超出 4e-3 的稳定窗口（seed 1337 既不 grok 也从不记忆），"不稳定≠慢"防护首次真实触发。描述性结算：**d=64 的洞在 4× lr 下关闭**（同一批 100k 删失的 seed 现在 1,800–3,000 步 grok，heldout 至 1.0，ρ=1.71 收敛）；**窄加速消失并反转**（w=32 的 ρ=3.36，比 d=128 慢 3 倍）；可用 lr 窗口随宽度上移（broken/变慢/收敛三种后果同图可见）。新现象 banked：两个 w=16 seed 从未记忆却泛化到 ~0.96。v1277 简报已回写如实标注 review 的结算注记。cache 与五格式报告见 `f/1282/解释/grok_width_lr_v1282/`，单图见 `f/1282/图片/grok-width-lr-v1282.png`。

## v1281

v1281 跑 v1280 bank 下的对照：匹配相对步长（lr/α）下，满范数模型是否和救活的小范数模型一样快？预注册 `ea074da1`（其后代码零改动），10/12 GPU，参照全部读 v1279/v1280 提交缓存（G0 中位锚逐字校验）。判决 **`review`（mixed_pairs）**：ρ(r=4×)=0.359（α=1 反而快 2.8 倍）、ρ(r=8×)=0.778（平价），但两对一致排除小范数分支——v1280 救活的速度从来不是范数的功劳。描述性结算：**绝对 lr 主导时钟**（α=1 剂量 11,400→~1,400，lr≥4e-3 处饱和 ~1k 步）；同步长三范数 (0.5,1,2)→(3,900,1,400,900)，足够 lr 下范数越大略快，α=2 @ 8e-3 以 900 步 + heldout 1.0 成为全弧最快最准；v1277–v1280 的宽度/范数效应大多归结为"经典配方 lr 饥饿"。resolution note 已回写 v1280 简报。cache 与五格式报告见 `f/1281/解释/grok_norm_vs_step_v1281/`，单图见 `f/1281/图片/grok-norm-vs-step-v1281.png`。

## v1280

v1280 用最锋利的通缩假说压测 v1279 的头条：小初始化杀死 grokking 是不是学习率假象？P1 先纠正了 v1279 的解读（死格瞬间记忆、t_mem≈300——死的是记忆→泛化转变；更正 `784f91c2`），预注册 `2cd7590c` 后在 α=0.5 扫 16 倍 lr 范围 + α 剂量臂，参照全部读 v1279 提交缓存（零重跑），预算 14/18。判决 **`norm_clock_revived_under_lr_scaling`**：lr 减半毫无作用（步长对称理论被否），lr 提到 2–4 倍全部获救——lr=4e-3 时 1,300/2,300 步 grok、heldout 0.9992，全弧最快最准的 d=128 格；v1279 头条正式降级为"lr 条件性"，降级通告按预注册义务回写 v1279 简报。剂量臂：冻结 lr 下死亡边界是 (0.7, 0.85] 的 cliff，关闭前先减速 3–4 倍。cache 与五格式报告见 `f/1280/解释/grok_init_rescue_v1280/`，单图见 `f/1280/图片/grok-init-rescue-v1280.png`。
