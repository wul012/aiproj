...
MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_REPAIR_TRAINING_RUN_CSV_FILENAME = "model_capability_route_promotion_bounded_real_replay_repair_training_run.csv"
MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_REPAIR_TRAINING_RUN_TEXT_FILENAME = "model_capability_route_promotion_bounded_real_replay_repair_training_run.txt"
MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_REPAIR_TRAINING_RUN_MARKDOWN_FILENAME = "model_capability_route_promotion_bounded_real_replay_repair_training_run.md"
MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_REPAIR_TRAINING_RUN_HTML_FILENAME = "model_capability_route_promotion_bounded_real_replay_repair_training_run.html"


def locate_route_promotion_bounded_real_replay_repair_seed(path: str | Path) -> Path:
    source = Path(path)
    if source.is_dir():
...
def read_json_report(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("route promotion bounded repair training input must be a JSON object")
    return dict(payload)


def build_model_capability_route_promotion_bounded_real_replay_repair_training_run(
    repair_seed_report: dict[str, Any],
    run_dir: str | Path,
    *,
    repair_seed_path: str | Path | None = None,
    title: str = "MiniGPT model capability route promotion bounded real replay repair training run",
    ...
    checks = _checks(repair_seed_report, seed_summary, artifacts, metrics, train_config)
    issues = [row for row in checks if row["status"] != "pass"]
    status = "pass" if not issues else "fail"
    training = _training(status, artifacts, metrics, train_config, manifest)
    return {
        "schema_version": 1,
        "title": title,
...
def _checks(
    ...
    return [
        _check("repair_seed_passed", seed.get("status") == "pass", seed.get("status"), "repair seed must pass"),
        _check("repair_seed_ready", seed_summary.get("bounded_real_replay_repair_seed_ready") is True, seed_summary.get("bounded_real_replay_repair_seed_ready"), "repair seed must be ready"),
...
def _training(
    ...
    return {
        ...
        "next_step": "run_repair_checkpoint_bounded_replay" if status == "pass" else "repair_bounded_replay_training_run",
    }


def _summary(status: str, checks: list[dict[str, Any]], training: dict[str, Any]) -> dict[str, Any]:
    return {
        "bounded_real_replay_repair_training_ready": status == "pass" and training.get("ready") is True,
...
def _decision(status: str) -> str:
    if status == "pass":
        return "model_capability_route_promotion_bounded_real_replay_repair_training_run_ready"
    return "fix_model_capability_route_promotion_bounded_real_replay_repair_training_run"


def _interpretation(status: str, training: dict[str, Any]) -> dict[str, Any]:
    if status != "pass":
        return {"model_quality_claim": "not_claimed", "reason": "Repair training artifacts are incomplete.", "next_action": "repair training run"}
    return {
        "model_quality_claim": "training_artifact_only",
        "reason": "A bounded repair checkpoint exists, but model improvement must be proven by replaying the benchmark.",
