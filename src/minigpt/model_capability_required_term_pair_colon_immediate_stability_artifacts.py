...
def render_model_capability_required_term_pair_colon_immediate_stability_text(report: dict[str, Any]) -> str:
...
def write_model_capability_required_term_pair_colon_immediate_stability_csv(report: dict[str, Any], path: str | Path) -> None:
    fieldnames = [
        "seed",
        "status",
        "decision",
...
def render_model_capability_required_term_pair_colon_immediate_stability_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    rows = [
    ...
    return "\n".join(
        [
            ...
            f"- Pair-full seeds: `{summary.get('pair_full_seed_count')}/{summary.get('seed_count')}`",
...
def render_model_capability_required_term_pair_colon_immediate_stability_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ("Status", report.get("status")),
        ...
        ("Pair-full seeds", summary.get("pair_full_seed_count")),
        ("Pair-full rate", summary.get("pair_full_seed_rate")),
        ("Stable", summary.get("stable_pair_full")),
    ]
...
<title>MiniGPT colon-immediate stability</title>
{_style()}
</head>
<body>
<main>
<header><h1>MiniGPT colon-immediate stability</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
<section class="stats">{''.join(_card(label, value) for label, value in stats)}</section>
<section class="panel"><h2>Next Action</h2><p>{html_escape(interpretation.get('next_action'))}</p></section>
<section class="panel">
<h2>Seeds</h2>
<div class="table-wrap"><table>
<thead><tr><th>Seed</th><th>Status</th><th>Pair full</th><th>Default full</th><th>Suppression full</th><th>Output</th></tr></thead>
...
def write_model_capability_required_term_pair_colon_immediate_stability_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    sidecar_outputs = {}
    for seed_report in list_of_dicts(report.get("seed_reports")):
        seed = as_dict(seed_report.get("settings")).get("seed")
        if seed:
            sidecar_outputs[str(seed)] = write_model_capability_required_term_pair_coexistence_refresh_outputs(
                seed_report,
                root / "seed-reports" / f"seed-{seed}",
            )
    report["sidecar_outputs"] = {"seed_reports": sidecar_outputs}
    paths = {
        "json": root / PAIR_COLON_IMMEDIATE_STABILITY_JSON_FILENAME,
        "csv": root / PAIR_COLON_IMMEDIATE_STABILITY_CSV_FILENAME,
        "text": root / PAIR_COLON_IMMEDIATE_STABILITY_TEXT_FILENAME,
        "markdown": root / PAIR_COLON_IMMEDIATE_STABILITY_MARKDOWN_FILENAME,
        "html": root / PAIR_COLON_IMMEDIATE_STABILITY_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_model_capability_required_term_pair_colon_immediate_stability_csv(report, paths["csv"])
    paths["text"].write_text(render_model_capability_required_term_pair_colon_immediate_stability_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_model_capability_required_term_pair_colon_immediate_stability_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_model_capability_required_term_pair_colon_immediate_stability_html(report), encoding="utf-8")
