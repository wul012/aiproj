...
def write_aligned_candidate_seed_stability_csv(report: dict[str, Any], path: str | Path) -> None:
    fieldnames = [
        "seed",
        "status",
        "decision",
...
def render_aligned_candidate_seed_stability_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    ...
    return "\n".join(
        [
            ...
            f"- Corpus mode: `{settings.get('corpus_mode')}`",
...
def render_aligned_candidate_seed_stability_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    settings = as_dict(report.get("settings"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
...
<title>MiniGPT aligned-candidate seed stability</title>
{_style()}
</head>
<body>
<main>
<header>
<h1>MiniGPT aligned-candidate seed stability</h1>
<p>{html_escape(interpretation.get('reason'))}</p>
</header>
<section class="stats">{''.join(_card(label, value) for label, value in stats)}</section>
<section class="panel"><h2>Corpus Mode</h2><p>{html_escape(settings.get('corpus_mode'))}</p></section>
...
def write_aligned_candidate_seed_stability_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    ...
    paths = {
        "json": root / PAIR_ALIGNED_CANDIDATE_SEED_STABILITY_JSON_FILENAME,
        "csv": root / PAIR_ALIGNED_CANDIDATE_SEED_STABILITY_CSV_FILENAME,
        "text": root / PAIR_ALIGNED_CANDIDATE_SEED_STABILITY_TEXT_FILENAME,
        "markdown": root / PAIR_ALIGNED_CANDIDATE_SEED_STABILITY_MARKDOWN_FILENAME,
        "html": root / PAIR_ALIGNED_CANDIDATE_SEED_STABILITY_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_aligned_candidate_seed_stability_csv(report, paths["csv"])
    paths["text"].write_text(render_aligned_candidate_seed_stability_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_aligned_candidate_seed_stability_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_aligned_candidate_seed_stability_html(report), encoding="utf-8")
