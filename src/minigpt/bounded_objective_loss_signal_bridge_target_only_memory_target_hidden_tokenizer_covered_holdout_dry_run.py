...
TARGET_ONLY_MEMORY_TARGET_HIDDEN_TOKENIZER_COVERED_HOLDOUT_DRY_RUN_JSON_FILENAME = (
    "bounded_objective_loss_signal_bridge_target_only_memory_target_hidden_tokenizer_covered_holdout_dry_run.json"
)
TARGET_ONLY_MEMORY_TARGET_HIDDEN_TOKENIZER_COVERED_HOLDOUT_DRY_RUN_CSV_FILENAME = (
    "bounded_objective_loss_signal_bridge_target_only_memory_target_hidden_tokenizer_covered_holdout_dry_run.csv"
)
TARGET_ONLY_MEMORY_TARGET_HIDDEN_TOKENIZER_COVERED_HOLDOUT_DRY_RUN_TEXT_FILENAME = (
    "bounded_objective_loss_signal_bridge_target_only_memory_target_hidden_tokenizer_covered_holdout_dry_run.txt"
)
TARGET_ONLY_MEMORY_TARGET_HIDDEN_TOKENIZER_COVERED_HOLDOUT_DRY_RUN_MARKDOWN_FILENAME = (
    "bounded_objective_loss_signal_bridge_target_only_memory_target_hidden_tokenizer_covered_holdout_dry_run.md"
)
TARGET_ONLY_MEMORY_TARGET_HIDDEN_TOKENIZER_COVERED_HOLDOUT_DRY_RUN_HTML_FILENAME = (
    "bounded_objective_loss_signal_bridge_target_only_memory_target_hidden_tokenizer_covered_holdout_dry_run.html"
)
...
def read_json_report(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("target-hidden tokenizer-covered holdout dry-run input must be a JSON object")
    return dict(payload)


def build_target_hidden_tokenizer_covered_holdout_dry_run(
    ...
    *,
    ...
    title: str = "MiniGPT bounded objective loss signal bridge target-only memory target-hidden tokenizer-covered holdout dry-run",
...
def _checks(
    ...
    ready_key = "bounded_objective_loss_signal_bridge_target_only_memory_target_hidden_tokenizer_covered_holdout_suite_ready"
    return [
        _check("holdout_suite_passed", holdout_suite_report.get("status") == "pass", holdout_suite_report.get("status"), "target-hidden holdout suite must pass"),
        _check("holdout_suite_ready", source_summary.get(ready_key) is True, source_summary.get(ready_key), "target-hidden suite summary must be ready"),
        ...
        _check("target_hidden_cases_present", source_summary.get("target_hidden_case_count") == len(cases), source_summary.get("target_hidden_case_count"), "every source case must be target-hidden"),
...
def _summary(status: str, cases: list[dict[str, Any]], rows: list[dict[str, Any]], issues: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "bounded_objective_loss_signal_bridge_target_only_memory_target_hidden_tokenizer_covered_holdout_dry_run_ready": status == "pass",
...
def _decision(status: str) -> str:
    if status == "pass":
        return "bounded_objective_loss_signal_bridge_target_only_memory_target_hidden_tokenizer_covered_holdout_dry_run_passed"
    return "fix_bounded_objective_loss_signal_bridge_target_only_memory_target_hidden_tokenizer_covered_holdout_dry_run"


def _interpretation(status: str, summary: dict[str, Any]) -> dict[str, Any]:
    if status != "pass":
        return {"model_quality_claim": "not_claimed", "reason": "Target-hidden holdout dry-run checks failed.", "next_action": "repair_target_hidden_holdout_dry_run_inputs"}
    return {
        "model_quality_claim": summary.get("model_quality_claim"),
        "reason": "The target-hidden tokenizer-covered holdout suite scoring contract passes dry-run checks; real replay is now the next evidence step.",
