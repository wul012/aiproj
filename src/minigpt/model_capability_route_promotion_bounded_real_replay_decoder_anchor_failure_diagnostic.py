...
MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_DECODER_ANCHOR_FAILURE_DIAGNOSTIC_JSON_FILENAME = "model_capability_route_promotion_bounded_real_replay_decoder_anchor_failure_diagnostic.json"
MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_DECODER_ANCHOR_FAILURE_DIAGNOSTIC_CSV_FILENAME = "model_capability_route_promotion_bounded_real_replay_decoder_anchor_failure_diagnostic.csv"
MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_DECODER_ANCHOR_FAILURE_DIAGNOSTIC_TEXT_FILENAME = "model_capability_route_promotion_bounded_real_replay_decoder_anchor_failure_diagnostic.txt"
MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_DECODER_ANCHOR_FAILURE_DIAGNOSTIC_MARKDOWN_FILENAME = "model_capability_route_promotion_bounded_real_replay_decoder_anchor_failure_diagnostic.md"
MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_DECODER_ANCHOR_FAILURE_DIAGNOSTIC_HTML_FILENAME = "model_capability_route_promotion_bounded_real_replay_decoder_anchor_failure_diagnostic.html"


def locate_decoder_anchor_replay(path: str | Path) -> Path:
    source = Path(path)
    if source.is_dir():
...
def locate_decoder_anchor_comparison(path: str | Path) -> Path:
    source = Path(path)
    if source.is_dir():
        source = source / MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_DECODER_ANCHOR_CHECKPOINT_COMPARISON_JSON_FILENAME
    return source
...
def read_json_report(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("decoder anchor failure diagnostic input must be a JSON object")
    return dict(payload)


def build_model_capability_route_promotion_bounded_real_replay_decoder_anchor_failure_diagnostic(
    decoder_anchor_replay: dict[str, Any],
    decoder_anchor_comparison: dict[str, Any],
    decoder_anchor_seed_revision: dict[str, Any],
    decoder_anchor_training_run: dict[str, Any],
    *,
    ...
    title: str = "MiniGPT model capability route promotion bounded real replay decoder anchor failure diagnostic",
    generated_at: str | None = None,
) -> dict[str, Any]:
    corpus = Path(corpus_path).read_text(encoding="utf-8-sig") if Path(corpus_path).is_file() else ""
    replay_summary = as_dict(decoder_anchor_replay.get("summary"))
    comparison_summary = as_dict(decoder_anchor_comparison.get("summary"))
    seed_summary = as_dict(decoder_anchor_seed_revision.get("summary"))
    training_summary = as_dict(decoder_anchor_training_run.get("summary"))
    seed_text = "\n".join(str(row.get("text", "")) for row in list_of_dicts(decoder_anchor_seed_revision.get("seed_examples")))
    case_rows = [_case_diagnostic(row, seed_text, corpus) for row in list_of_dicts(decoder_anchor_replay.get("replay_rows"))]
    root_causes = _root_causes(case_rows, comparison_summary, seed_summary, training_summary)
    checks = _checks(decoder_anchor_replay, decoder_anchor_comparison, decoder_anchor_seed_revision, decoder_anchor_training_run, corpus, case_rows)
    ...
    return {
        ...
        "issues": issues,
        "source_decoder_anchor_replay": str(replay_path or ""),
        "source_decoder_anchor_comparison": str(comparison_path or ""),
        "source_decoder_anchor_seed_revision": str(seed_revision_path or ""),
        "source_decoder_anchor_training_run": str(training_run_path or ""),
...
def _diagnosis(prompt_in_corpus: bool, zero_hit: bool, fragment_like: bool, term_corpus_count: dict[str, int]) -> str:
    ...
    if all(count > 0 for count in term_corpus_count.values()):
        return "decoder_anchor_terms_present_but_not_decoded"
    return "decoder_anchor_terms_underrepresented"
...
def _recommended_action(prompt_in_corpus: bool, zero_hit: bool, fragment_like: bool) -> str:
    ...
    if zero_hit and fragment_like:
        return "audit_decoder_anchor_distribution_and_sampling"
    if zero_hit:
        return "add_shorter_unanchored_output_examples"
...
def _root_causes(
    ...
    bridge_count = int(seed_summary.get("bridge_example_count") or 0)
    direct_count = int(seed_summary.get("unanchored_direct_example_count") or 0)
    if failed and bridge_count and direct_count and all(row.get("prompt_in_corpus") is True for row in case_rows):
        causes.append({"cause_id": "decoder_anchor_examples_present_but_replay_zero_hit", "severity": "high", "evidence": {"bridge": bridge_count, "direct": direct_count}, "detail": "The corpus contains decoder-anchor bridge/direct examples, but replay still misses fixed/loss."})
    zero_hit_count = sum(1 for row in case_rows if row.get("zero_hit") is True)
    if zero_hit_count:
        causes.append({"cause_id": "zero_required_term_hits", "severity": "high", "evidence": zero_hit_count, "detail": "Decoder-anchor checkpoint continuations did not contain any expected required terms."})
    ...
    if comparison_summary.get("decoder_anchor_checkpoint_regressed_from_baseline") is True and training_summary.get("final_val_loss") is not None:
        causes.append({"cause_id": "loss_reduction_did_not_transfer_to_bounded_replay", "severity": "medium", "evidence": training_summary.get("final_val_loss"), "detail": "The decoder-anchor run trained successfully, but replay still failed against baseline."})
    return causes
...
def _checks(
    ...
    return [
        _check("decoder_anchor_replay_passed", replay.get("status") == "pass", replay.get("status"), "decoder anchor replay execution must pass"),
        _check("decoder_anchor_comparison_passed", comparison.get("status") == "pass", comparison.get("status"), "decoder anchor comparison must pass"),
        ...
        _check("decoder_anchor_training_passed", training.get("status") == "pass", training.get("status"), "decoder anchor training run must pass"),
        _check("corpus_present", bool(corpus.strip()), len(corpus), "decoder anchor corpus must be readable"),
...
def _diagnostic(status: str, case_rows: list[dict[str, Any]], root_causes: list[dict[str, Any]], comparison_summary: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "decoder_vs_baseline_pass_rate_delta": comparison_summary.get("decoder_vs_baseline_pass_rate_delta"),
        "decoder_vs_prompt_pass_rate_delta": comparison_summary.get("decoder_vs_prompt_pass_rate_delta"),
        "root_cause_count": len(root_causes),
        "proposed_next_artifact": "model_capability_route_promotion_bounded_real_replay_decoder_anchor_distribution_audit",
        "next_step": "audit_decoder_anchor_training_distribution_before_more_training" if status == "pass" else "repair_decoder_anchor_failure_diagnostic_inputs",
    }


def _summary(status: str, checks: list[dict[str, Any]], diagnostic: dict[str, Any]) -> dict[str, Any]:
    return {
        "decoder_anchor_failure_diagnostic_ready": status == "pass" and diagnostic.get("ready") is True,
        ...
        "decoder_vs_baseline_pass_rate_delta": diagnostic.get("decoder_vs_baseline_pass_rate_delta"),
        "decoder_vs_prompt_pass_rate_delta": diagnostic.get("decoder_vs_prompt_pass_rate_delta"),
...
def _decision(status: str) -> str:
    if status == "pass":
        return "model_capability_route_promotion_bounded_real_replay_decoder_anchor_failure_diagnostic_ready"
    return "fix_model_capability_route_promotion_bounded_real_replay_decoder_anchor_failure_diagnostic"


def _interpretation(status: str, diagnostic: dict[str, Any]) -> dict[str, Any]:
    if status != "pass":
        return {"model_quality_claim": "not_claimed", "reason": "Decoder anchor failure diagnostic inputs are incomplete.", "next_action": "repair diagnostic inputs"}
    return {
        "model_quality_claim": "not_improved",
        "reason": "Decoder-anchor examples are present, but replay output still fails to emit required terms.",
        "next_action": diagnostic.get("next_step"),
    }


__all__ = [
    "MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_DECODER_ANCHOR_FAILURE_DIAGNOSTIC_CSV_FILENAME",
    "MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_DECODER_ANCHOR_FAILURE_DIAGNOSTIC_HTML_FILENAME",
    "MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_DECODER_ANCHOR_FAILURE_DIAGNOSTIC_JSON_FILENAME",
    "MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_DECODER_ANCHOR_FAILURE_DIAGNOSTIC_MARKDOWN_FILENAME",
