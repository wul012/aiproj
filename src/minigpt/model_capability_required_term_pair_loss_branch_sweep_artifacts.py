...
from minigpt.model_capability_required_term_pair_loss_branch_sweep import (
    REQUIRED_TERM_PAIR_LOSS_BRANCH_SWEEP_HTML_FILENAME,
...
def render_model_capability_required_term_pair_loss_branch_sweep_text(report: dict[str, Any]) -> str:
    ...
    rows = [
        ...
        ("source_contrast_free_training_decision", summary.get("source_contrast_free_training_decision")),
        ...
        ("best_variant_focus_hit", summary.get("best_variant_focus_hit")),
...
def write_model_capability_required_term_pair_loss_branch_sweep_csv(
    report: dict[str, Any],
    path: str | Path,
) -> None:
    fieldnames = [
...
def render_model_capability_required_term_pair_loss_branch_sweep_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    variant_table = [
        ...
        variant_table.append(
            ...
            + " | ".join(
                [
                    ...
                    markdown_cell(row.get("focus_missed_term")),
                    markdown_cell(", ".join(str(term) for term in row.get("hit_terms") or [])),
                    markdown_cell(", ".join(str(term) for term in row.get("dropped_source_hit_terms") or [])),
                    markdown_cell(row.get("pair_full_hit")),
                    markdown_cell(row.get("branch_tradeoff")),
                ]
            )
            + " |"
        )
        ...
        target_table.append(
            ...
            + " | ".join(
                [
                    ...
                    markdown_cell(row.get("focus_term_hit_variant_count")),
                    markdown_cell(row.get("pair_full_hit_variant_count")),
    ...
    return "\n".join(
        [
            ...
            f"- Sweep decision: `{summary.get('loss_branch_sweep_decision')}`",
            f"- Source contrast-free decision: `{summary.get('source_contrast_free_training_decision')}`",
            ...
            f"- Focus-hit variants: `{summary.get('focus_term_hit_variant_count')}`",
...
def render_model_capability_required_term_pair_loss_branch_sweep_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ("Status", report.get("status")),
        ("Decision", summary.get("loss_branch_sweep_decision")),
        ("Targets", summary.get("target_count")),
        ("Variants", summary.get("variant_count")),
        ("Probe hits", summary.get("probe_hit_count")),
        ("Focus hits", summary.get("focus_term_hit_variant_count")),
...
<title>MiniGPT pair loss-branch sweep</title>
{_style()}
</head>
<body>
<main>
<header><h1>MiniGPT pair loss-branch sweep</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
<section class="stats">{''.join(_card(label, value) for label, value in stats)}</section>
<section class="panel"><h2>Next Action</h2><p>{html_escape(interpretation.get('next_action'))}</p></section>
<section class="panel">
<h2>Variant Results</h2>
<div class="table-wrap"><table>
<thead><tr><th>Pair</th><th>Variant</th><th>Focus</th><th>Hit terms</th><th>Dropped</th><th>Full</th><th>Tradeoff</th></tr></thead>
...
<h2>Training Runs</h2>
<div class="table-wrap"><table>
<thead><tr><th>Run</th><th>Lines</th><th>Order</th><th>Weight</th><th>Anchor</th><th>Checkpoint</th></tr></thead>
...
def write_model_capability_required_term_pair_loss_branch_sweep_outputs(
    report: dict[str, Any],
    out_dir: str | Path,
) -> dict[str, str]:
    root = Path(out_dir)
    ...
    paths = {
        "json": root / REQUIRED_TERM_PAIR_LOSS_BRANCH_SWEEP_JSON_FILENAME,
        "csv": root / "model_capability_required_term_pair_loss_branch_sweep.csv",
        "text": root / REQUIRED_TERM_PAIR_LOSS_BRANCH_SWEEP_TEXT_FILENAME,
        "markdown": root / REQUIRED_TERM_PAIR_LOSS_BRANCH_SWEEP_MARKDOWN_FILENAME,
        "html": root / REQUIRED_TERM_PAIR_LOSS_BRANCH_SWEEP_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_model_capability_required_term_pair_loss_branch_sweep_csv(report, paths["csv"])
    paths["text"].write_text(render_model_capability_required_term_pair_loss_branch_sweep_text(report), encoding="utf-8")
    paths["markdown"].write_text(
        render_model_capability_required_term_pair_loss_branch_sweep_markdown(report),
        encoding="utf-8",
    )
    paths["html"].write_text(render_model_capability_required_term_pair_loss_branch_sweep_html(report), encoding="utf-8")
...
def _variant_row_html(row: dict[str, Any]) -> str:
    return (
        ...
        f"<td>{html_escape(row.get('focus_missed_term'))}</td>"
        f"<td>{html_escape(', '.join(str(term) for term in row.get('hit_terms') or []))}</td>"
        f"<td>{html_escape(', '.join(str(term) for term in row.get('dropped_source_hit_terms') or []))}</td>"
        f"<td>{html_escape(row.get('pair_full_hit'))}</td>"
        f"<td>{html_escape(row.get('branch_tradeoff'))}</td>"
        "</tr>"
    )
...
def _training_row_html(row: dict[str, Any]) -> str:
    return (
        "<tr>"
        f"<td>{html_escape(row.get('loss_branch_run_id'))}</td>"
        f"<td>{html_escape(row.get('loss_branch_line_count'))}</td>"
        f"<td>{html_escape(row.get('term_order'))}</td>"
        f"<td>{html_escape(row.get('missed_weight'))}</td>"
        f"<td>{html_escape(row.get('missed_anchor_repeat'))}</td>"
...
def _style() -> str:
    return """<style>
:root{color-scheme:light;--ink:#172026;--muted:#5d6975;--line:#d8dee4;--panel:#f7f9fb;--accent:#9f1239}
