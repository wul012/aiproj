...
def render_loss_internal_preference_objective_comparison_text(report: dict[str, Any]) -> str:
...
def render_loss_internal_preference_objective_comparison_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    return "\n".join(
        [
            ...
            f"- Fixed-only reports: `{summary.get('fixed_only_tradeoff_report_count')}`",
            f"- Loss-only reports: `{summary.get('loss_only_tradeoff_report_count')}`",
            f"- Union hit terms: `{summary.get('union_hit_terms')}`",
            "",
            "## Branch Rows",
            "",
...
def render_loss_internal_preference_objective_comparison_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ("Status", report.get("status")),
...
<title>MiniGPT loss-internal-preference objective comparison</title>
{_style()}
</head>
<body>
<main>
<header><h1>MiniGPT loss-internal-preference objective comparison</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
<section class="stats">{''.join(_card(label, value) for label, value in stats)}</section>
<section class="panel"><h2>Next Action</h2><p>{html_escape(interpretation.get('next_action'))}</p></section>
<section class="panel"><h2>Branch Rows</h2><div class="table-wrap"><table>
<thead><tr><th>Route</th><th>Hit Terms</th><th>Missed Terms</th><th>Pair Full</th></tr></thead>
<tbody>{rows}</tbody>
</table></div></section>
</main>
</body>
...
def write_loss_internal_preference_objective_comparison_outputs(
    report: dict[str, Any], out_dir: str | Path
) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / PAIR_LOSS_INTERNAL_PREFERENCE_OBJECTIVE_COMPARISON_JSON_FILENAME,
        "csv": root / PAIR_LOSS_INTERNAL_PREFERENCE_OBJECTIVE_COMPARISON_CSV_FILENAME,
        "text": root / PAIR_LOSS_INTERNAL_PREFERENCE_OBJECTIVE_COMPARISON_TEXT_FILENAME,
        "markdown": root / PAIR_LOSS_INTERNAL_PREFERENCE_OBJECTIVE_COMPARISON_MARKDOWN_FILENAME,
        "html": root / PAIR_LOSS_INTERNAL_PREFERENCE_OBJECTIVE_COMPARISON_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    _write_csv(report, paths["csv"])
    paths["text"].write_text(render_loss_internal_preference_objective_comparison_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_loss_internal_preference_objective_comparison_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_loss_internal_preference_objective_comparison_html(report), encoding="utf-8")
...
def _branch_markdown_rows(report: dict[str, Any]) -> list[str]:
    rows = ["| Route | Hit Terms | Missed Terms | Pair Full |", "| --- | --- | --- | --- |"]
    for row in list_of_dicts(report.get("branch_rows")):
        rows.append(
            "| "
            + " | ".join(
