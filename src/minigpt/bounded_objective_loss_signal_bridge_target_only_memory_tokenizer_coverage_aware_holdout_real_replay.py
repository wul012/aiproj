...
TARGET_ONLY_MEMORY_TOKENIZER_COVERAGE_AWARE_HOLDOUT_REAL_REPLAY_JSON_FILENAME = (
    "bounded_objective_loss_signal_bridge_target_only_memory_tokenizer_coverage_aware_holdout_real_replay.json"
)
TARGET_ONLY_MEMORY_TOKENIZER_COVERAGE_AWARE_HOLDOUT_REAL_REPLAY_CSV_FILENAME = (
    "bounded_objective_loss_signal_bridge_target_only_memory_tokenizer_coverage_aware_holdout_real_replay.csv"
)
TARGET_ONLY_MEMORY_TOKENIZER_COVERAGE_AWARE_HOLDOUT_REAL_REPLAY_TEXT_FILENAME = (
    "bounded_objective_loss_signal_bridge_target_only_memory_tokenizer_coverage_aware_holdout_real_replay.txt"
)
TARGET_ONLY_MEMORY_TOKENIZER_COVERAGE_AWARE_HOLDOUT_REAL_REPLAY_MARKDOWN_FILENAME = (
    "bounded_objective_loss_signal_bridge_target_only_memory_tokenizer_coverage_aware_holdout_real_replay.md"
)
TARGET_ONLY_MEMORY_TOKENIZER_COVERAGE_AWARE_HOLDOUT_REAL_REPLAY_HTML_FILENAME = (
    "bounded_objective_loss_signal_bridge_target_only_memory_tokenizer_coverage_aware_holdout_real_replay.html"
)
...
def locate_tokenizer_coverage_aware_holdout_dry_run(path: str | Path) -> Path:
    source = Path(path)
    if source.is_dir():
        source = source / TARGET_ONLY_MEMORY_TOKENIZER_COVERAGE_AWARE_HOLDOUT_DRY_RUN_JSON_FILENAME
    return source
...
def read_json_report(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("tokenizer-coverage-aware holdout real replay input must be a JSON object")
    return dict(payload)


def build_tokenizer_coverage_aware_holdout_real_replay(
    ...
    *,
    ...
    title: str = "MiniGPT bounded objective loss signal bridge target-only memory tokenizer-coverage-aware holdout real replay",
...
def _run_cases(
    ...
    for case in cases:
        ...
        except Exception as exc:  # pragma: no cover - exercised by fake runner tests.
...
def _checks(
    ...
    return [
        _check("holdout_suite_passed", suite_report.get("status") == "pass", suite_report.get("status"), "holdout suite must pass"),
        _check("holdout_suite_ready", suite_summary.get("bounded_objective_loss_signal_bridge_target_only_memory_tokenizer_coverage_aware_holdout_suite_ready") is True, suite_summary.get("bounded_objective_loss_signal_bridge_target_only_memory_tokenizer_coverage_aware_holdout_suite_ready"), "holdout suite summary must be ready"),
        _check("dry_run_passed", dry_run_report.get("status") == "pass", dry_run_report.get("status"), "dry-run must pass before real replay"),
        _check("dry_run_ready", dry_summary.get("bounded_objective_loss_signal_bridge_target_only_memory_tokenizer_coverage_aware_holdout_dry_run_ready") is True, dry_summary.get("bounded_objective_loss_signal_bridge_target_only_memory_tokenizer_coverage_aware_holdout_dry_run_ready"), "dry-run summary must be ready"),
...
def _summary(status: str, cases: list[dict[str, Any]], rows: list[dict[str, Any]], issues: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "bounded_objective_loss_signal_bridge_target_only_memory_tokenizer_coverage_aware_holdout_real_replay_ready": status == "pass",
...
def _decision(status: str, summary: dict[str, Any]) -> str:
    if status != "pass":
        return "fix_bounded_objective_loss_signal_bridge_target_only_memory_tokenizer_coverage_aware_holdout_real_replay_inputs"
    if summary.get("holdout_model_quality_ready") is True:
        return "bounded_objective_loss_signal_bridge_target_only_memory_tokenizer_coverage_aware_holdout_real_replay_passed_review_required"
    if int(summary.get("any_hit_case_count") or 0) > 0:
        return "bounded_objective_loss_signal_bridge_target_only_memory_tokenizer_coverage_aware_holdout_real_replay_partial_model_gap"
    return "bounded_objective_loss_signal_bridge_target_only_memory_tokenizer_coverage_aware_holdout_real_replay_zero_hit_model_gap"


def _interpretation(status: str, summary: dict[str, Any]) -> dict[str, Any]:
    if status != "pass":
        return {"model_quality_claim": "not_claimed", "reason": "Tokenizer-covered real replay inputs failed.", "next_action": "repair_real_replay_inputs"}
    if summary.get("holdout_model_quality_ready") is True:
        return {"model_quality_claim": summary.get("model_quality_claim"), "reason": "The checkpoint passed every tokenizer-covered holdout case; review remains required before promotion.", "next_action": summary.get("next_step")}
    return {"model_quality_claim": summary.get("model_quality_claim"), "reason": "The tokenizer-covered holdout suite executed, but the checkpoint did not pass every case.", "next_action": summary.get("next_step")}
