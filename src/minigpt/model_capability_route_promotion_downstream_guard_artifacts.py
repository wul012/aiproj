...
def render_model_capability_route_promotion_downstream_guard_text(report: dict[str, Any]) -> str:
    ...
    rows = [
        ...
        ("downstream_guard_ready", summary.get("downstream_guard_ready")),
        ("access_allowed", summary.get("access_allowed")),
...
def write_model_capability_route_promotion_downstream_guard_csv(report: dict[str, Any], path: str | Path) -> None:
...
def render_model_capability_route_promotion_downstream_guard_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT model capability route promotion downstream guard'))}",
        "",
        f"- Status: `{report.get('status')}`",
        f"- Decision: `{report.get('decision')}`",
        f"- Access allowed: `{summary.get('access_allowed')}`",
...
def render_model_capability_route_promotion_downstream_guard_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    stats = [
        ("Status", report.get("status")),
        ("Decision", report.get("decision")),
        ("Allowed", summary.get("access_allowed")),
        ("Consumer", summary.get("consumer_name")),
        ("Route", summary.get("route_id")),
        ("Scope", summary.get("allowed_scope")),
        ("Boundary", summary.get("boundary")),
    ]
...
<title>{html_escape(report.get('title', 'MiniGPT model capability route promotion downstream guard'))}</title>
{_style()}
</head>
<body>
<main>
<header><h1>{html_escape(report.get('title', 'MiniGPT model capability route promotion downstream guard'))}</h1><p>Checks whether a downstream consumer may use a contract-verified bounded route.</p></header>
...
def write_model_capability_route_promotion_downstream_guard_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / MODEL_CAPABILITY_ROUTE_PROMOTION_DOWNSTREAM_GUARD_JSON_FILENAME,
        "csv": root / MODEL_CAPABILITY_ROUTE_PROMOTION_DOWNSTREAM_GUARD_CSV_FILENAME,
        "text": root / MODEL_CAPABILITY_ROUTE_PROMOTION_DOWNSTREAM_GUARD_TEXT_FILENAME,
        "markdown": root / MODEL_CAPABILITY_ROUTE_PROMOTION_DOWNSTREAM_GUARD_MARKDOWN_FILENAME,
        "html": root / MODEL_CAPABILITY_ROUTE_PROMOTION_DOWNSTREAM_GUARD_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_model_capability_route_promotion_downstream_guard_csv(report, paths["csv"])
    paths["text"].write_text(render_model_capability_route_promotion_downstream_guard_text(report), encoding="utf-8")
...
body{margin:0;background:#f2f4f8;color:var(--ink);font-family:Arial,"Microsoft YaHei",sans-serif}
