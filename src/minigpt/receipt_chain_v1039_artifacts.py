...
from minigpt.receipt_chain_v1039 import (
    RECEIPT_CHAIN_V1039_CSV_FILENAME,
    RECEIPT_CHAIN_V1039_HTML_FILENAME,
    RECEIPT_CHAIN_V1039_JSON_FILENAME,
    RECEIPT_CHAIN_V1039_MARKDOWN_FILENAME,
    RECEIPT_CHAIN_V1039_TEXT_FILENAME,
    READY_KEY,
)
...
def render_receipt_v1039_artifacts_text(report: dict[str, Any]) -> str:
...
def write_receipt_v1039_artifacts_csv(report: dict[str, Any], path: str | Path) -> None:
...
def render_receipt_v1039_artifacts_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
...
def render_receipt_v1039_artifacts_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    receipt = as_dict(report.get("receipt"))
...
def write_receipt_v1039_artifacts_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / RECEIPT_CHAIN_V1039_JSON_FILENAME,
        "csv": root / RECEIPT_CHAIN_V1039_CSV_FILENAME,
        "text": root / RECEIPT_CHAIN_V1039_TEXT_FILENAME,
        "markdown": root / RECEIPT_CHAIN_V1039_MARKDOWN_FILENAME,
        "html": root / RECEIPT_CHAIN_V1039_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_receipt_v1039_artifacts_csv(report, paths["csv"])
    paths["text"].write_text(render_receipt_v1039_artifacts_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_receipt_v1039_artifacts_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_receipt_v1039_artifacts_html(report), encoding="utf-8")
