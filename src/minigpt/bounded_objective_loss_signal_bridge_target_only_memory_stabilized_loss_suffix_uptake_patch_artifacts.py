...
def render_stabilized_loss_suffix_uptake_patch_text(report: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"stabilized_loss_suffix_uptake_patch_ready={summary.get('bounded_objective_loss_signal_bridge_target_only_memory_stabilized_loss_suffix_uptake_patch_ready')}",
        f"patch_example_count={summary.get('patch_example_count')}",
        f"fixed_l_to_loss_uptake_count={summary.get('fixed_l_to_loss_uptake_count')}",
        f"fixed_lo_to_loss_uptake_count={summary.get('fixed_lo_to_loss_uptake_count')}",
        f"global_suffix_uptake_count={summary.get('global_suffix_uptake_count')}",
        f"surface_pair_carry_forward_count={summary.get('surface_pair_carry_forward_count')}",
...
def render_stabilized_loss_suffix_uptake_patch_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT bounded objective loss signal bridge target-only memory stabilized loss-suffix uptake patch'))}",
        "",
        f"- Status: `{report.get('status')}`",
        f"- Decision: `{report.get('decision')}`",
        f"- Patch examples: `{summary.get('patch_example_count')}`",
        f"- Fixed-l uptake: `{summary.get('fixed_l_to_loss_uptake_count')}`",
        f"- Fixed-lo uptake: `{summary.get('fixed_lo_to_loss_uptake_count')}`",
        f"- Global suffix: `{summary.get('global_suffix_uptake_count')}`",
        f"- Surface carry-forward: `{summary.get('surface_pair_carry_forward_count')}`",
        "",
        "## Patch Examples",
        "",
...
def render_stabilized_loss_suffix_uptake_patch_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    ...
    stats = [
        ...
        ("Fixed-l", summary.get("fixed_l_to_loss_uptake_count")),
        ("Fixed-lo", summary.get("fixed_lo_to_loss_uptake_count")),
        ("Global", summary.get("global_suffix_uptake_count")),
        ("Carry", summary.get("surface_pair_carry_forward_count")),
...
<title>{html_escape(report.get('title', 'MiniGPT bounded objective loss signal bridge target-only memory stabilized loss-suffix uptake patch'))}</title>{_style()}</head>
<body><main>
<header><h1>{html_escape(report.get('title', 'MiniGPT bounded objective loss signal bridge target-only memory stabilized loss-suffix uptake patch'))}</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
...
def write_stabilized_loss_suffix_uptake_patch_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / TARGET_ONLY_MEMORY_STABILIZED_LOSS_SUFFIX_UPTAKE_PATCH_JSON_FILENAME,
        "csv": root / TARGET_ONLY_MEMORY_STABILIZED_LOSS_SUFFIX_UPTAKE_PATCH_CSV_FILENAME,
        "jsonl": root / TARGET_ONLY_MEMORY_STABILIZED_LOSS_SUFFIX_UPTAKE_PATCH_JSONL_FILENAME,
        "corpus": root / TARGET_ONLY_MEMORY_STABILIZED_LOSS_SUFFIX_UPTAKE_PATCH_CORPUS_FILENAME,
        "text": root / TARGET_ONLY_MEMORY_STABILIZED_LOSS_SUFFIX_UPTAKE_PATCH_TEXT_FILENAME,
        "markdown": root / TARGET_ONLY_MEMORY_STABILIZED_LOSS_SUFFIX_UPTAKE_PATCH_MARKDOWN_FILENAME,
        "html": root / TARGET_ONLY_MEMORY_STABILIZED_LOSS_SUFFIX_UPTAKE_PATCH_HTML_FILENAME,
    }
    ...
    paths["text"].write_text(render_stabilized_loss_suffix_uptake_patch_text(report), encoding="utf-8")
...
def _style() -> str:
    return """<style>
:root{color-scheme:light;--ink:#17212b;--muted:#617182;--line:#d7dee8;--panel:#f8fafc;--accent:#1f6f4a}
*{box-sizing:border-box}body{margin:0;background:#f6f8f5;color:var(--ink);font-family:Arial,"Microsoft YaHei",sans-serif}
