...
def render_training_scale_plan_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- Suite: `{_dict(report.get('suite')).get('path')}`",
...
def render_training_scale_plan_html(report: dict[str, Any]) -> str:
    ...
    stats = [
        ...
        ("Suite", _dict(report.get("suite")).get("path")),
    ]
    return "\n".join(
        [
            "<!doctype html>",
            ...
            _batch_panel(batch, _dict(report.get("suite"))),
...
def _batch_panel(batch: dict[str, Any], suite: dict[str, Any]) -> str:
    return (
        ...
        f"<p><strong>Suite:</strong> {_e(suite.get('path'))}</p>"
