...
PAIR_FIXED_RETENTION_OBJECTIVE_COMPARISON_JSON_FILENAME = "model_capability_required_term_pair_fixed_retention_objective_comparison.json"
PAIR_FIXED_RETENTION_OBJECTIVE_COMPARISON_CSV_FILENAME = "model_capability_required_term_pair_fixed_retention_objective_comparison.csv"
PAIR_FIXED_RETENTION_OBJECTIVE_COMPARISON_TEXT_FILENAME = "model_capability_required_term_pair_fixed_retention_objective_comparison.txt"
PAIR_FIXED_RETENTION_OBJECTIVE_COMPARISON_MARKDOWN_FILENAME = "model_capability_required_term_pair_fixed_retention_objective_comparison.md"
PAIR_FIXED_RETENTION_OBJECTIVE_COMPARISON_HTML_FILENAME = "model_capability_required_term_pair_fixed_retention_objective_comparison.html"
TARGET_TERMS = ("fixed", "loss")


def locate_fixed_retention_objective_refresh_report(path: str | Path) -> Path:
    source = Path(path)
    if source.is_dir():
...
def read_fixed_retention_objective_refresh_report(path: str | Path) -> dict[str, Any]:
    payload = json.loads(Path(path).read_text(encoding="utf-8-sig"))
    if not isinstance(payload, dict):
        raise ValueError("fixed-retention objective comparison input must be a JSON object")
    return dict(payload)


def build_model_capability_required_term_pair_fixed_retention_objective_comparison(
    ...
    *,
    ...
    return {
        "schema_version": 1,
        "title": "MiniGPT required-term pair fixed-retention objective comparison",
...
def _source_label(labels: Sequence[str], index: int, report: dict[str, Any], paths: Sequence[str]) -> str:
    ...
    if "first_token" in mode:
        return "fixed-retention-first-token"
    if "prompt_guard" in mode:
        return "fixed-retention-prompt-guard"
    if "balanced" in mode:
        return "fixed-retention-balanced"
    if index < len(paths) and paths[index]:
        return Path(paths[index]).parent.name
    return f"fixed-retention-report-{index + 1}"
...
def _issues(report_rows: list[dict[str, Any]], term_rows: list[dict[str, Any]]) -> list[str]:
    ...
    if len(report_rows) < 2:
        issues.append("at least two fixed-retention objective reports are required")
    for row in report_rows:
        ...
        if "fixed_retention" not in str(row.get("corpus_mode") or ""):
            issues.append(f"{label} is not a fixed-retention objective corpus mode")
    if not term_rows:
...
def _summary(report_rows: list[dict[str, Any]], branch_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "fixed_recovery_route": _first_route(branch_rows, "fixed_only_tradeoff"),
    }
...
def _decision(status: str, summary: dict[str, Any]) -> str:
    if status != "pass":
        return "fix_required_term_pair_fixed_retention_comparison_inputs"
    if int(summary.get("pair_full_report_count") or 0) > 0:
        return "promote_fixed_retention_pair_full_candidate"
    if summary.get("mixed_tradeoff_observed"):
        return "fixed_retention_objectives_confirm_branch_tradeoff"
    if int(summary.get("fixed_only_tradeoff_report_count") or 0) > 0:
        return "select_fixed_retention_route_for_loss_rebalance"
    return "fixed_retention_objectives_did_not_recover_fixed"
...
def _interpretation(status: str, summary: dict[str, Any]) -> dict[str, Any]:
    if status != "pass":
        return {
            ...
            "reason": "The fixed-retention objective reports are not structurally comparable.",
            "next_action": "repair comparison inputs before another training run",
        }
    if int(summary.get("pair_full_report_count") or 0) > 0:
        return {
            "model_quality_claim": "pair_full_candidate",
            "reason": "At least one fixed-retention objective reached pair-full.",
    ...
    if summary.get("mixed_tradeoff_observed"):
        return {
            ...
            "next_action": "select the fixed recovery route for a loss-rebalance objective or stability replay",
        }
    return {
        "model_quality_claim": "comparison_only",
        "reason": "The compared routes do not yet provide pair-full evidence.",
        "next_action": "inspect first-token preferences before another objective",
    }
