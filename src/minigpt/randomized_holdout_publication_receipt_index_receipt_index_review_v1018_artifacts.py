...
def render_randomized_holdout_publication_receipt_index_receipt_index_review_v1018_text(report: dict[str, Any]) -> str:
...
def write_randomized_holdout_publication_receipt_index_receipt_index_review_v1018_csv(report: dict[str, Any], path: str | Path) -> None:
...
def render_randomized_holdout_publication_receipt_index_receipt_index_review_v1018_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    review = as_dict(report.get("review"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT randomized holdout publication receipt index receipt index review v1018'))}",
        "",
...
def render_randomized_holdout_publication_receipt_index_receipt_index_review_v1018_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    review = as_dict(report.get("review"))
...
def write_randomized_holdout_publication_receipt_index_receipt_index_review_v1018_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_INDEX_RECEIPT_INDEX_REVIEW_V1018_JSON_FILENAME,
        "csv": root / RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_INDEX_RECEIPT_INDEX_REVIEW_V1018_CSV_FILENAME,
        "text": root / RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_INDEX_RECEIPT_INDEX_REVIEW_V1018_TEXT_FILENAME,
        "markdown": root / RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_INDEX_RECEIPT_INDEX_REVIEW_V1018_MARKDOWN_FILENAME,
        "html": root / RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_INDEX_RECEIPT_INDEX_REVIEW_V1018_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_randomized_holdout_publication_receipt_index_receipt_index_review_v1018_csv(report, paths["csv"])
    paths["text"].write_text(render_randomized_holdout_publication_receipt_index_receipt_index_review_v1018_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_randomized_holdout_publication_receipt_index_receipt_index_review_v1018_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_randomized_holdout_publication_receipt_index_receipt_index_review_v1018_html(report), encoding="utf-8")
    return {key: str(value) for key, value in paths.items()}


def _receipt_index_row(row: dict[str, Any]) -> str:
    keys = ["lookup_key", "receipt_id", "receipt_status", "granted_use", "contract_check_ready", "promotion_ready"]
...
__all__ = [
    "render_randomized_holdout_publication_receipt_index_receipt_index_review_v1018_html",
    "render_randomized_holdout_publication_receipt_index_receipt_index_review_v1018_markdown",
    "render_randomized_holdout_publication_receipt_index_receipt_index_review_v1018_text",
    "write_randomized_holdout_publication_receipt_index_receipt_index_review_v1018_outputs",
]
