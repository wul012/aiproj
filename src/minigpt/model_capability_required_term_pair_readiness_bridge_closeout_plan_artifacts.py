...
def render_bridge_closeout_plan_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- Closed route: `{plan.get('closed_route')}`",
            f"- Proposed next artifact: `{plan.get('proposed_next_artifact')}`",
            "",
            "## Next Contract Requirements",
            "",
            *[f"- {markdown_cell(item)}" for item in plan.get("next_contract_requirements", [])],
            "",
            "## Checks",
            "",
            *checks,
...
def render_bridge_closeout_plan_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    plan = as_dict(report.get("plan"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ...
        ("Closed", summary.get("closed_route")),
        ("Next", summary.get("proposed_next_artifact")),
    ]
    requirements = "".join(f"<li>{html_escape(item)}</li>" for item in plan.get("next_contract_requirements", []))
...
<title>MiniGPT bridge closeout plan</title>
{_style()}
</head>
<body>
<main>
<header><h1>MiniGPT bridge closeout plan</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
<section class="stats">{''.join(_card(label, value) for label, value in stats)}</section>
<section class="panel"><h2>Next Contract Requirements</h2><ul>{requirements}</ul></section>
...
def write_bridge_closeout_plan_csv(report: dict[str, Any], path: str | Path) -> None:
...
def write_bridge_closeout_plan_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / PAIR_READINESS_BRIDGE_CLOSEOUT_PLAN_JSON_FILENAME,
        "csv": root / PAIR_READINESS_BRIDGE_CLOSEOUT_PLAN_CSV_FILENAME,
        "text": root / PAIR_READINESS_BRIDGE_CLOSEOUT_PLAN_TEXT_FILENAME,
        "markdown": root / PAIR_READINESS_BRIDGE_CLOSEOUT_PLAN_MARKDOWN_FILENAME,
        "html": root / PAIR_READINESS_BRIDGE_CLOSEOUT_PLAN_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_bridge_closeout_plan_csv(report, paths["csv"])
    paths["text"].write_text(render_bridge_closeout_plan_text(report), encoding="utf-8")
