...
def render_randomized_target_hidden_holdout_dry_run_text(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    rows = [
        ("status", report.get("status")),
        ("decision", report.get("decision")),
        ("failed_count", report.get("failed_count")),
        ("randomized_target_hidden_holdout_dry_run_ready", summary.get("randomized_target_hidden_holdout_dry_run_ready")),
        ...
        ("source_unique_prompt_count", summary.get("source_unique_prompt_count")),
...
def write_randomized_target_hidden_holdout_dry_run_csv(report: dict[str, Any], path: str | Path) -> None:
    fieldnames = ["case_id", "draw", "positive_case_pass", "negative_case_pass", "positive_hit_terms", "negative_hit_terms", "negative_missed_terms"]
    out_path = Path(path)
...
def render_randomized_target_hidden_holdout_dry_run_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT randomized target-hidden holdout dry-run'))}",
        "",
        f"- Status: `{report.get('status')}`",
        f"- Decision: `{report.get('decision')}`",
        f"- Cases: `{summary.get('case_count')}`",
        f"- Source random seed: `{summary.get('source_random_seed')}`",
        f"- Source randomized factor: `{summary.get('source_randomized_case_factor')}`",
...
def render_randomized_target_hidden_holdout_dry_run_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ("Status", report.get("status")),
        ("Cases", summary.get("case_count")),
        ("Seed", summary.get("source_random_seed")),
        ("Factor", summary.get("source_randomized_case_factor")),
    ...
    return f"""<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><link rel="icon" href="data:,"><title>{html_escape(report.get('title', 'MiniGPT randomized target-hidden holdout dry-run'))}</title>{_style()}</head>
<body><main><header><h1>{html_escape(report.get('title', 'MiniGPT randomized target-hidden holdout dry-run'))}</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
<section class="stats">{''.join(_card(label, value) for label, value in stats)}</section>
<section class="panel"><h2>Dry-Run Rows</h2><div class="table-wrap"><table><thead><tr><th>Case</th><th>Draw</th><th>Positive pass</th><th>Negative pass</th><th>Positive hit</th><th>Negative hit</th><th>Negative missed</th></tr></thead><tbody>{rows}</tbody></table></div></section>
...
def write_randomized_target_hidden_holdout_dry_run_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    ...
    paths["text"].write_text(render_randomized_target_hidden_holdout_dry_run_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_randomized_target_hidden_holdout_dry_run_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_randomized_target_hidden_holdout_dry_run_html(report), encoding="utf-8")
