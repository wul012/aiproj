...
) + PAIR_BRANCH_BINDING_CORPUS_MODES + PAIR_TARGET_ANCHOR_CORPUS_MODES + PAIR_LOSS_BRANCH_OBJECTIVE_CORPUS_MODES
...
def source_prompts(corpus_mode: str) -> tuple[str, str]:
    if corpus_mode in (
        ...
        "equals_surface_no_pair_id_loss_balanced_first_token_repair",
        "equals_surface_no_pair_id_loss_balanced_light_first_token_repair",
    ) or is_pair_branch_binding_corpus_mode(corpus_mode) or is_pair_target_anchor_corpus_mode(corpus_mode) or is_pair_loss_branch_objective_corpus_mode(corpus_mode):
        return "fixed=", "loss="
    return "fixed:", "loss:"
