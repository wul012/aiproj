...
def render_pair_readiness_split_contract_text(report: dict[str, Any]) -> str:
    ...
    rows = [
        ...
        ("heldout_pair_probe", summary.get("heldout_pair_probe")),
...
def render_pair_readiness_split_contract_markdown(report: dict[str, Any]) -> str:
...
def render_pair_readiness_split_contract_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    contract = as_dict(report.get("contract"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ...
        ("Train rows", summary.get("training_row_count")),
        ("Eval probes", summary.get("evaluation_probe_count")),
        ("Heldout", summary.get("heldout_pair_probe")),
    ]
    checks = "".join(_check_html(row) for row in list_of_dicts(report.get("check_rows")))
    train_rows = "".join(f"<li>{html_escape(row)}</li>" for row in contract.get("training_rows", []))
...
<title>MiniGPT pair-readiness split contract</title>
{_style()}
</head>
<body>
<main>
<header><h1>MiniGPT pair-readiness split contract</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
...
<section class="grid"><article class="panel"><h2>Training Rows</h2><ul>{train_rows}</ul></article><article class="panel"><h2>Evaluation Probes</h2><ul>{probes}</ul></article></section>
...
def write_pair_readiness_split_contract_csv(report: dict[str, Any], path: str | Path) -> None:
...
def write_pair_readiness_split_contract_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / PAIR_READINESS_SPLIT_CONTRACT_JSON_FILENAME,
        "csv": root / PAIR_READINESS_SPLIT_CONTRACT_CSV_FILENAME,
        "text": root / PAIR_READINESS_SPLIT_CONTRACT_TEXT_FILENAME,
        "markdown": root / PAIR_READINESS_SPLIT_CONTRACT_MARKDOWN_FILENAME,
        "html": root / PAIR_READINESS_SPLIT_CONTRACT_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_pair_readiness_split_contract_csv(report, paths["csv"])
    paths["text"].write_text(render_pair_readiness_split_contract_text(report), encoding="utf-8")
...
.stats,.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:10px;margin:18px 0}
