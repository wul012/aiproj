...
def render_loss_retention_repair_plan_text(report: dict[str, Any]) -> str:
...
def render_loss_retention_repair_plan_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# MiniGPT Pair-Readiness Loss-Retention Repair Plan",
            "",
...
def render_loss_retention_repair_plan_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    plan = as_dict(report.get("plan"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
...
<title>MiniGPT loss-retention repair plan</title>
{_style()}
</head>
<body>
<main>
<header><h1>MiniGPT loss-retention repair plan</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
...
def write_loss_retention_repair_plan_csv(report: dict[str, Any], path: str | Path) -> None:
...
def write_loss_retention_repair_plan_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / PAIR_READINESS_LOSS_RETENTION_REPAIR_PLAN_JSON_FILENAME,
        "csv": root / PAIR_READINESS_LOSS_RETENTION_REPAIR_PLAN_CSV_FILENAME,
        "text": root / PAIR_READINESS_LOSS_RETENTION_REPAIR_PLAN_TEXT_FILENAME,
        "markdown": root / PAIR_READINESS_LOSS_RETENTION_REPAIR_PLAN_MARKDOWN_FILENAME,
        "html": root / PAIR_READINESS_LOSS_RETENTION_REPAIR_PLAN_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_loss_retention_repair_plan_csv(report, paths["csv"])
    paths["text"].write_text(render_loss_retention_repair_plan_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_loss_retention_repair_plan_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_loss_retention_repair_plan_html(report), encoding="utf-8")
