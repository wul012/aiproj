...
def render_packet_v1013_text(report: dict[str, Any]) -> str:
...
def write_packet_v1013_csv(report: dict[str, Any], path: str | Path) -> None:
...
def render_packet_v1013_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
...
def render_packet_v1013_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    index = as_dict(report.get("receipt_index"))
...
def write_packet_v1013_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / PACKET_V1013_JSON_FILENAME,
        "csv": root / PACKET_V1013_CSV_FILENAME,
        "text": root / PACKET_V1013_TEXT_FILENAME,
        "markdown": root / PACKET_V1013_MARKDOWN_FILENAME,
        "html": root / PACKET_V1013_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_packet_v1013_csv(report, paths["csv"])
    paths["text"].write_text(render_packet_v1013_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_packet_v1013_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_packet_v1013_html(report), encoding="utf-8")
