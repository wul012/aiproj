...
def render_randomized_holdout_publication_registry_lookup_index_text(report: dict[str, Any]) -> str:
    ...
    rows = [
        ...
        ("randomized_holdout_publication_registry_lookup_index_ready", summary.get("randomized_holdout_publication_registry_lookup_index_ready")),
        ("lookup_index_id", summary.get("lookup_index_id")),
...
def write_randomized_holdout_publication_registry_lookup_index_csv(report: dict[str, Any], path: str | Path) -> None:
    index = as_dict(report.get("lookup_index"))
    ...
    with out_path.open("w", encoding="utf-8", newline="") as handle:
        ...
        writer.writeheader()
        for row in list_of_dicts(index.get("lookup_entries")):
            writer.writerow({field: csv_cell(row.get(field)) for field in fieldnames})


def render_randomized_holdout_publication_registry_lookup_index_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    index = as_dict(report.get("lookup_index"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT randomized holdout publication registry lookup index'))}",
        "",
        f"- Status: `{report.get('status')}`",
        f"- Decision: `{report.get('decision')}`",
        f"- Index ready: `{summary.get('randomized_holdout_publication_registry_lookup_index_ready')}`",
...
def render_randomized_holdout_publication_registry_lookup_index_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    index = as_dict(report.get("lookup_index"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ("Status", report.get("status")),
        ("Index ready", summary.get("randomized_holdout_publication_registry_lookup_index_ready")),
    ...
    entries = "".join(_entry_row(row) for row in list_of_dicts(index.get("lookup_entries")))
...
<title>{html_escape(report.get('title', 'MiniGPT randomized holdout publication registry lookup index'))}</title>
{_style()}
</head>
<body>
<main>
<header><h1>{html_escape(report.get('title', 'MiniGPT randomized holdout publication registry lookup index'))}</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
<section class="stats">{''.join(_card(label, value) for label, value in stats)}</section>
<section class="panel"><h2>Lookup Index Boundary</h2><dl>
...
<dt>Evidence count</dt><dd>{html_escape(summary.get('evidence_count'))}</dd>
...
def write_randomized_holdout_publication_registry_lookup_index_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_LOOKUP_INDEX_JSON_FILENAME,
        "csv": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_LOOKUP_INDEX_CSV_FILENAME,
        "text": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_LOOKUP_INDEX_TEXT_FILENAME,
        "markdown": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_LOOKUP_INDEX_MARKDOWN_FILENAME,
        "html": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_LOOKUP_INDEX_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_randomized_holdout_publication_registry_lookup_index_csv(report, paths["csv"])
    paths["text"].write_text(render_randomized_holdout_publication_registry_lookup_index_text(report), encoding="utf-8")
