...
def render_target_hidden_semantic_holdout_suite_text(report: dict[str, Any]) -> str:
    ...
    rows = [
        ...
        ("target_hidden_semantic_holdout_suite_ready", summary.get("target_hidden_semantic_holdout_suite_ready")),
        ...
        ("source_task_hint_case_count", summary.get("source_task_hint_case_count")),
...
def write_target_hidden_semantic_holdout_suite_csv(report: dict[str, Any], path: str | Path) -> None:
    fieldnames = ["case_id", "source_case_id", "tokenizer_covered", "target_hidden", "task_hint", "prompt_unknown_count", "prompt"]
...
def render_target_hidden_semantic_holdout_suite_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT target-hidden semantic holdout suite'))}",
        "",
        ...
        f"- Source task-hint cases: `{summary.get('source_task_hint_case_count')}`",
...
def render_target_hidden_semantic_holdout_suite_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    ...
    stats = [
        ...
        ("Task hints", summary.get("task_hint_case_count")),
        ("Source hints", summary.get("source_task_hint_case_count")),
...
<title>{html_escape(report.get('title', 'MiniGPT target-hidden semantic holdout suite'))}</title>{_style()}</head>
<body><main><header><h1>{html_escape(report.get('title', 'MiniGPT target-hidden semantic holdout suite'))}</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
<section class="stats">{''.join(_card(label, value) for label, value in stats)}</section>
<section class="panel"><h2>Coverage Rows</h2><div class="table-wrap"><table><thead><tr><th>Case</th><th>Source</th><th>Covered</th><th>Target Hidden</th><th>Task Hint</th><th>Unknown</th><th>Prompt</th></tr></thead><tbody>{rows}</tbody></table></div></section>
...
def write_target_hidden_semantic_holdout_suite_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    ...
    paths["text"].write_text(render_target_hidden_semantic_holdout_suite_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_target_hidden_semantic_holdout_suite_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_target_hidden_semantic_holdout_suite_html(report), encoding="utf-8")
...
def _style() -> str:
    return """<style>
:root{color-scheme:light;--ink:#17212b;--muted:#62717d;--line:#d7dee5;--panel:#f8fafc;--accent:#047857}
