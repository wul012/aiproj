...
from minigpt.training_scale_promotion_guards import (
    clean_batch_review_guard as _clean_batch_review_guard,
    reason_detail as _reason_detail,
...
def _summary(
    ...
    return {
        ...
        "handoff_batch_maturity_suite_design_regression_names": suite_guard.get(
...
def _recommendations(summary: dict[str, Any], blockers: list[str], review_items: list[str]) -> list[str]:
    ...
    if summary.get("handoff_require_clean_batch_review") and (
        ...
        or int(summary.get("handoff_batch_maturity_suite_design_regression_count") or 0)
    ):
