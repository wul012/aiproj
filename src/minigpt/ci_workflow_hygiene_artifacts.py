...
def render_ci_workflow_hygiene_markdown(report: dict[str, Any]) -> str:
    ...
    for key in [
        ...
        "execution_policy_check_count",
        "execution_policy_violation_count",
        "main_branch_push_scope_ready",
        "tag_push_suppressed",
        "pip_dependency_cache_ready",
        "concurrency_cancel_ready",
        "ci_execution_economy_ready",
        ...
        "aiproj_track_closeout_present",
        "aiproj_track_closeout_order_ready",
        "aiproj_track_closeout_ready",
    ...
    lines.extend(
        ["", "## Actions", "", "| Repository | Version | Line | Node 24 Native |", "| --- | --- | --- | --- |"]
    )
    for item in _actions(report):
        lines.append(
            f"| {_md(item.get('repository'))} | {_md(item.get('version'))} | {_md(item.get('line'))} | {_md(item.get('node24_native'))} |"
        )
...
def render_ci_workflow_hygiene_html(report: dict[str, Any]) -> str:
    ...
    stats = [
        ...
        ("Execution economy", summary.get("ci_execution_economy_ready")),
        ("Tag push suppressed", summary.get("tag_push_suppressed")),
        ("Pip cache", summary.get("pip_dependency_cache_ready")),
        ("Cancel superseded", summary.get("concurrency_cancel_ready")),
        ...
        ("Archived paths", summary.get("archived_path_portability_check_ready")),
        ...
        ("A-track closeout", summary.get("aiproj_track_closeout_ready")),
    ...
    return "\n".join(
        [
            ...
            + checks
            + "</table></section>",
            "<section><h2>Actions</h2><table><tr><th>Repository</th><th>Version</th><th>Line</th><th>Node 24 Native</th></tr>"
            + actions
            + "</table></section>",
