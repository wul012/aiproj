...
BOUNDED_OBJECTIVE_UNASSISTED_REPAIR_SEED_REVISION_TRAINING_RUN_CSV_FILENAME = "model_capability_route_promotion_bounded_objective_unassisted_repair_seed_revision_training_run.csv"
...
def read_json_report(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("bounded objective unassisted repair seed revision training run input must be a JSON object")
    return dict(payload)


def build_model_capability_route_promotion_bounded_objective_unassisted_repair_seed_revision_training_run(
    seed_revision_report: dict[str, Any],
    run_dir: str | Path,
    *,
    seed_revision_path: str | Path | None = None,
    title: str = "MiniGPT model capability route promotion bounded objective unassisted repair seed revision training run",
    generated_at: str | None = None,
) -> dict[str, Any]:
    report = build_model_capability_route_promotion_bounded_objective_unassisted_repair_training_run(
        _adapt_seed_revision(seed_revision_report),
        run_dir,
        unassisted_repair_seed_path=seed_revision_path,
        title=title,
...
def _adapt_seed_revision(seed_revision_report: dict[str, Any]) -> dict[str, Any]:
    adapted = dict(seed_revision_report)
    summary = dict(as_dict(seed_revision_report.get("summary")))
    summary["bounded_objective_unassisted_repair_seed_ready"] = summary.get("bounded_objective_unassisted_repair_seed_revision_ready")
    summary["proposed_next_artifact"] = "model_capability_route_promotion_bounded_objective_unassisted_repair_training_run"
    summary["direct_example_count"] = summary.get("example_count")
    adapted["summary"] = summary
    return adapted


def _adapt_training_report(report: dict[str, Any], seed_revision_report: dict[str, Any], source_path: str | Path | None) -> dict[str, Any]:
    adapted = dict(report)
    adapted["decision"] = _decision(str(report.get("decision") or ""))
    adapted["source_seed_revision"] = str(source_path or "")
    adapted["seed_revision_summary"] = as_dict(seed_revision_report.get("summary"))
...
def _training(base: dict[str, Any]) -> dict[str, Any]:
    training = dict(base)
    training["proposed_next_artifact"] = "model_capability_route_promotion_bounded_objective_unassisted_repair_seed_revision_replay_comparison"
    training["next_step"] = "run_bounded_objective_unassisted_repair_seed_revision_replay_comparison" if training.get("ready") is True else "repair_bounded_objective_unassisted_repair_seed_revision_training_run"
    return training


def _summary(base: dict[str, Any]) -> dict[str, Any]:
    summary = dict(base)
    ready = summary.get("bounded_objective_unassisted_repair_training_ready") is True
    summary["bounded_objective_unassisted_repair_seed_revision_training_ready"] = ready
    summary["proposed_next_artifact"] = "model_capability_route_promotion_bounded_objective_unassisted_repair_seed_revision_replay_comparison" if ready else ""
    summary["next_step"] = "run_bounded_objective_unassisted_repair_seed_revision_replay_comparison" if ready else "repair_bounded_objective_unassisted_repair_seed_revision_training_run"
    return summary


def _decision(base_decision: str) -> str:
    mapping = {
        "model_capability_route_promotion_bounded_objective_unassisted_repair_training_run_ready": "model_capability_route_promotion_bounded_objective_unassisted_repair_seed_revision_training_run_ready",
        "fix_model_capability_route_promotion_bounded_objective_unassisted_repair_training_run": "fix_model_capability_route_promotion_bounded_objective_unassisted_repair_seed_revision_training_run",
    }
...
def _interpretation(base: dict[str, Any], training: dict[str, Any]) -> dict[str, Any]:
    ...
    if training.get("ready") is True:
        ...
        interpretation["reason"] = "A checkpoint was trained from the revised unassisted seed, but capability still requires replay comparison."
