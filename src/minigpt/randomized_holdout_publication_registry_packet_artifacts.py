...
def render_randomized_holdout_publication_registry_packet_text(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    rows = [
        ("status", report.get("status")),
        ("decision", report.get("decision")),
        ("failed_count", report.get("failed_count")),
        ("randomized_holdout_publication_registry_packet_ready", summary.get("randomized_holdout_publication_registry_packet_ready")),
...
def write_randomized_holdout_publication_registry_packet_csv(report: dict[str, Any], path: str | Path) -> None:
    fieldnames = ["kind", "exists", "path"]
    out_path = Path(path)
...
def render_randomized_holdout_publication_registry_packet_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT randomized holdout publication registry packet'))}",
        "",
        f"- Status: `{report.get('status')}`",
        f"- Decision: `{report.get('decision')}`",
        f"- Packet ready: `{summary.get('randomized_holdout_publication_registry_packet_ready')}`",
        f"- Handoff: `{summary.get('handoff_status')}`",
        f"- Entry id: `{summary.get('entry_id')}`",
        f"- Registry status: `{summary.get('registry_status')}`",
        f"- Contract check: `{summary.get('contract_check_ready')}`",
    ...
    for row in list_of_dicts(report.get("evidence_rows")):
        lines.append("| " + " | ".join([markdown_cell(row.get("kind")), markdown_cell(row.get("exists")), markdown_cell(row.get("path"))]) + " |")
...
def render_randomized_holdout_publication_registry_packet_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ("Status", report.get("status")),
        ("Packet ready", summary.get("randomized_holdout_publication_registry_packet_ready")),
...
<title>{html_escape(report.get('title', 'MiniGPT randomized holdout publication registry packet'))}</title>
{_style()}
</head>
<body>
<main>
<header><h1>{html_escape(report.get('title', 'MiniGPT randomized holdout publication registry packet'))}</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
...
def write_randomized_holdout_publication_registry_packet_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_PACKET_JSON_FILENAME,
        "csv": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_PACKET_CSV_FILENAME,
        "text": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_PACKET_TEXT_FILENAME,
        "markdown": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_PACKET_MARKDOWN_FILENAME,
        "html": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_PACKET_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_randomized_holdout_publication_registry_packet_csv(report, paths["csv"])
    paths["text"].write_text(render_randomized_holdout_publication_registry_packet_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_randomized_holdout_publication_registry_packet_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_randomized_holdout_publication_registry_packet_html(report), encoding="utf-8")
    return {key: str(value) for key, value in paths.items()}


def _evidence_row(row: dict[str, Any]) -> str:
    return "<tr>" + "".join(f"<td>{html_escape(row.get(key))}</td>" for key in ["kind", "exists", "path"]) + "</tr>"
