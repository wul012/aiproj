...
from minigpt.registry_ack_packet import (
    REGISTRY_ACK_PACKET_CSV_FILENAME,
    REGISTRY_ACK_PACKET_HTML_FILENAME,
    REGISTRY_ACK_PACKET_JSON_FILENAME,
    REGISTRY_ACK_PACKET_MARKDOWN_FILENAME,
    REGISTRY_ACK_PACKET_TEXT_FILENAME,
)
...
def render_ack_packet_artifacts_text(report: dict[str, Any]) -> str:
...
def write_ack_packet_artifacts_csv(report: dict[str, Any], path: str | Path) -> None:
...
def render_ack_packet_artifacts_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
...
def render_ack_packet_artifacts_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    packet = as_dict(report.get("packet"))
...
def write_ack_packet_artifacts_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / REGISTRY_ACK_PACKET_JSON_FILENAME,
        "csv": root / REGISTRY_ACK_PACKET_CSV_FILENAME,
        "text": root / REGISTRY_ACK_PACKET_TEXT_FILENAME,
        "markdown": root / REGISTRY_ACK_PACKET_MARKDOWN_FILENAME,
        "html": root / REGISTRY_ACK_PACKET_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_ack_packet_artifacts_csv(report, paths["csv"])
    paths["text"].write_text(render_ack_packet_artifacts_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_ack_packet_artifacts_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_ack_packet_artifacts_html(report), encoding="utf-8")
