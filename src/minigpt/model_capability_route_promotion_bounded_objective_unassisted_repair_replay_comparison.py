...
BOUNDED_OBJECTIVE_UNASSISTED_REPAIR_REPLAY_COMPARISON_CSV_FILENAME = "model_capability_route_promotion_bounded_objective_unassisted_repair_replay_comparison.csv"
...
def read_json_report(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("bounded objective unassisted repair replay comparison input must be a JSON object")
    return dict(payload)


def build_model_capability_route_promotion_bounded_objective_unassisted_repair_replay_comparison(
    objective_contract_report: dict[str, Any],
    unassisted_repair_training_run_report: dict[str, Any],
    *,
    ...
    title: str = "MiniGPT model capability route promotion bounded objective unassisted repair replay comparison",
    generated_at: str | None = None,
) -> dict[str, Any]:
    adapted_training = _adapt_training_report(unassisted_repair_training_run_report)
    ...
    return _adapt_replay_report(report, unassisted_repair_training_run_report, unassisted_repair_training_run_path)
...
def _adapt_training_report(training_report: dict[str, Any]) -> dict[str, Any]:
...
def _adapt_replay_report(report: dict[str, Any], original_training_report: dict[str, Any], source_path: str | Path | None) -> dict[str, Any]:
    adapted = dict(report)
    adapted["source_unassisted_repair_training_run"] = str(source_path or "")
    adapted["unassisted_repair_training_summary"] = as_dict(original_training_report.get("summary"))
...
def _comparison(base: dict[str, Any]) -> dict[str, Any]:
    ...
    comparison["route"] = "bounded_objective_unassisted_repair"
    comparison["decoder_anchor_used"] = False
    comparison["training_source"] = "model_capability_route_promotion_bounded_objective_unassisted_repair_training_run"
...
def _summary(base: dict[str, Any], comparison: dict[str, Any]) -> dict[str, Any]:
    summary = dict(base)
    summary["bounded_objective_unassisted_repair_replay_comparison_ready"] = summary.get("bounded_objective_replay_comparison_ready", False)
...
def _decision(base_decision: str) -> str:
    mapping = {
        "fix_model_capability_route_promotion_bounded_objective_replay_comparison": "fix_model_capability_route_promotion_bounded_objective_unassisted_repair_replay_comparison",
        "model_capability_route_promotion_bounded_objective_replay_contract_recovered_holdout_required": "model_capability_route_promotion_bounded_objective_unassisted_repair_replay_contract_recovered_holdout_required",
        "model_capability_route_promotion_bounded_objective_replay_partial_required_term_hit": "model_capability_route_promotion_bounded_objective_unassisted_repair_replay_partial_required_term_hit",
        "model_capability_route_promotion_bounded_objective_replay_zero_hit": "model_capability_route_promotion_bounded_objective_unassisted_repair_replay_zero_hit",
    }
...
def _interpretation(base: dict[str, Any], comparison: dict[str, Any]) -> dict[str, Any]:
    ...
    if comparison.get("objective_contract_recovered") is True:
        ...
        interpretation["next_action"] = "diagnose_bounded_objective_unassisted_repair_partial_hit_before_more_training"
    else:
        interpretation["next_action"] = "diagnose_bounded_objective_unassisted_repair_replay_zero_hit_before_more_training"
    interpretation["reason"] = str(interpretation.get("reason") or "") + " This replay used no decoder anchors."
...
def _next_step(recovered: bool, any_hit_count: int) -> str:
    ...
    if any_hit_count > 0:
        return "diagnose_bounded_objective_unassisted_repair_partial_hit_before_more_training"
    return "diagnose_bounded_objective_unassisted_repair_replay_zero_hit_before_more_training"


__all__ = [
    "BOUNDED_OBJECTIVE_UNASSISTED_REPAIR_REPLAY_COMPARISON_CSV_FILENAME",
    "BOUNDED_OBJECTIVE_UNASSISTED_REPAIR_REPLAY_COMPARISON_HTML_FILENAME",
    "BOUNDED_OBJECTIVE_UNASSISTED_REPAIR_REPLAY_COMPARISON_JSON_FILENAME",
    "BOUNDED_OBJECTIVE_UNASSISTED_REPAIR_REPLAY_COMPARISON_MARKDOWN_FILENAME",
    "BOUNDED_OBJECTIVE_UNASSISTED_REPAIR_REPLAY_COMPARISON_TEXT_FILENAME",
