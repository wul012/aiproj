...
def render_governance_stabilization_markdown(report: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            "| Proposal | Target chain | Suggested chain | Decision | Reason | Expansion rule |",
            "| --- | --- | --- | --- | --- | --- |",
        ]
    )
...
def _proposal_routing_section(routing: dict[str, Any]) -> str:
    ...
    if not rows:
        rows = '<tr><td colspan="6" class="muted">No governance expansion proposals were provided.</td></tr>'
    summary = (
    ...
    return (
        ...
        + summary
        + "<table><tr><th>Proposal</th><th>Target Chain</th><th>Suggested Chain</th><th>Decision</th><th>Reason</th><th>Expansion Rule</th></tr>"
        + rows
        + "</table></section>"
    )
