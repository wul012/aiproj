...
def render_project_audit_markdown(audit: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- Benchmark history: `{audit.get('benchmark_history_path') or 'missing'}`",
        f"- CI workflow hygiene: `{audit.get('ci_workflow_hygiene_path') or 'missing'}`",
        f"- Test coverage report: `{audit.get('test_coverage_report_path') or 'missing'}`",
        "",
        "## Summary",
        "",
        *_markdown_table(
            [
                ...
                ("Benchmark history status", summary.get("benchmark_history_status")),
                ...
                ("CI archived path portability", summary.get("ci_archived_path_portability_check_ready")),
...
def render_project_audit_html(audit: dict[str, Any]) -> str:
    ...
    stats = [
        ...
        ("CI archived paths", summary.get("ci_archived_path_portability_check_ready")),
