...
from minigpt.report_utils import family_html as _family_html
from minigpt.report_utils import probe_html as _probe_html


def render_objective_level_contrast_contract_text(report: dict[str, Any]) -> str:
...
def render_objective_level_contrast_contract_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# MiniGPT Objective-Level Contrast Contract",
            "",
...
def render_objective_level_contrast_contract_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    contract = as_dict(report.get("contract"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ...
        ("Probes", summary.get("evaluation_probe_count")),
    ]
...
<title>MiniGPT objective-level contrast contract</title>
{_style()}
</head>
<body>
<main>
<header><h1>MiniGPT objective-level contrast contract</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
...
def write_objective_level_contrast_contract_csv(report: dict[str, Any], path: str | Path) -> None:
    fieldnames = ["family", "role", "target_term", "row_count"]
    out_path = Path(path)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    with out_path.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=fieldnames)
        writer.writeheader()
        for family in list_of_dicts(as_dict(report.get("contract")).get("row_families")):
            writer.writerow(
                {
                    "family": csv_cell(family.get("family")),
                    "role": csv_cell(family.get("role")),
                    "target_term": csv_cell(family.get("target_term")),
                    "row_count": csv_cell(len(family.get("rows", []))),
                }
            )


def write_objective_level_contrast_contract_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / PAIR_READINESS_OBJECTIVE_LEVEL_CONTRAST_CONTRACT_JSON_FILENAME,
        "csv": root / PAIR_READINESS_OBJECTIVE_LEVEL_CONTRAST_CONTRACT_CSV_FILENAME,
        "text": root / PAIR_READINESS_OBJECTIVE_LEVEL_CONTRAST_CONTRACT_TEXT_FILENAME,
        "markdown": root / PAIR_READINESS_OBJECTIVE_LEVEL_CONTRAST_CONTRACT_MARKDOWN_FILENAME,
        "html": root / PAIR_READINESS_OBJECTIVE_LEVEL_CONTRAST_CONTRACT_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_objective_level_contrast_contract_csv(report, paths["csv"])
    paths["text"].write_text(render_objective_level_contrast_contract_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_objective_level_contrast_contract_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_objective_level_contrast_contract_html(report), encoding="utf-8")
