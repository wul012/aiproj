...
RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_PACKET_JSON_FILENAME = "randomized_holdout_publication_registry_downstream_consumer_packet.json"
RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_PACKET_CSV_FILENAME = "randomized_holdout_publication_registry_downstream_consumer_packet.csv"
RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_PACKET_TEXT_FILENAME = "randomized_holdout_publication_registry_downstream_consumer_packet.txt"
RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_PACKET_MARKDOWN_FILENAME = "randomized_holdout_publication_registry_downstream_consumer_packet.md"
RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_PACKET_HTML_FILENAME = "randomized_holdout_publication_registry_downstream_consumer_packet.html"

PACKET_ID = "randomized-holdout-publication-registry-downstream-consumer-packet-v941"


def locate_randomized_holdout_publication_registry_downstream_receipt_review(path: str | Path) -> Path:
    source = Path(path)
    if source.is_dir():
        source = source / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_RECEIPT_REVIEW_JSON_FILENAME
    return source
...
def read_json_report(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("randomized holdout publication registry downstream consumer packet input must be a JSON object")
    return dict(payload)


def build_randomized_holdout_publication_registry_downstream_consumer_packet(
    receipt_review_report: dict[str, Any],
    *,
    receipt_review_path: str | Path | None = None,
    title: str = "MiniGPT randomized holdout publication registry downstream consumer packet",
    ...
    entries = list_of_dicts(receipt_review_report.get("entry_rows"))
    checks = _checks(receipt_review_report, review_summary, review, consumer_receipts, entries, receipt_review_path)
    issues = [row for row in checks if row["status"] != "pass"]
    status = "pass" if not issues else "fail"
    packet = _packet(status, review_summary, review, consumer_receipts, entries, receipt_review_path)
    return {
        "schema_version": 1,
        "title": title,
...
def resolve_exit_code(
    ...
    *,
    ...
    if require_packet_ready and summary.get("randomized_holdout_publication_registry_downstream_consumer_packet_ready") is not True:
        return 1
...
def _checks(
    ...
    return [
        ...
        _check("receipt_review_decision_ready", review_report.get("decision") == "randomized_holdout_publication_registry_downstream_receipt_review_ready", review_report.get("decision"), "receipt review decision must be ready"),
        _check("review_summary_ready", review_summary.get("randomized_holdout_publication_registry_downstream_receipt_review_ready") is True and review.get("review_ready") is True, {"summary": review_summary.get("randomized_holdout_publication_registry_downstream_receipt_review_ready"), "review": review.get("review_ready")}, "receipt review summary and body must be ready"),
        _check("review_status_packet_ready", review_summary.get("review_status") == "approved_for_downstream_consumer_packet" and review.get("review_status") == "approved_for_downstream_consumer_packet", {"summary": review_summary.get("review_status"), "review": review.get("review_status")}, "review must approve consumer packet construction"),
        _check("consumer_ready", review_summary.get("consumer_ready") is True and review.get("consumer_ready") is True, {"summary": review_summary.get("consumer_ready"), "review": review.get("consumer_ready")}, "consumer readiness must be true"),
        ...
        _check("promotion_still_false", review_summary.get("promotion_ready") is False and review.get("promotion_ready") is False, {"summary": review_summary.get("promotion_ready"), "review": review.get("promotion_ready")}, "packet must not enable promotion"),
        ...
        _check("source_guard_digest_shape", is_sha256(review.get("source_guard_sha256")), review.get("source_guard_sha256"), "source guard digest must be a lowercase sha256"),
        _check("consumer_receipts_present", len(consumer_receipts) == int(review_summary.get("entry_count") or 0) and len(consumer_receipts) > 0, {"consumer_receipts": len(consumer_receipts), "entry_count": review_summary.get("entry_count")}, "consumer packet must include all consumer receipt rows"),
        _check("entries_present", len(entries) == int(review_summary.get("entry_count") or 0) and len(entries) > 0, {"entries": len(entries), "entry_count": review_summary.get("entry_count")}, "consumer packet must include all entries"),
        ...
        _check("lookup_keys_publication_namespace", all(str(row.get("lookup_key")).startswith("publication:") for row in consumer_receipts), [row.get("lookup_key") for row in consumer_receipts], "consumer lookup keys must use publication namespace"),
        ...
        _check("source_next_step_matches", review_summary.get("next_step") == RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_RECEIPT_REVIEW_NEXT_STEP, review_summary.get("next_step"), "source review must route to consumer packet"),
    ]
...
def _packet(
    ...
    return {
        ...
        "packet_status": "downstream_consumer_packet_ready" if ready else "blocked",
        ...
        "lookup_keys": [str(row.get("lookup_key")) for row in consumer_receipts] if ready else [],
        "source_guard_sha256": review.get("source_guard_sha256") if ready else "",
        "promotion_ready": False,
        "approved_for_promotion": False,
        "model_quality_claim": RANDOMIZED_HOLDOUT_PUBLICATION_MODEL_QUALITY_CLAIM if ready else "not_claimed",
        "next_step": RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_PACKET_NEXT_STEP if ready else "repair_randomized_holdout_publication_registry_downstream_consumer_packet",
    }
...
def _summary(status: str, checks: list[dict[str, Any]], packet: dict[str, Any]) -> dict[str, Any]:
    return {
        "randomized_holdout_publication_registry_downstream_consumer_packet_ready": status == "pass" and packet.get("packet_ready") is True,
        "packet_id": packet.get("packet_id"),
        "packet_status": packet.get("packet_status"),
        "consumer_name": packet.get("consumer_name"),
...
def _decision(status: str) -> str:
    if status == "pass":
        return "randomized_holdout_publication_registry_downstream_consumer_packet_ready"
    return "fix_randomized_holdout_publication_registry_downstream_consumer_packet"
...
def _interpretation(status: str, packet: dict[str, Any]) -> dict[str, str]:
    if status != "pass":
        return {
            ...
            "reason": "The receipt review is not ready to become a consumer packet.",
            "next_action": "repair receipt review before consumer packet",
        }
    return {
        "model_quality_claim": RANDOMIZED_HOLDOUT_PUBLICATION_MODEL_QUALITY_CLAIM,
        "reason": "The downstream consumer packet is ready for lookup-only consumption while production promotion remains blocked.",
        "next_action": str(packet.get("next_step")),
    }


__all__ = [
    "RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_PACKET_CSV_FILENAME",
    "RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_PACKET_HTML_FILENAME",
    "RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_PACKET_JSON_FILENAME",
    "RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_PACKET_MARKDOWN_FILENAME",
    "RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_PACKET_TEXT_FILENAME",
