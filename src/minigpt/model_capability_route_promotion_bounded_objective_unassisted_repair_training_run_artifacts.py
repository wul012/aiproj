...
def render_bounded_objective_unassisted_repair_training_run_text(report: dict[str, Any]) -> str:
    ...
    rows = [
        ...
        ("bounded_objective_unassisted_repair_training_ready", summary.get("bounded_objective_unassisted_repair_training_ready")),
        ("final_step", summary.get("final_step")),
        ("final_train_loss", summary.get("final_train_loss")),
        ("final_val_loss", summary.get("final_val_loss")),
        ("train_loss_delta", summary.get("train_loss_delta")),
        ("repair_example_count", summary.get("repair_example_count")),
...
def write_bounded_objective_unassisted_repair_training_run_csv(report: dict[str, Any], path: str | Path) -> None:
...
def render_bounded_objective_unassisted_repair_training_run_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT bounded objective unassisted repair training run'))}",
        "",
        f"- Status: `{report.get('status')}`",
        f"- Decision: `{report.get('decision')}`",
        f"- Ready: `{summary.get('bounded_objective_unassisted_repair_training_ready')}`",
        ...
        f"- Repair examples: `{summary.get('repair_example_count')}`",
        f"- Neutral prompts: `{summary.get('neutral_prompt_example_count')}`",
...
def render_bounded_objective_unassisted_repair_training_run_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ("Status", report.get("status")),
        ("Decision", report.get("decision")),
        ("Ready", summary.get("bounded_objective_unassisted_repair_training_ready")),
        ("Final step", summary.get("final_step")),
        ("Val loss", summary.get("final_val_loss")),
        ("Neutral prompts", summary.get("neutral_prompt_example_count")),
...
<title>{html_escape(report.get('title', 'MiniGPT bounded objective unassisted repair training run'))}</title>
{_style()}
</head>
<body>
<main>
<header><h1>{html_escape(report.get('title', 'MiniGPT bounded objective unassisted repair training run'))}</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
...
def write_bounded_objective_unassisted_repair_training_run_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / BOUNDED_OBJECTIVE_UNASSISTED_REPAIR_TRAINING_RUN_JSON_FILENAME,
        "csv": root / BOUNDED_OBJECTIVE_UNASSISTED_REPAIR_TRAINING_RUN_CSV_FILENAME,
        "text": root / BOUNDED_OBJECTIVE_UNASSISTED_REPAIR_TRAINING_RUN_TEXT_FILENAME,
        "markdown": root / BOUNDED_OBJECTIVE_UNASSISTED_REPAIR_TRAINING_RUN_MARKDOWN_FILENAME,
        "html": root / BOUNDED_OBJECTIVE_UNASSISTED_REPAIR_TRAINING_RUN_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_bounded_objective_unassisted_repair_training_run_csv(report, paths["csv"])
    paths["text"].write_text(render_bounded_objective_unassisted_repair_training_run_text(report), encoding="utf-8")
