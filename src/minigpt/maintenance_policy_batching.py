...
from minigpt.maintenance_policy_common import (
    HIGH_RISK_FLAGS,
...
from minigpt.report_utils import list_of_dicts as _list_of_dicts, utc_now
...
def build_maintenance_proposal_decision(
    ...
    *,
    ...
    groups = category_groups(batchable)
    if split_items:
        decision = "split"
        target_kind = "focused"
...
def _normalize_release_entry(item: dict[str, Any], index: int) -> dict[str, Any]:
    ...
    category_name = category(item, title)
    module_names = modules(item)
    flags = risk_flags(item)
    low_risk_utils = is_low_risk_utils(category_name, title)
    high_risk = bool(set(flags) & set(HIGH_RISK_FLAGS))
    single_module_utils = low_risk_utils and not high_risk and len(module_names) <= 1
    return {
        "version": version,
        "title": title,
...
def _normalize_proposal_item(item: dict[str, Any], index: int) -> dict[str, Any]:
    ...
    category_name = category(item, name)
    flags = risk_flags(item)
    high_risk_flags = [flag for flag in flags if flag in HIGH_RISK_FLAGS]
    split_reasons = [HIGH_RISK_FLAGS[flag] for flag in high_risk_flags]
    low_risk = category_name in LOW_RISK_MAINTENANCE_CATEGORIES
