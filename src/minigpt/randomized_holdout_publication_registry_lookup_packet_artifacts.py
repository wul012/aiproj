...
def render_randomized_holdout_publication_registry_lookup_packet_text(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    rows = [
        ("status", report.get("status")),
        ("decision", report.get("decision")),
        ("failed_count", report.get("failed_count")),
        ("randomized_holdout_publication_registry_lookup_packet_ready", summary.get("randomized_holdout_publication_registry_lookup_packet_ready")),
        ("lookup_packet_id", summary.get("lookup_packet_id")),
...
def write_randomized_holdout_publication_registry_lookup_packet_csv(report: dict[str, Any], path: str | Path) -> None:
    packet = as_dict(report.get("lookup_packet"))
    ...
    with out_path.open("w", encoding="utf-8", newline="") as handle:
        ...
        writer.writeheader()
        for row in list_of_dicts(packet.get("lookup_entries")):
            writer.writerow({field: csv_cell(row.get(field)) for field in fieldnames})


def render_randomized_holdout_publication_registry_lookup_packet_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    packet = as_dict(report.get("lookup_packet"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT randomized holdout publication registry lookup packet'))}",
        "",
        f"- Status: `{report.get('status')}`",
        f"- Decision: `{report.get('decision')}`",
        f"- Packet ready: `{summary.get('randomized_holdout_publication_registry_lookup_packet_ready')}`",
...
def render_randomized_holdout_publication_registry_lookup_packet_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    packet = as_dict(report.get("lookup_packet"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ("Status", report.get("status")),
        ("Packet ready", summary.get("randomized_holdout_publication_registry_lookup_packet_ready")),
    ...
    entries = "".join(_entry_row(row) for row in list_of_dicts(packet.get("lookup_entries")))
...
<title>{html_escape(report.get('title', 'MiniGPT randomized holdout publication registry lookup packet'))}</title>
{_style()}
</head>
<body>
<main>
<header><h1>{html_escape(report.get('title', 'MiniGPT randomized holdout publication registry lookup packet'))}</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
...
<section class="panel"><h2>Lookup Entries</h2><div class="table-wrap"><table>
...
def write_randomized_holdout_publication_registry_lookup_packet_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_LOOKUP_PACKET_JSON_FILENAME,
        "csv": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_LOOKUP_PACKET_CSV_FILENAME,
        "text": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_LOOKUP_PACKET_TEXT_FILENAME,
        "markdown": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_LOOKUP_PACKET_MARKDOWN_FILENAME,
        "html": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_LOOKUP_PACKET_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_randomized_holdout_publication_registry_lookup_packet_csv(report, paths["csv"])
    paths["text"].write_text(render_randomized_holdout_publication_registry_lookup_packet_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_randomized_holdout_publication_registry_lookup_packet_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_randomized_holdout_publication_registry_lookup_packet_html(report), encoding="utf-8")
