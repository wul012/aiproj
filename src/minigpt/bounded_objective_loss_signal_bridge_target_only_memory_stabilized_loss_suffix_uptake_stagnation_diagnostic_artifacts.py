...
def render_stabilized_loss_suffix_uptake_stagnation_diagnostic_text(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    rows = [
        ("status", report.get("status")),
        ("decision", report.get("decision")),
        ("stabilized_loss_suffix_uptake_stagnation_diagnostic_ready", summary.get("bounded_objective_loss_signal_bridge_target_only_memory_stabilized_loss_suffix_uptake_stagnation_diagnostic_ready")),
        ("no_contract_gain_confirmed", summary.get("no_contract_gain_confirmed")),
        ("stagnation_confirmed", summary.get("stagnation_confirmed")),
        ("surface_format_changed_without_suffix_gain", summary.get("surface_format_changed_without_suffix_gain")),
...
def render_stabilized_loss_suffix_uptake_stagnation_diagnostic_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT bounded objective loss signal bridge target-only memory stabilized loss-suffix uptake stagnation diagnostic'))}",
        "",
        f"- Status: `{report.get('status')}`",
        f"- Decision: `{report.get('decision')}`",
        f"- Stagnation confirmed: `{summary.get('stagnation_confirmed')}`",
        f"- No contract gain confirmed: `{summary.get('no_contract_gain_confirmed')}`",
        f"- Surface format changed without suffix gain: `{summary.get('surface_format_changed_without_suffix_gain')}`",
...
def render_stabilized_loss_suffix_uptake_stagnation_diagnostic_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ("Status", report.get("status")),
        ("Decision", report.get("decision")),
        ("Stagnated", summary.get("stagnation_confirmed")),
        ("No contract gain", summary.get("no_contract_gain_confirmed")),
        ("Format delta only", summary.get("surface_format_changed_without_suffix_gain")),
...
<title>{html_escape(report.get('title', 'MiniGPT bounded objective loss signal bridge target-only memory stabilized loss-suffix uptake stagnation diagnostic'))}</title>{_style()}</head>
<body><main>
<header><h1>{html_escape(report.get('title', 'MiniGPT bounded objective loss signal bridge target-only memory stabilized loss-suffix uptake stagnation diagnostic'))}</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
...
def write_stabilized_loss_suffix_uptake_stagnation_diagnostic_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / TARGET_ONLY_MEMORY_STABILIZED_LOSS_SUFFIX_UPTAKE_STAGNATION_DIAGNOSTIC_JSON_FILENAME,
        "csv": root / TARGET_ONLY_MEMORY_STABILIZED_LOSS_SUFFIX_UPTAKE_STAGNATION_DIAGNOSTIC_CSV_FILENAME,
        "text": root / TARGET_ONLY_MEMORY_STABILIZED_LOSS_SUFFIX_UPTAKE_STAGNATION_DIAGNOSTIC_TEXT_FILENAME,
        "markdown": root / TARGET_ONLY_MEMORY_STABILIZED_LOSS_SUFFIX_UPTAKE_STAGNATION_DIAGNOSTIC_MARKDOWN_FILENAME,
        "html": root / TARGET_ONLY_MEMORY_STABILIZED_LOSS_SUFFIX_UPTAKE_STAGNATION_DIAGNOSTIC_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    _write_csv(report, paths["csv"])
    paths["text"].write_text(render_stabilized_loss_suffix_uptake_stagnation_diagnostic_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_stabilized_loss_suffix_uptake_stagnation_diagnostic_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_stabilized_loss_suffix_uptake_stagnation_diagnostic_html(report), encoding="utf-8")
...
def _style() -> str:
    return """<style>
:root{color-scheme:light;--ink:#17212b;--muted:#617182;--line:#d7dee8;--panel:#f8fafc;--accent:#7f1d1d}
