...
def render_balanced_repair_plan_text(report: dict[str, Any]) -> str:
...
def render_balanced_repair_plan_markdown(report: dict[str, Any]) -> str:
    plan = as_dict(report.get("plan"))
    interpretation = as_dict(report.get("interpretation"))
...
def render_balanced_repair_plan_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    plan = as_dict(report.get("plan"))
    interpretation = as_dict(report.get("interpretation"))
...
<title>MiniGPT balanced repair plan</title>
{_style()}
</head>
<body>
<main>
<header><h1>MiniGPT balanced repair plan</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
...
def write_balanced_repair_plan_csv(report: dict[str, Any], path: str | Path) -> None:
...
def write_balanced_repair_plan_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / PAIR_MINIMAL_PROMPT_BALANCED_REPAIR_PLAN_JSON_FILENAME,
        "csv": root / PAIR_MINIMAL_PROMPT_BALANCED_REPAIR_PLAN_CSV_FILENAME,
        "text": root / PAIR_MINIMAL_PROMPT_BALANCED_REPAIR_PLAN_TEXT_FILENAME,
        "markdown": root / PAIR_MINIMAL_PROMPT_BALANCED_REPAIR_PLAN_MARKDOWN_FILENAME,
        "html": root / PAIR_MINIMAL_PROMPT_BALANCED_REPAIR_PLAN_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_balanced_repair_plan_csv(report, paths["csv"])
    paths["text"].write_text(render_balanced_repair_plan_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_balanced_repair_plan_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_balanced_repair_plan_html(report), encoding="utf-8")
