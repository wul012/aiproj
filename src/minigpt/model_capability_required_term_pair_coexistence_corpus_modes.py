"""Built-in corpus-mode implementations for pair coexistence refresh."""
...
EQUALS_SURFACE_MODES = frozenset(mode for mode in BUILTIN_PAIR_COEXISTENCE_CORPUS_MODES if mode.startswith("equals_surface_"))
...
_MODE_BUILDERS = {
    "spaced_answer": _extend_spaced_answer,
    "colon_immediate": _extend_colon_immediate,
    "colon_immediate_first_token_boost": _extend_first_token_boost,
    "colon_immediate_isolated_prompt": _extend_isolated_prompt,
    "colon_immediate_loss_calibrated": _extend_loss_calibrated,
    "equals_surface_fixed_repair": _extend_equals_surface_fixed_repair,
    "equals_surface_balanced_repair": _extend_equals_surface_balanced_repair,
    "equals_surface_tied_repair": _extend_equals_surface_tied_repair,
    "equals_surface_no_pair_id_repair": _extend_equals_surface_no_pair_id_repair,
    "equals_surface_no_pair_id_loss_balanced_repair": _extend_equals_surface_no_pair_id_loss_balanced_repair,
    "equals_surface_no_pair_id_loss_balanced_first_token_repair": _extend_equals_surface_no_pair_id_loss_balanced_first_token_repair,
    "equals_surface_no_pair_id_loss_balanced_light_first_token_repair": _extend_equals_surface_no_pair_id_loss_balanced_light_first_token_repair,
}


def extend_builtin_pair_coexistence_corpus(
    lines: list[str], *, corpus_mode: str, repeat: int, bridge_repeat: int
) -> bool:
    builder = _MODE_BUILDERS.get(corpus_mode)
    if builder is None:
        return False
    builder(lines, repeat=repeat, bridge_repeat=bridge_repeat)
    return True


def uses_equals_surface(corpus_mode: str) -> bool:
    return corpus_mode in EQUALS_SURFACE_MODES
