...
def render_model_capability_route_promotion_bounded_real_replay_prompt_aligned_failure_diagnostic_text(report: dict[str, Any]) -> str:
    ...
    rows = [
        ...
        ("diagnostic_ready", summary.get("prompt_aligned_failure_diagnostic_ready")),
...
def write_model_capability_route_promotion_bounded_real_replay_prompt_aligned_failure_diagnostic_csv(report: dict[str, Any], path: str | Path) -> None:
...
def render_model_capability_route_promotion_bounded_real_replay_prompt_aligned_failure_diagnostic_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT model capability route promotion bounded real replay prompt-aligned failure diagnostic'))}",
        "",
        f"- Status: `{report.get('status')}`",
        f"- Decision: `{report.get('decision')}`",
        f"- Ready: `{summary.get('prompt_aligned_failure_diagnostic_ready')}`",
...
def render_model_capability_route_promotion_bounded_real_replay_prompt_aligned_failure_diagnostic_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    stats = [
        ("Status", report.get("status")),
        ("Decision", report.get("decision")),
...
<title>{html_escape(report.get('title', 'MiniGPT model capability route promotion bounded real replay prompt-aligned failure diagnostic'))}</title>
{_style()}
</head>
<body>
<main>
<header><h1>{html_escape(report.get('title', 'MiniGPT model capability route promotion bounded real replay prompt-aligned failure diagnostic'))}</h1><p>Diagnoses why exact prompt-aligned training still produced zero required-term hits in bounded replay.</p></header>
...
def write_model_capability_route_promotion_bounded_real_replay_prompt_aligned_failure_diagnostic_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_PROMPT_ALIGNED_FAILURE_DIAGNOSTIC_JSON_FILENAME,
        "csv": root / MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_PROMPT_ALIGNED_FAILURE_DIAGNOSTIC_CSV_FILENAME,
        "text": root / MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_PROMPT_ALIGNED_FAILURE_DIAGNOSTIC_TEXT_FILENAME,
        "markdown": root / MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_PROMPT_ALIGNED_FAILURE_DIAGNOSTIC_MARKDOWN_FILENAME,
        "html": root / MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_PROMPT_ALIGNED_FAILURE_DIAGNOSTIC_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_model_capability_route_promotion_bounded_real_replay_prompt_aligned_failure_diagnostic_csv(report, paths["csv"])
    paths["text"].write_text(render_model_capability_route_promotion_bounded_real_replay_prompt_aligned_failure_diagnostic_text(report), encoding="utf-8")
...
__all__ = [
    "render_model_capability_route_promotion_bounded_real_replay_prompt_aligned_failure_diagnostic_html",
    "render_model_capability_route_promotion_bounded_real_replay_prompt_aligned_failure_diagnostic_markdown",
