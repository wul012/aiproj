...
def render_check_v1102_artifacts_text(report: dict[str, Any]) -> str:
...
def write_check_v1102_artifacts_csv(report: dict[str, Any], path: str | Path) -> None:
...
def render_check_v1102_artifacts_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    lines = [
...
def render_check_v1102_artifacts_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
...
def write_check_v1102_artifacts_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / RECEIPT_CHECK_V1102_JSON_FILENAME,
        "csv": root / RECEIPT_CHECK_V1102_CSV_FILENAME,
        "text": root / RECEIPT_CHECK_V1102_TEXT_FILENAME,
        "markdown": root / RECEIPT_CHECK_V1102_MARKDOWN_FILENAME,
        "html": root / RECEIPT_CHECK_V1102_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_check_v1102_artifacts_csv(report, paths["csv"])
    paths["text"].write_text(render_check_v1102_artifacts_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_check_v1102_artifacts_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_check_v1102_artifacts_html(report), encoding="utf-8")
