...
def render_randomized_holdout_publication_receipt_packet_index_publication_receipt_index_v985_text(report: dict[str, Any]) -> str:
    ...
    rows = [
        ...
        ("index_ready", summary.get("randomized_holdout_publication_receipt_packet_index_publication_receipt_index_v985_ready")),
...
def write_randomized_holdout_publication_receipt_packet_index_publication_receipt_index_v985_csv(report: dict[str, Any], path: str | Path) -> None:
    fieldnames = [
        "receipt_index_id",
        "lookup_key",
        "receipt_id",
...
def render_randomized_holdout_publication_receipt_packet_index_publication_receipt_index_v985_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    index = as_dict(report.get("receipt_index"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT randomized holdout publication receipt packet index publication receipt index v985'))}",
        "",
        f"- Status: `{report.get('status')}`",
        f"- Decision: `{report.get('decision')}`",
        f"- Index ready: `{summary.get('randomized_holdout_publication_receipt_packet_index_publication_receipt_index_v985_ready')}`",
...
def render_randomized_holdout_publication_receipt_packet_index_publication_receipt_index_v985_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    index = as_dict(report.get("receipt_index"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ("Status", report.get("status")),
        ("Index ready", summary.get("randomized_holdout_publication_receipt_packet_index_publication_receipt_index_v985_ready")),
...
<title>{html_escape(report.get('title', 'MiniGPT randomized holdout publication receipt packet index publication receipt index v985'))}</title>
{_style()}
</head>
<body>
<main>
<header><h1>{html_escape(report.get('title', 'MiniGPT randomized holdout publication receipt packet index publication receipt index v985'))}</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
...
def write_randomized_holdout_publication_receipt_packet_index_publication_receipt_index_v985_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_PACKET_INDEX_PUBLICATION_RECEIPT_INDEX_V985_JSON_FILENAME,
        "csv": root / RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_PACKET_INDEX_PUBLICATION_RECEIPT_INDEX_V985_CSV_FILENAME,
        "text": root / RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_PACKET_INDEX_PUBLICATION_RECEIPT_INDEX_V985_TEXT_FILENAME,
        "markdown": root / RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_PACKET_INDEX_PUBLICATION_RECEIPT_INDEX_V985_MARKDOWN_FILENAME,
        "html": root / RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_PACKET_INDEX_PUBLICATION_RECEIPT_INDEX_V985_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_randomized_holdout_publication_receipt_packet_index_publication_receipt_index_v985_csv(report, paths["csv"])
    paths["text"].write_text(render_randomized_holdout_publication_receipt_packet_index_publication_receipt_index_v985_text(report), encoding="utf-8")
...
__all__ = [
    "render_randomized_holdout_publication_receipt_packet_index_publication_receipt_index_v985_html",
    "render_randomized_holdout_publication_receipt_packet_index_publication_receipt_index_v985_markdown",
    "render_randomized_holdout_publication_receipt_packet_index_publication_receipt_index_v985_text",
