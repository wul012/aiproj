...
def render_randomized_holdout_publication_registry_downstream_receipt_text(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    rows = [
        ("status", report.get("status")),
        ("decision", report.get("decision")),
        ("failed_count", report.get("failed_count")),
        ("randomized_holdout_publication_registry_downstream_receipt_ready", summary.get("randomized_holdout_publication_registry_downstream_receipt_ready")),
...
def write_randomized_holdout_publication_registry_downstream_receipt_csv(report: dict[str, Any], path: str | Path) -> None:
...
def render_randomized_holdout_publication_registry_downstream_receipt_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT randomized holdout publication registry downstream receipt'))}",
        "",
        f"- Status: `{report.get('status')}`",
        f"- Decision: `{report.get('decision')}`",
        f"- Receipt ready: `{summary.get('randomized_holdout_publication_registry_downstream_receipt_ready')}`",
...
def render_randomized_holdout_publication_registry_downstream_receipt_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ("Status", report.get("status")),
        ("Receipt ready", summary.get("randomized_holdout_publication_registry_downstream_receipt_ready")),
...
<title>{html_escape(report.get('title', 'MiniGPT randomized holdout publication registry downstream receipt'))}</title>
{_style()}
</head>
<body>
<main>
<header><h1>{html_escape(report.get('title', 'MiniGPT randomized holdout publication registry downstream receipt'))}</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
...
<dt>Source digest</dt><dd>{html_escape(report.get('downstream_guard_sha256'))}</dd>
</dl></section>
...
def write_randomized_holdout_publication_registry_downstream_receipt_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_RECEIPT_JSON_FILENAME,
        "csv": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_RECEIPT_CSV_FILENAME,
        "text": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_RECEIPT_TEXT_FILENAME,
        "markdown": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_RECEIPT_MARKDOWN_FILENAME,
        "html": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_RECEIPT_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_randomized_holdout_publication_registry_downstream_receipt_csv(report, paths["csv"])
    paths["text"].write_text(render_randomized_holdout_publication_registry_downstream_receipt_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_randomized_holdout_publication_registry_downstream_receipt_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_randomized_holdout_publication_registry_downstream_receipt_html(report), encoding="utf-8")
