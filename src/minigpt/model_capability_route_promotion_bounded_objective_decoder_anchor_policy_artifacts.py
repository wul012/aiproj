...
def render_bounded_objective_decoder_anchor_policy_text(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    rows = [
        ("status", report.get("status")),
        ("decision", report.get("decision")),
        ("bounded_objective_decoder_anchor_policy_ready", summary.get("bounded_objective_decoder_anchor_policy_ready")),
...
def write_bounded_objective_decoder_anchor_policy_csv(report: dict[str, Any], path: str | Path) -> None:
...
def render_bounded_objective_decoder_anchor_policy_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT bounded objective decoder anchor policy'))}",
        "",
        f"- Status: `{report.get('status')}`",
        f"- Decision: `{report.get('decision')}`",
        f"- Ready: `{summary.get('bounded_objective_decoder_anchor_policy_ready')}`",
    ...
    for row in list_of_dicts(report.get("policy_rows")):
        lines.append("| " + " | ".join([
            ...
            markdown_cell(row.get("claim_boundary")),
            markdown_cell(row.get("recommended_use")),
            markdown_cell(row.get("combined_preview")),
        ]) + " |")
...
def render_bounded_objective_decoder_anchor_policy_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ("Status", report.get("status")),
...
<title>{html_escape(report.get('title', 'MiniGPT bounded objective decoder anchor policy'))}</title>
{_style()}
</head>
<body>
<main>
<header><h1>{html_escape(report.get('title', 'MiniGPT bounded objective decoder anchor policy'))}</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
<section class="stats">{''.join(_card(label, value) for label, value in stats)}</section>
<section class="panel"><h2>Policy Rows</h2><div class="table-wrap"><table>
<thead><tr><th>Case</th><th>Profile</th><th>Anchor</th><th>Hits</th><th>Boundary</th><th>Use</th><th>Preview</th></tr></thead>
...
def write_bounded_objective_decoder_anchor_policy_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / BOUNDED_OBJECTIVE_DECODER_ANCHOR_POLICY_JSON_FILENAME,
        "csv": root / BOUNDED_OBJECTIVE_DECODER_ANCHOR_POLICY_CSV_FILENAME,
        "text": root / BOUNDED_OBJECTIVE_DECODER_ANCHOR_POLICY_TEXT_FILENAME,
        "markdown": root / BOUNDED_OBJECTIVE_DECODER_ANCHOR_POLICY_MARKDOWN_FILENAME,
        "html": root / BOUNDED_OBJECTIVE_DECODER_ANCHOR_POLICY_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_bounded_objective_decoder_anchor_policy_csv(report, paths["csv"])
    paths["text"].write_text(render_bounded_objective_decoder_anchor_policy_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_bounded_objective_decoder_anchor_policy_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_bounded_objective_decoder_anchor_policy_html(report), encoding="utf-8")
...
def _policy_row(row: dict[str, Any]) -> str:
    return (
        ...
        f"<td>{html_escape(row.get('combined_preview'))}</td>"
        "</tr>"
    )
