...
MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_DECODER_ANCHOR_TRAINING_RUN_CSV_FILENAME = "model_capability_route_promotion_bounded_real_replay_decoder_anchor_training_run.csv"
MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_DECODER_ANCHOR_TRAINING_RUN_TEXT_FILENAME = "model_capability_route_promotion_bounded_real_replay_decoder_anchor_training_run.txt"
MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_DECODER_ANCHOR_TRAINING_RUN_MARKDOWN_FILENAME = "model_capability_route_promotion_bounded_real_replay_decoder_anchor_training_run.md"
MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_DECODER_ANCHOR_TRAINING_RUN_HTML_FILENAME = "model_capability_route_promotion_bounded_real_replay_decoder_anchor_training_run.html"
...
def read_json_report(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("decoder anchor training run input must be a JSON object")
    return dict(payload)


def build_model_capability_route_promotion_bounded_real_replay_decoder_anchor_training_run(
    decoder_anchor_seed_report: dict[str, Any],
    run_dir: str | Path,
    *,
    decoder_anchor_seed_path: str | Path | None = None,
    title: str = "MiniGPT model capability route promotion bounded real replay decoder anchor training run",
    generated_at: str | None = None,
) -> dict[str, Any]:
    root = Path(run_dir)
    seed_summary = as_dict(decoder_anchor_seed_report.get("summary"))
    artifacts = _artifacts(root)
    metrics = _metrics(root / "metrics.jsonl")
    train_config = _read_json(root / "train_config.json")
    manifest = _read_json(root / "run_manifest.json")
    checks = _checks(decoder_anchor_seed_report, seed_summary, artifacts, metrics, train_config)
    ...
    return {
        ...
        "issues": issues,
        "source_decoder_anchor_seed_revision": str(decoder_anchor_seed_path or ""),
        "run_dir": str(root),
        "decoder_anchor_seed_summary": seed_summary,
        "artifacts": artifacts,
        "metrics": metrics,
        "train_config": train_config,
        "manifest": manifest,
        "check_rows": checks,
        "decoder_anchor_training": training,
...
def _checks(
    ...
    return [
        ...
        _check("decoder_anchor_seed_ready", seed_summary.get("decoder_anchor_seed_revision_ready") is True, seed_summary.get("decoder_anchor_seed_revision_ready"), "decoder anchor seed revision must be ready"),
        _check("bridge_examples_present", int(seed_summary.get("bridge_example_count") or 0) > 0, seed_summary.get("bridge_example_count"), "seed must contain decoder bridge examples"),
        _check("direct_examples_present", int(seed_summary.get("unanchored_direct_example_count") or 0) > 0, seed_summary.get("unanchored_direct_example_count"), "seed must contain direct answer examples"),
...
def _training(
    ...
    return {
        ...
        "decoder_anchor_example_count": seed_summary.get("example_count"),
        "bridge_example_count": seed_summary.get("bridge_example_count"),
        "unanchored_direct_example_count": seed_summary.get("unanchored_direct_example_count"),
        "proposed_next_artifact": "model_capability_route_promotion_bounded_real_replay_decoder_anchor_checkpoint_replay",
        "next_step": "run_decoder_anchor_checkpoint_bounded_replay" if status == "pass" else "repair_decoder_anchor_training_run",
    }


def _summary(status: str, checks: list[dict[str, Any]], training: dict[str, Any]) -> dict[str, Any]:
    return {
        "decoder_anchor_training_ready": status == "pass" and training.get("ready") is True,
        ...
        "bridge_example_count": training.get("bridge_example_count"),
        "unanchored_direct_example_count": training.get("unanchored_direct_example_count"),
...
def _decision(status: str) -> str:
    if status == "pass":
        return "model_capability_route_promotion_bounded_real_replay_decoder_anchor_training_run_ready"
    return "fix_model_capability_route_promotion_bounded_real_replay_decoder_anchor_training_run"


def _interpretation(status: str, training: dict[str, Any]) -> dict[str, Any]:
    if status != "pass":
        return {"model_quality_claim": "not_claimed", "reason": "Decoder anchor training artifacts are incomplete.", "next_action": "repair decoder anchor training run"}
    return {
        "model_quality_claim": "training_artifact_only",
        "reason": "A decoder-anchor-informed checkpoint exists, but model improvement still requires bounded replay comparison.",
...
__all__ = [
    "MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_DECODER_ANCHOR_TRAINING_RUN_CSV_FILENAME",
    "MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_DECODER_ANCHOR_TRAINING_RUN_HTML_FILENAME",
    "MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_DECODER_ANCHOR_TRAINING_RUN_JSON_FILENAME",
    "MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_DECODER_ANCHOR_TRAINING_RUN_MARKDOWN_FILENAME",
    "MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_DECODER_ANCHOR_TRAINING_RUN_TEXT_FILENAME",
