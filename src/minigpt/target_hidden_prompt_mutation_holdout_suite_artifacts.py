...
def render_target_hidden_prompt_mutation_holdout_suite_text(report: dict[str, Any]) -> str:
    ...
    rows = [
        ...
        ("target_hidden_prompt_mutation_holdout_suite_ready", summary.get("target_hidden_prompt_mutation_holdout_suite_ready")),
        ("source_case_count", summary.get("source_case_count")),
        ("candidate_case_count", summary.get("candidate_case_count")),
        ("mutation_factor", summary.get("mutation_factor")),
        ...
        ("source_clean_prompt_case_count", summary.get("source_clean_prompt_case_count")),
...
def write_target_hidden_prompt_mutation_holdout_suite_csv(report: dict[str, Any], path: str | Path) -> None:
    fieldnames = ["case_id", "source_case_id", "tokenizer_covered", "target_hidden", "task_hint", "prompt_mutated", "prompt_unknown_count", "prompt"]
...
def render_target_hidden_prompt_mutation_holdout_suite_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT target-hidden prompt-mutation holdout suite'))}",
        "",
        f"- Status: `{report.get('status')}`",
        f"- Decision: `{report.get('decision')}`",
        f"- Candidate cases: `{summary.get('candidate_case_count')}`",
        f"- Mutation factor: `{summary.get('mutation_factor')}`",
...
def render_target_hidden_prompt_mutation_holdout_suite_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ("Status", report.get("status")),
        ("Candidates", summary.get("candidate_case_count")),
        ("Mutation factor", summary.get("mutation_factor")),
        ...
        ("Mutated", summary.get("prompt_mutated_case_count")),
...
<title>{html_escape(report.get('title', 'MiniGPT target-hidden prompt-mutation holdout suite'))}</title>{_style()}</head>
<body><main><header><h1>{html_escape(report.get('title', 'MiniGPT target-hidden prompt-mutation holdout suite'))}</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
<section class="stats">{''.join(_card(label, value) for label, value in stats)}</section>
<section class="panel"><h2>Coverage Rows</h2><div class="table-wrap"><table><thead><tr><th>Case</th><th>Source</th><th>Covered</th><th>Target Hidden</th><th>Hint</th><th>Mutated</th><th>Unknown</th><th>Prompt</th></tr></thead><tbody>{rows}</tbody></table></div></section>
...
def write_target_hidden_prompt_mutation_holdout_suite_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / TARGET_HIDDEN_PROMPT_MUTATION_HOLDOUT_SUITE_JSON_FILENAME,
        "csv": root / TARGET_HIDDEN_PROMPT_MUTATION_HOLDOUT_SUITE_CSV_FILENAME,
        "text": root / TARGET_HIDDEN_PROMPT_MUTATION_HOLDOUT_SUITE_TEXT_FILENAME,
        "markdown": root / TARGET_HIDDEN_PROMPT_MUTATION_HOLDOUT_SUITE_MARKDOWN_FILENAME,
        "html": root / TARGET_HIDDEN_PROMPT_MUTATION_HOLDOUT_SUITE_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_target_hidden_prompt_mutation_holdout_suite_csv(report, paths["csv"])
    paths["text"].write_text(render_target_hidden_prompt_mutation_holdout_suite_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_target_hidden_prompt_mutation_holdout_suite_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_target_hidden_prompt_mutation_holdout_suite_html(report), encoding="utf-8")
...
def _coverage_row(row: dict[str, Any], report: dict[str, Any]) -> str:
    return (
        ...
        f"<td>{html_escape(row.get('prompt_mutated'))}</td>"
...
def _style() -> str:
    return """<style>
:root{color-scheme:light;--ink:#17212b;--muted:#62717d;--line:#d7dee5;--panel:#f8fafc;--accent:#7c3aed}
