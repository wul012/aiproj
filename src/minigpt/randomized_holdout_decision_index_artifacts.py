...
def render_randomized_holdout_decision_index_text(report: dict[str, Any]) -> str:
    ...
    rows = [
        ...
        ("randomized_holdout_decision_index_ready", summary.get("randomized_holdout_decision_index_ready")),
...
def write_randomized_holdout_decision_index_csv(report: dict[str, Any], path: str | Path) -> None:
    fieldnames = ["kind", "exists", "status", "decision", "ready_key", "ready_value", "candidate_case_count", "random_seed", "pass_rate", "model_quality_claim", "path"]
    out_path = Path(path)
...
def render_randomized_holdout_decision_index_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT randomized holdout decision index'))}",
        "",
        f"- Status: `{report.get('status')}`",
        f"- Decision: `{report.get('decision')}`",
        f"- Index ready: `{summary.get('randomized_holdout_decision_index_ready')}`",
...
def render_randomized_holdout_decision_index_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ("Status", report.get("status")),
        ("Index ready", summary.get("randomized_holdout_decision_index_ready")),
...
<title>{html_escape(report.get('title', 'MiniGPT randomized holdout decision index'))}</title>
{_style()}
</head>
<body>
<main>
<header><h1>{html_escape(report.get('title', 'MiniGPT randomized holdout decision index'))}</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
<section class="stats">{''.join(_card(label, value) for label, value in stats)}</section>
<section class="panel"><h2>Source Rows</h2><div class="table-wrap"><table>
<thead><tr><th>Kind</th><th>Status</th><th>Ready</th><th>Cases</th><th>Seed</th><th>Claim</th><th>Path</th></tr></thead>
<tbody>{sources}</tbody>
</table></div></section>
...
def write_randomized_holdout_decision_index_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / RANDOMIZED_HOLDOUT_DECISION_INDEX_JSON_FILENAME,
        "csv": root / RANDOMIZED_HOLDOUT_DECISION_INDEX_CSV_FILENAME,
        "text": root / RANDOMIZED_HOLDOUT_DECISION_INDEX_TEXT_FILENAME,
        "markdown": root / RANDOMIZED_HOLDOUT_DECISION_INDEX_MARKDOWN_FILENAME,
        "html": root / RANDOMIZED_HOLDOUT_DECISION_INDEX_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_randomized_holdout_decision_index_csv(report, paths["csv"])
    paths["text"].write_text(render_randomized_holdout_decision_index_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_randomized_holdout_decision_index_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_randomized_holdout_decision_index_html(report), encoding="utf-8")
...
def _source_row(row: dict[str, Any]) -> str:
    return (
        ...
        f"<td>{html_escape(row.get('random_seed'))}</td>"
