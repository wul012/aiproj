...
def render_ack_index_artifacts_text(report: dict[str, Any]) -> str:
...
def write_ack_index_artifacts_csv(report: dict[str, Any], path: str | Path) -> None:
...
def render_ack_index_artifacts_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    index = as_dict(report.get("publication_index"))
    lines = [
...
def render_ack_index_artifacts_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    index = as_dict(report.get("publication_index"))
...
def write_ack_index_artifacts_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / ACK_BUNDLE_INDEX_JSON_FILENAME,
        "csv": root / ACK_BUNDLE_INDEX_CSV_FILENAME,
        "text": root / ACK_BUNDLE_INDEX_TEXT_FILENAME,
        "markdown": root / ACK_BUNDLE_INDEX_MARKDOWN_FILENAME,
        "html": root / ACK_BUNDLE_INDEX_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_ack_index_artifacts_csv(report, paths["csv"])
    paths["text"].write_text(render_ack_index_artifacts_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_ack_index_artifacts_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_ack_index_artifacts_html(report), encoding="utf-8")
