...
from minigpt.randomized_holdout_publication_constants import (
    ...
    RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_INDEX_RECEIPT_V1015_NEXT_STEP,
...
RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_INDEX_RECEIPT_V1015_JSON_FILENAME = "randomized_holdout_publication_receipt_index_receipt_v1015.json"
RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_INDEX_RECEIPT_V1015_CSV_FILENAME = "randomized_holdout_publication_receipt_index_receipt_v1015.csv"
RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_INDEX_RECEIPT_V1015_TEXT_FILENAME = "randomized_holdout_publication_receipt_index_receipt_v1015.txt"
RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_INDEX_RECEIPT_V1015_MARKDOWN_FILENAME = "randomized_holdout_publication_receipt_index_receipt_v1015.md"
RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_INDEX_RECEIPT_V1015_HTML_FILENAME = "randomized_holdout_publication_receipt_index_receipt_v1015.html"

READY_KEY = "randomized_holdout_publication_receipt_index_receipt_v1015_ready"
SOURCE_REVIEW_READY_KEY = "randomized_holdout_publication_receipt_packet_index_publication_receipt_index_receipt_index_publication_index_receipt_index_receipt_index_receipt_index_receipt_index_receipt_index_review_v1014_ready"
SOURCE_REVIEW_STATUS = "approved_for_publication_index_receipt_index_receipt_index_receipt_index_receipt_index_receipt_index_receipt_lookup_only"
RECEIPT_ID = "randomized-holdout-publication-receipt-index-receipt-v1015"
RECEIPT_TYPE = "randomized_holdout_publication_receipt_index_receipt"
RECEIPT_STATUS = "publication_receipt_index_receipt_v1015_lookup_receipted"
DEFAULT_CONSUMER_NAME = "publication_receipt_index_receipt_v1015_lookup_reader"
LOOKUP_KEY_PREFIX = "publication-index-receipt-index-receipt-index-receipt-index-receipt:"
...
def read_json_report(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("randomized holdout publication receipt index receipt v1015 input must be a JSON object")
    return dict(payload)


def build_randomized_holdout_publication_receipt_index_receipt_v1015(
    ...
    *,
    ...
    title: str = "MiniGPT randomized holdout publication receipt index receipt v1015",
...
def _checks(
    ...
    return [
        ...
        _check("receipt_index_review_decision_ready", review_report.get("decision") == SOURCE_REVIEW_READY_KEY, review_report.get("decision"), "receipt index review decision must be ready"),
        ...
        _check("lookup_keys_publication_index_receipt_namespace", len(lookup_keys) == len(index_rows) and all(str(key).startswith(LOOKUP_KEY_PREFIX) for key in lookup_keys), lookup_keys, "lookup keys must use publication-index receipt namespace"),
        ...
        _check("source_next_step_matches", review_summary.get("next_step") == RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_PACKET_INDEX_PUBLICATION_RECEIPT_INDEX_RECEIPT_INDEX_PUBLICATION_INDEX_RECEIPT_INDEX_RECEIPT_INDEX_RECEIPT_INDEX_RECEIPT_INDEX_RECEIPT_INDEX_REVIEW_V1014_NEXT_STEP, review_summary.get("next_step"), "source receipt index review must route to receipt"),
    ]
...
def _receipt(
    ...
    return {
        ...
        "next_step": RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_INDEX_RECEIPT_V1015_NEXT_STEP if ready else "repair_randomized_holdout_publication_receipt_index_receipt_v1015",
    }
...
def _decision(status: str) -> str:
    if status == "pass":
        return READY_KEY
    return "fix_randomized_holdout_publication_receipt_index_receipt_v1015"


def _interpretation(status: str, receipt: dict[str, Any]) -> dict[str, str]:
    if status != "pass":
        return {"model_quality_claim": "not_claimed", "reason": "The publication receipt index review is not ready to be receipted.", "next_action": "repair receipt index review before recording receipt"}
    return {
        "model_quality_claim": RANDOMIZED_HOLDOUT_PUBLICATION_MODEL_QUALITY_CLAIM,
        "reason": "The reviewed publication receipt index is receipted for downstream lookup only; production promotion remains blocked.",
        "next_action": str(receipt.get("next_step")),
    }


__all__ = [
    "RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_INDEX_RECEIPT_V1015_CSV_FILENAME",
    "RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_INDEX_RECEIPT_V1015_HTML_FILENAME",
    "RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_INDEX_RECEIPT_V1015_JSON_FILENAME",
    "RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_INDEX_RECEIPT_V1015_MARKDOWN_FILENAME",
    "RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_INDEX_RECEIPT_V1015_TEXT_FILENAME",
