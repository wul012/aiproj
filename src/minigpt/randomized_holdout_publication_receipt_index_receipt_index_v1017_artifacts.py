...
def render_randomized_holdout_publication_receipt_index_receipt_index_v1017_text(report: dict[str, Any]) -> str:
...
def write_randomized_holdout_publication_receipt_index_receipt_index_v1017_csv(report: dict[str, Any], path: str | Path) -> None:
...
def render_randomized_holdout_publication_receipt_index_receipt_index_v1017_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    index = as_dict(report.get("receipt_index"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT randomized holdout publication receipt index receipt index v1017'))}",
        "",
...
def render_randomized_holdout_publication_receipt_index_receipt_index_v1017_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    index = as_dict(report.get("receipt_index"))
...
def write_randomized_holdout_publication_receipt_index_receipt_index_v1017_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_INDEX_RECEIPT_INDEX_V1017_JSON_FILENAME,
        "csv": root / RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_INDEX_RECEIPT_INDEX_V1017_CSV_FILENAME,
        "text": root / RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_INDEX_RECEIPT_INDEX_V1017_TEXT_FILENAME,
        "markdown": root / RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_INDEX_RECEIPT_INDEX_V1017_MARKDOWN_FILENAME,
        "html": root / RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_INDEX_RECEIPT_INDEX_V1017_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_randomized_holdout_publication_receipt_index_receipt_index_v1017_csv(report, paths["csv"])
    paths["text"].write_text(render_randomized_holdout_publication_receipt_index_receipt_index_v1017_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_randomized_holdout_publication_receipt_index_receipt_index_v1017_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_randomized_holdout_publication_receipt_index_receipt_index_v1017_html(report), encoding="utf-8")
