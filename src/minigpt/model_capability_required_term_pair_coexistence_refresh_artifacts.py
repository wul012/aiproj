from __future__ import annotations

from pathlib import Path
from typing import Any

from minigpt.model_capability_required_term_pair_coexistence_refresh import (
    PAIR_COEXISTENCE_REFRESH_HTML_FILENAME,
    PAIR_COEXISTENCE_REFRESH_JSON_FILENAME,
    PAIR_COEXISTENCE_REFRESH_MARKDOWN_FILENAME,
    PAIR_COEXISTENCE_REFRESH_TEXT_FILENAME,
)
from minigpt.model_capability_required_term_pair_generation_profile_replay_artifacts import (
    write_model_capability_required_term_pair_generation_profile_replay_outputs,
)
from minigpt.report_utils import as_dict, html_escape, list_of_dicts, markdown_cell, write_json_payload
from minigpt.report_utils import html_card as _card


def render_model_capability_required_term_pair_coexistence_refresh_text(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    rows = [
        ("status", report.get("status")),
        ("decision", report.get("decision")),
        ("failed_count", report.get("failed_count")),
        ("training_status", summary.get("training_status")),
        ("training_mode", summary.get("training_mode")),
        ("resume_checkpoint_exists", summary.get("resume_checkpoint_exists")),
        ("checkpoint_exists", summary.get("checkpoint_exists")),
        ("default_pair_full_variant_count", summary.get("default_pair_full_variant_count")),
        ("suppression_pair_full_variant_count", summary.get("suppression_pair_full_variant_count")),
        ("pair_full_observed", summary.get("pair_full_observed")),
        ("model_quality_claim", interpretation.get("model_quality_claim")),
        ("next_action", interpretation.get("next_action")),
    ]
    return "\n".join(f"{key}={value}" for key, value in rows) + "\n"


def render_model_capability_required_term_pair_coexistence_refresh_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    replay = as_dict(report.get("replay_report"))
    variant_rows = [
        "| Variant | Profile | Hit terms | Missed terms | Pair full |",
        "| --- | --- | --- | --- | --- |",
    ]
    for row in list_of_dicts(replay.get("variant_rows")):
        variant_rows.append(
            "| "
            + " | ".join(
                [
                    markdown_cell(row.get("variant_id")),
                    markdown_cell(row.get("profile_id")),
                    markdown_cell(",".join(str(item) for item in row.get("hit_terms", []))),
                    markdown_cell(",".join(str(item) for item in row.get("missed_terms", []))),
                    markdown_cell(row.get("pair_full_hit")),
                ]
            )
            + " |"
        )
    return "\n".join(
        [
            "# MiniGPT Required-Term Pair Coexistence Refresh",
            "",
            f"- Status: `{report.get('status')}`",
            f"- Decision: `{report.get('decision')}`",
            f"- Training status: `{summary.get('training_status')}`",
            f"- Training mode: `{summary.get('training_mode')}`",
            f"- Pair full observed: `{summary.get('pair_full_observed')}`",
            f"- Model quality claim: `{interpretation.get('model_quality_claim')}`",
            "",
            "## Replay Variants",
            "",
            *variant_rows,
            "",
            "## Boundary",
            "",
            f"- Reason: {interpretation.get('reason')}",
            f"- Next action: {interpretation.get('next_action')}",
            "",
        ]
    )


def render_model_capability_required_term_pair_coexistence_refresh_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    replay = as_dict(report.get("replay_report"))
    stats = [
        ("Status", report.get("status")),
        ("Decision", report.get("decision")),
        ("Training", summary.get("training_status")),
        ("Mode", summary.get("training_mode")),
        ("Checkpoint", summary.get("checkpoint_exists")),
        ("Default pair full", summary.get("default_pair_full_variant_count")),
        ("Suppression pair full", summary.get("suppression_pair_full_variant_count")),
        ("Pair full observed", summary.get("pair_full_observed")),
    ]
    variant_rows = "\n".join(_variant_html(row) for row in list_of_dicts(replay.get("variant_rows")))
    return f"""<!doctype html>
<html lang="zh-CN">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<link rel="icon" href="data:,">
<title>MiniGPT pair coexistence refresh</title>
{_style()}
</head>
<body>
<main>
<header><h1>MiniGPT pair coexistence refresh</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
<section class="stats">{''.join(_card(label, value) for label, value in stats)}</section>
<section class="panel"><h2>Next Action</h2><p>{html_escape(interpretation.get('next_action'))}</p></section>
<section class="panel">
<h2>Replay Variants</h2>
<div class="table-wrap"><table>
<thead><tr><th>Variant</th><th>Profile</th><th>Hit terms</th><th>Missed terms</th><th>Pair full</th></tr></thead>
<tbody>{variant_rows}</tbody>
</table></div>
</section>
</main>
</body>
</html>
"""


def write_model_capability_required_term_pair_coexistence_refresh_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    replay_report = as_dict(report.get("replay_report"))
    if replay_report:
        report["sidecar_outputs"] = {
            "pair_generation_profile_replay": write_model_capability_required_term_pair_generation_profile_replay_outputs(
                replay_report,
                root / "pair-generation-profile-replay",
            )
        }
    paths = {
        "json": root / PAIR_COEXISTENCE_REFRESH_JSON_FILENAME,
        "text": root / PAIR_COEXISTENCE_REFRESH_TEXT_FILENAME,
        "markdown": root / PAIR_COEXISTENCE_REFRESH_MARKDOWN_FILENAME,
        "html": root / PAIR_COEXISTENCE_REFRESH_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    paths["text"].write_text(render_model_capability_required_term_pair_coexistence_refresh_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_model_capability_required_term_pair_coexistence_refresh_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_model_capability_required_term_pair_coexistence_refresh_html(report), encoding="utf-8")
    return {key: str(value) for key, value in paths.items()}


def _variant_html(row: dict[str, Any]) -> str:
    return (
        "<tr>"
        f"<td>{html_escape(row.get('variant_id'))}</td>"
        f"<td>{html_escape(row.get('profile_id'))}</td>"
        f"<td>{html_escape(','.join(str(item) for item in row.get('hit_terms', [])))}</td>"
        f"<td>{html_escape(','.join(str(item) for item in row.get('missed_terms', [])))}</td>"
        f"<td>{html_escape(row.get('pair_full_hit'))}</td>"
        "</tr>"
    )


def _style() -> str:
    return """<style>
:root{color-scheme:light;--ink:#172026;--muted:#5d6975;--line:#d8dee4;--panel:#f7f9fb;--accent:#0f766e}
*{box-sizing:border-box}
body{margin:0;background:#eef2f5;color:var(--ink);font-family:Arial,"Microsoft YaHei",sans-serif}
main{max-width:1120px;margin:0 auto;padding:28px}
h1{font-size:30px;margin:0 0 8px;letter-spacing:0}
h2{font-size:18px;margin:0 0 12px;letter-spacing:0}
p{color:var(--muted);line-height:1.55}
.stats{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px;margin:18px 0}
.card,.panel{background:white;border:1px solid var(--line);border-radius:8px}
.card{padding:14px}
.card span{display:block;color:var(--muted);font-size:12px;text-transform:uppercase}
.card strong{display:block;margin-top:6px;font-size:18px;line-height:1.2;color:var(--accent);overflow-wrap:anywhere}
.panel{padding:16px;margin:14px 0}
.table-wrap{overflow:auto}
table{width:100%;border-collapse:collapse;font-size:13px}
th,td{border-bottom:1px solid var(--line);padding:9px;text-align:left;vertical-align:top}
th{background:var(--panel);color:#334155}
</style>"""


__all__ = [
    "render_model_capability_required_term_pair_coexistence_refresh_html",
    "render_model_capability_required_term_pair_coexistence_refresh_markdown",
    "render_model_capability_required_term_pair_coexistence_refresh_text",
    "write_model_capability_required_term_pair_coexistence_refresh_outputs",
]
