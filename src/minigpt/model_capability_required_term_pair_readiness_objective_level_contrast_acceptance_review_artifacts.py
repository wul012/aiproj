...
def render_objective_level_contrast_acceptance_review_text(report: dict[str, Any]) -> str:
    ...
    rows = [
        ...
        ("acceptance_review_passed", summary.get("acceptance_review_passed")),
        ("promotion_allowed", summary.get("promotion_allowed")),
        ("accepted_route", summary.get("accepted_route")),
        ("model_quality_boundary", summary.get("model_quality_boundary")),
...
def render_objective_level_contrast_acceptance_review_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    ...
    return "\n".join(
        [
            ...
            f"- Accepted route: `{summary.get('accepted_route')}`",
            f"- Boundary: `{summary.get('model_quality_boundary')}`",
            "",
            "## Checks",
            "",
            *checks,
...
def render_objective_level_contrast_acceptance_review_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ("Status", report.get("status")),
        ...
        ("Route", summary.get("accepted_route") or "blocked"),
        ("Min pair-full", summary.get("min_pair_full_count")),
...
<title>MiniGPT objective-level contrast acceptance review</title>
{_style()}
</head>
<body>
<main>
<header><h1>MiniGPT objective-level contrast acceptance review</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
<section class="stats">{''.join(_card(label, value) for label, value in stats)}</section>
<section class="panel"><h2>Seed Replays</h2><div class="table-wrap"><table>
<thead><tr><th>Seed</th><th>Ready</th><th>Pair Full</th><th>Rate</th><th>Source</th></tr></thead>
...
def write_objective_level_contrast_acceptance_review_csv(report: dict[str, Any], path: str | Path) -> None:
...
def write_objective_level_contrast_acceptance_review_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / PAIR_READINESS_OBJECTIVE_LEVEL_CONTRAST_ACCEPTANCE_REVIEW_JSON_FILENAME,
        "csv": root / PAIR_READINESS_OBJECTIVE_LEVEL_CONTRAST_ACCEPTANCE_REVIEW_CSV_FILENAME,
        "text": root / PAIR_READINESS_OBJECTIVE_LEVEL_CONTRAST_ACCEPTANCE_REVIEW_TEXT_FILENAME,
        "markdown": root / PAIR_READINESS_OBJECTIVE_LEVEL_CONTRAST_ACCEPTANCE_REVIEW_MARKDOWN_FILENAME,
        "html": root / PAIR_READINESS_OBJECTIVE_LEVEL_CONTRAST_ACCEPTANCE_REVIEW_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_objective_level_contrast_acceptance_review_csv(report, paths["csv"])
    paths["text"].write_text(render_objective_level_contrast_acceptance_review_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_objective_level_contrast_acceptance_review_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_objective_level_contrast_acceptance_review_html(report), encoding="utf-8")
...
def _seed_row_html(row: dict[str, Any]) -> str:
    return (
        ...
        f"<td>{html_escape(row.get('pair_full_rate'))}</td>"
