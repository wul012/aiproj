...
GENERATION_ROWS_NAME = "unassisted_loss_suffix_repair_replay_generation_rows_v1155.json"
GeneratorRunner = Callable[[dict[str, Any], Path, Path, str, int], dict[str, Any]]


def default_v1154_training_handoff_path(repo_root: str | Path) -> Path:
    return (
        Path(repo_root)
        / "f"
        / "1154"
        / EXPLAIN_DIR_NAME
        / "unassisted-loss-suffix-repair-training-run-v1154"
        / TRAINING_HANDOFF_NAME
    )


def locate_v1154_training_handoff(path: str | Path) -> Path:
    source = Path(path)
    if source.is_dir():
...
def build_unassisted_loss_suffix_repair_replay_comparison_v1155(
    ...
    *,
    ...
    prompts = holdout_prompts if holdout_prompts is not None else read_json_rows(holdout_path, description="v1153 target-free holdout prompts")
    ...
    if not preflight_issues:
        checks.append(
            _check(
                ...
                "each v1153 holdout prompt must produce one generation row",
            )
        )
    ...
    return {
        "schema_version": 1,
        "title": "MiniGPT unassisted loss suffix repair replay comparison v1155",
...
def write_unassisted_loss_suffix_repair_replay_comparison_v1155_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    outputs = write_readability_outputs(
        report,
        out_dir,
        stem=UNASSISTED_LOSS_SUFFIX_REPAIR_REPLAY_COMPARISON_V1155_STEM,
...
def _preflight_checks(
    ...
    return [
        _check("training_handoff_passed", training_handoff.get("status") == "pass", training_handoff.get("status"), "v1154 training handoff must pass"),
        _check("handoff_next_step_matches_replay", training_handoff.get("next_step") == "run_unassisted_loss_suffix_repair_replay_comparison", training_handoff.get("next_step"), "v1154 handoff must point to replay comparison"),
        _check("checkpoint_exists", checkpoint.is_file(), str(checkpoint), "v1154 checkpoint.pt must exist"),
        _check("tokenizer_exists", tokenizer.is_file(), str(tokenizer), "v1154 tokenizer.json must exist"),
        _check("holdout_prompts_exist", holdout_path.is_file() or bool(prompts), str(holdout_path), "v1153 holdout prompts must be available"),
        _check("holdout_prompt_count", len(prompts) >= 5, len(prompts), "replay should cover the target-free v1153 holdout prompts"),
        ...
        _check("promotion_boundary_kept", training_handoff.get("promotion_ready") is False, training_handoff.get("promotion_ready"), "v1155 replay is evidence, not automatic promotion"),
    ]
...
def _generate_case(row: dict[str, Any], checkpoint: Path, tokenizer: Path, device: str, index: int) -> dict[str, Any]:
    request = GenerationRequest(
        ...
        seed=int(row.get("seed") or (115500 + index)),
    )
    response = MiniGPTGenerator(checkpoint, tokenizer, device=device).generate(request).to_dict()
    response["case_id"] = row.get("case_id") or f"unassisted-loss-suffix-holdout-{index:02d}"
...
def _scored_generation(row: dict[str, Any], index: int) -> dict[str, Any]:
    ...
    return {
        "case_id": str(row.get("case_id") or f"unassisted-loss-suffix-holdout-{index:02d}"),
        "prompt": str(row.get("prompt") or ""),
...
def _summary(status: str, checks: list[dict[str, Any]], comparison: dict[str, Any]) -> dict[str, Any]:
    return {
        "unassisted_loss_suffix_repair_replay_comparison_ready": status == "pass",
        ...
        "promotion_ready": False,
        "next_step": comparison.get("next_step") if status == "pass" else "repair_unassisted_loss_suffix_repair_replay_inputs",
...
def _decision(status: str, comparison: dict[str, Any]) -> str:
    if status != "pass":
        return "fix_unassisted_loss_suffix_repair_replay_comparison"
    if comparison.get("all_full_pair_hit") is True:
        return "unassisted_loss_suffix_repair_replay_full_pair_recovered_candidate"
    if int(comparison.get("any_hit_case_count") or 0) > 0:
        return "unassisted_loss_suffix_repair_replay_partial_signal"
    return "unassisted_loss_suffix_repair_replay_zero_hit"
...
def _interpretation(status: str, comparison: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "reason": "This replay used the unchanged v1153 target-free holdout prompts against the v1154 checkpoint.",
...
def _next_step(all_full_pair: bool, any_hit_count: int) -> str:
    if all_full_pair:
        return "repeat_unassisted_loss_suffix_repair_replay_across_seeds_before_promotion"
    if any_hit_count > 0:
...
__all__ = [
    ...
    "UNASSISTED_LOSS_SUFFIX_REPAIR_REPLAY_COMPARISON_V1155_STEM",
