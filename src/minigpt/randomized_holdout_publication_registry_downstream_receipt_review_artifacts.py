...
def render_randomized_holdout_publication_registry_downstream_receipt_review_text(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    rows = [
        ("status", report.get("status")),
        ("decision", report.get("decision")),
        ("failed_count", report.get("failed_count")),
        ("randomized_holdout_publication_registry_downstream_receipt_review_ready", summary.get("randomized_holdout_publication_registry_downstream_receipt_review_ready")),
        ("review_id", summary.get("review_id")),
        ("review_status", summary.get("review_status")),
        ("consumer_name", summary.get("consumer_name")),
        ("consumer_ready", summary.get("consumer_ready")),
...
def write_randomized_holdout_publication_registry_downstream_receipt_review_csv(report: dict[str, Any], path: str | Path) -> None:
    fieldnames = ["consumer_name", "lookup_key", "entry_id", "granted_use", "blocked_uses", "promotion_ready", "receipt_status"]
    out_path = Path(path)
...
def render_randomized_holdout_publication_registry_downstream_receipt_review_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT randomized holdout publication registry downstream receipt review'))}",
        "",
        f"- Status: `{report.get('status')}`",
        f"- Decision: `{report.get('decision')}`",
        f"- Review ready: `{summary.get('randomized_holdout_publication_registry_downstream_receipt_review_ready')}`",
        f"- Review status: `{summary.get('review_status')}`",
        f"- Consumer ready: `{summary.get('consumer_ready')}`",
...
def render_randomized_holdout_publication_registry_downstream_receipt_review_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    review = as_dict(report.get("review"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ("Status", report.get("status")),
        ("Review ready", summary.get("randomized_holdout_publication_registry_downstream_receipt_review_ready")),
...
<title>{html_escape(report.get('title', 'MiniGPT randomized holdout publication registry downstream receipt review'))}</title>
{_style()}
</head>
<body>
<main>
<header><h1>{html_escape(report.get('title', 'MiniGPT randomized holdout publication registry downstream receipt review'))}</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
...
<dt>Source guard digest</dt><dd>{html_escape(review.get('source_guard_sha256'))}</dd>
...
<thead><tr><th>Consumer</th><th>Lookup key</th><th>Entry</th><th>Granted use</th><th>Blocked uses</th><th>Promotion</th><th>Status</th></tr></thead>
...
def write_randomized_holdout_publication_registry_downstream_receipt_review_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_RECEIPT_REVIEW_JSON_FILENAME,
        "csv": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_RECEIPT_REVIEW_CSV_FILENAME,
        "text": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_RECEIPT_REVIEW_TEXT_FILENAME,
        "markdown": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_RECEIPT_REVIEW_MARKDOWN_FILENAME,
        "html": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_RECEIPT_REVIEW_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_randomized_holdout_publication_registry_downstream_receipt_review_csv(report, paths["csv"])
    paths["text"].write_text(render_randomized_holdout_publication_registry_downstream_receipt_review_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_randomized_holdout_publication_registry_downstream_receipt_review_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_randomized_holdout_publication_registry_downstream_receipt_review_html(report), encoding="utf-8")
...
def _receipt_row(row: dict[str, Any]) -> str:
    return (
        ...
        f"<td>{html_escape(row.get('receipt_status'))}</td>"
        "</tr>"
    )
