...
TARGET_HIDDEN_SEMANTIC_HOLDOUT_DRY_RUN_CSV_FILENAME = "target_hidden_semantic_holdout_dry_run.csv"
...
def read_json_report(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("target-hidden semantic holdout dry-run input must be a JSON object")
    return dict(payload)


def build_target_hidden_semantic_holdout_dry_run(
    ...
    *,
    ...
    title: str = "MiniGPT target-hidden semantic paraphrase holdout dry-run",
...
def _checks(
    ...
    return [
        ...
        _check("holdout_suite_ready", source_summary.get("target_hidden_semantic_holdout_suite_ready") is True, source_summary.get("target_hidden_semantic_holdout_suite_ready"), "semantic holdout suite summary must be ready"),
        ...
        _check("semantic_no_task_hints", source_summary.get("task_hint_case_count") == 0, source_summary.get("task_hint_case_count"), "semantic holdout should have no known task hints"),
...
def _summary(status: str, cases: list[dict[str, Any]], rows: list[dict[str, Any]], issues: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "target_hidden_semantic_holdout_dry_run_ready": status == "pass",
...
def _decision(status: str) -> str:
    return "target_hidden_semantic_holdout_dry_run_passed" if status == "pass" else "fix_target_hidden_semantic_holdout_dry_run"


def _interpretation(status: str, summary: dict[str, Any]) -> dict[str, Any]:
    if status != "pass":
        return {"model_quality_claim": "not_claimed", "reason": "Semantic holdout dry-run checks failed.", "next_action": "repair_target_hidden_semantic_holdout_dry_run"}
    return {
        "model_quality_claim": summary.get("model_quality_claim"),
        "reason": "The semantic target-hidden holdout scoring contract passes dry-run checks; real replay is next.",
