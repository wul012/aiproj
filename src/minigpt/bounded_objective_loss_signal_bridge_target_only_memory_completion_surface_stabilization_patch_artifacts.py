...
def render_completion_surface_stabilization_patch_text(report: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"completion_surface_stabilization_patch_ready={summary.get('bounded_objective_loss_signal_bridge_target_only_memory_completion_surface_stabilization_patch_ready')}",
        ...
        f"answer_surface_carry_forward_count={summary.get('answer_surface_carry_forward_count')}",
        f"prefix_fragment_bridge_count={summary.get('prefix_fragment_bridge_count')}",
        f"completion_fragment_resistance_count={summary.get('completion_fragment_resistance_count')}",
...
def render_completion_surface_stabilization_patch_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT bounded objective loss signal bridge target-only memory completion-surface stabilization patch'))}",
        "",
        f"- Status: `{report.get('status')}`",
        f"- Decision: `{report.get('decision')}`",
        f"- Patch examples: `{summary.get('patch_example_count')}`",
        f"- Completion examples: `{summary.get('completion_surface_example_count')}`",
        f"- Answer carry-forward: `{summary.get('answer_surface_carry_forward_count')}`",
        f"- Prefix bridges: `{summary.get('prefix_fragment_bridge_count')}`",
        f"- Fragment resistance: `{summary.get('completion_fragment_resistance_count')}`",
        "",
        "## Patch Examples",
        "",
...
def render_completion_surface_stabilization_patch_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    ...
    stats = [
        ...
        ("Completion", summary.get("completion_surface_example_count")),
        ("Answer carry", summary.get("answer_surface_carry_forward_count")),
        ("Bridge", summary.get("prefix_fragment_bridge_count")),
        ("Resist", summary.get("completion_fragment_resistance_count")),
...
<title>{html_escape(report.get('title', 'MiniGPT bounded objective loss signal bridge target-only memory completion-surface stabilization patch'))}</title>{_style()}</head>
<body><main>
<header><h1>{html_escape(report.get('title', 'MiniGPT bounded objective loss signal bridge target-only memory completion-surface stabilization patch'))}</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
...
def write_completion_surface_stabilization_patch_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / TARGET_ONLY_MEMORY_COMPLETION_SURFACE_STABILIZATION_PATCH_JSON_FILENAME,
        "csv": root / TARGET_ONLY_MEMORY_COMPLETION_SURFACE_STABILIZATION_PATCH_CSV_FILENAME,
        "jsonl": root / TARGET_ONLY_MEMORY_COMPLETION_SURFACE_STABILIZATION_PATCH_JSONL_FILENAME,
        "corpus": root / TARGET_ONLY_MEMORY_COMPLETION_SURFACE_STABILIZATION_PATCH_CORPUS_FILENAME,
        "text": root / TARGET_ONLY_MEMORY_COMPLETION_SURFACE_STABILIZATION_PATCH_TEXT_FILENAME,
        "markdown": root / TARGET_ONLY_MEMORY_COMPLETION_SURFACE_STABILIZATION_PATCH_MARKDOWN_FILENAME,
        "html": root / TARGET_ONLY_MEMORY_COMPLETION_SURFACE_STABILIZATION_PATCH_HTML_FILENAME,
    }
    ...
    paths["text"].write_text(render_completion_surface_stabilization_patch_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_completion_surface_stabilization_patch_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_completion_surface_stabilization_patch_html(report), encoding="utf-8")
    return {key: str(value) for key, value in paths.items()}


def _write_csv(report: dict[str, Any], path: Path) -> None:
    fieldnames = ["example_id", "kind", "text", "source_case_id", "source_case_label", "decoder_anchor"]
...
def _style() -> str:
    return """<style>
:root{color-scheme:light;--ink:#17212b;--muted:#617182;--line:#d7dee8;--panel:#f8fafc;--accent:#245b8f}
*{box-sizing:border-box}body{margin:0;background:#f6f8fa;color:var(--ink);font-family:Arial,"Microsoft YaHei",sans-serif}
