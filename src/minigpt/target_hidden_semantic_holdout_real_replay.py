...
TARGET_HIDDEN_SEMANTIC_HOLDOUT_REAL_REPLAY_CSV_FILENAME = "target_hidden_semantic_holdout_real_replay.csv"
TARGET_HIDDEN_SEMANTIC_HOLDOUT_REAL_REPLAY_TEXT_FILENAME = "target_hidden_semantic_holdout_real_replay.txt"
TARGET_HIDDEN_SEMANTIC_HOLDOUT_REAL_REPLAY_MARKDOWN_FILENAME = "target_hidden_semantic_holdout_real_replay.md"
...
def locate_target_hidden_semantic_holdout_dry_run(path: str | Path) -> Path:
    source = Path(path)
    if source.is_dir():
        source = source / TARGET_HIDDEN_SEMANTIC_HOLDOUT_DRY_RUN_JSON_FILENAME
    return source
...
def read_json_report(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("target-hidden semantic holdout real replay input must be a JSON object")
    return dict(payload)


def build_target_hidden_semantic_holdout_real_replay(
    ...
    *,
    ...
    title: str = "MiniGPT target-hidden semantic paraphrase holdout real replay",
...
def _checks(
    ...
    return [
        _check("holdout_suite_passed", suite_report.get("status") == "pass", suite_report.get("status"), "semantic holdout suite must pass"),
        _check("holdout_suite_ready", suite_summary.get("target_hidden_semantic_holdout_suite_ready") is True, suite_summary.get("target_hidden_semantic_holdout_suite_ready"), "semantic holdout suite summary must be ready"),
        _check("target_hidden_cases_present", suite_summary.get("target_hidden_case_count") == len(cases), suite_summary.get("target_hidden_case_count"), "every semantic suite case must remain target-hidden"),
        _check("semantic_no_task_hints", suite_summary.get("task_hint_case_count") == 0, suite_summary.get("task_hint_case_count"), "semantic holdout should have no known task hints"),
        _check("dry_run_passed", dry_run_report.get("status") == "pass", dry_run_report.get("status"), "dry-run must pass before real replay"),
        _check("dry_run_ready", dry_summary.get("target_hidden_semantic_holdout_dry_run_ready") is True, dry_summary.get("target_hidden_semantic_holdout_dry_run_ready"), "dry-run summary must be ready"),
        ...
        _check("cases_present", bool(cases), len(cases), "semantic holdout suite must include cases"),
...
def _summary(status: str, cases: list[dict[str, Any]], rows: list[dict[str, Any]], issues: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "target_hidden_semantic_holdout_real_replay_ready": status == "pass",
        ...
        "semantic_holdout_model_quality_ready": status == "pass" and bool(cases) and passed == len(cases),
...
def _decision(status: str, summary: dict[str, Any]) -> str:
    if status != "pass":
        return "fix_target_hidden_semantic_holdout_real_replay_inputs"
    if summary.get("semantic_holdout_model_quality_ready") is True:
        return "target_hidden_semantic_holdout_real_replay_passed_review_required"
    if int(summary.get("any_hit_case_count") or 0) > 0:
        return "target_hidden_semantic_holdout_real_replay_partial_model_gap"
    return "target_hidden_semantic_holdout_real_replay_zero_hit_model_gap"


def _interpretation(status: str, summary: dict[str, Any]) -> dict[str, Any]:
    if status != "pass":
        return {"model_quality_claim": "not_claimed", "reason": "Semantic target-hidden real replay inputs failed.", "next_action": "repair_target_hidden_semantic_real_replay_inputs"}
    if summary.get("semantic_holdout_model_quality_ready") is True:
        return {"model_quality_claim": summary.get("model_quality_claim"), "reason": "The checkpoint passed every semantic target-hidden holdout case; review remains required before promotion.", "next_action": summary.get("next_step")}
    return {"model_quality_claim": summary.get("model_quality_claim"), "reason": "The semantic target-hidden holdout suite executed, but the checkpoint did not pass every case.", "next_action": summary.get("next_step")}


__all__ = [
    "TARGET_HIDDEN_SEMANTIC_HOLDOUT_REAL_REPLAY_CSV_FILENAME",
    "TARGET_HIDDEN_SEMANTIC_HOLDOUT_REAL_REPLAY_HTML_FILENAME",
