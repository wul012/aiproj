...
def render_training_portfolio_comparison_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        (
            f"| Best score CI archived path regressions | "
            f"{_md(summary.get('best_score_maturity_release_readiness_ci_archived_path_portability_check_ready_regression_count'))} |"
        ),
            ...
            + " | ".join(
                [
                    ...
                    _md(
                        ...
                        f"ci_archived_paths={portfolio.get('maturity_release_readiness_ci_archived_path_portability_check_ready_regression_count') or 0} / "
    ...
    lines.extend(["", "## Review Actions", "", "| ID | Portfolio | Severity | Category | Reason | Action |", "| --- | --- | --- | --- | --- | --- |"])
    review_actions = _list_of_dicts(report.get("review_actions"))
    for action in review_actions:
        lines.append(
            ...
            + " | ".join(
                [
                    _md(action.get("id")),
                    _md(action.get("portfolio")),
                    _md(action.get("severity")),
                    _md(action.get("category")),
                    _md(action.get("reason")),
                    _md(action.get("action")),
                ]
            )
            + " |"
        )
    if not review_actions:
        lines.append("| none | none | none | none | none | No review actions. |")
...
def render_training_portfolio_comparison_html(report: dict[str, Any]) -> str:
    ...
    stats = [
        ...
        (
            "Best score CI archived paths",
            summary.get("best_score_maturity_release_readiness_ci_archived_path_portability_check_ready_regression_count"),
        ),
...
def _review_action_table(report: dict[str, Any]) -> str:
    rows = []
    for action in _list_of_dicts(report.get("review_actions")):
        rows.append(
            "<tr>"
            f"<td>{_e(action.get('id'))}</td>"
            f"<td>{_e(action.get('portfolio'))}</td>"
            f"<td>{_e(action.get('severity'))}</td>"
            f"<td>{_e(action.get('category'))}</td>"
            f"<td>{_e(action.get('reason'))}</td>"
            f"<td>{_e(action.get('action'))}</td>"
            "</tr>"
        )
    if not rows:
        rows.append('<tr><td colspan="6">No review actions.</td></tr>')
    return (
        '<section class="panel"><h2>Review Actions</h2>'
        '<table><thead><tr><th>ID</th><th>Portfolio</th><th>Severity</th><th>Category</th><th>Reason</th><th>Action</th></tr></thead><tbody>'
        + "".join(rows)
        + "</tbody></table></section>"
    )
