...
PAIR_LOSS_BRANCH_OBJECTIVE_COMPARISON_CSV_FILENAME = "model_capability_required_term_pair_loss_branch_objective_comparison.csv"
PAIR_LOSS_BRANCH_OBJECTIVE_COMPARISON_TEXT_FILENAME = "model_capability_required_term_pair_loss_branch_objective_comparison.txt"
PAIR_LOSS_BRANCH_OBJECTIVE_COMPARISON_MARKDOWN_FILENAME = "model_capability_required_term_pair_loss_branch_objective_comparison.md"
PAIR_LOSS_BRANCH_OBJECTIVE_COMPARISON_HTML_FILENAME = "model_capability_required_term_pair_loss_branch_objective_comparison.html"

TARGET_TERMS = ("fixed", "loss")


def locate_loss_branch_objective_refresh_report(path: str | Path) -> Path:
    source = Path(path)
    if source.is_dir():
...
def read_loss_branch_objective_refresh_report(path: str | Path) -> dict[str, Any]:
    payload = json.loads(Path(path).read_text(encoding="utf-8-sig"))
    if not isinstance(payload, dict):
        raise ValueError("loss-branch objective comparison input must be a JSON object")
    return dict(payload)


def build_model_capability_required_term_pair_loss_branch_objective_comparison(
    ...
    *,
    ...
    return {
        "schema_version": 1,
        "title": "MiniGPT required-term pair loss-branch objective comparison",
...
def _source_label(labels: Sequence[str], index: int, report: dict[str, Any], paths: Sequence[str]) -> str:
    ...
    if "targeted" in mode:
        return "loss-branch-targeted"
    if "dual_anchor" in mode:
        return "loss-branch-dual-anchor"
    if "micro_span" in mode:
        return "loss-branch-micro-span"
    if index < len(paths) and paths[index]:
        return Path(paths[index]).parent.name
    return f"loss-branch-report-{index + 1}"
...
def _issues(report_rows: list[dict[str, Any]], term_rows: list[dict[str, Any]]) -> list[str]:
    ...
    if len(report_rows) < 2:
        issues.append("at least two loss-branch objective reports are required")
    for row in report_rows:
        ...
        if "loss_branch" not in str(row.get("corpus_mode") or ""):
            issues.append(f"{label} is not a loss-branch objective corpus mode")
    if not term_rows:
...
def _summary(report_rows: list[dict[str, Any]], branch_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    loss_hit_count = sum(1 for row in branch_rows if "loss" in row.get("hit_terms", []))
    fixed_hit_count = sum(1 for row in branch_rows if "fixed" in row.get("hit_terms", []))
    ...
    return {
        ...
        "loss_hit_report_count": loss_hit_count,
        "fixed_hit_report_count": fixed_hit_count,
        "union_hit_terms": union_terms,
        "all_loss_branch_reports_tradeoff": bool(report_rows) and loss_only_count == len(report_rows),
...
def _decision(status: str, summary: dict[str, Any]) -> str:
    if status != "pass":
        return "fix_required_term_pair_loss_branch_objective_comparison_inputs"
    if int(summary.get("pair_full_report_count") or 0) > 0:
        return "promote_loss_branch_pair_full_candidate"
    if summary.get("all_loss_branch_reports_tradeoff"):
        return "loss_branch_objectives_confirm_loss_only_tradeoff"
    return "record_loss_branch_objective_comparison"
...
def _interpretation(status: str, summary: dict[str, Any]) -> dict[str, Any]:
    if status != "pass":
        return {
            ...
            "reason": "The loss-branch objective reports are not structurally comparable.",
            "next_action": "repair comparison inputs before another training run",
        }
    if int(summary.get("pair_full_report_count") or 0) > 0:
        return {
            "model_quality_claim": "pair_full_candidate",
            "reason": "At least one loss-branch objective reached pair-full.",
            "next_action": "run a seed stability check before promotion",
        }
    if summary.get("all_loss_branch_reports_tradeoff"):
        return {
            "model_quality_claim": "tradeoff_only",
            "reason": "All compared loss-branch objectives recover loss but drop fixed.",
            "next_action": "build a route decision and test whether one route is worth seed stability replay",
        }
    return {
        "model_quality_claim": "comparison_only",
        "reason": "The compared objectives show mixed branch behavior without a pair-full route.",
        "next_action": "inspect best routes before selecting the next objective",
    }


__all__ = [
    "PAIR_LOSS_BRANCH_OBJECTIVE_COMPARISON_CSV_FILENAME",
    "PAIR_LOSS_BRANCH_OBJECTIVE_COMPARISON_HTML_FILENAME",
    "PAIR_LOSS_BRANCH_OBJECTIVE_COMPARISON_JSON_FILENAME",
    "PAIR_LOSS_BRANCH_OBJECTIVE_COMPARISON_MARKDOWN_FILENAME",
    "PAIR_LOSS_BRANCH_OBJECTIVE_COMPARISON_TEXT_FILENAME",
