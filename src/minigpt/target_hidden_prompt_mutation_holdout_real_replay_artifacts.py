...
def render_target_hidden_prompt_mutation_holdout_real_replay_text(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    rows = [
        ("status", report.get("status")),
        ("decision", report.get("decision")),
        ("failed_count", report.get("failed_count")),
        ("target_hidden_prompt_mutation_holdout_real_replay_ready", summary.get("target_hidden_prompt_mutation_holdout_real_replay_ready")),
        ("prompt_mutation_holdout_model_quality_ready", summary.get("prompt_mutation_holdout_model_quality_ready")),
...
def render_target_hidden_prompt_mutation_holdout_real_replay_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT target-hidden prompt-mutation holdout real replay'))}",
        "",
        f"- Status: `{report.get('status')}`",
        f"- Decision: `{report.get('decision')}`",
        f"- Model quality ready: `{summary.get('prompt_mutation_holdout_model_quality_ready')}`",
    ...
    for row in list_of_dicts(report.get("replay_rows")):
        lines.append("| " + " | ".join([markdown_cell(row.get("case_id")), markdown_cell(row.get("case_pass")), markdown_cell(_join_terms(row.get("hit_terms"))), markdown_cell(_join_terms(row.get("missed_terms"))), markdown_cell(row.get("continuation"))]) + " |")
...
def render_target_hidden_prompt_mutation_holdout_real_replay_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ("Status", report.get("status")),
        ("Quality ready", summary.get("prompt_mutation_holdout_model_quality_ready")),
...
<title>{html_escape(report.get('title', 'MiniGPT target-hidden prompt-mutation holdout real replay'))}</title>{_style()}</head>
<body><main><header><h1>{html_escape(report.get('title', 'MiniGPT target-hidden prompt-mutation holdout real replay'))}</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
<section class="stats">{''.join(_card(label, value) for label, value in stats)}</section>
<section class="panel"><h2>Replay Rows</h2><div class="table-wrap"><table><thead><tr><th>Case</th><th>Pass</th><th>Hit</th><th>Missed</th><th>Continuation</th></tr></thead><tbody>{rows}</tbody></table></div></section>
...
def write_target_hidden_prompt_mutation_holdout_real_replay_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / TARGET_HIDDEN_PROMPT_MUTATION_HOLDOUT_REAL_REPLAY_JSON_FILENAME,
        "csv": root / TARGET_HIDDEN_PROMPT_MUTATION_HOLDOUT_REAL_REPLAY_CSV_FILENAME,
        "text": root / TARGET_HIDDEN_PROMPT_MUTATION_HOLDOUT_REAL_REPLAY_TEXT_FILENAME,
        "markdown": root / TARGET_HIDDEN_PROMPT_MUTATION_HOLDOUT_REAL_REPLAY_MARKDOWN_FILENAME,
        "html": root / TARGET_HIDDEN_PROMPT_MUTATION_HOLDOUT_REAL_REPLAY_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_model_capability_route_promotion_bounded_real_replay_csv(report, paths["csv"])
    paths["text"].write_text(render_target_hidden_prompt_mutation_holdout_real_replay_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_target_hidden_prompt_mutation_holdout_real_replay_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_target_hidden_prompt_mutation_holdout_real_replay_html(report), encoding="utf-8")
