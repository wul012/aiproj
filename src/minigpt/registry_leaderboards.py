...
def release_readiness_delta_leaderboard_html(leaderboard: Any, base_dir: str | Path | None) -> str:
    ...
    for item in leaderboard[:10]:
        ...
        rows.append(
            ...
            f"<td>{_e(item.get('baseline_benchmark_history_status'))} -> {_e(item.get('compared_benchmark_history_status'))}<br><span>case={_e(_fmt_delta(item.get('benchmark_history_case_regression_delta')))} flags={_e(_fmt_delta(item.get('benchmark_history_generation_flag_regression_delta')))}</span><br><span>req={_e(item.get('baseline_benchmark_history_readiness_requirement_status'))} -> {_e(item.get('compared_benchmark_history_readiness_requirement_status'))} exit={_e(_fmt_delta(item.get('benchmark_history_readiness_requirement_exit_code_delta')))}</span><br><span>boundary={_e(item.get('benchmark_history_latest_boundary_changed'))}</span></td>"
