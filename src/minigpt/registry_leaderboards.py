...
def release_readiness_delta_leaderboard_html(leaderboard: Any, base_dir: str | Path | None) -> str:
    ...
    for item in leaderboard[:10]:
        ...
        rows.append(
            ...
            f"<td>{_e(item.get('baseline_ci_workflow_status'))} -> {_e(item.get('compared_ci_workflow_status'))}<br><span>failed={_e(_fmt_delta(item.get('ci_workflow_failed_check_delta')))} order={_e(_fmt_delta(item.get('ci_workflow_order_violation_delta')))}</span><br><span>reasons={_e('; '.join(_as_str_list(item.get('ci_workflow_regression_reasons'))))}</span></td>"
            ...
            f"<td>{_release_readiness_benchmark_cell(item)}</td>"
...
def _release_readiness_benchmark_cell(item: dict[str, Any]) -> str:
    return (
        f"{_e(item.get('baseline_benchmark_history_status'))} -> {_e(item.get('compared_benchmark_history_status'))}"
        f"<br><span>case={_e(_fmt_delta(item.get('benchmark_history_case_regression_delta')))} "
        f"flags={_e(_fmt_delta(item.get('benchmark_history_generation_flag_regression_delta')))}</span>"
        f"<br><span>suite={_e(_fmt_delta(item.get('benchmark_history_suite_design_non_comparison_ready_entries_delta')))} "
        f"design={_e(_fmt_delta(item.get('benchmark_history_design_comparison_changed_entries_delta')))}</span>"
        f"<br><span>req={_e(item.get('baseline_benchmark_history_readiness_requirement_status'))} -> "
        f"{_e(item.get('compared_benchmark_history_readiness_requirement_status'))} "
        f"exit={_e(_fmt_delta(item.get('benchmark_history_readiness_requirement_exit_code_delta')))}</span>"
        f"<br><span>boundary={_e(item.get('benchmark_history_latest_boundary_changed'))}</span>"
    )
