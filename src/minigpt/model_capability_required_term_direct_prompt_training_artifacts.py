...
from minigpt.model_capability_required_term_direct_prompt_training import (
    REQUIRED_TERM_DIRECT_PROMPT_TRAINING_HTML_FILENAME,
    REQUIRED_TERM_DIRECT_PROMPT_TRAINING_JSON_FILENAME,
    REQUIRED_TERM_DIRECT_PROMPT_TRAINING_MARKDOWN_FILENAME,
    REQUIRED_TERM_DIRECT_PROMPT_TRAINING_TEXT_FILENAME,
)
from minigpt.report_utils import as_dict, csv_cell, html_escape, list_of_dicts, markdown_cell, write_json_payload


def render_model_capability_required_term_direct_prompt_training_text(report: dict[str, Any]) -> str:
...
def write_model_capability_required_term_direct_prompt_training_csv(report: dict[str, Any], path: str | Path) -> None:
    fieldnames = [
        "generation_seed",
        "case",
        "term",
        "scaffold_prompt",
        "direct_prompt_line_count",
        "direct_prompt_pattern_count",
        "direct_prompt_repeat",
        "prompt_hit_count",
        "generated_hit_count",
...
def render_model_capability_required_term_direct_prompt_training_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    ...
    return "\n".join(
        [
            ...
            f"- Direct prompt training decision: `{summary.get('direct_prompt_training_decision')}`",
            ...
            f"- Direct prompt lines: `{summary.get('direct_prompt_line_count')}`",
            f"- Source corpus: `{corpus.get('path')}`",
...
def render_model_capability_required_term_direct_prompt_training_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    corpus = as_dict(report.get("corpus"))
    training = as_dict(report.get("training"))
    stats = [
        ("Status", report.get("status")),
        ("Decision", summary.get("direct_prompt_training_decision")),
        ...
        ("Direct prompt ready", summary.get("direct_prompt_ready")),
...
<title>MiniGPT required-term direct prompt training</title>
{_style()}
</head>
<body>
<main>
<header><h1>MiniGPT required-term direct prompt training</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
...
<section class="panel"><h2>Direct Prompt Boundary</h2><p>Direct prompt lines: {html_escape(summary.get('direct_prompt_line_count'))}; previous continuation hits: {html_escape(summary.get('previous_continuation_hit_count'))}; current continuation hits: {html_escape(summary.get('continuation_hit_count'))}; delta: {html_escape(summary.get('continuation_hit_delta'))}.</p><p>{html_escape(interpretation.get('next_action'))}</p></section>
<section class="panel">
...
def write_model_capability_required_term_direct_prompt_training_outputs(
    report: dict[str, Any],
    out_dir: str | Path,
) -> dict[str, str]:
    root = Path(out_dir)
    ...
    paths = {
        "json": root / REQUIRED_TERM_DIRECT_PROMPT_TRAINING_JSON_FILENAME,
        "csv": root / "model_capability_required_term_direct_prompt_training.csv",
        "text": root / REQUIRED_TERM_DIRECT_PROMPT_TRAINING_TEXT_FILENAME,
        "markdown": root / REQUIRED_TERM_DIRECT_PROMPT_TRAINING_MARKDOWN_FILENAME,
        "html": root / REQUIRED_TERM_DIRECT_PROMPT_TRAINING_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_model_capability_required_term_direct_prompt_training_csv(report, paths["csv"])
    paths["text"].write_text(render_model_capability_required_term_direct_prompt_training_text(report), encoding="utf-8")
    paths["markdown"].write_text(
        render_model_capability_required_term_direct_prompt_training_markdown(report),
        encoding="utf-8",
    )
    paths["html"].write_text(render_model_capability_required_term_direct_prompt_training_html(report), encoding="utf-8")
...
def _style() -> str:
    return """<style>
:root { color-scheme: light; font-family: "Segoe UI", Arial, sans-serif; background: #f3f6f3; color: #172221; }
body { margin: 0; padding: 28px; }
main { max-width: 1180px; margin: 0 auto; }
header { border-bottom: 1px solid #d9e2da; padding-bottom: 16px; margin-bottom: 18px; }
h1 { font-size: 30px; margin: 0 0 8px; letter-spacing: 0; }
h2 { font-size: 18px; margin: 0 0 12px; letter-spacing: 0; }
p { color: #596762; line-height: 1.55; overflow-wrap: anywhere; }
.stats { display: grid; grid-template-columns: repeat(auto-fit, minmax(155px, 1fr)); gap: 10px; margin-bottom: 18px; }
.card, .panel { background: #fff; border: 1px solid #d9e2da; border-radius: 8px; padding: 14px; box-shadow: 0 1px 2px rgba(23, 34, 33, 0.05); }
.card span { display: block; color: #66736c; font-size: 12px; }
.card strong { display: block; margin-top: 6px; font-size: 16px; overflow-wrap: anywhere; }
.panel { margin-bottom: 18px; }
.table-wrap { overflow-x: auto; }
table { width: 100%; border-collapse: collapse; min-width: 900px; }
th, td { text-align: left; border-bottom: 1px solid #e3ebe4; padding: 10px; vertical-align: top; }
th { color: #52605b; font-size: 12px; text-transform: uppercase; }
