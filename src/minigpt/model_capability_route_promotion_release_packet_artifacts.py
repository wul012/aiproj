...
def render_model_capability_route_promotion_release_packet_text(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    rows = [
        ("status", report.get("status")),
        ("decision", report.get("decision")),
        ("failed_count", report.get("failed_count")),
        ("release_packet_ready", summary.get("release_packet_ready")),
...
def write_model_capability_route_promotion_release_packet_csv(report: dict[str, Any], path: str | Path) -> None:
...
def render_model_capability_route_promotion_release_packet_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT model capability route promotion release packet'))}",
        "",
        f"- Status: `{report.get('status')}`",
        f"- Decision: `{report.get('decision')}`",
        f"- Release packet ready: `{summary.get('release_packet_ready')}`",
...
def render_model_capability_route_promotion_release_packet_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ("Status", report.get("status")),
        ("Decision", report.get("decision")),
        ("Packet ready", summary.get("release_packet_ready")),
...
<title>{html_escape(report.get('title', 'MiniGPT model capability route promotion release packet'))}</title>
{_style()}
</head>
<body>
<main>
<header><h1>{html_escape(report.get('title', 'MiniGPT model capability route promotion release packet'))}</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
...
def write_model_capability_route_promotion_release_packet_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / MODEL_CAPABILITY_ROUTE_PROMOTION_RELEASE_PACKET_JSON_FILENAME,
        "csv": root / MODEL_CAPABILITY_ROUTE_PROMOTION_RELEASE_PACKET_CSV_FILENAME,
        "text": root / MODEL_CAPABILITY_ROUTE_PROMOTION_RELEASE_PACKET_TEXT_FILENAME,
        "markdown": root / MODEL_CAPABILITY_ROUTE_PROMOTION_RELEASE_PACKET_MARKDOWN_FILENAME,
        "html": root / MODEL_CAPABILITY_ROUTE_PROMOTION_RELEASE_PACKET_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_model_capability_route_promotion_release_packet_csv(report, paths["csv"])
    paths["text"].write_text(render_model_capability_route_promotion_release_packet_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_model_capability_route_promotion_release_packet_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_model_capability_route_promotion_release_packet_html(report), encoding="utf-8")
