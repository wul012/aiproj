from __future__ import annotations

import csv
from pathlib import Path
from typing import Any

from minigpt.randomized_target_hidden_holdout_replay_review import (
    RANDOMIZED_TARGET_HIDDEN_HOLDOUT_REPLAY_REVIEW_CSV_FILENAME,
    RANDOMIZED_TARGET_HIDDEN_HOLDOUT_REPLAY_REVIEW_HTML_FILENAME,
    RANDOMIZED_TARGET_HIDDEN_HOLDOUT_REPLAY_REVIEW_JSON_FILENAME,
    RANDOMIZED_TARGET_HIDDEN_HOLDOUT_REPLAY_REVIEW_MARKDOWN_FILENAME,
    RANDOMIZED_TARGET_HIDDEN_HOLDOUT_REPLAY_REVIEW_TEXT_FILENAME,
)
from minigpt.report_utils import as_dict, csv_cell, html_escape, list_of_dicts, markdown_cell, write_json_payload


def render_randomized_target_hidden_holdout_replay_review_text(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    rows = [
        ("status", report.get("status")),
        ("decision", report.get("decision")),
        ("failed_count", report.get("failed_count")),
        ("randomized_target_hidden_holdout_replay_review_ready", summary.get("randomized_target_hidden_holdout_replay_review_ready")),
        ("source_randomized_holdout_model_quality_ready", summary.get("source_randomized_holdout_model_quality_ready")),
        ("case_count", summary.get("case_count")),
        ("passed_case_count", summary.get("passed_case_count")),
        ("pass_rate", summary.get("pass_rate")),
        ("source_random_seed", summary.get("source_random_seed")),
        ("source_randomized_case_factor", summary.get("source_randomized_case_factor")),
        ("target_leakage_case_count", summary.get("target_leakage_case_count")),
        ("task_hint_case_count", summary.get("task_hint_case_count")),
        ("unique_prompt_count", summary.get("unique_prompt_count")),
        ("randomized_prompt_count", summary.get("randomized_prompt_count")),
        ("clean_randomized_case_count", summary.get("clean_randomized_case_count")),
        ("approved_for_candidate_promotion_packet", summary.get("approved_for_candidate_promotion_packet")),
        ("approved_for_promotion", summary.get("approved_for_promotion")),
        ("promotion_ready", summary.get("promotion_ready")),
        ("model_quality_claim", summary.get("model_quality_claim")),
        ("next_step", summary.get("next_step")),
    ]
    return "\n".join(f"{key}={value}" for key, value in rows) + "\n"


def write_randomized_target_hidden_holdout_replay_review_csv(report: dict[str, Any], path: str | Path) -> None:
    fieldnames = ["case_id", "source_case_id", "draw", "target_leakage", "task_hint", "unique_prompt", "randomized_prompt", "review_status", "detail"]
    out_path = Path(path)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    with out_path.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=fieldnames)
        writer.writeheader()
        for row in list_of_dicts(report.get("review_rows")):
            writer.writerow(
                {
                    "case_id": csv_cell(row.get("case_id")),
                    "source_case_id": csv_cell(row.get("source_case_id")),
                    "draw": csv_cell(row.get("random_draw_index")),
                    "target_leakage": csv_cell(row.get("target_leakage")),
                    "task_hint": csv_cell(row.get("task_hint")),
                    "unique_prompt": csv_cell(row.get("unique_prompt")),
                    "randomized_prompt": csv_cell(row.get("randomized_prompt")),
                    "review_status": csv_cell(row.get("review_status")),
                    "detail": csv_cell(row.get("detail")),
                }
            )


def render_randomized_target_hidden_holdout_replay_review_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT randomized target-hidden holdout replay review'))}",
        "",
        f"- Status: `{report.get('status')}`",
        f"- Decision: `{report.get('decision')}`",
        f"- Randomized model quality ready: `{summary.get('source_randomized_holdout_model_quality_ready')}`",
        f"- Target leakage cases: `{summary.get('target_leakage_case_count')}`",
        f"- Task hint cases: `{summary.get('task_hint_case_count')}`",
        f"- Unique prompts: `{summary.get('unique_prompt_count')}`",
        f"- Clean randomized cases: `{summary.get('clean_randomized_case_count')}`",
        f"- Candidate packet: `{summary.get('approved_for_candidate_promotion_packet')}`",
        f"- Promotion: `{summary.get('approved_for_promotion')}`",
        f"- Next step: `{summary.get('next_step')}`",
        "",
        "## Review Rows",
        "",
        "| Case | Source | Draw | Leakage | Hint | Unique | Randomized | Status | Detail |",
        "| --- | --- | --- | --- | --- | --- | --- | --- | --- |",
    ]
    for row in list_of_dicts(report.get("review_rows")):
        lines.append(
            "| "
            + " | ".join(
                [
                    markdown_cell(row.get("case_id")),
                    markdown_cell(row.get("source_case_id")),
                    markdown_cell(row.get("random_draw_index")),
                    markdown_cell(row.get("target_leakage")),
                    markdown_cell(row.get("task_hint")),
                    markdown_cell(row.get("unique_prompt")),
                    markdown_cell(row.get("randomized_prompt")),
                    markdown_cell(row.get("review_status")),
                    markdown_cell(row.get("detail")),
                ]
            )
            + " |"
        )
    lines.extend(["", "## Checks", "", "| Check | Status | Actual | Detail |", "| --- | --- | --- | --- |"])
    for row in list_of_dicts(report.get("check_rows")):
        lines.append("| " + " | ".join([markdown_cell(row.get("id")), markdown_cell(row.get("status")), markdown_cell(row.get("actual")), markdown_cell(row.get("detail"))]) + " |")
    return "\n".join(lines).rstrip() + "\n"


def render_randomized_target_hidden_holdout_replay_review_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ("Status", report.get("status")),
        ("Quality ready", summary.get("source_randomized_holdout_model_quality_ready")),
        ("Leakage", summary.get("target_leakage_case_count")),
        ("Hints", summary.get("task_hint_case_count")),
        ("Clean", summary.get("clean_randomized_case_count")),
        ("Candidate packet", summary.get("approved_for_candidate_promotion_packet")),
        ("Promotion", summary.get("approved_for_promotion")),
        ("Next", summary.get("next_step")),
    ]
    rows = "".join(_row(item) for item in list_of_dicts(report.get("review_rows")))
    checks = "".join(_check_row(item) for item in list_of_dicts(report.get("check_rows")))
    return f"""<!doctype html>
<html lang="zh-CN">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><link rel="icon" href="data:,">
<title>{html_escape(report.get('title', 'MiniGPT randomized target-hidden holdout replay review'))}</title>{_style()}</head>
<body><main><header><h1>{html_escape(report.get('title', 'MiniGPT randomized target-hidden holdout replay review'))}</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
<section class="stats">{''.join(_card(label, value) for label, value in stats)}</section>
<section class="panel"><h2>Review Rows</h2><div class="table-wrap"><table><thead><tr><th>Case</th><th>Source</th><th>Draw</th><th>Leakage</th><th>Hint</th><th>Unique</th><th>Randomized</th><th>Status</th><th>Detail</th></tr></thead><tbody>{rows}</tbody></table></div></section>
<section class="panel"><h2>Checks</h2><div class="table-wrap"><table><thead><tr><th>Check</th><th>Status</th><th>Actual</th><th>Detail</th></tr></thead><tbody>{checks}</tbody></table></div></section>
</main></body></html>
"""


def write_randomized_target_hidden_holdout_replay_review_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / RANDOMIZED_TARGET_HIDDEN_HOLDOUT_REPLAY_REVIEW_JSON_FILENAME,
        "csv": root / RANDOMIZED_TARGET_HIDDEN_HOLDOUT_REPLAY_REVIEW_CSV_FILENAME,
        "text": root / RANDOMIZED_TARGET_HIDDEN_HOLDOUT_REPLAY_REVIEW_TEXT_FILENAME,
        "markdown": root / RANDOMIZED_TARGET_HIDDEN_HOLDOUT_REPLAY_REVIEW_MARKDOWN_FILENAME,
        "html": root / RANDOMIZED_TARGET_HIDDEN_HOLDOUT_REPLAY_REVIEW_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_randomized_target_hidden_holdout_replay_review_csv(report, paths["csv"])
    paths["text"].write_text(render_randomized_target_hidden_holdout_replay_review_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_randomized_target_hidden_holdout_replay_review_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_randomized_target_hidden_holdout_replay_review_html(report), encoding="utf-8")
    return {key: str(value) for key, value in paths.items()}


def _row(row: dict[str, Any]) -> str:
    return "<tr>" + "".join(
        f"<td>{html_escape(value)}</td>"
        for value in [
            row.get("case_id"),
            row.get("source_case_id"),
            row.get("random_draw_index"),
            row.get("target_leakage"),
            row.get("task_hint"),
            row.get("unique_prompt"),
            row.get("randomized_prompt"),
            row.get("review_status"),
            row.get("detail"),
        ]
    ) + "</tr>"


def _check_row(row: dict[str, Any]) -> str:
    return "<tr>" + "".join(f"<td>{html_escape(row.get(key))}</td>" for key in ["id", "status", "actual", "detail"]) + "</tr>"


def _card(label: str, value: Any) -> str:
    return f"<div class=\"card\"><span>{html_escape(label)}</span><strong>{html_escape(value)}</strong></div>"


def _style() -> str:
    return """<style>:root{color-scheme:light;--ink:#17212b;--muted:#62717d;--line:#d7dee5;--panel:#f8fafc;--accent:#0f766e}*{box-sizing:border-box}body{margin:0;background:#f2f4f7;color:var(--ink);font-family:Arial,"Microsoft YaHei",sans-serif}main{max-width:1240px;margin:0 auto;padding:28px}h1{font-size:30px;margin:0 0 8px;letter-spacing:0}h2{font-size:18px;margin:0 0 12px;letter-spacing:0}p{color:var(--muted);line-height:1.55;overflow-wrap:anywhere}.stats{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px;margin:18px 0}.card,.panel{background:white;border:1px solid var(--line);border-radius:8px}.card{padding:14px}.card span{display:block;color:var(--muted);font-size:12px;text-transform:uppercase}.card strong{display:block;margin-top:6px;font-size:18px;line-height:1.2;color:var(--accent);overflow-wrap:anywhere}.panel{padding:16px;margin:14px 0}.table-wrap{overflow:auto}table{width:100%;border-collapse:collapse;font-size:13px}th,td{border-bottom:1px solid var(--line);padding:9px;text-align:left;vertical-align:top}th{background:var(--panel);color:#334155}td{overflow-wrap:anywhere}</style>"""


__all__ = [
    "render_randomized_target_hidden_holdout_replay_review_html",
    "render_randomized_target_hidden_holdout_replay_review_markdown",
    "render_randomized_target_hidden_holdout_replay_review_text",
    "write_randomized_target_hidden_holdout_replay_review_outputs",
]
