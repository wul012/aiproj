...
def render_exact_surface_repair_plan_text(report: dict[str, Any]) -> str:
    ...
    rows = [
        ...
        ("repair_row_budget", summary.get("repair_row_budget")),
        ("missed_required_surfaces", summary.get("missed_required_surfaces")),
        ("observed_pair_full_surfaces", summary.get("observed_pair_full_surfaces")),
...
def render_exact_surface_repair_plan_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- Repair row budget: `{plan.get('repair_row_budget')}`",
            "",
            "## Checks",
            "",
            *checks,
...
def render_exact_surface_repair_plan_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    plan = as_dict(report.get("plan"))
    stats = [
        ...
        ("Row budget", summary.get("repair_row_budget")),
...
<title>MiniGPT exact-surface repair plan</title>
{_style()}
</head>
<body>
<main>
<header><h1>MiniGPT exact-surface repair plan</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
...
def write_exact_surface_repair_plan_csv(report: dict[str, Any], path: str | Path) -> None:
    out_path = Path(path)
...
def write_exact_surface_repair_plan_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / PAIR_READINESS_EXACT_SURFACE_REPAIR_PLAN_JSON_FILENAME,
        "csv": root / PAIR_READINESS_EXACT_SURFACE_REPAIR_PLAN_CSV_FILENAME,
        "text": root / PAIR_READINESS_EXACT_SURFACE_REPAIR_PLAN_TEXT_FILENAME,
        "markdown": root / PAIR_READINESS_EXACT_SURFACE_REPAIR_PLAN_MARKDOWN_FILENAME,
        "html": root / PAIR_READINESS_EXACT_SURFACE_REPAIR_PLAN_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_exact_surface_repair_plan_csv(report, paths["csv"])
    paths["text"].write_text(render_exact_surface_repair_plan_text(report), encoding="utf-8")
