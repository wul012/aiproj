...
def render_pub_receipt_artifacts_text(report: dict[str, Any]) -> str:
...
def write_pub_receipt_artifacts_csv(report: dict[str, Any], path: str | Path) -> None:
...
def render_pub_receipt_artifacts_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    lines = [
...
def render_pub_receipt_artifacts_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
...
def write_pub_receipt_artifacts_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / ACK_PUB_RECEIPT_JSON_FILENAME,
        "csv": root / ACK_PUB_RECEIPT_CSV_FILENAME,
        "text": root / ACK_PUB_RECEIPT_TEXT_FILENAME,
        "markdown": root / ACK_PUB_RECEIPT_MARKDOWN_FILENAME,
        "html": root / ACK_PUB_RECEIPT_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_pub_receipt_artifacts_csv(report, paths["csv"])
    paths["text"].write_text(render_pub_receipt_artifacts_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_pub_receipt_artifacts_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_pub_receipt_artifacts_html(report), encoding="utf-8")
