...
def read_json_report(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("randomized holdout publication index receipt check v996 input must be a JSON object")
    return dict(payload)
...
def build_randomized_holdout_publication_receipt_packet_index_publication_receipt_index_receipt_index_publication_index_receipt_check_v996(
    ...
    *,
    ...
    title: str = "MiniGPT randomized holdout publication receipt packet index publication receipt index receipt index publication index receipt check v996",
    ...
    return {
        ...
        "source_publication_index_review": str(source_review or ""),
...
def _checks(
    ...
    checks = [
        _check("source_publication_index_review_exists", source_review is not None and source_review.exists(), str(source_review or ""), "source publication index review must exist"),
...
def _resolve_source_review_path(report: dict[str, Any], receipt: dict[str, Any], receipt_path: str | Path | None) -> Path | None:
    raw = report.get("publication_index_review_path") or receipt.get("publication_index_review_path")
    if not raw:
        return None
    source = Path(str(raw))
...
def _summary(
    ...
    return {
        ...
        "next_step": RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_PACKET_INDEX_PUBLICATION_RECEIPT_INDEX_RECEIPT_INDEX_PUBLICATION_INDEX_RECEIPT_CHECK_V996_NEXT_STEP if status == "pass" else "repair_randomized_holdout_publication_receipt_packet_index_publication_receipt_index_receipt_index_publication_index_receipt_check_v996",
...
def _decision(status: str) -> str:
    if status == "pass":
        return "randomized_holdout_publication_receipt_packet_index_publication_receipt_index_receipt_index_publication_index_receipt_contract_check_v996_passed"
    return "fix_randomized_holdout_publication_receipt_packet_index_publication_receipt_index_receipt_index_publication_index_receipt_check_v996"


def _interpretation(status: str) -> dict[str, str]:
    if status != "pass":
        return {"model_quality_claim": "not_claimed", "reason": "The publication index receipt does not rebuild from its source review.", "next_action": "repair or regenerate publication index receipt"}
    return {
        "model_quality_claim": "bounded_randomized_target_hidden_holdout_claim_only",
        "reason": "The publication index receipt rebuilds from the source review and remains lookup-only.",
        "next_action": RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_PACKET_INDEX_PUBLICATION_RECEIPT_INDEX_RECEIPT_INDEX_PUBLICATION_INDEX_RECEIPT_CHECK_V996_NEXT_STEP,
    }
