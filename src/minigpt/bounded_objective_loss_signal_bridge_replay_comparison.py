...
LOSS_SIGNAL_BRIDGE_REPLAY_COMPARISON_CSV_FILENAME = "bounded_objective_loss_signal_bridge_replay_comparison.csv"
...
def locate_loss_signal_bridge_training_run(path: str | Path) -> Path:
    source = Path(path)
    if source.is_dir():
        source = source / LOSS_SIGNAL_BRIDGE_TRAINING_RUN_JSON_FILENAME
    return source
...
def read_json_report(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("bounded objective loss signal bridge replay comparison input must be a JSON object")
    return dict(payload)


def build_bounded_objective_loss_signal_bridge_replay_comparison(
    objective_contract_report: dict[str, Any],
    loss_signal_bridge_training_run_report: dict[str, Any],
    *,
    checkpoint_path: str | Path | None = None,
    tokenizer_path: str | Path | None = None,
    device: str = "auto",
    objective_contract_path: str | Path | None = None,
    loss_signal_bridge_training_run_path: str | Path | None = None,
    generator_runner: GeneratorRunner | None = None,
    title: str = "MiniGPT bounded objective loss signal bridge replay comparison",
    generated_at: str | None = None,
) -> dict[str, Any]:
    report = build_model_capability_route_promotion_bounded_objective_replay_comparison(
        objective_contract_report,
        _adapt_training_run(loss_signal_bridge_training_run_report),
        checkpoint_path=checkpoint_path,
        tokenizer_path=tokenizer_path,
        device=device,
        objective_contract_path=objective_contract_path,
        objective_training_run_path=loss_signal_bridge_training_run_path,
        generator_runner=generator_runner,
        title=title,
        generated_at=generated_at,
    )
    return _adapt_replay_report(report, loss_signal_bridge_training_run_report, loss_signal_bridge_training_run_path)
...
def _adapt_training_run(training_report: dict[str, Any]) -> dict[str, Any]:
    ...
    summary["bounded_objective_training_ready"] = summary.get("bounded_objective_loss_signal_bridge_training_ready")
...
def _adapt_replay_report(report: dict[str, Any], training_report: dict[str, Any], source_path: str | Path | None) -> dict[str, Any]:
    adapted = dict(report)
    adapted["source_loss_signal_bridge_training_run"] = str(source_path or "")
    adapted["loss_signal_bridge_training_summary"] = as_dict(training_report.get("summary"))
...
def _comparison(base: dict[str, Any]) -> dict[str, Any]:
    ...
    comparison["route"] = "bounded_objective_loss_signal_bridge"
    comparison["decoder_anchor_used"] = False
    comparison["training_source"] = "bounded_objective_loss_signal_bridge_training_run"
...
def _summary(base: dict[str, Any], comparison: dict[str, Any]) -> dict[str, Any]:
    summary = dict(base)
    summary["bounded_objective_loss_signal_bridge_replay_comparison_ready"] = summary.get(
        "bounded_objective_replay_comparison_ready", False
    )
...
def _decision(base_decision: str) -> str:
    mapping = {
        "fix_model_capability_route_promotion_bounded_objective_replay_comparison": "fix_bounded_objective_loss_signal_bridge_replay_comparison",
        "model_capability_route_promotion_bounded_objective_replay_contract_recovered_holdout_required": "bounded_objective_loss_signal_bridge_replay_contract_recovered_holdout_required",
        "model_capability_route_promotion_bounded_objective_replay_partial_required_term_hit": "bounded_objective_loss_signal_bridge_replay_partial_required_term_hit",
        "model_capability_route_promotion_bounded_objective_replay_zero_hit": "bounded_objective_loss_signal_bridge_replay_zero_hit",
    }
...
def _interpretation(base: dict[str, Any], comparison: dict[str, Any]) -> dict[str, Any]:
    ...
    interpretation["model_quality_claim"] = str(base.get("model_quality_claim") or "not_claimed")
    if comparison.get("objective_contract_recovered") is True:
        ...
        interpretation["reason"] = "The loss-signal bridge checkpoint passed the objective contract, but unchanged holdout replay remains required."
    elif int(comparison.get("any_hit_case_count") or 0) > 0:
        interpretation["next_action"] = "diagnose_bounded_objective_loss_signal_bridge_partial_hit_before_more_training"
        interpretation["reason"] = "The loss-signal bridge checkpoint produced partial required-term signal without recovering the full contract."
    else:
        interpretation["next_action"] = "diagnose_bounded_objective_loss_signal_bridge_zero_hit_before_more_training"
        interpretation["reason"] = "The loss-signal bridge checkpoint produced zero required-term hits on the objective contract."
...
def _next_step(recovered: bool, any_hit_count: int) -> str:
    ...
    if any_hit_count > 0:
        return "diagnose_bounded_objective_loss_signal_bridge_partial_hit_before_more_training"
    return "diagnose_bounded_objective_loss_signal_bridge_zero_hit_before_more_training"


__all__ = [
    "LOSS_SIGNAL_BRIDGE_REPLAY_COMPARISON_CSV_FILENAME",
    "LOSS_SIGNAL_BRIDGE_REPLAY_COMPARISON_HTML_FILENAME",
    "LOSS_SIGNAL_BRIDGE_REPLAY_COMPARISON_JSON_FILENAME",
    "LOSS_SIGNAL_BRIDGE_REPLAY_COMPARISON_MARKDOWN_FILENAME",
    "LOSS_SIGNAL_BRIDGE_REPLAY_COMPARISON_TEXT_FILENAME",
