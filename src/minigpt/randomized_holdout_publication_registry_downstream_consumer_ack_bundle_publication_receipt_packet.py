...
from minigpt.randomized_holdout_publication_registry_downstream_consumer_ack_bundle_publication_receipt_review import (
    RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_ACK_BUNDLE_PUBLICATION_RECEIPT_REVIEW_JSON_FILENAME,
)
...
RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_ACK_BUNDLE_PUBLICATION_RECEIPT_PACKET_CSV_FILENAME = "randomized_holdout_publication_registry_downstream_consumer_ack_bundle_publication_receipt_packet.csv"
RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_ACK_BUNDLE_PUBLICATION_RECEIPT_PACKET_TEXT_FILENAME = "randomized_holdout_publication_registry_downstream_consumer_ack_bundle_publication_receipt_packet.txt"
RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_ACK_BUNDLE_PUBLICATION_RECEIPT_PACKET_MARKDOWN_FILENAME = "randomized_holdout_publication_registry_downstream_consumer_ack_bundle_publication_receipt_packet.md"
RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_ACK_BUNDLE_PUBLICATION_RECEIPT_PACKET_HTML_FILENAME = "randomized_holdout_publication_registry_downstream_consumer_ack_bundle_publication_receipt_packet.html"

PACKET_ID = "randomized-holdout-publication-registry-downstream-consumer-ack-bundle-publication-receipt-packet-v955"


def locate_randomized_holdout_publication_registry_downstream_consumer_ack_bundle_publication_receipt_review(path: str | Path) -> Path:
    source = Path(path)
    if source.is_dir():
        source = source / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_ACK_BUNDLE_PUBLICATION_RECEIPT_REVIEW_JSON_FILENAME
    return source
...
def read_json_report(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("randomized holdout publication registry downstream consumer ack bundle publication receipt packet input must be a JSON object")
    return dict(payload)


def build_randomized_holdout_publication_registry_downstream_consumer_ack_bundle_publication_receipt_packet(
    receipt_review_report: dict[str, Any],
    *,
    receipt_review_path: str | Path | None = None,
    title: str = "MiniGPT randomized holdout publication registry downstream consumer ack bundle publication receipt packet",
    ...
    publication_rows = list_of_dicts(receipt_review_report.get("publication_rows"))
    source_evidence_rows = list_of_dicts(receipt_review_report.get("source_evidence_rows"))
    checks = _checks(receipt_review_report, review_summary, review, consumer_receipts, publication_rows, source_evidence_rows, receipt_review_path)
    issues = [row for row in checks if row["status"] != "pass"]
    status = "pass" if not issues else "fail"
    packet = _packet(status, review_summary, review, consumer_receipts, publication_rows, source_evidence_rows, receipt_review_path)
    return {
        "schema_version": 1,
        "title": title,
...
def resolve_exit_code(
    ...
    *,
    ...
    if require_packet_ready and summary.get("randomized_holdout_publication_registry_downstream_consumer_ack_bundle_publication_receipt_packet_ready") is not True:
        return 1
...
def _checks(
    ...
    return [
        ...
        _check("receipt_review_decision_ready", review_report.get("decision") == "randomized_holdout_publication_registry_downstream_consumer_ack_bundle_publication_receipt_review_ready", review_report.get("decision"), "receipt review decision must be ready"),
        _check("review_summary_ready", review_summary.get("randomized_holdout_publication_registry_downstream_consumer_ack_bundle_publication_receipt_review_ready") is True and review.get("review_ready") is True, {"summary": review_summary.get("randomized_holdout_publication_registry_downstream_consumer_ack_bundle_publication_receipt_review_ready"), "review": review.get("review_ready")}, "receipt review summary and body must be ready"),
        _check("review_status_packet_ready", review_summary.get("review_status") == "approved_for_downstream_publication_receipt_packet" and review.get("review_status") == "approved_for_downstream_publication_receipt_packet", {"summary": review_summary.get("review_status"), "review": review.get("review_status")}, "review must approve receipt packet construction"),
        ...
        _check("receipt_status_ready", review_summary.get("receipt_status") == "downstream_publication_lookup_receipted" and review.get("receipt_status") == "downstream_publication_lookup_receipted", {"summary": review_summary.get("receipt_status"), "review": review.get("receipt_status")}, "source receipt must stay lookup receipted"),
        ...
        _check("publication_rows_present", len(publication_rows) == int(review_summary.get("publication_row_count") or 0) == 1, {"rows": len(publication_rows), "summary": review_summary.get("publication_row_count")}, "packet must include one publication row"),
        _check("source_evidence_rows_present", len(source_evidence_rows) == int(review_summary.get("source_evidence_count") or 0) == 2, {"rows": len(source_evidence_rows), "summary": review_summary.get("source_evidence_count")}, "packet must include two source evidence rows"),
        ...
        _check("source_next_step_matches", review_summary.get("next_step") == RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_ACK_BUNDLE_PUBLICATION_RECEIPT_REVIEW_NEXT_STEP, review_summary.get("next_step"), "source review must route to receipt packet"),
    ]
...
def _packet(
    ...
    return {
        ...
        "packet_status": "downstream_publication_receipt_packet_ready" if ready else "blocked",
...
def _summary(status: str, checks: list[dict[str, Any]], packet: dict[str, Any]) -> dict[str, Any]:
    return {
        "randomized_holdout_publication_registry_downstream_consumer_ack_bundle_publication_receipt_packet_ready": status == "pass" and packet.get("packet_ready") is True,
        "packet_id": packet.get("packet_id"),
        "packet_status": packet.get("packet_status"),
        "consumer_name": packet.get("consumer_name"),
        ...
        "granted_use": packet.get("granted_use"),
        "blocked_uses": packet.get("blocked_uses"),
        "publication_row_count": packet.get("publication_row_count"),
        "source_evidence_count": packet.get("source_evidence_count"),
...
def _decision(status: str) -> str:
    if status == "pass":
        return "randomized_holdout_publication_registry_downstream_consumer_ack_bundle_publication_receipt_packet_ready"
    return "fix_randomized_holdout_publication_registry_downstream_consumer_ack_bundle_publication_receipt_packet"
...
def _interpretation(status: str, packet: dict[str, Any]) -> dict[str, str]:
    ...
    return {
        ...
        "reason": "The downstream publication receipt packet is ready for contract checking while promotion remains blocked.",
        "next_action": str(packet.get("next_step")),
    }


__all__ = [
    "RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_ACK_BUNDLE_PUBLICATION_RECEIPT_PACKET_CSV_FILENAME",
    "RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_ACK_BUNDLE_PUBLICATION_RECEIPT_PACKET_HTML_FILENAME",
    "RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_ACK_BUNDLE_PUBLICATION_RECEIPT_PACKET_JSON_FILENAME",
    "RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_ACK_BUNDLE_PUBLICATION_RECEIPT_PACKET_MARKDOWN_FILENAME",
    "RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_ACK_BUNDLE_PUBLICATION_RECEIPT_PACKET_TEXT_FILENAME",
