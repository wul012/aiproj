...
def render_surface_policy_selector_text(report: dict[str, Any]) -> str:
    ...
    rows = [
        ...
        ("selected_leakage_level", summary.get("selected_leakage_level")),
...
def write_surface_policy_selector_csv(report: dict[str, Any], path: str | Path) -> None:
    fieldnames = [
        "policy_id",
        "stable_pair_full",
        "leakage_level",
...
def render_surface_policy_selector_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    rows = [
        ...
        rows.append(
            ...
            + " | ".join(
                [
                    ...
                    markdown_cell(row.get("uses_boundary_sentence")),
                    markdown_cell(row.get("selection_score")),
                    markdown_cell(row.get("selection_reason")),
                ]
            )
            + " |"
        )
...
def render_surface_policy_selector_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ("Status", report.get("status")),
        ("Decision", report.get("decision")),
        ("Stable candidates", summary.get("stable_candidate_count")),
        ("Selected", summary.get("selected_policy_id")),
        ("Leakage", summary.get("selected_leakage_level")),
        ("Promotion ready", summary.get("promotion_ready")),
    ]
    candidate_rows = "\n".join(_candidate_html(row) for row in list_of_dicts(report.get("candidate_rows")))
...
<title>MiniGPT surface policy selector</title>
{_style()}
</head>
<body>
<main>
<header><h1>MiniGPT surface policy selector</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
<section class="stats">{''.join(_card(label, value) for label, value in stats)}</section>
<section class="panel"><h2>Next Action</h2><p>{html_escape(interpretation.get('next_action'))}</p></section>
<section class="panel">
<h2>Candidates</h2>
<div class="table-wrap"><table>
<thead><tr><th>Policy</th><th>Stable</th><th>Leakage</th><th>Boundary</th><th>Score</th><th>Reason</th></tr></thead>
...
def write_surface_policy_selector_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / PAIR_SURFACE_POLICY_SELECTOR_JSON_FILENAME,
        "csv": root / PAIR_SURFACE_POLICY_SELECTOR_CSV_FILENAME,
        "text": root / PAIR_SURFACE_POLICY_SELECTOR_TEXT_FILENAME,
        "markdown": root / PAIR_SURFACE_POLICY_SELECTOR_MARKDOWN_FILENAME,
        "html": root / PAIR_SURFACE_POLICY_SELECTOR_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_surface_policy_selector_csv(report, paths["csv"])
    paths["text"].write_text(render_surface_policy_selector_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_surface_policy_selector_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_surface_policy_selector_html(report), encoding="utf-8")
...
def _candidate_html(row: dict[str, Any]) -> str:
    return (
        ...
        f"<td>{html_escape(row.get('uses_boundary_sentence'))}</td>"
