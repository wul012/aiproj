...
from minigpt.promoted_training_scale_comparison_csv_rows import (
...
def write_promoted_training_scale_comparison_csv(report: dict[str, Any], path: str | Path) -> None:
    ...
    with out_path.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=promoted_comparison_csv_fieldnames())
        writer.writeheader()
        writer.writerows(promoted_comparison_csv_rows(report))
...
def render_promoted_training_scale_comparison_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- Comparison-ready selected batch reviews: `{summary.get('comparison_ready_handoff_selected_batch_review_count')}`",
        f"- Comparison-ready selected batch blockers: `{summary.get('comparison_ready_handoff_selected_batch_blocker_count')}`",
        f"- Comparison-ready batch review actions: `{summary.get('comparison_ready_handoff_selected_batch_comparison_review_action_total')}`",
        f"- Comparison-ready batch blocker actions: `{summary.get('comparison_ready_handoff_selected_batch_comparison_blocker_action_total')}`",
        "",
        "## Promoted Inputs",
        "",
        ...
        lines.append(
            ...
            + " | ".join(
                [
                    ...
                    _md(row.get("handoff_batch_maturity_suite_design_regression_count")),
                    _md(", ".join(_string_list(row.get("comparison_exclusion_reasons")))),
...
def render_promoted_training_scale_comparison_html(report: dict[str, Any]) -> str:
    ...
    stats = [
        ...
        ("Handoff require clean batch review", summary.get("handoff_require_clean_batch_review_count")),
        ("Handoff clean batch review", summary.get("handoff_clean_batch_review_count")),
        ("Handoff unclean batch review", summary.get("handoff_unclean_batch_review_count")),
        ...
        ("Handoff selected CI reasons", _fmt_mapping(summary.get("handoff_selected_batch_maturity_ci_regression_reason_counts"))),
        (
        ...
        ("Handoff selected suite-design regressions", summary.get("handoff_selected_batch_maturity_suite_design_regression_total")),
        (
        ...
        ("Ready clean batch review", summary.get("comparison_ready_handoff_clean_batch_review_count")),
        ("Ready unclean batch review", summary.get("comparison_ready_handoff_unclean_batch_review_count")),
        ("Ready CI regressions", summary.get("comparison_ready_handoff_batch_maturity_ci_regression_count")),
        ("Ready CI reasons", _fmt_mapping(summary.get("comparison_ready_handoff_batch_maturity_ci_regression_reason_counts"))),
        (
        ...
        ("Ready suite-design regressions", summary.get("comparison_ready_handoff_batch_maturity_suite_design_regression_count")),
        (
        ...
        (
            ...
            ", ".join(_string_list(summary.get("comparison_ready_handoff_selected_batch_maturity_suite_design_regression_names"))),
        ),
...
def _promotion_table(report: dict[str, Any]) -> str:
    ...
    for row in _list_of_dicts(report.get("promotions")):
        rows.append(
            ...
            f"<td>{_e(row.get('handoff_batch_maturity_suite_design_regression_count'))}</td>"
            f"<td>{_e(', '.join(_string_list(row.get('comparison_exclusion_reasons'))))}</td>"
    ...
    return (
        ...
        "<thead><tr><th>Name</th><th>Status</th><th>Compare</th><th>Readiness</th><th>Handoff Suite</th><th>Mismatches</th><th>Clean Required</th><th>Clean Status</th><th>CI Regressions</th><th>Suite-Design Regressions</th><th>Exclusion</th><th>Selected Suite</th><th>Batch Review</th><th>Batch Blockers</th><th>Run</th></tr></thead>"
