...
from minigpt.randomized_holdout_publication_downstream_common import blocked_uses, blocked_uses_complete, downstream_lookup_use, sha256_file
from minigpt.randomized_holdout_publication_receipt_packet_index_publication_receipt_index_review_v986 import (
...
RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_PACKET_INDEX_PUBLICATION_RECEIPT_INDEX_RECEIPT_V987_JSON_FILENAME = "randomized_holdout_publication_receipt_packet_index_publication_receipt_index_receipt_v987.json"
RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_PACKET_INDEX_PUBLICATION_RECEIPT_INDEX_RECEIPT_V987_CSV_FILENAME = "randomized_holdout_publication_receipt_packet_index_publication_receipt_index_receipt_v987.csv"
RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_PACKET_INDEX_PUBLICATION_RECEIPT_INDEX_RECEIPT_V987_TEXT_FILENAME = "randomized_holdout_publication_receipt_packet_index_publication_receipt_index_receipt_v987.txt"
RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_PACKET_INDEX_PUBLICATION_RECEIPT_INDEX_RECEIPT_V987_MARKDOWN_FILENAME = "randomized_holdout_publication_receipt_packet_index_publication_receipt_index_receipt_v987.md"
RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_PACKET_INDEX_PUBLICATION_RECEIPT_INDEX_RECEIPT_V987_HTML_FILENAME = "randomized_holdout_publication_receipt_packet_index_publication_receipt_index_receipt_v987.html"

RECEIPT_ID = "randomized-holdout-publication-receipt-packet-index-publication-receipt-index-receipt-v987"
RECEIPT_TYPE = "randomized_holdout_publication_receipt_packet_index_publication_receipt_index"


def locate_receipt_index_review_v987(path: str | Path) -> Path:
    source = Path(path)
    if source.is_dir():
        source = source / RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_PACKET_INDEX_PUBLICATION_RECEIPT_INDEX_REVIEW_V986_JSON_FILENAME
    return source
...
def read_json_report(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("randomized holdout publication receipt packet index publication receipt index receipt v987 input must be a JSON object")
    return dict(payload)


def build_randomized_holdout_publication_receipt_packet_index_publication_receipt_index_receipt_v987(
    review_report: dict[str, Any],
    *,
    receipt_index_review_path: str | Path | None = None,
    consumer_name: str = "publication_receipt_index_governance_lookup_reader",
    requested_use: str = "downstream_governance_lookup_only",
    title: str = "MiniGPT randomized holdout publication receipt packet index publication receipt index receipt v987",
...
def resolve_exit_code(report: dict[str, Any], *, require_receipt_ready: bool, require_promotion_ready: bool = False) -> int:
    ...
    if require_receipt_ready and summary.get("randomized_holdout_publication_receipt_packet_index_publication_receipt_index_receipt_v987_ready") is not True:
        return 1
...
def _checks(
    ...
    return [
        ...
        _check("receipt_index_review_decision_ready", review_report.get("decision") == "randomized_holdout_publication_receipt_packet_index_publication_receipt_index_review_v986_ready", review_report.get("decision"), "receipt index review decision must be ready"),
        _check("receipt_index_review_summary_ready", review_summary.get("randomized_holdout_publication_receipt_packet_index_publication_receipt_index_review_v986_ready") is True and review.get("review_ready") is True, {"summary": review_summary.get("randomized_holdout_publication_receipt_packet_index_publication_receipt_index_review_v986_ready"), "review": review.get("review_ready")}, "receipt index review summary and body must be ready"),
        _check("review_status_allowed", review_summary.get("review_status") == "approved_for_publication_receipt_index_receipt_lookup_only" and review.get("review_status") == "approved_for_publication_receipt_index_receipt_lookup_only", {"summary": review_summary.get("review_status"), "review": review.get("review_status")}, "review must approve lookup-only receipt"),
        ...
        _check("receipt_lookup_ready", review_summary.get("receipt_ready") is True and review_summary.get("lookup_ready") is True, {"receipt": review_summary.get("receipt_ready"), "lookup": review_summary.get("lookup_ready")}, "receipt lookup must be ready"),
        ...
        _check("lookup_keys_receipt_index_namespace", len(lookup_keys) == len(index_rows) and all(str(key).startswith("receipt-index:") for key in lookup_keys), lookup_keys, "lookup keys must use receipt-index namespace"),
        ...
        _check("source_next_step_matches", review_summary.get("next_step") == RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_PACKET_INDEX_PUBLICATION_RECEIPT_INDEX_REVIEW_V986_NEXT_STEP, review_summary.get("next_step"), "source receipt index review must route to receipt"),
    ]
...
def _receipt(
    ...
    return {
        ...
        "receipt_status": "publication_receipt_index_lookup_receipted" if ready else "blocked",
        ...
        "next_step": RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_PACKET_INDEX_PUBLICATION_RECEIPT_INDEX_RECEIPT_V987_NEXT_STEP if ready else "repair_randomized_holdout_publication_receipt_packet_index_publication_receipt_index_receipt_v987",
    }
...
def _summary(status: str, checks: list[dict[str, Any]], receipt: dict[str, Any]) -> dict[str, Any]:
    return {
        "randomized_holdout_publication_receipt_packet_index_publication_receipt_index_receipt_v987_ready": status == "pass" and receipt.get("receipt_ready") is True,
...
def _decision(status: str) -> str:
    if status == "pass":
        return "randomized_holdout_publication_receipt_packet_index_publication_receipt_index_receipt_v987_ready"
    return "fix_randomized_holdout_publication_receipt_packet_index_publication_receipt_index_receipt_v987"
...
def _interpretation(status: str, receipt: dict[str, Any]) -> dict[str, str]:
    if status != "pass":
        return {
            ...
            "reason": "The receipt index review is not ready to be receipted.",
            "next_action": "repair receipt index review before recording receipt",
        }
    return {
        "model_quality_claim": RANDOMIZED_HOLDOUT_PUBLICATION_MODEL_QUALITY_CLAIM,
        "reason": "The reviewed receipt index is receipted for downstream lookup only; production promotion remains blocked.",
        "next_action": str(receipt.get("next_step")),
    }


__all__ = [
    "RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_PACKET_INDEX_PUBLICATION_RECEIPT_INDEX_RECEIPT_V987_CSV_FILENAME",
    "RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_PACKET_INDEX_PUBLICATION_RECEIPT_INDEX_RECEIPT_V987_HTML_FILENAME",
    "RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_PACKET_INDEX_PUBLICATION_RECEIPT_INDEX_RECEIPT_V987_JSON_FILENAME",
    "RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_PACKET_INDEX_PUBLICATION_RECEIPT_INDEX_RECEIPT_V987_MARKDOWN_FILENAME",
    "RANDOMIZED_HOLDOUT_PUBLICATION_RECEIPT_PACKET_INDEX_PUBLICATION_RECEIPT_INDEX_RECEIPT_V987_TEXT_FILENAME",
    "build_randomized_holdout_publication_receipt_packet_index_publication_receipt_index_receipt_v987",
    "locate_receipt_index_review_v987",
    "read_json_report",
    "resolve_exit_code",
]
