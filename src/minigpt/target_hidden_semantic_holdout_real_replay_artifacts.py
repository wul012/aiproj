...
def render_target_hidden_semantic_holdout_real_replay_text(report: dict[str, Any]) -> str:
    ...
    rows = [
        ...
        ("target_hidden_semantic_holdout_real_replay_ready", summary.get("target_hidden_semantic_holdout_real_replay_ready")),
        ("holdout_model_quality_ready", summary.get("holdout_model_quality_ready")),
        ("semantic_holdout_model_quality_ready", summary.get("semantic_holdout_model_quality_ready")),
...
def render_target_hidden_semantic_holdout_real_replay_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT target-hidden semantic holdout real replay'))}",
        "",
        f"- Status: `{report.get('status')}`",
        f"- Decision: `{report.get('decision')}`",
        f"- Holdout model quality ready: `{summary.get('holdout_model_quality_ready')}`",
        f"- Semantic model quality ready: `{summary.get('semantic_holdout_model_quality_ready')}`",
...
def render_target_hidden_semantic_holdout_real_replay_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ("Status", report.get("status")),
        ("Quality ready", summary.get("holdout_model_quality_ready")),
        ("Semantic ready", summary.get("semantic_holdout_model_quality_ready")),
...
<title>{html_escape(report.get('title', 'MiniGPT target-hidden semantic holdout real replay'))}</title>{_style()}</head>
<body><main>
<header><h1>{html_escape(report.get('title', 'MiniGPT target-hidden semantic holdout real replay'))}</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
...
def write_target_hidden_semantic_holdout_real_replay_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / TARGET_HIDDEN_SEMANTIC_HOLDOUT_REAL_REPLAY_JSON_FILENAME,
        "csv": root / TARGET_HIDDEN_SEMANTIC_HOLDOUT_REAL_REPLAY_CSV_FILENAME,
        "text": root / TARGET_HIDDEN_SEMANTIC_HOLDOUT_REAL_REPLAY_TEXT_FILENAME,
        "markdown": root / TARGET_HIDDEN_SEMANTIC_HOLDOUT_REAL_REPLAY_MARKDOWN_FILENAME,
        "html": root / TARGET_HIDDEN_SEMANTIC_HOLDOUT_REAL_REPLAY_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_model_capability_route_promotion_bounded_real_replay_csv(report, paths["csv"])
    paths["text"].write_text(render_target_hidden_semantic_holdout_real_replay_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_target_hidden_semantic_holdout_real_replay_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_target_hidden_semantic_holdout_real_replay_html(report), encoding="utf-8")
...
def _style() -> str:
    return """<style>
:root{color-scheme:light;--ink:#17212b;--muted:#62717d;--line:#d7dee5;--panel:#f8fafc;--accent:#2563eb}
