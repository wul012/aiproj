...
def render_randomized_holdout_publication_registry_downstream_consumer_packet_text(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    rows = [
        ("status", report.get("status")),
        ("decision", report.get("decision")),
        ("failed_count", report.get("failed_count")),
        ("randomized_holdout_publication_registry_downstream_consumer_packet_ready", summary.get("randomized_holdout_publication_registry_downstream_consumer_packet_ready")),
...
def write_randomized_holdout_publication_registry_downstream_consumer_packet_csv(report: dict[str, Any], path: str | Path) -> None:
    fieldnames = ["packet_id", "consumer_name", "lookup_key", "entry_id", "granted_use", "blocked_uses", "promotion_ready", "packet_status"]
    out_path = Path(path)
...
def render_randomized_holdout_publication_registry_downstream_consumer_packet_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT randomized holdout publication registry downstream consumer packet'))}",
        "",
        f"- Status: `{report.get('status')}`",
        f"- Decision: `{report.get('decision')}`",
        f"- Packet ready: `{summary.get('randomized_holdout_publication_registry_downstream_consumer_packet_ready')}`",
    ...
    for row in list_of_dicts(report.get("packet_rows")):
        lines.append(
            ...
            + " | ".join(
                [
                    markdown_cell(row.get("packet_id")),
                    ...
                    markdown_cell(row.get("packet_status")),
                ]
            )
            + " |"
        )
...
def render_randomized_holdout_publication_registry_downstream_consumer_packet_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    packet = as_dict(report.get("packet"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ("Status", report.get("status")),
        ("Packet ready", summary.get("randomized_holdout_publication_registry_downstream_consumer_packet_ready")),
    ...
    packet_rows = "".join(_packet_row(row) for row in list_of_dicts(report.get("packet_rows")))
...
<title>{html_escape(report.get('title', 'MiniGPT randomized holdout publication registry downstream consumer packet'))}</title>
{_style()}
</head>
<body>
<main>
<header><h1>{html_escape(report.get('title', 'MiniGPT randomized holdout publication registry downstream consumer packet'))}</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
...
<dt>Source guard digest</dt><dd>{html_escape(packet.get('source_guard_sha256'))}</dd>
<dt>Next step</dt><dd>{html_escape(summary.get('next_step'))}</dd>
</dl></section>
<section class="panel"><h2>Packet Rows</h2><div class="table-wrap"><table>
<thead><tr><th>Packet</th><th>Consumer</th><th>Lookup key</th><th>Entry</th><th>Granted use</th><th>Blocked uses</th><th>Promotion</th><th>Status</th></tr></thead>
...
def write_randomized_holdout_publication_registry_downstream_consumer_packet_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_PACKET_JSON_FILENAME,
        "csv": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_PACKET_CSV_FILENAME,
        "text": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_PACKET_TEXT_FILENAME,
        "markdown": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_PACKET_MARKDOWN_FILENAME,
        "html": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_PACKET_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_randomized_holdout_publication_registry_downstream_consumer_packet_csv(report, paths["csv"])
    paths["text"].write_text(render_randomized_holdout_publication_registry_downstream_consumer_packet_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_randomized_holdout_publication_registry_downstream_consumer_packet_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_randomized_holdout_publication_registry_downstream_consumer_packet_html(report), encoding="utf-8")
...
def _packet_row(row: dict[str, Any]) -> str:
    return (
        "<tr>"
        f"<td>{html_escape(row.get('packet_id'))}</td>"
        ...
        f"<td>{html_escape(row.get('packet_status'))}</td>"
        "</tr>"
    )
