...
def render_target_hidden_tokenizer_covered_holdout_suite_text(report: dict[str, Any]) -> str:
    ...
    rows = [
        ...
        (
            "target_hidden_tokenizer_covered_holdout_suite_ready",
            summary.get("bounded_objective_loss_signal_bridge_target_only_memory_target_hidden_tokenizer_covered_holdout_suite_ready"),
        ),
        ...
        ("source_target_leakage_case_count", summary.get("source_target_leakage_case_count")),
...
def write_target_hidden_tokenizer_covered_holdout_suite_csv(report: dict[str, Any], path: str | Path) -> None:
    fieldnames = [
        "case_id",
        "source_case_id",
        "tokenizer_covered",
    ...
    with out_path.open("w", encoding="utf-8", newline="") as handle:
        ...
        for row in list_of_dicts(report.get("coverage_rows")):
            prompt_case = as_dict(cases_by_id.get(str(row.get("case_id")))).get("prompt_case")
            writer.writerow(
                {
                    ...
                    "leaked_terms": csv_cell(row.get("leaked_terms")),
                    "prompt": csv_cell(as_dict(prompt_case).get("prompt")),
                }
            )


def render_target_hidden_tokenizer_covered_holdout_suite_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT target-hidden tokenizer-covered holdout suite'))}",
        "",
        ...
        f"- Source leakage cases: `{summary.get('source_target_leakage_case_count')}`",
    ...
    for row in list_of_dicts(report.get("coverage_rows")):
        prompt = _prompt_for_case(report, row.get("case_id"))
        lines.append(
            "| "
            + " | ".join(
                [
                    ...
                    markdown_cell(row.get("leaked_terms")),
...
def render_target_hidden_tokenizer_covered_holdout_suite_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    ...
    stats = [
        ...
        ("Target-hidden cases", summary.get("target_hidden_case_count")),
        ("Unknown tokens", summary.get("candidate_prompt_unknown_token_count")),
        ("Source leakage", summary.get("source_target_leakage_case_count")),
...
<title>{html_escape(report.get('title', 'MiniGPT target-hidden tokenizer-covered holdout suite'))}</title>
{_style()}
</head>
<body>
<main>
<header><h1>{html_escape(report.get('title', 'MiniGPT target-hidden tokenizer-covered holdout suite'))}</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
<section class="stats">{''.join(_card(label, value) for label, value in stats)}</section>
<section class="panel"><h2>Coverage Rows</h2><div class="table-wrap"><table>
<thead><tr><th>Case</th><th>Source</th><th>Covered</th><th>Target Hidden</th><th>Unknown</th><th>Leaked Terms</th><th>Prompt</th></tr></thead>
<tbody>{rows}</tbody>
</table></div></section>
...
def write_target_hidden_tokenizer_covered_holdout_suite_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / TARGET_ONLY_MEMORY_TARGET_HIDDEN_TOKENIZER_COVERED_HOLDOUT_SUITE_JSON_FILENAME,
        "csv": root / TARGET_ONLY_MEMORY_TARGET_HIDDEN_TOKENIZER_COVERED_HOLDOUT_SUITE_CSV_FILENAME,
        "text": root / TARGET_ONLY_MEMORY_TARGET_HIDDEN_TOKENIZER_COVERED_HOLDOUT_SUITE_TEXT_FILENAME,
        "markdown": root / TARGET_ONLY_MEMORY_TARGET_HIDDEN_TOKENIZER_COVERED_HOLDOUT_SUITE_MARKDOWN_FILENAME,
        "html": root / TARGET_ONLY_MEMORY_TARGET_HIDDEN_TOKENIZER_COVERED_HOLDOUT_SUITE_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_target_hidden_tokenizer_covered_holdout_suite_csv(report, paths["csv"])
    paths["text"].write_text(render_target_hidden_tokenizer_covered_holdout_suite_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_target_hidden_tokenizer_covered_holdout_suite_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_target_hidden_tokenizer_covered_holdout_suite_html(report), encoding="utf-8")
...
def _coverage_row(row: dict[str, Any], report: dict[str, Any]) -> str:
    ...
    return (
        ...
        f"<td>{html_escape(row.get('leaked_terms'))}</td>"
...
__all__ = [
    ...
    "write_target_hidden_tokenizer_covered_holdout_suite_csv",
