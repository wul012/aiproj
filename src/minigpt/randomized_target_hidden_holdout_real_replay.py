...
RANDOMIZED_TARGET_HIDDEN_HOLDOUT_REAL_REPLAY_CSV_FILENAME = "randomized_target_hidden_holdout_real_replay.csv"
RANDOMIZED_TARGET_HIDDEN_HOLDOUT_REAL_REPLAY_TEXT_FILENAME = "randomized_target_hidden_holdout_real_replay.txt"
RANDOMIZED_TARGET_HIDDEN_HOLDOUT_REAL_REPLAY_MARKDOWN_FILENAME = "randomized_target_hidden_holdout_real_replay.md"
...
def read_json_report(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("randomized target-hidden holdout real replay input must be a JSON object")
    return dict(payload)


def build_randomized_target_hidden_holdout_real_replay(
    ...
    *,
    ...
    title: str = "MiniGPT randomized target-hidden holdout real replay",
...
def resolve_exit_code(report: dict[str, Any], *, require_execution_pass: bool, require_model_pass: bool = False) -> int:
    if require_execution_pass and report.get("status") != "pass":
        return 1
    if require_model_pass and as_dict(report.get("summary")).get("randomized_holdout_model_quality_ready") is not True:
        return 1
    return 0
...
def _checks(
    ...
    return [
        ...
        _check("holdout_suite_ready", suite_summary.get("randomized_target_hidden_holdout_suite_ready") is True, suite_summary.get("randomized_target_hidden_holdout_suite_ready"), "randomized holdout suite summary must be ready"),
        _check("randomized_case_factor_at_least_two", float(suite_summary.get("randomized_case_factor") or 0.0) >= 2.0, suite_summary.get("randomized_case_factor"), "randomized suite should expand source cases"),
        _check("all_prompts_unique", suite_summary.get("unique_prompt_count") == len(cases), suite_summary.get("unique_prompt_count"), "randomized prompts should be unique"),
        _check("all_prompts_target_hidden", suite_summary.get("target_hidden_case_count") == len(cases), suite_summary.get("target_hidden_case_count"), "randomized prompts should remain target-hidden"),
        ...
        _check("negative_control_clean", dry_summary.get("negative_control_passed") is False, dry_summary.get("negative_control_passed"), "negative dry-run control must fail all cases"),
...
def _summary(
    ...
    return {
        "randomized_target_hidden_holdout_real_replay_ready": status == "pass",
        ...
        "source_unique_prompt_count": suite_summary.get("unique_prompt_count"),
        ...
        "randomized_holdout_model_quality_ready": model_ready,
...
def _decision(status: str, summary: dict[str, Any]) -> str:
    if status != "pass":
        return "fix_randomized_target_hidden_holdout_real_replay_inputs"
    if summary.get("randomized_holdout_model_quality_ready") is True:
        return "randomized_target_hidden_holdout_real_replay_passed_review_required"
    if int(summary.get("any_hit_case_count") or 0) > 0:
        return "randomized_target_hidden_holdout_real_replay_partial_model_gap"
    return "randomized_target_hidden_holdout_real_replay_zero_hit_model_gap"


def _interpretation(status: str, summary: dict[str, Any]) -> dict[str, Any]:
    if status != "pass":
        return {"model_quality_claim": "not_claimed", "reason": "Randomized target-hidden real replay inputs failed.", "next_action": "repair_randomized_target_hidden_real_replay_inputs"}
    if summary.get("randomized_holdout_model_quality_ready") is True:
        return {"model_quality_claim": summary.get("model_quality_claim"), "reason": "The checkpoint passed every randomized target-hidden holdout case; review remains required before promotion.", "next_action": summary.get("next_step")}
    return {"model_quality_claim": summary.get("model_quality_claim"), "reason": "The randomized target-hidden holdout suite executed, but the checkpoint did not pass every case.", "next_action": summary.get("next_step")}
