...
def render_randomized_holdout_publication_registry_downstream_consumer_packet_check_text(report: dict[str, Any]) -> str:
...
def write_randomized_holdout_publication_registry_downstream_consumer_packet_check_csv(report: dict[str, Any], path: str | Path) -> None:
...
def render_randomized_holdout_publication_registry_downstream_consumer_packet_check_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT randomized holdout publication registry downstream consumer packet contract check'))}",
        "",
...
def render_randomized_holdout_publication_registry_downstream_consumer_packet_check_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
...
<title>{html_escape(report.get('title', 'MiniGPT randomized holdout publication registry downstream consumer packet contract check'))}</title>
{_style()}
</head>
<body>
<main>
<header><h1>{html_escape(report.get('title', 'MiniGPT randomized holdout publication registry downstream consumer packet contract check'))}</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
...
def write_randomized_holdout_publication_registry_downstream_consumer_packet_check_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_PACKET_CHECK_JSON_FILENAME,
        "csv": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_PACKET_CHECK_CSV_FILENAME,
        "text": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_PACKET_CHECK_TEXT_FILENAME,
        "markdown": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_PACKET_CHECK_MARKDOWN_FILENAME,
        "html": root / RANDOMIZED_HOLDOUT_PUBLICATION_REGISTRY_DOWNSTREAM_CONSUMER_PACKET_CHECK_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_randomized_holdout_publication_registry_downstream_consumer_packet_check_csv(report, paths["csv"])
    paths["text"].write_text(render_randomized_holdout_publication_registry_downstream_consumer_packet_check_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_randomized_holdout_publication_registry_downstream_consumer_packet_check_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_randomized_holdout_publication_registry_downstream_consumer_packet_check_html(report), encoding="utf-8")
