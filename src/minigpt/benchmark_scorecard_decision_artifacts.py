...
def render_benchmark_scorecard_decision_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- Remediation blocker count: `{summary.get('remediation_blocker_count')}`",
        f"- Remediation review count: `{summary.get('remediation_review_count')}`",
        f"- Dominant remediation kind: `{summary.get('dominant_remediation_kind')}`",
        f"- Dominant remediation category: `{summary.get('dominant_remediation_category')}`",
        f"- Dominant remediation action: `{summary.get('dominant_remediation_action')}`",
    ...
    plan = _list_of_dicts(report.get("remediation_plan"))
    if plan:
        lines.extend(
            [
                "",
...
def _remediation_table(report: dict[str, Any]) -> str:
    rows = []
    for item in _list_of_dicts(report.get("remediation_plan")):
        rows.append(
            "<tr>"
