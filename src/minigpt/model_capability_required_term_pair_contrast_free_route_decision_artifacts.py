...
def render_model_capability_required_term_pair_contrast_free_route_decision_text(report: dict[str, Any]) -> str:
    ...
    rows = [
        ...
        ("fixed_only_route_count", summary.get("fixed_only_route_count")),
        ("prior_loss_rebalance_stopped", summary.get("prior_loss_rebalance_stopped")),
        ("requires_forced_choice_diagnostic", summary.get("requires_forced_choice_diagnostic")),
...
def render_model_capability_required_term_pair_contrast_free_route_decision_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    return "\n".join(
        [
            ...
            f"- Pair-full route count: `{summary.get('pair_full_route_count')}`",
            f"- Fixed-only route count: `{summary.get('fixed_only_route_count')}`",
            f"- Requires forced-choice diagnostic: `{summary.get('requires_forced_choice_diagnostic')}`",
            "",
            "## Evidence",
            "",
...
def render_model_capability_required_term_pair_contrast_free_route_decision_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ("Status", report.get("status")),
        ...
        ("Fixed-only", summary.get("fixed_only_route_count")),
        ("Forced choice", summary.get("requires_forced_choice_diagnostic")),
    ]
...
<title>MiniGPT contrast-free route decision</title>
{_style()}
</head>
<body>
<main>
<header><h1>MiniGPT contrast-free route decision</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
...
def write_model_capability_required_term_pair_contrast_free_route_decision_outputs(
    report: dict[str, Any], out_dir: str | Path
) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / PAIR_CONTRAST_FREE_ROUTE_DECISION_JSON_FILENAME,
        "csv": root / PAIR_CONTRAST_FREE_ROUTE_DECISION_CSV_FILENAME,
        "text": root / PAIR_CONTRAST_FREE_ROUTE_DECISION_TEXT_FILENAME,
        "markdown": root / PAIR_CONTRAST_FREE_ROUTE_DECISION_MARKDOWN_FILENAME,
        "html": root / PAIR_CONTRAST_FREE_ROUTE_DECISION_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    _write_csv(report, paths["csv"])
    paths["text"].write_text(render_model_capability_required_term_pair_contrast_free_route_decision_text(report), encoding="utf-8")
    paths["markdown"].write_text(
        render_model_capability_required_term_pair_contrast_free_route_decision_markdown(report),
        encoding="utf-8",
    )
    paths["html"].write_text(render_model_capability_required_term_pair_contrast_free_route_decision_html(report), encoding="utf-8")
...
def _style() -> str:
    return """<style>
:root{color-scheme:light;--ink:#172026;--muted:#5d6975;--line:#d8dee4;--panel:#f7f9fb;--accent:#6a4c93}
