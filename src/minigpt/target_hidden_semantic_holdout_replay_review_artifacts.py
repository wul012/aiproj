...
def render_target_hidden_semantic_holdout_replay_review_text(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    rows = [
        ("status", report.get("status")),
        ("decision", report.get("decision")),
        ("failed_count", report.get("failed_count")),
        ("target_hidden_semantic_holdout_replay_review_ready", summary.get("target_hidden_semantic_holdout_replay_review_ready")),
        ("source_holdout_model_quality_ready", summary.get("source_holdout_model_quality_ready")),
        ("source_semantic_holdout_model_quality_ready", summary.get("source_semantic_holdout_model_quality_ready")),
        ...
        ("clean_prompt_case_count", summary.get("clean_prompt_case_count")),
        ("approved_for_prompt_mutation_holdout", summary.get("approved_for_prompt_mutation_holdout")),
...
def write_target_hidden_semantic_holdout_replay_review_csv(report: dict[str, Any], path: str | Path) -> None:
...
def render_target_hidden_semantic_holdout_replay_review_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT target-hidden semantic holdout replay review'))}",
        "",
        f"- Status: `{report.get('status')}`",
        f"- Decision: `{report.get('decision')}`",
        f"- Source semantic model quality ready: `{summary.get('source_semantic_holdout_model_quality_ready')}`",
        f"- Target leakage cases: `{summary.get('target_leakage_case_count')}`",
        f"- Task hint cases: `{summary.get('task_hint_case_count')}`",
        f"- Clean prompt cases: `{summary.get('clean_prompt_case_count')}`",
        f"- Approved for prompt mutation holdout: `{summary.get('approved_for_prompt_mutation_holdout')}`",
...
def render_target_hidden_semantic_holdout_replay_review_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ("Status", report.get("status")),
        ("Semantic ready", summary.get("source_semantic_holdout_model_quality_ready")),
        ("Leakage cases", summary.get("target_leakage_case_count")),
        ("Hint cases", summary.get("task_hint_case_count")),
        ("Clean cases", summary.get("clean_prompt_case_count")),
        ("Prompt mutation", summary.get("approved_for_prompt_mutation_holdout")),
...
<title>{html_escape(report.get('title', 'MiniGPT target-hidden semantic holdout replay review'))}</title>{_style()}</head>
<body><main><header><h1>{html_escape(report.get('title', 'MiniGPT target-hidden semantic holdout replay review'))}</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
...
def write_target_hidden_semantic_holdout_replay_review_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / TARGET_HIDDEN_SEMANTIC_HOLDOUT_REPLAY_REVIEW_JSON_FILENAME,
        "csv": root / TARGET_HIDDEN_SEMANTIC_HOLDOUT_REPLAY_REVIEW_CSV_FILENAME,
        "text": root / TARGET_HIDDEN_SEMANTIC_HOLDOUT_REPLAY_REVIEW_TEXT_FILENAME,
        "markdown": root / TARGET_HIDDEN_SEMANTIC_HOLDOUT_REPLAY_REVIEW_MARKDOWN_FILENAME,
        "html": root / TARGET_HIDDEN_SEMANTIC_HOLDOUT_REPLAY_REVIEW_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_target_hidden_semantic_holdout_replay_review_csv(report, paths["csv"])
    paths["text"].write_text(render_target_hidden_semantic_holdout_replay_review_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_target_hidden_semantic_holdout_replay_review_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_target_hidden_semantic_holdout_replay_review_html(report), encoding="utf-8")
