...
from minigpt.bounded_objective_loss_signal_bridge_target_only_memory_target_hidden_tokenizer_covered_holdout_dry_run import (
    TARGET_ONLY_MEMORY_TARGET_HIDDEN_TOKENIZER_COVERED_HOLDOUT_DRY_RUN_CSV_FILENAME,
    TARGET_ONLY_MEMORY_TARGET_HIDDEN_TOKENIZER_COVERED_HOLDOUT_DRY_RUN_HTML_FILENAME,
    TARGET_ONLY_MEMORY_TARGET_HIDDEN_TOKENIZER_COVERED_HOLDOUT_DRY_RUN_JSON_FILENAME,
    TARGET_ONLY_MEMORY_TARGET_HIDDEN_TOKENIZER_COVERED_HOLDOUT_DRY_RUN_MARKDOWN_FILENAME,
    TARGET_ONLY_MEMORY_TARGET_HIDDEN_TOKENIZER_COVERED_HOLDOUT_DRY_RUN_TEXT_FILENAME,
)
...
def render_target_hidden_tokenizer_covered_holdout_dry_run_text(report: dict[str, Any]) -> str:
    ...
    rows = [
        ...
        (
            "target_hidden_tokenizer_covered_holdout_dry_run_ready",
            summary.get("bounded_objective_loss_signal_bridge_target_only_memory_target_hidden_tokenizer_covered_holdout_dry_run_ready"),
        ),
...
def write_target_hidden_tokenizer_covered_holdout_dry_run_csv(report: dict[str, Any], path: str | Path) -> None:
    fieldnames = [
        "case_id",
        "positive_case_pass",
        "negative_case_pass",
            ...
            writer.writerow(
                {
                    ...
                    "positive_hit_terms": csv_cell(_join_terms(row.get("positive_hit_terms"))),
                    "negative_hit_terms": csv_cell(_join_terms(row.get("negative_hit_terms"))),
                    "negative_missed_terms": csv_cell(_join_terms(row.get("negative_missed_terms"))),
                }
            )


def render_target_hidden_tokenizer_covered_holdout_dry_run_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT target-hidden tokenizer-covered holdout dry-run'))}",
        "",
        ...
        lines.append(
            ...
            + " | ".join(
                [
                    ...
                    markdown_cell(_join_terms(row.get("positive_hit_terms"))),
                    markdown_cell(_join_terms(row.get("negative_hit_terms"))),
                    markdown_cell(_join_terms(row.get("negative_missed_terms"))),
                ]
            )
            + " |"
        )
...
def render_target_hidden_tokenizer_covered_holdout_dry_run_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
...
<title>{html_escape(report.get('title', 'MiniGPT target-hidden tokenizer-covered holdout dry-run'))}</title>
{_style()}
</head>
<body>
<main>
<header><h1>{html_escape(report.get('title', 'MiniGPT target-hidden tokenizer-covered holdout dry-run'))}</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
<section class="stats">{''.join(_card(label, value) for label, value in stats)}</section>
<section class="panel"><h2>Dry-Run Rows</h2><div class="table-wrap"><table>
<thead><tr><th>Case</th><th>Positive pass</th><th>Negative pass</th><th>Positive hit</th><th>Negative hit</th><th>Negative missed</th></tr></thead>
<tbody>{rows}</tbody>
</table></div></section>
...
def write_target_hidden_tokenizer_covered_holdout_dry_run_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / TARGET_ONLY_MEMORY_TARGET_HIDDEN_TOKENIZER_COVERED_HOLDOUT_DRY_RUN_JSON_FILENAME,
        "csv": root / TARGET_ONLY_MEMORY_TARGET_HIDDEN_TOKENIZER_COVERED_HOLDOUT_DRY_RUN_CSV_FILENAME,
        "text": root / TARGET_ONLY_MEMORY_TARGET_HIDDEN_TOKENIZER_COVERED_HOLDOUT_DRY_RUN_TEXT_FILENAME,
        "markdown": root / TARGET_ONLY_MEMORY_TARGET_HIDDEN_TOKENIZER_COVERED_HOLDOUT_DRY_RUN_MARKDOWN_FILENAME,
        "html": root / TARGET_ONLY_MEMORY_TARGET_HIDDEN_TOKENIZER_COVERED_HOLDOUT_DRY_RUN_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_target_hidden_tokenizer_covered_holdout_dry_run_csv(report, paths["csv"])
    paths["text"].write_text(render_target_hidden_tokenizer_covered_holdout_dry_run_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_target_hidden_tokenizer_covered_holdout_dry_run_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_target_hidden_tokenizer_covered_holdout_dry_run_html(report), encoding="utf-8")
...
def _row(row: dict[str, Any]) -> str:
    return (
        ...
        f"<td>{html_escape(_join_terms(row.get('positive_hit_terms')))}</td>"
        f"<td>{html_escape(_join_terms(row.get('negative_hit_terms')))}</td>"
