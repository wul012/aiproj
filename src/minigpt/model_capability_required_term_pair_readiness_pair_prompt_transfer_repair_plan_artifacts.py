...
def render_pair_prompt_transfer_repair_plan_text(report: dict[str, Any]) -> str:
    ...
    rows = [
        ...
        ("repair_requirement_count", summary.get("repair_requirement_count")),
...
def render_pair_prompt_transfer_repair_plan_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- Next artifact: `{plan.get('proposed_next_artifact')}`",
            "",
            "## Repair Requirements",
            "",
            *[f"- {item}" for item in plan.get("repair_requirements", [])],
            "",
            "## Checks",
            "",
            *checks,
...
def render_pair_prompt_transfer_repair_plan_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    plan = as_dict(report.get("plan"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ...
        ("Requirements", summary.get("repair_requirement_count")),
    ]
    checks = "".join(_check_html(row) for row in list_of_dicts(report.get("check_rows")))
    requirements = "".join(f"<li>{html_escape(item)}</li>" for item in plan.get("repair_requirements", []))
...
<title>MiniGPT pair prompt transfer repair plan</title>
{_style()}
</head>
<body>
<main>
<header><h1>MiniGPT pair prompt transfer repair plan</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
<section class="stats">{''.join(_card(label, value) for label, value in stats)}</section>
<section class="panel"><h2>Repair Requirements</h2><ul>{requirements}</ul></section>
...
def write_pair_prompt_transfer_repair_plan_csv(report: dict[str, Any], path: str | Path) -> None:
...
def write_pair_prompt_transfer_repair_plan_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / PAIR_READINESS_PAIR_PROMPT_TRANSFER_REPAIR_PLAN_JSON_FILENAME,
        "csv": root / PAIR_READINESS_PAIR_PROMPT_TRANSFER_REPAIR_PLAN_CSV_FILENAME,
        "text": root / PAIR_READINESS_PAIR_PROMPT_TRANSFER_REPAIR_PLAN_TEXT_FILENAME,
        "markdown": root / PAIR_READINESS_PAIR_PROMPT_TRANSFER_REPAIR_PLAN_MARKDOWN_FILENAME,
        "html": root / PAIR_READINESS_PAIR_PROMPT_TRANSFER_REPAIR_PLAN_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_pair_prompt_transfer_repair_plan_csv(report, paths["csv"])
    paths["text"].write_text(render_pair_prompt_transfer_repair_plan_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_pair_prompt_transfer_repair_plan_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_pair_prompt_transfer_repair_plan_html(report), encoding="utf-8")
