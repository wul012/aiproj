...
def render_model_capability_route_promotion_bounded_real_replay_decoder_anchor_probe_text(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    rows = [
        ("status", report.get("status")),
        ("decision", report.get("decision")),
        ("probe_ready", summary.get("decoder_anchor_probe_ready")),
...
def write_model_capability_route_promotion_bounded_real_replay_decoder_anchor_probe_csv(report: dict[str, Any], path: str | Path) -> None:
    fieldnames = ["case_id", "profile_id", "anchor", "anchor_assisted_pass", "completion_pass", "new_text_pass", "anchor_assisted_hit_terms", "new_text_hit_terms", "completion_hit_terms"]
    out_path = Path(path)
...
def render_model_capability_route_promotion_bounded_real_replay_decoder_anchor_probe_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT model capability route promotion bounded real replay decoder anchor probe'))}",
        "",
        f"- Status: `{report.get('status')}`",
        f"- Decision: `{report.get('decision')}`",
        f"- Ready: `{summary.get('decoder_anchor_probe_ready')}`",
        f"- Probe rows: `{summary.get('probe_row_count')}`",
        f"- Anchor-assisted passes: `{summary.get('anchor_assisted_pass_count')}`",
        f"- Completion passes: `{summary.get('completion_pass_count')}`",
        f"- New-text passes: `{summary.get('new_text_pass_count')}`",
...
def render_model_capability_route_promotion_bounded_real_replay_decoder_anchor_probe_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    stats = [
        ("Status", report.get("status")),
        ("Decision", report.get("decision")),
...
<title>{html_escape(report.get('title', 'MiniGPT model capability route promotion bounded real replay decoder anchor probe'))}</title>
{_style()}
</head>
<body>
<main>
<header><h1>{html_escape(report.get('title', 'MiniGPT model capability route promotion bounded real replay decoder anchor probe'))}</h1><p>Tests whether small output anchors help the v818 checkpoint complete required terms. Anchor-assisted hits are not treated as unassisted model success.</p></header>
<section class="stats">{''.join(_card(label, value) for label, value in stats)}</section>
<section class="panel"><h2>Probe Rows</h2><div class="table-wrap"><table>
<thead><tr><th>Case</th><th>Profile</th><th>Anchor</th><th>Assisted</th><th>Completion</th><th>New text</th><th>Assisted hits</th><th>New hits</th><th>Completion hits</th></tr></thead>
<tbody>{rows}</tbody>
</table></div></section>
</main>
</body>
...
def write_model_capability_route_promotion_bounded_real_replay_decoder_anchor_probe_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_DECODER_ANCHOR_PROBE_JSON_FILENAME,
        "csv": root / MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_DECODER_ANCHOR_PROBE_CSV_FILENAME,
        "text": root / MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_DECODER_ANCHOR_PROBE_TEXT_FILENAME,
        "markdown": root / MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_DECODER_ANCHOR_PROBE_MARKDOWN_FILENAME,
        "html": root / MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_DECODER_ANCHOR_PROBE_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_model_capability_route_promotion_bounded_real_replay_decoder_anchor_probe_csv(report, paths["csv"])
    paths["text"].write_text(render_model_capability_route_promotion_bounded_real_replay_decoder_anchor_probe_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_model_capability_route_promotion_bounded_real_replay_decoder_anchor_probe_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_model_capability_route_promotion_bounded_real_replay_decoder_anchor_probe_html(report), encoding="utf-8")
