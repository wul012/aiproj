...
def render_model_capability_required_term_pair_loss_alias_newline_suppression_text(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    rows = [
        ("status", report.get("status")),
        ("decision", report.get("decision")),
        ("newline_suppression_decision", summary.get("newline_suppression_decision")),
...
def write_model_capability_required_term_pair_loss_alias_newline_suppression_csv(
    report: dict[str, Any],
    path: str | Path,
) -> None:
    fieldnames = [
...
def render_model_capability_required_term_pair_loss_alias_newline_suppression_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    profile_table = [
        ...
        profile_table.append(
            ...
            + " | ".join(
                [
                    ...
                    markdown_cell(row.get("strict_hit_count")),
                    markdown_cell(row.get("strict_full_coverage")),
                    markdown_cell(row.get("focus_strict_hit_count")),
                    markdown_cell(row.get("focus_strict_full_coverage")),
                    markdown_cell(row.get("strict_gain_count")),
                ]
            )
            + " |"
        )
    ...
    return "\n".join(
        [
            ...
            f"- Suppression decision: `{summary.get('newline_suppression_decision')}`",
            f"- Baseline strict hits: `{summary.get('baseline_strict_hit_count')}/{summary.get('case_count')}`",
            f"- Suppressed strict hits: `{summary.get('suppressed_strict_hit_count')}/{summary.get('case_count')}`",
...
def render_model_capability_required_term_pair_loss_alias_newline_suppression_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ("Status", report.get("status")),
        ...
        ("Suppressed strict", summary.get("suppressed_strict_hit_count")),
        ("Strict gains", summary.get("suppressed_strict_gain_count")),
    ]
...
<title>MiniGPT loss-alias newline suppression</title>
{_style()}
</head>
<body>
<main>
<header><h1>MiniGPT loss-alias newline suppression</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
<section class="stats">{''.join(_card(label, value) for label, value in stats)}</section>
<section class="panel"><h2>Next Action</h2><p>{html_escape(interpretation.get('next_action'))}</p></section>
<section class="panel">
<h2>Profiles</h2>
<div class="table-wrap"><table>
<thead><tr><th>Profile</th><th>Strict hits</th><th>Strict full</th><th>Focus hits</th><th>Focus full</th><th>Gains</th></tr></thead>
...
def write_model_capability_required_term_pair_loss_alias_newline_suppression_outputs(
    report: dict[str, Any],
    out_dir: str | Path,
) -> dict[str, str]:
    root = Path(out_dir)
    ...
    paths = {
        "json": root / REQUIRED_TERM_PAIR_LOSS_ALIAS_NEWLINE_SUPPRESSION_JSON_FILENAME,
        "csv": root / "model_capability_required_term_pair_loss_alias_newline_suppression_probe.csv",
        "text": root / REQUIRED_TERM_PAIR_LOSS_ALIAS_NEWLINE_SUPPRESSION_TEXT_FILENAME,
        "markdown": root / REQUIRED_TERM_PAIR_LOSS_ALIAS_NEWLINE_SUPPRESSION_MARKDOWN_FILENAME,
        "html": root / REQUIRED_TERM_PAIR_LOSS_ALIAS_NEWLINE_SUPPRESSION_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_model_capability_required_term_pair_loss_alias_newline_suppression_csv(report, paths["csv"])
    paths["text"].write_text(render_model_capability_required_term_pair_loss_alias_newline_suppression_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_model_capability_required_term_pair_loss_alias_newline_suppression_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_model_capability_required_term_pair_loss_alias_newline_suppression_html(report), encoding="utf-8")
...
def _profile_html(row: dict[str, Any]) -> str:
    return (
        ...
        f"<td>{html_escape(row.get('strict_hit_count'))}/{html_escape(row.get('case_count'))}</td>"
        f"<td>{html_escape(row.get('strict_full_coverage'))}</td>"
        f"<td>{html_escape(row.get('focus_strict_hit_count'))}/{html_escape(row.get('focus_case_count'))}</td>"
        f"<td>{html_escape(row.get('focus_strict_full_coverage'))}</td>"
        f"<td>{html_escape(row.get('strict_gain_count'))}</td>"
        "</tr>"
    )
