...
MODEL_CAPABILITY_ROUTE_PROMOTION_BOUNDED_REAL_REPLAY_PROMPT_ALIGNED_TRAINING_RUN_CSV_FILENAME = "model_capability_route_promotion_bounded_real_replay_prompt_aligned_training_run.csv"
...
def read_json_report(path: str | Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("prompt-aligned training run input must be a JSON object")
    return dict(payload)


def build_model_capability_route_promotion_bounded_real_replay_prompt_aligned_training_run(
    prompt_aligned_seed_report: dict[str, Any],
    run_dir: str | Path,
    *,
    prompt_aligned_seed_path: str | Path | None = None,
    title: str = "MiniGPT model capability route promotion bounded real replay prompt-aligned training run",
    generated_at: str | None = None,
) -> dict[str, Any]:
    root = Path(run_dir)
    seed_summary = as_dict(prompt_aligned_seed_report.get("summary"))
    artifacts = _artifacts(root)
    metrics = _metrics(root / "metrics.jsonl")
    train_config = _read_json(root / "train_config.json")
    manifest = _read_json(root / "run_manifest.json")
    checks = _checks(prompt_aligned_seed_report, seed_summary, artifacts, metrics, train_config)
    ...
    return {
        ...
        "issues": issues,
        "source_prompt_aligned_seed_revision": str(prompt_aligned_seed_path or ""),
        "run_dir": str(root),
        "prompt_aligned_seed_summary": seed_summary,
        "artifacts": artifacts,
        "metrics": metrics,
        "train_config": train_config,
        "manifest": manifest,
        "check_rows": checks,
        "prompt_aligned_training": training,
...
def _checks(
    ...
    return [
        ...
        _check("prompt_aligned_seed_ready", seed_summary.get("prompt_aligned_seed_revision_ready") is True, seed_summary.get("prompt_aligned_seed_revision_ready"), "prompt-aligned seed revision must be ready"),
        _check("exact_prompt_answers_present", int(seed_summary.get("exact_prompt_answer_count") or 0) > 0, seed_summary.get("exact_prompt_answer_count"), "seed must contain exact benchmark prompt answers"),
...
def _training(
    ...
    return {
        ...
        "prompt_aligned_example_count": seed_summary.get("example_count"),
        "exact_prompt_answer_count": seed_summary.get("exact_prompt_answer_count"),
        "proposed_next_artifact": "model_capability_route_promotion_bounded_real_replay_prompt_aligned_checkpoint_replay",
        "next_step": "run_prompt_aligned_checkpoint_bounded_replay" if status == "pass" else "repair_prompt_aligned_training_run",
    }


def _summary(status: str, checks: list[dict[str, Any]], training: dict[str, Any]) -> dict[str, Any]:
    return {
        "prompt_aligned_training_ready": status == "pass" and training.get("ready") is True,
        ...
        "prompt_aligned_example_count": training.get("prompt_aligned_example_count"),
        "exact_prompt_answer_count": training.get("exact_prompt_answer_count"),
...
def _decision(status: str) -> str:
    if status == "pass":
        return "model_capability_route_promotion_bounded_real_replay_prompt_aligned_training_run_ready"
    return "fix_model_capability_route_promotion_bounded_real_replay_prompt_aligned_training_run"


def _interpretation(status: str, training: dict[str, Any]) -> dict[str, Any]:
    if status != "pass":
        return {"model_quality_claim": "not_claimed", "reason": "Prompt-aligned training artifacts are incomplete.", "next_action": "repair prompt-aligned training run"}
    return {
        "model_quality_claim": "training_artifact_only",
        "reason": "A prompt-aligned checkpoint exists, but model improvement still requires bounded replay comparison.",
