...
def render_loss_suffix_patch_text(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    return "\n".join([
        f"status={report.get('status')}",
        f"decision={report.get('decision')}",
        f"loss_suffix_patch_ready={summary.get('bounded_objective_loss_signal_bridge_target_only_memory_loss_suffix_patch_ready')}",
        f"patch_example_count={summary.get('patch_example_count')}",
        f"target_pair_example_count={summary.get('target_pair_example_count')}",
        f"loss_suffix_example_count={summary.get('loss_suffix_example_count')}",
        f"loss_prefix_bridge_example_count={summary.get('loss_prefix_bridge_example_count')}",
        f"minimal_label_pair_example_count={summary.get('minimal_label_pair_example_count')}",
...
def render_loss_suffix_patch_markdown(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    lines = [
        f"# {markdown_cell(report.get('title', 'MiniGPT bounded objective loss signal bridge target-only memory loss-suffix patch'))}",
        "",
        f"- Status: `{report.get('status')}`",
        f"- Decision: `{report.get('decision')}`",
        f"- Patch examples: `{summary.get('patch_example_count')}`",
        f"- Target-pair examples: `{summary.get('target_pair_example_count')}`",
        f"- Loss-suffix examples: `{summary.get('loss_suffix_example_count')}`",
        f"- Loss-prefix bridge examples: `{summary.get('loss_prefix_bridge_example_count')}`",
        "",
        "## Patch Examples",
        "",
...
def render_loss_suffix_patch_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    ...
    stats = [
        ...
        ("Pair", summary.get("target_pair_example_count")),
        ("Loss", summary.get("loss_suffix_example_count")),
        ("Bridge", summary.get("loss_prefix_bridge_example_count")),
...
<title>{html_escape(report.get('title', 'MiniGPT bounded objective loss signal bridge target-only memory loss-suffix patch'))}</title>{_style()}</head>
<body><main>
<header><h1>{html_escape(report.get('title', 'MiniGPT bounded objective loss signal bridge target-only memory loss-suffix patch'))}</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
...
def write_loss_suffix_patch_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / TARGET_ONLY_MEMORY_LOSS_SUFFIX_PATCH_JSON_FILENAME,
        "csv": root / TARGET_ONLY_MEMORY_LOSS_SUFFIX_PATCH_CSV_FILENAME,
        "jsonl": root / TARGET_ONLY_MEMORY_LOSS_SUFFIX_PATCH_JSONL_FILENAME,
        "corpus": root / TARGET_ONLY_MEMORY_LOSS_SUFFIX_PATCH_CORPUS_FILENAME,
        "text": root / TARGET_ONLY_MEMORY_LOSS_SUFFIX_PATCH_TEXT_FILENAME,
        "markdown": root / TARGET_ONLY_MEMORY_LOSS_SUFFIX_PATCH_MARKDOWN_FILENAME,
        "html": root / TARGET_ONLY_MEMORY_LOSS_SUFFIX_PATCH_HTML_FILENAME,
    }
    ...
    paths["text"].write_text(render_loss_suffix_patch_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_loss_suffix_patch_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_loss_suffix_patch_html(report), encoding="utf-8")
...
*{box-sizing:border-box}body{margin:0;background:#f7f6f2;color:var(--ink);font-family:Arial,"Microsoft YaHei",sans-serif}
