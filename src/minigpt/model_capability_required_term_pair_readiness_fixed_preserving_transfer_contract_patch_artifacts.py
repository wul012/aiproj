...
def render_fixed_preserving_transfer_contract_patch_text(report: dict[str, Any]) -> str:
    ...
    rows = [
        ...
        ("fixed_preserving_transfer_row_count", summary.get("fixed_preserving_transfer_row_count")),
...
def render_fixed_preserving_transfer_contract_patch_markdown(report: dict[str, Any]) -> str:
...
def render_fixed_preserving_transfer_contract_patch_html(report: dict[str, Any]) -> str:
    summary = as_dict(report.get("summary"))
    patch = as_dict(report.get("patch"))
    interpretation = as_dict(report.get("interpretation"))
    stats = [
        ...
        ("Transfer rows", summary.get("fixed_preserving_transfer_row_count")),
    ]
...
<title>MiniGPT fixed-preserving transfer contract patch</title>
{_style()}
</head>
<body>
<main>
<header><h1>MiniGPT fixed-preserving transfer contract patch</h1><p>{html_escape(interpretation.get('reason'))}</p></header>
...
def write_fixed_preserving_transfer_contract_patch_csv(report: dict[str, Any], path: str | Path) -> None:
...
def write_fixed_preserving_transfer_contract_patch_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    root = Path(out_dir)
    root.mkdir(parents=True, exist_ok=True)
    paths = {
        "json": root / PAIR_READINESS_FIXED_PRESERVING_TRANSFER_CONTRACT_PATCH_JSON_FILENAME,
        "csv": root / PAIR_READINESS_FIXED_PRESERVING_TRANSFER_CONTRACT_PATCH_CSV_FILENAME,
        "text": root / PAIR_READINESS_FIXED_PRESERVING_TRANSFER_CONTRACT_PATCH_TEXT_FILENAME,
        "markdown": root / PAIR_READINESS_FIXED_PRESERVING_TRANSFER_CONTRACT_PATCH_MARKDOWN_FILENAME,
        "html": root / PAIR_READINESS_FIXED_PRESERVING_TRANSFER_CONTRACT_PATCH_HTML_FILENAME,
    }
    write_json_payload(report, paths["json"])
    write_fixed_preserving_transfer_contract_patch_csv(report, paths["csv"])
    paths["text"].write_text(render_fixed_preserving_transfer_contract_patch_text(report), encoding="utf-8")
    paths["markdown"].write_text(render_fixed_preserving_transfer_contract_patch_markdown(report), encoding="utf-8")
    paths["html"].write_text(render_fixed_preserving_transfer_contract_patch_html(report), encoding="utf-8")
