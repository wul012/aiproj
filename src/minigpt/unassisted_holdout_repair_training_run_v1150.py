...
def build_unassisted_holdout_repair_training_run_v1150(
    ...
    *,
    ...
    return {
        "schema_version": 1,
        "title": "MiniGPT unassisted holdout repair training run v1150",
        "generated_at": generated_at or utc_now(),
        "status": status,
        "decision": _decision(status),
        "failed_count": len(issues),
        "issues": issues,
        "source_unassisted_holdout_repair_seed_corpus": str(seed_corpus_path or ""),
        "run_dir": str(root),
...
def write_unassisted_holdout_repair_training_run_v1150_outputs(report: dict[str, Any], out_dir: str | Path) -> dict[str, str]:
    outputs = write_readability_outputs(
        report,
        out_dir,
        stem=UNASSISTED_HOLDOUT_REPAIR_TRAINING_RUN_V1150_STEM,
...
def _checks(
    ...
    return [
        _check("seed_corpus_passed", seed_corpus_report.get("status") == "pass", seed_corpus_report.get("status"), "v1149 seed corpus must pass"),
        _check("seed_corpus_ready", seed_summary.get("unassisted_holdout_repair_seed_corpus_ready") is True, seed_summary.get("unassisted_holdout_repair_seed_corpus_ready"), "v1149 seed corpus ready flag must be true"),
        _check("seed_corpus_next_step_matches_training", seed_summary.get("next_step") == "run_unassisted_holdout_repair_training", seed_summary.get("next_step"), "v1149 must point to bounded training"),
        ...
        _check("prepared_corpus_matches_seed", bool(expected_corpus) and prepared_text == expected_corpus, artifact_map["prepared_corpus"]["path"], "training must consume the exact v1149 seed corpus text"),
...
def _training(
    ...
    return {
        ...
        "next_step": "run_unassisted_holdout_repair_replay_comparison" if status == "pass" else "repair_unassisted_holdout_repair_training_run",
    }


def _summary(status: str, checks: list[dict[str, Any]], training: dict[str, Any]) -> dict[str, Any]:
    return {
        "unassisted_holdout_repair_training_ready": status == "pass" and training.get("ready") is True,
...
def _decision(status: str) -> str:
    if status == "pass":
        return "unassisted_holdout_repair_training_run_ready"
    return "fix_unassisted_holdout_repair_training_run"
...
def _interpretation(status: str, training: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "model_quality_claim": "training_artifact_only",
        "reason": "A bounded checkpoint was trained from the v1149 seed corpus, but capability improvement must be proven by replaying the unchanged target-free holdout prompts.",
...
def _handoff(report: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "source_unassisted_holdout_repair_seed_corpus": report.get("source_unassisted_holdout_repair_seed_corpus"),
        "run_dir": report.get("run_dir"),
        "checkpoint": training.get("checkpoint"),
        "tokenizer": training.get("tokenizer"),
        "holdout_prompts": str(Path(str(report.get("source_unassisted_holdout_repair_seed_corpus") or "")).parent / HOLDOUT_PROMPTS_NAME),
        "model_quality_claim": training.get("model_quality_claim"),
        "promotion_ready": False,
        "next_step": training.get("next_step"),
    }
