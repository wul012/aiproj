...
1265-v1308-core-migration.md
 -> v1308 code explanation: the core owner package is migrated ATOMICALLY and flat_dir_file_count drops for the FIRST TIME since it was frozen at v1293: 1,355 -> 1,350. Executed straight off v1307's design doc, in its stated order. (1) The checkpoint-compatibility gate ran FIRST, before touching anything: reading pickle opcodes directly (no payload execution) showed both checkpoints — including the v1185 canonical grok one — store only state_dicts and plain metadata with NO minigpt class refs, so class paths are not baked into artifacts; the invariant STATE_SHA256=7b4b9761... was recorded and is bit-identical after the move. (2) model.py was split along its natural seam into core/layers.py (165 lines: GPTConfig/CausalSelfAttention/MLP/Block) and core/model.py (207: select_next_token/MiniGPT, re-exporting GPTConfig so historical imports are unchanged), both under the 220-line cap, definitions byte-identical. (3) All five primitives moved together with sys.modules shims at the flat paths; v1307's circular import did not recur because core/* now references only siblings. A trap was caught and fully reverted rather than patched: ast node.lineno excludes DECORATORS, so slicing silently dropped @dataclass and turned GPTConfig into a plain class ("takes no arguments") — no syntax error, only a runtime signature change. The contract's facade-only rule retired as the "transitional" phase always implied, with compensating strictness: facade rules now key on SHAPE not location, the 220-line cap and layering prohibitions are unchanged, and a NEW rule forbids an implementation submodule from importing the flat namespace it left — exactly the v1307 cycle, now guarded.

1264-v1307-normalization-pilot-negative.md
 -> v1307 code explanation: a NEGATIVE result on the normalization migration — the dominant remaining term for 9/10 — delivered as documentation with zero code change (the pilot was applied, measured, and fully reverted; repo stands at v1306). Three findings. (1) Owner packages are pure facades and the contract test actively forbids implementation inside them, which is exactly why flat_dir_file_count has never moved: while every submodule is a facade, the flat module stays the only home for the code. (2) The mechanism works and was measured — moving implementation in and leaving a sys.modules shim at the flat path drops the metric by the number migrated (1,355 -> 1,351 for four core primitives) while preserving module identity; all gates were green at that point. (3) The killer constraint: an owner package CANNOT be half-migrated, because its __init__ eagerly wires every submodule, so a migrated shim reaches minigpt.core, which loads a still-facade sibling pointing back into the half-initialized flat module — a circular import. The migration unit is the whole package, atomically. For `core` that means splitting the 299-line model.py under the 220-line cap first, which is NOT mechanical: it is the transformer itself, and GPTConfig/MiniGPT class paths may be referenced by saved checkpoints, so checkpoint loadability must be verified before any split. Mechanism, constraint, the required contract change with its compensating strictness, and a recommended order are recorded in docs/elegance/normalization-migration-design.md.

1263-v1306-symbol-batch2.md
 -> v1306 code explanation: symbol batch 2 finishes the chain-shaped families (packet_chain / registry_ack / ack_bundle / target_hidden) — 353 symbols, 190 files, stock 6,494 -> 6,141 (three-version total 1,350 resolved, 0 new). Two correctness constraints were caught by assertions before any write: (1) a short name must be derived from the module that DEFINES a symbol, not one that references it — the same long name lived in two modules via import and was asked to become two different names; names defined in several modules are genuinely ambiguous (dedup-arc leftovers, 27 of them) and get excluded; (2) collapsed cores must be globally unique — `registry_ack_packet_artifacts` dropping `registry` and `ack_bundle_packet_artifacts` dropping `bundle` both landed on `ack_packet_artifacts`, which would have MERGED two distinct symbols, so core selection became first-come claiming. P1 was also refined: an old name may legitimately survive inside a dotted module path (scripts are named after the function they wrap; that is the class-9 shield working), so P1 now permits only that form — 0 non-module-path survivals, and P2 against git HEAD compared 3,214 files with 0 artifact-value changes. The real headline is the 53% EXCLUSION RATE (59 modules / 398 symbols): the chain families' homogeneity is now exhausted, so the remaining ~4,500 violations need per-family rules rather than another generic run — and two structural leftovers (27 ambiguous duplicate definitions, 75 dead case-alias constants) are cleanup problems, not renaming problems.

1262-v1305-symbol-batch1.md
 -> v1305 code explanation: symbol-rename batch 1 scales the v1304 pilot to the receipt_chain family — 964 symbols across 177 modules, 373 files rewritten, name-budget stock 7,458 -> 6,494 (964 resolved, 0 new; two-version total 997). Naming is a BUDGET-DRIVEN COLLAPSE LADDER: the symbol core starts as the module's own short stem and drops a token only when a derived name would exceed 40 chars (96 modules forced to drop `chain`, 49 `receipt`, 32 kept the full stem) — nothing shortened for taste, every forced collapse counted and reported. 15 modules were EXCLUDED wholesale rather than half-renamed: they carry pre-existing alias assignments where a lowercase-v constant points at its uppercase-V twin, so renaming one side would leave a 31-char constant beside a 184-char alias to it. Those 75 aliases are probably dead and would resolve 75 more violations, but deletion needs its own evidence and does not ride along in a rename batch (same discipline as _style in v1303). Coupling class 9 did NOT recur: the tool's import-path shield plus test_import_target_resolution_v1304 reported 0 unresolvable imports — the pilot's guard proven under load.

1261-v1304-symbol-rename-pilot.md
 -> v1304 code explanation: the symbol-rename arc's PILOT — 33 symbols in the packet_index v1012-v1014 family, name-budget stock 7,491 -> 7,458 (33 resolved, 0 new). It first FALSIFIES the obvious approach: a compat alias `OLD = new` is itself counted by the scanner as a `variable` violation, so aliasing merely converts a `function` violation into a `variable` one and the stock never moves — the same trap as wrappers/partial in the dedup arc's blocked clusters. The way through is no alias at all: retire the old name and rewrite every reference, which is safe only because this repo has no external consumers (an asset worth naming, since the conclusion reverses the day it does). Proof P2 is the important one: a whole-repo string-constant multiset comparison shows the ONLY strings that changed are the renamed identifiers themselves (they appear in `__all__`), proving no artifact filename VALUE moved — renaming a constant changes its name, not its value. Coupling class 9 found: scripts here are named after the function they wrap, so the same 191-char phrase is both a symbol and a module name, and identifier-level rewriting silently repointed a dotted import at a nonexistent module — invisible to ruff and mypy. Fixed, swept repo-wide (exactly one instance), promoted to a permanent guard (test_import_target_resolution_v1304), and the batch tool now shields import paths before rewriting.

1260-v1303-dedup-batch3.md
 -> v1303 code explanation: dedup batch 3 takes the MULTI-VARIANT families (one local name covering 2-6 genuinely different bodies), 23 clusters / 166 copies, dup_def_stock 125 -> 102 (arc total 227 -> 102, 2,275 inline copies removed). Two judgment calls are the substance: (1) the _style family (27 variants, 295 copies, the largest remaining block) is deliberately NOT touched — analysis shows they are one stylesheet copy-pasted and drifted across ink/background/accent/width/grid/selector axes, so verbatim extraction would institutionalize the drift while convergence would change every shipped HTML artifact's bytes. That is a product decision, handed back to the user with the evidence rather than smuggled into an engineering batch that has held a byte-preserving line since batch 1. (2) mechanical naming was abandoned when it produced `read_json_report_json_report` and a name truncated mid-word by the 40-char budget — the v1298 signal — so the generator kept the screening/assertions and the names were hand-curated. Also: `html_card` was already owned by a batch-2 variant, so first-come is treated as contract; and the Any-contagion class recurred (json.loads into mypy-strict host) and was fixed WITHOUT adopting the house validating reader, since the inline originals deliberately did not validate.

1259-v1302-dedup-batch2.md
 -> v1302 code explanation: dedup batch 2 — 79 clusters / 502 inline copies into 22 hosts, dup_def_stock 204 -> 125. New routing rule: host by DEPENDENCY DIRECTION — the locate_* family moves into the modules owning their filename constants (members already import them, so zero new edges) rather than into a shared common. Batch 1's second-order effect landed as predicted (_field_checks, 41 copies, became extractable once _check was hosted). The close exposed a metric blind spot: dup_def_stock keys on ast.dump, which includes the function NAME, so two byte-identical bodies under different names count as two clusters — the plan faithfully built two identical host functions. Fixed by collapsing each pair to one canonical public (members keep their local names via `as`) and minting all new publics inside the 40-char budget, which banked all 24 resolutions: name-budget stock 7,515 -> 7,491, its first real reduction ever. Two duplicate-definition defects found (one nearly introduced by a name-keyed idempotency check, one PRE-EXISTING and shadowed) — both fixed and the adversarial scan promoted to a permanent repo-wide test, since within-module shadowing is structurally invisible to the cross-module dup metric.

1258-v1301-dedup-batch1.md
 -> v1301 code explanation: dedup batch 1 opens the post-closeout chase — 23 duplicate-body clusters (the check/exit-code family and the HTML/path helper family, 1,607 inline copies across 968 modules, ~4,000 copy-pasted lines) collapse into 18 host functions in report_check_common / report_utils, members importing under their old local names so the contract is byte-preserved. resolve_exit_code's dozen "variants" turn out to be keyword-name API differences — each keeps its exact signature under a systematic host name instead of a forced unification. Deleting code (unlike renaming it) orphans imports: the executor AST-drops imports whose only user was the removed def and noqa-tags pure re-exports. The full sweep caught coupling class 8 — Any-contagion: the host was outside the mypy strict target set, so strict-typed consumers saw resolve_exit_code as Any; fixed by adding report_check_common to the scope (22 -> 23 targets, a tightening). dup_def_stock 227 -> 204, identity of >1,500 import bindings asserted in both runners.
