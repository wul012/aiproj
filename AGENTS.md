...
- CI-parity close gate (promoted 2026-07-18 after two same-class misses in the v1287 close; sharpened 2026-07-20 after a third near-miss in v1290): before any close commit, run the gates in CI's shape, not just the local habit — (1) `python -B scripts/check_static_analysis.py` (the ratchet counts ALL issues; strict lint alone is not the gate), and the pass signal is `status=pass` / `new_issue_count=0` in the JSON — `baseline_update_allowed=True` is trivially true when no update is requested and proves nothing; (2) new test modules must not `import pytest` (CI's unit-test step is plain unittest with no pytest installed; the arc's test files are pytest-style but pytest-import-free — assert raises via try/except, tolerances via `abs(a-b) < tol`), verified with `PYTHONPATH=src python -m unittest discover -s tests -p "<new_test_file>.py"` expecting a clean import.
