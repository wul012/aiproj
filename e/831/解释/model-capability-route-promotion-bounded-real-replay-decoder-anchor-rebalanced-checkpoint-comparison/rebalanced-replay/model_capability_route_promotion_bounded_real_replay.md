...
| --- | --- | --- | --- | --- |
| objective-answer-direct | False |  | fixed,loss | 两 rrssu siesmssitsssseee |
| objective-answer-role | False |  | fixed,loss | aired required terms 的两个角色词。 输出：   iiir  lxssssos  x:  t |
| objective-answer-contrast | False |  | fixed,loss | fis_  owsossttssoswsssss |
| objective-answer-jsonish | False |  | fixed,loss | wer_terms，包含两个英文词。 answer_terms:otssssssmssssssemssssoss |
| objective-answer-check | False |  | fixed,loss | fim  resssesser seemse   |
...
| checkpoint_exists | pass | e\830\解释\model-capability-route-promotion-bounded-real-replay-decoder-anchor-rebalanced-training-run\run\checkpoint.pt | checkpoint.pt must exist |
| tokenizer_exists | pass | e\830\解释\model-capability-route-promotion-bounded-real-replay-decoder-anchor-rebalanced-training-run\run\tokenizer.json | tokenizer.json must exist |
