...
| `promoted_training*` | 46 | Governance/training handoff boundary; keep decision/review contract facades, seed/handoff summary builders, receipt automation checks, receipt sections, contract/check rendering, CSV field contracts, CSV row models, HTML stats, CSV/report rendering, assurance rendering, and artifact rendering split by role. |
