...
| `research` | 36 | Versioned experiments around adaptation, decoding, interpretability, and training behavior. |
...
| `promoted_training*` | 44 | Governance/training handoff boundary; keep decision review facade, seed/handoff summary builders, receipt automation checks, receipt sections, contract/check rendering, CSV field contracts, CSV row models, CSV/report rendering, assurance rendering, and artifact rendering split by role. |
...
|---|---|---|
| Adaptation | `lora_*`, `sft_*`, `dpo_*`, `distill_*` | Keep isolated until promoted by tests and docs. |
| Decoding/runtime | `spec_decode*`, `kv_cache*`, `rope_eval*` | Promote only stable primitives into core. |
| Interpretability/behavior | `grok_*`, `induction_*`, `similarity*` | Research/evaluation boundary. |
