...
| `promoted_training*` | 47 | Governance/training handoff boundary; keep decision/review contract facades, seed/handoff summary builders, receipt automation checks, receipt sections, contract/check rendering, CSV field contracts, CSV row models, seed/handoff HTML stats, CSV/report rendering, assurance rendering, and artifact rendering split by role. |
