...
本目录用于永久记录对 `MiniGPT From Scratch` 项目的第一阶段分层代码讲解。

本目录保留 v1-v30 的历史记录：从 tokenizer、dataset、model、train/generate，一直推进到 registry、model card、project audit、release bundle、release gate 和 generation quality policy。45 篇讲解还不需要迁移，后续只在这里做历史修正，不再把新版本持续堆进同一个目录。
...
后续版本讲解还要向下面这篇 mini-kv 参考文档靠齐：

```text
D:\C\mini-kv\代码讲解记录\111-restart-recovery-evidence-v55.md
```

靠齐的重点不是复用内容，而是复用讲解密度和证据链写法：

- 开头写清楚本版目标、来源、边界和不做什么。
- 逐节解释关键文件、核心结构、字段语义、输入输出和运行流程。
- 对报告、fixture、截图、归档等产物说明它们为何能作为证据。
- 测试部分写出真实覆盖链路和关键断言保护点。
- 末尾用“一句话总结”说明本版把项目推进到哪一层。

## 阶段拆分规则

从 v31 开始，新的代码讲解写入同级目录：

```text
代码讲解记录_发布治理阶段
```

拆分依据不是简单按文件数量机械切分，而是按项目阶段切分：当项目从“从零复现 MiniGPT 和补齐实验证据链”进入“发布治理、策略门禁、质量闭环和可复查交付”时，使用新的阶段目录。以后如果进入更大的新阶段，再新建同级目录，命名格式继续沿用：
