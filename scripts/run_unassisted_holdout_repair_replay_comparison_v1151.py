...
from minigpt.unassisted_holdout_repair_replay_comparison_v1151 import (  # noqa: E402
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Replay v1149 target-free holdout prompts against the v1150 repair checkpoint.")
    parser.add_argument("--handoff", type=Path, default=default_v1150_training_handoff_path(ROOT), help="v1150 training handoff JSON or output directory.")
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "output" / "unassisted-holdout-repair-replay-comparison-v1151")
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    handoff_path = locate_v1150_training_handoff(args.handoff)
    prepare_output_dir(args.out_dir, force=args.force)
    handoff = read_json_report(handoff_path, description="v1150 unassisted holdout repair training handoff")
    holdout_prompts = read_json_rows(args.holdout_prompts, description="v1149 holdout prompts") if args.holdout_prompts is not None else None
    ...
    outputs = write_unassisted_holdout_repair_replay_comparison_v1151_outputs(report, args.out_dir)
