...
from minigpt.model_capability_route_promotion_bounded_objective_unassisted_repair_seed_revision_training_run import (  # noqa: E402
...
from minigpt.model_capability_route_promotion_bounded_objective_unassisted_repair_seed_revision_training_run_artifacts import (  # noqa: E402
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build evidence for a bounded objective unassisted repair seed revision training run.")
    parser.add_argument("--seed-revision", type=Path, required=True)
    parser.add_argument("--run-dir", type=Path, required=True)
    parser.add_argument("--out-dir", type=Path, default=ROOT / "runs" / "model-capability-route-promotion-bounded-objective-unassisted-repair-seed-revision-training-run")
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    outputs = write_bounded_objective_unassisted_repair_seed_revision_training_run_outputs(report, args.out_dir)
    print(render_bounded_objective_unassisted_repair_seed_revision_training_run_text(report), end="")
