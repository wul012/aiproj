...
from minigpt.model_capability_required_term_pair_loss_alias_focus import (  # noqa: E402
...
from minigpt.model_capability_required_term_pair_loss_alias_focus_artifacts import (  # noqa: E402
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Boost v515 missed loss-alias rows and rerun focused tiny training.")
    parser.add_argument("stability", type=Path, help="v515 loss-alias stability JSON or output directory.")
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=ROOT / "runs" / "model-capability-required-term-pair-loss-alias-focus",
    )
    parser.add_argument("--seeds", type=int, nargs="+", default=[515])
    ...
    parser.add_argument("--require-pass", action="store_true", help="Return exit code 1 when the focus report fails structurally.")
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    source = locate_model_capability_required_term_pair_loss_alias_focus_source(args.stability)
    ...
    outputs = write_model_capability_required_term_pair_loss_alias_focus_outputs(report, args.out_dir)
    print(render_model_capability_required_term_pair_loss_alias_focus_text(report), end="")
