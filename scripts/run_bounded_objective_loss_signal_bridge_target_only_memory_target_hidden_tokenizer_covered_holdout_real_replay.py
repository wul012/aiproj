...
from minigpt.bounded_objective_loss_signal_bridge_target_only_memory_target_hidden_tokenizer_covered_holdout_real_replay import (  # noqa: E402
...
from minigpt.bounded_objective_loss_signal_bridge_target_only_memory_target_hidden_tokenizer_covered_holdout_real_replay_artifacts import (  # noqa: E402
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run real replay for the target-hidden tokenizer-covered holdout suite.")
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=ROOT / "runs" / "bounded-objective-loss-signal-bridge-target-only-memory-target-hidden-tokenizer-covered-holdout-real-replay",
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    dry_run_path = locate_target_hidden_tokenizer_covered_holdout_dry_run(args.dry_run)
    ...
    outputs = write_target_hidden_tokenizer_covered_holdout_real_replay_outputs(report, args.out_dir)
    print(render_target_hidden_tokenizer_covered_holdout_real_replay_text(report), end="")
