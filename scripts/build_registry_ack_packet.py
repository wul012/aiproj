...
from minigpt.registry_ack_packet import (  # noqa: E402
    build_registry_ack_packet,
    locate_registry_ack_packet,
    read_json_report,
    resolve_exit_code,
)
from minigpt.registry_ack_packet_artifacts import (  # noqa: E402
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    review_path = locate_registry_ack_packet(args.receipt_review)
    prepare_output_dir(args.out_dir, force=args.force)
    report = build_registry_ack_packet(
        read_json_report(review_path),
        receipt_review_path=review_path,
    )
    outputs = write_ack_packet_artifacts_outputs(report, args.out_dir)
    if args.check_out_dir:
        prepare_output_dir(args.check_out_dir, force=args.force)
        check_report = build_ack_packet_check(
            report,
            receipt_packet_path=outputs["json"],
        )
        outputs["check"] = write_p_et_check_artifacts_outputs(check_report, args.check_out_dir)["json"]
    print(render_ack_packet_artifacts_text(report), end="")
