...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    review_path = locate_ack_packet_pub(args.receipt_packet_index_review)
    prepare_output_dir(args.out_dir, force=args.force)
    report = build_ack_packet_pub(
        read_json_report(review_path),
        receipt_packet_index_review_path=review_path,
    )
    outputs = write_ack_packet_pub_artifacts_outputs(report, args.out_dir)
    print(render_ack_packet_pub_artifacts_text(report), end="")
