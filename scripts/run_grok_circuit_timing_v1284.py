"""v1284 Phase A: train the 6 trajectory cells (full + truncation ladder) once.

Example:
    python scripts/run_grok_circuit_timing_v1284.py            # full plan
    python scripts/run_grok_circuit_timing_v1284.py --probe    # P2: w=28 seed 1337
"""
...
from minigpt.grok_circuit_timing_v1284 import TimingConfig, run_cell, run_phase_a  # noqa: E402


def main(argv=None) -> None:
    parser = argparse.ArgumentParser(description="v1284 Phase A trainer")
    parser.add_argument("--out-dir", type=Path,
                        default=ROOT / "output" / "grok-circuit-timing-v1284")
    parser.add_argument("--probe", action="store_true",
                        help="P2 probe: the full ladder for w=28, seed 1337")
    args = parser.parse_args(argv)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    cfg = TimingConfig()
    args.out_dir.mkdir(parents=True, exist_ok=True)
    if args.probe:
        cell = run_cell(cfg, 28, 1337, "delayed", device)
        torch.save(cell, args.out_dir / "p2_probe.pt")
        print(f"probe w=28 seed=1337 prefix_ok={cell['prefix_ok']}"
    ...
    for c in cache["cells"]:
        print(f"{c['expected_phase']} w={c['width']} seed={c['seed']}:"
              f" prefix_ok={c['prefix_ok']} t_mem={c['t_mem']}"
