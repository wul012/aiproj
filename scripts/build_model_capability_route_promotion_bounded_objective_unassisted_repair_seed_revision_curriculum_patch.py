...
from minigpt.model_capability_route_promotion_bounded_objective_unassisted_repair_seed_revision_curriculum_patch import (  # noqa: E402
...
from minigpt.model_capability_route_promotion_bounded_objective_unassisted_repair_seed_revision_curriculum_patch_artifacts import (  # noqa: E402
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build a curriculum patch for bounded objective unassisted repair seed revision partial hits.")
    parser.add_argument("--partial-hit-diagnostic", type=Path, required=True)
    parser.add_argument("--seed-revision", type=Path, required=True)
    parser.add_argument("--objective-contract", type=Path, required=True)
    parser.add_argument("--source-corpus", type=Path, required=True)
    parser.add_argument("--out-dir", type=Path, default=ROOT / "runs" / "model-capability-route-promotion-bounded-objective-unassisted-repair-seed-revision-curriculum-patch")
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    diagnostic_path = locate_partial_hit_diagnostic(args.partial_hit_diagnostic)
    ...
    report = build_model_capability_route_promotion_bounded_objective_unassisted_repair_seed_revision_curriculum_patch(
        ...
        partial_hit_diagnostic_path=diagnostic_path,
        seed_revision_path=seed_path,
        objective_contract_path=contract_path,
    )
    outputs = write_bounded_objective_unassisted_repair_seed_revision_curriculum_patch_outputs(report, args.out_dir)
    print(render_bounded_objective_unassisted_repair_seed_revision_curriculum_patch_text(report), end="")
