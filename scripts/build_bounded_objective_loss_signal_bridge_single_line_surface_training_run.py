...
from minigpt.bounded_objective_loss_signal_bridge_single_line_surface_training_run import (  # noqa: E402
    build_bounded_objective_loss_signal_bridge_single_line_surface_training_run,
    locate_single_line_surface_patch,
    read_json_report,
    resolve_exit_code,
)
from minigpt.bounded_objective_loss_signal_bridge_single_line_surface_training_run_artifacts import (  # noqa: E402
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build evidence for a single-line surface patch training run.")
    parser.add_argument("--single-line-surface-patch", type=Path, required=True)
    parser.add_argument("--run-dir", type=Path, required=True)
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=ROOT / "runs" / "bounded-objective-loss-signal-bridge-single-line-surface-training-run",
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    patch_path = locate_single_line_surface_patch(args.single_line_surface_patch)
    ...
    outputs = write_single_line_surface_training_run_outputs(report, args.out_dir)
    print(render_single_line_surface_training_run_text(report), end="")
