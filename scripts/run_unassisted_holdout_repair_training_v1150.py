...
from minigpt.unassisted_holdout_repair_training_run_v1150 import (  # noqa: E402
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run and verify the v1150 bounded unassisted holdout repair training.")
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "output" / "unassisted-holdout-repair-training-run-v1150")
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    seed_report = read_json_report(seed_path, description="v1149 unassisted holdout repair seed corpus")
    ...
    outputs = write_unassisted_holdout_repair_training_run_v1150_outputs(report, args.out_dir)
...
def run_training(args: argparse.Namespace, prepared_data: Path, run_dir: Path) -> None:
    if not prepared_data.is_file():
        raise SystemExit(f"v1149 seed corpus text is missing: {prepared_data}")
    command = [
        sys.executable,
