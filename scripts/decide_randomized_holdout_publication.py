...
from minigpt.randomized_holdout_publication_decision import (  # noqa: E402
    build_randomized_holdout_publication_decision,
    locate_randomized_holdout_publication_packet_review,
    read_json_report,
    resolve_exit_code,
)
from minigpt.randomized_holdout_publication_decision_artifacts import (  # noqa: E402
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Record a bounded randomized holdout publication decision.")
    parser.add_argument("--publication-review", type=Path, required=True, help="Publication packet review JSON or output directory.")
    parser.add_argument("--out-dir", type=Path, default=ROOT / "runs" / "randomized-holdout-publication-decision")
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    report = build_randomized_holdout_publication_decision(read_json_report(review_path), publication_review_path=review_path)
    outputs = write_randomized_holdout_publication_decision_outputs(report, args.out_dir)
    print(render_randomized_holdout_publication_decision_text(report), end="")
