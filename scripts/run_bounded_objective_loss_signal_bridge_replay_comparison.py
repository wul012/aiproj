...
from minigpt.bounded_objective_loss_signal_bridge_replay_comparison import (  # noqa: E402
...
from minigpt.bounded_objective_loss_signal_bridge_replay_comparison_artifacts import (  # noqa: E402
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Replay the bounded objective contract against a loss-signal bridge checkpoint.")
    parser.add_argument("--objective-contract", type=Path, required=True)
    parser.add_argument("--training-run", type=Path, required=True)
    parser.add_argument("--checkpoint", type=Path, default=None)
    parser.add_argument("--tokenizer", type=Path, default=None)
    parser.add_argument("--device", choices=["auto", "cpu", "cuda"], default="auto")
    parser.add_argument("--out-dir", type=Path, default=ROOT / "runs" / "bounded-objective-loss-signal-bridge-replay-comparison")
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    training_path = locate_loss_signal_bridge_training_run(args.training_run)
    ...
    report = build_bounded_objective_loss_signal_bridge_replay_comparison(
        ...
        loss_signal_bridge_training_run_path=training_path,
    )
    outputs = write_loss_signal_bridge_replay_comparison_outputs(report, args.out_dir)
    print(render_loss_signal_bridge_replay_comparison_text(report), end="")
