...
from minigpt.model_capability_route_promotion_bounded_real_replay_prompt_aligned_checkpoint_comparison import (  # noqa: E402
...
from minigpt.model_capability_route_promotion_bounded_real_replay_prompt_aligned_checkpoint_comparison_artifacts import (  # noqa: E402
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compare a bounded real replay baseline with a prompt-aligned checkpoint replay.")
    ...
    parser.add_argument("--training-evidence", type=Path, default=None, help="Optional prompt-aligned training evidence JSON or output directory.")
    parser.add_argument("--out-dir", type=Path, default=ROOT / "runs" / "model-capability-route-promotion-bounded-real-replay-prompt-aligned-checkpoint-comparison")
    parser.add_argument("--require-comparison-pass", action="store_true", help="Return exit code 1 when comparison inputs are invalid.")
    parser.add_argument("--require-improvement", action="store_true", help="Return exit code 1 unless the prompt-aligned checkpoint improves over baseline.")
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    training_path = locate_prompt_aligned_training_run(args.training_evidence) if args.training_evidence else None
    ...
    outputs = write_model_capability_route_promotion_bounded_real_replay_prompt_aligned_checkpoint_comparison_outputs(report, args.out_dir)
    print(render_model_capability_route_promotion_bounded_real_replay_prompt_aligned_checkpoint_comparison_text(report), end="")
