...
from minigpt.model_capability_required_term_pair_loss_alias_objective import (  # noqa: E402
...
from minigpt.model_capability_required_term_pair_loss_alias_objective_artifacts import (  # noqa: E402
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a tiny loss-alias objective from the v513 heldout alias matrix.")
    parser.add_argument("heldout", type=Path, help="v513 heldout alias matrix JSON or output directory.")
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=ROOT / "runs" / "model-capability-required-term-pair-loss-alias-objective",
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    source = locate_model_capability_required_term_pair_loss_alias_objective_source(args.heldout)
    ...
    outputs = write_model_capability_required_term_pair_loss_alias_objective_outputs(report, args.out_dir)
    print(render_model_capability_required_term_pair_loss_alias_objective_text(report), end="")
